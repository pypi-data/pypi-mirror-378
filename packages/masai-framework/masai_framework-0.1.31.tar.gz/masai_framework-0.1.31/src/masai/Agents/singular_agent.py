...
class Agent(BaseAgent): # Inherit from the modified BaseAgent
    ...
    async def initiate_agent(self, query: str, passed_from: Optional[str] = None) -> Dict:
        """
        ...
        initial_state = State(
            ...
            previous_node=None, plan={}, passed_from=passed_from,
    ...
    async def initiate_agent_astream(self, query: str, passed_from: Optional[str] = None) -> AsyncGenerator[Dict, None]:
        """
        ...
        initial_state = State(
            ...
            current_tool="", tool_input=None, tool_output="", answer="",
            satisfied=False, reasoning="", delegate_to_agent=None,
            current_node='planner' if self.plan else 'router',previous_node=None, plan={}, passed_from=passed_from,
        reflection_counter=0, tool_loop_counter=0)
        configuration = {"recursion_limit": config.MAX_RECURSION_LIMIT}
