...
def build_query():
    ...
    for cls_name in actor_instances:
        ...
        for field0 in fields:
            ...
            mutation_name = make_mutation_name(cls, field0.name)
            field.graphql_name = mutation_name
            field.__skill__ = field0.__skill__

            if field.__skill__.response and inspect.isclass(field.__skill__.response):
                make_response_field(Query, field, mutation_name)
            ...
            if field.__skill__.response and not inspect.isclass(field.__skill__.response):
                ...
                Query.__annotations__[mutation_name] = cls.__annotations__[field.name]
                Query.__dataclass_fields__[mutation_name] = field
                Query.__strawberry_definition__.fields.append(field)
            else:
                if len(inst_list) == 1:
                ...
                Mutation.__annotations__[mutation_name] = cls.__annotations__[field.name]
                Mutation.__dataclass_fields__[mutation_name] = field
