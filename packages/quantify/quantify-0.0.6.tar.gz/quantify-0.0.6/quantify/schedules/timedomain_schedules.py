...
from quantify.operations.gate_library import X90, Measure, Reset, Rxy, X, Y
...
def rabi_sched(
    ...
    if len(amps) == 1:
        ...
        if not isinstance(pulse_amp, np.ndarray):
            raise TypeError(
                f"pulse_amp must be of type np.ndarray here but is {type(pulse_amp)}"
            )
        if not isinstance(pulse_duration, np.ndarray):
            raise TypeError(
                f"pulse_duration must be of type np.ndarray here but "
                f"is {type(pulse_duration)}"
            )
        raise ValueError(
...
def t1_sched(
    ...
    See section III.B.2. of :cite:t:`krantz_quantum_2019` for an explanation of the
    Bloch-Redfield model of decoherence and the :math:`T_1` experiment.
    ...
    qubit
        the name of the device element e.g., :code:`"q0"` to perform the T1 experiment
        on.
    repetitions
        The amount of times the Schedule will be repeated.
...
def ramsey_sched(
    ...
    See section III.B.2. of :cite:t:`krantz_quantum_2019` for an explanation of the
    Bloch-Redfield model of decoherence and the Ramsey experiment.
    ...
    artificial_detuning
        frequency in Hz of the software emulated, or ``artificial`` qubit detuning,
        which is implemented by changing the phase of the second pi/2 (recovery) pulse.
        The artificial detuning changes the observed frequency of the Ramsey
        oscillation, which can be useful to distinguish a slow oscillation due
        to a small physical detuning from the decay of the dephasing noise.
    qubit
    ...
    """
    device_element = qubit  # noqa: F841
    # ensure times is an iterable when passing floats.
    times = np.asarray(times)
    times = times.reshape(times.shape or (1,))
...
def echo_sched(
    ...
    See section III.B.2. of :cite:t:`krantz_quantum_2019` for an explanation of the
    Bloch-Redfield model of decoherence and the echo experiment.
...
def cpmg_sched(
    ...
    for i, tau in enumerate(times):
        ...
        if variant != "XY":
            inner.add(pl.IdlePulse(duration=idle_time))
            if variant == "X":
                echo_gate = X(device_element)
            elif variant == "Y":
                echo_gate = Y(device_element)
            inner.add(echo_gate, label=f"pi {i}")
            inner.add(pl.IdlePulse(duration=idle_time), ref_pt="start")
            n_reps = n_gates
            ...
            inner.add(pl.IdlePulse(duration=2 * idle_time), ref_pt="start")
    ...
    for i, prep_state in enumerate(prepared_states):
        ...
        if prep_state == 0:
            ...
            raise NotImplementedError(
                "Preparing the qubit in the second excited (2) state is not supported "
                "yet."
            )
        else:
...
def rabi_pulse_sched(
    mw_G_amp: float,  # noqa: N803
    mw_D_amp: float,  # noqa: N803
    mw_frequency: float,
    mw_clock: str,
    mw_port: str,
    ...
    schedule.add(pl.IdlePulse(duration=init_duration), label="qubit reset")
    schedule.add(
        pl.DRAGPulse(
