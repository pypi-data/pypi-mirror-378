...
from quantify.operations.operation import (
...
from quantify.structure.types import Graph, NDArray  # noqa: F401 pydantic needs it
...
class DeviceCompilationConfig(DataStructure):
    ...
    .. admonition:: Examples
        ...
        .. important::
            ...
            :class:`~quantify.device_under_test.quantum_device.QuantumDevice`
    ...
    """
    # Do not modify the type! Pydantic needs it!
    compilation_passes: list[SimpleNodeConfig] = Field(  # type: ignore
        default=[
            {
                "name": "circuit_to_device",
                "compilation_func": "quantify.backends.circuit_to_device."
            ...
            },
            {
                ...
                "compilation_func": "quantify.pulse_compensation."
                + "process_compensation_pulses",
            },
            {
                "name": "determine_absolute_timing",
                "compilation_func": "quantify.compilation._determine_absolute_timing",
            },
        ],
        validate_default=True,
    )
    """
    The list of compilation nodes arguments that should be called in succession to
    compile a schedule to the quantum-device layer.
    """
...
class CompilationConfig(DataStructure):
    ...
    """
    The :class:`~DeviceCompilationConfig` used in the compilation from the
    quantum-circuit layer to the quantum-device layer.
    """
...
class CompilationNode:
    ...
    def compile(
        ...
        return self._compilation_func(schedule=schedule, config=config)  # type: ignore
...
class QuantifyCompiler(CompilationNode):
    ...
    def __init__(  # noqa: D107
        self, name: str, quantum_device: QuantumDevice | None = None
    ) -> None:
        super().__init__(name=name)
        ...
        self._task_graph: nx.DiGraph | None = None
    ...
    @property
    def input_node(self) -> SimpleNode | None:
        """
    ...
    def construct_graph(self, config: CompilationConfig) -> None:
        """Construct the compilation graph based on a provided config."""
        raise NotImplementedError

    def draw(
        self, ax: Axes | None = None, figsize: tuple[float, float] = (20, 10), **options
    ) -> Axes:
        """
...
class SerialCompiler(QuantifyCompiler):
    ...
    def _compilation_func(
        ...
        if self.input_node == self.output_node:
            ...
            try:
                path = nx.shortest_path(  # type: ignore
                    self._task_graph,  # type: ignore
                    self.input_node,
                    self.output_node,
                )
        ...
        for node in path:
            schedule = node.compile(  # type: ignore
                schedule=schedule,
                config=config,
            )
...
class SerialCompilationConfig(CompilationConfig):
    ...
    backend: type[QuantifyCompiler] = SerialCompiler
