			...
			with Session(ENGINE) as session:
				...
				if sai:
					...
					for x in sai.__table__.columns:
						if getattr(sai,str(x.name)) not in [None]:
				...
				if glucose:
					...
					for x in glucose.__table__.columns:
						if getattr(glucose,str(x.name)) not in [None]:
				...
				height=session.query(HealthLog).filter(and_(HealthLog.BloodSugar!=None,))
				...
				if height:
					...
					for x in height.__table__.columns:
						if getattr(height,str(x.name)) not in [None]:
				...
				weight=session.query(HealthLog).filter(and_(HealthLog.BloodSugar!=None,))
				...
				heartRate=session.query(HealthLog).filter(and_(HealthLog.BloodSugar!=None,))
