...
class DubboKit:
    ...
    class InnterClazz():
        ...
        def __init__(self, zk_host,clazz_name):
            try:
                self.zk_host = zk_host
                self.clazz_name = clazz_name
                from dubbo.client import DubboClient, ZkRegister
                zk = ZkRegister(self.zk_host)
                self.dubbo_cli = DubboClient(self.clazz_name, zk_register=zk)
            except Exception as e:
                if "No module named 'dubbo'" in str(e):
                    print("ERROR: Dubbo模块未安装, 请使用以下命令进行安装:")
                    print("  pip3 install python3-dubbo  ")
                else:
                    print(f"ERROR Dubbo调用异常: {e}")

        def __getattr__(self, name,):
            # print(f"你想要调用{self.clazz_name}.{name}方法")
            return lambda *args, **kwargs: self.dubbo_cli.call(name, args)

    def __init__(self, zk_host):
        self.zk_host = zk_host

    def new(self,clazz_name):
        return self.InnterClazz(self.zk_host,clazz_name)