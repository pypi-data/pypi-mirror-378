...
export class APIConfigurator {
  static async setupAPIKey(
    ...
    // Wait for settings form to be visible - check for authentication status
    ...
    const authStatus = await page.locator('.sage-ai-auth-status-text').textContent();
    
    if (!authStatus?.includes('✅ Authenticated')) {
      console.log('User not authenticated. Authentication is required for API access.');
      // Note: In a real test scenario, you would need to handle authentication
      // The JWT token is now automatically used as the API key when authenticated
    } else {
      console.log('✅ User is authenticated. JWT token will be used automatically as API key.');
    }
