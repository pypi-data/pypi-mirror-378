...
from jaxtyping import ArrayLike, Float, PyTree, jaxtyped
...
def solve_transcription_splicing_model(
    ...
    Examples:
        >>> from stormi.models import solve_transcription_splicing_model
...
def solve_transcription_splicing_model_analytical(
    ...
    Examples:
        >>> from stormi.models import solve_transcription_splicing_model_analytical
        >>> from jax import numpy as jnp
        >>> ts0 = jnp.linspace(0, 4, 40)
        >>> ts1 = jnp.linspace(4 + (10 - 4) / 20, 10, 20)
        >>> ts = jnp.concatenate([ts0, ts1])
