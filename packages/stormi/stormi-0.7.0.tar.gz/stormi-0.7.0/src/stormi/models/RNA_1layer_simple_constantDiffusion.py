...
def diffusion_fn(t, state, args):
    ...
    return jnp.stack([u_coeff, s_coeff], axis=-1)  # Shape: (2G,)
...
def model(
    ...
    d_constant = numpyro.sample(
        "d_constant",
        dist.Uniform(diffusion_constant_prior_lower, diffusion_constant_prior_upper),
    )
    ...
    #   So we set them to -200 and 200, and then grab the values at the time points we want after the fact.
    brownian_motion = dfx.VirtualBrownianTree(
        -200, 200.0, tol=1e-1, shape=(), key=sde_rng_key
    )
