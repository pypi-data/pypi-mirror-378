...
# %% auto 0
__all__ = ['T', 'coerce_to_list_nonnull', 'coerce_to_list', 'is_hex', 'validate_action_id', 'validate_string',
           'validate_string_nonnull', 'validate_int', 'fetch_sample']
...
def fetch_sample(path: Union[Path, str]) -> str:
    ...
    def get_project_root(marker_file: str = ".gitignore") -> Path:
        """
        Walk upward from the current file/cwd until a marker (like .git) is found.
        This defines the project root.
        """
        try:
            # Start from the script file
            current = Path(__file__).resolve()
        except NameError:
            # In a notebook, use current working directory
    ...
    with open(os.path.join(get_project_root(),path), "r") as file_:
        return file_.read()
