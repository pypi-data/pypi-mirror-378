...
if firsttime:
    ...
    try:
        result = pickle.load(open(resultname, "rb"))
    except UnicodeDecodeError:
        result = pickle.load(open(resultname, "rb"), encoding='latin1')
...
def test(atoms, name, dostress=True):
    ...
    if result:
        ...
        data = result[name]
        ReportTest(name + ' (energy)', energy, data['energy'], 1e-9)
        compare(name, 'forces', forces, data['forces'], ids)
        compare(name, 'positions', positions, data['positions'], ids)
        if dostress:
            for i in range(6):
                ReportTest(name + f'(stress {i})', 
                           stress[i], data['stress'][i], 1e-9)
...
# Lennard-Jones
atoms = myfcc('Cu', pbc=True)
atoms.calc = LennardJones([29], [0.15], [2.7], 3*2.7, False)
...
atoms.calc = Morse(elements, epsilon, alpha, rmin)
...
if 0:
    # Gupta is broken with Python 3
    atoms = myalloy(('Y', 'Pt'), pbc=True)
    atoms.calc = Gupta(PtY_Gupta)
...
# EMT2013
atoms = myalloy(('Y', 'Pt'), pbc=True)
atoms.calc = EMT2013(PtY_EMT2013)
