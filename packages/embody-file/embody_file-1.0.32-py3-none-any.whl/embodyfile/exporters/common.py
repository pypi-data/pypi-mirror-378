...
def store_hdf_frequency_metadata(store: pd.HDFStore, schema_name: str, data: Data) -> None:
    ...
    if schema_name == SchemaRegistry.SCHEMAS[DataType.ECG_PPG].name:
        # Store both ECG and PPG sampling frequencies as metadata
        storer = store.get_storer(schema_name)
        if storer:
            if data.ecg_sample_frequency:
                storer.attrs.ecg_sample_frequency_hz = data.ecg_sample_frequency
                storer.attrs.ecg_sample_period_ms = 1000.0 / data.ecg_sample_frequency
                # Legacy compatibility
                storer.attrs.sample_frequency_hz = data.ecg_sample_frequency
                storer.attrs.sample_period_ms = 1000.0 / data.ecg_sample_frequency
            if data.ppg_sample_frequency:
                storer.attrs.ppg_sample_frequency_hz = data.ppg_sample_frequency
                storer.attrs.ppg_sample_period_ms = 1000.0 / data.ppg_sample_frequency
