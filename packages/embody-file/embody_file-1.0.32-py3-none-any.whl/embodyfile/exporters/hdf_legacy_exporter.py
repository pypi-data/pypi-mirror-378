        ...
        if not df_multidata.empty:
            with pd.HDFStore(output_path, mode="a") as store:
                ...
                if data.ecg_sample_frequency:
                    # Store the ECG sampling frequency as metadata for legacy compatibility
                    store.get_storer("multidata").attrs.sample_frequency_hz = data.ecg_sample_frequency
                    store.get_storer("multidata").attrs.sample_period_ms = 1000.0 / data.ecg_sample_frequency
