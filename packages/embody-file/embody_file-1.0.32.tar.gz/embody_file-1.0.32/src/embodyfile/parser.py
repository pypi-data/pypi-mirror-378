...
@dataclass
class SampleRates:
    """Container for ECG and PPG sample rates."""
...
def read_data(
    ...
    ppg_sample_rate: float | None = None,
    max_ecg_channels: int = MAX_ECG_CHANNELS,
    max_ppg_channels: int = MAX_PPG_CHANNELS,
) -> Data:
    """Parse binary file data with separate ECG and PPG sample rate handling."""
    ...
    if file_codec.PulseBlockEcg in collections or file_codec.PulseBlockPpg in collections:
        if sample_rate is None and ppg_sample_rate is None:
            sample_rates = _estimate_samplerate(collections)
        else:
            # Use provided rates or estimate missing ones
            estimated_rates = (
                _estimate_samplerate(collections) if sample_rate is None or ppg_sample_rate is None else None
            )
            ecg_rate = (
                sample_rate
                if sample_rate is not None
                else (estimated_rates.ecg_hz if estimated_rates else DEFAULT_ECG_PPG_SAMPLERATE)
            )
            ppg_rate = (
                ppg_sample_rate
                if ppg_sample_rate is not None
                else (
                    sample_rate
                    if sample_rate is not None
                    else (estimated_rates.ppg_hz if estimated_rates else DEFAULT_ECG_PPG_SAMPLERATE)
                )
            )
            sample_rates = SampleRates(ecg_hz=ecg_rate, ppg_hz=ppg_rate)
        __convert_block_messages_to_pulse_list(collections, sample_rates, max_ecg_channels, max_ppg_channels)
    else:
        # No ECG/PPG data, use default rates
        sample_rates = SampleRates(ecg_hz=DEFAULT_ECG_PPG_SAMPLERATE, ppg_hz=DEFAULT_ECG_PPG_SAMPLERATE)
...
def __convert_block_messages_to_pulse_list(
    ...
    """Convert ECG and PPG block messages to pulse lists using separate sample rates."""
    ...
    ecg_sampleinterval_ms = 1000 / sample_rates.ecg_hz
    ppg_sampleinterval_ms = 1000 / sample_rates.ppg_hz
    merged_data: dict[int, file_codec.PulseRawList] = {}
    ...
    if ecg_messages:
        _process_sensor_channel_data(
            ...
            sampleinterval_ms=ecg_sampleinterval_ms,
            stamp_tol=stamp_tol,
    ...
    if ppg_messages:
        _process_sensor_channel_data(
            ...
            merged_data=merged_data,
            sampleinterval_ms=ppg_sampleinterval_ms,
            stamp_tol=stamp_tol,
            stamp_gap_limit=stamp_gap_limit,
            overall_max_ecg_channels=current_max_ecg_channels,
            overall_max_ppg_channels=current_max_ppg_channels,
...
def _estimate_samplerate(collections: ProtocolMessageDict) -> SampleRates:
    """Estimate separate ECG and PPG sample rates from block messages."""
    ...
    # Calculate rates for each sensor type
    ecg_rate = calculate_rate_from_intervals(ecg_intervals_ms, "ECG")
    ppg_rate = calculate_rate_from_intervals(ppg_intervals_ms, "PPG")

    return SampleRates(ecg_hz=ecg_rate, ppg_hz=ppg_rate)
