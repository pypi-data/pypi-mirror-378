...
def print_example_report():
    ...
    print()
    print("  model-checker path/to/examples.py")
    print()
    ...
    by_subtheory = {}
    for example_name, meta in example_metadata.items():
        subtheory = meta['subtheory']
        if subtheory not in by_subtheory:
            by_subtheory[subtheory] = []
        by_subtheory[subtheory].append((example_name, meta['path']))
    ...
    for subtheory in sorted(by_subtheory.keys()):
        ...
        # Get unique path (all examples in a subtheory share the same file)
        if examples:
            path = examples[0][1]
            print(f"Source: {path}")
            print("\nActive examples:")
            ...
            # Separate countermodels and theorems
            cms = [e[0] for e in examples if '_CM_' in e[0]]
            ths = [e[0] for e in examples if '_TH_' in e[0]]
            defs = [e[0] for e in examples if '_DEF_' in e[0]]
            
            if cms:
                print("  Countermodels:", ", ".join(cms))
            if ths:
                print("  Theorems:", ", ".join(ths))
            if defs:
                print("  Definitions:", ", ".join(defs))
    ...
    total_cm = sum(1 for name in example_metadata if '_CM_' in name)
    total_th = sum(1 for name in example_metadata if '_TH_' in name)
    total_def = sum(1 for name in example_metadata if '_DEF_' in name)
    
    print(f"Total Examples: {len(example_metadata)}")
