...
from ctf_orchestra.challenge import app as challenge_app
from ctf_orchestra.config import app as config_app
from ctf_orchestra.ctf import app as ctf_app

from ctf_orchestra.utils.themes import print_info, print_warning, print_success, print_error
from ctf_orchestra.utils.state import StateManager
...
def cleanup(mode: Annotated[CleanupOptions, typer.Option(prompt=True)]):
    ...
    if mode.value == "ctf":
                ...
                for name in progress:
                    ...
                    if not ctf_directory.exists():
                        ...
                        if not delete_input:
                            ...
                            print_success(f"{"Deactivated and r" if initial_active_ctf == name else "R"}emoved [\green]{name}[green] from the CTF list.")
        ...
        if removed_counter == 0 and not_exists_counter == 0:
            ...
            print_info(f"Found [\green]{not_exists_counter}[green] non-existing CTF{'s' if not_exists_counter > 1 else ''} and removed [\green]{removed_counter} CTF{'s' if removed_counter > 1 or removed_counter == 0 else ''}.", True)
