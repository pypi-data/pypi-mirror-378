...
class MQConsumer:
    ...
    async def init(option: Dict[str, Any]) -> None:
        ...
        # 优雅的日志解决方案：支持外部传入logger
        external_logger = option.get('logger', None)
        ...
        if external_logger is not None:
            ...
            # 向后兼容：使用主程序的日志器
            logger = logging.getLogger('ImageRestorationService')
        
        # 仅设置pika库日志级别为ERROR，避免干扰
        ...
        # 使用log_config记录初始化信息
        if external_logger is not None:
            log_config(f"Consumer使用外部传入的logger: {logger.name}")
        else:
            log_config(f"Consumer使用默认logger: {logger.name}")
                ...
                if body is not None:
                    try:
                        ...
                        def handle_message():
                            try:
                                # 使用传入的logger记录回调执行
                                log_debug(f"开始执行回调函数")
                                try:
                                    if asyncio.iscoroutinefunction(callback):
                                        # 异步回调 - 保持与主程序相同的事件循环和日志配置
                                        log_debug(f"检测到异步回调函数")
                                        try:
                                            # 尝试获取现有的事件循环
                                        ...
                                        log_debug(f"异步回调执行完成: {result}")
                                    else:
                                        # 同步回调
                                        log_debug(f"检测到同步回调函数")
                                        result = callback(content)
                                        log_debug(f"同步回调执行完成: {result}")
                                except Exception as callback_error:
                                    log_error(f"回调执行出错: {callback_error}")
                                    import traceback
