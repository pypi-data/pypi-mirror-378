...
class UnifiedIAMClient:
    ...
    def __init__(
        ...
        self.base_url = base_url or settings.IAM_SERVICE_URL
    ...
    async def login(self, login_data: UserLogin) -> AuthResponse:
        ...
        response_data = await self._make_request(
            "POST", "/api/v1/auth/login", data=login_data.model_dump()
        )
    ...
    async def logout(self, token: str) -> bool:
        ...
        try:
            await self._make_request(
                "POST", "/api/v1/auth/logout", auth_token=token
            )
            return True
        except Exception as e:
    ...
    async def verify_token(self, token: str) -> UserInfo:
        """JWT 토큰 검증"""
        response_data = await self._make_request(
            "POST", "/api/v1/auth/verify", data={"token": token}
        )
    ...
    async def batch_check_permissions(
        ...
        response_data = await self._make_request(
            "POST",
            "/api/v1/rbac/decisions/batch-check",
            data=request_data,
            auth_token=auth_token,
        )
    ...
    async def get_user_permissions(
        ...
        endpoint = f"/api/v1/rbac/decisions/user-permissions/{user_id}"
        if query_string:
    ...
    async def get_user(self, user_id: str, auth_token: str) -> UserInfo:
        """사용자 정보 조회"""
        response_data = await self._make_request(
            "GET", f"/api/v1/users/{user_id}", auth_token=auth_token
        )
        return UserInfo(**response_data)
    ...
    async def get_current_user(self, auth_token: str) -> UserInfo:
        """현재 로그인 사용자 정보 조회"""
        response_data = await self._make_request(
            "GET", "/api/v1/users/me", auth_token=auth_token
        )
        return UserInfo(**response_data)
    ...
    async def get_user_sessions(
        ...
        response_data = await self._make_request(
            "GET",
            f"/api/v1/sessions?page={page}&size={size}",
            auth_token=auth_token,
        )
        return [
            SessionInfo(**session)
    ...
    async def get_current_session(self, auth_token: str) -> SessionInfo:
        """현재 세션 정보 조회"""
        response_data = await self._make_request(
            "GET", "/api/v1/sessions/current", auth_token=auth_token
        )
    ...
    async def deactivate_session(
        ...
        try:
            await self._make_request(
                "POST",
                f"/api/v1/sessions/{session_id}/deactivate",
                auth_token=auth_token,
            )
            return True
        except Exception as e:
