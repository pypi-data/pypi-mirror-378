...
from typing import Optional, List, Set, Dict, Tuple, Any, Literal, Union
...
class LangGraphAgent:
    ...
    def __init__(
        ...
        if isinstance(required_agents, str) and required_agents == "chat":
        ...
        self._checkpointer = self._get_checkpointer(connection)
    ...
    def build_reflection(
            ...
            ouput_structure:BaseModel
    ):
        if not all(agent in self._required_agents for agent in ["reflector", "critique"]):
            msg = fmt_msg("Reflection graph: Two agents not in required agents 'reflector' and 'critique'", "engine/build_reflection", color="red")
            self._logger.error(msg)
            raise ValueError(msg)
        
        if not (isclass(ouput_structure) and issubclass(ouput_structure, BaseModel)):
