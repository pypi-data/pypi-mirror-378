...
def pixels_to_ascii(image):
    ...
    return "".join(
        ASCII_CHARS[min(pixel * n_chars // 256, n_chars - 1)] for pixel in pixels
    )
...
def run_cli():
    """CLI wrapper for pixascii command"""
    import argparse

    parser = argparse.ArgumentParser(description="Convert images to ASCII art")
    parser.add_argument("image", help="Path to input image")
    parser.add_argument(
        "-o", "--output", help="File to save ASCII art"
    )
    parser.add_argument(
        "-w", "--width", type=int, default=100, help="Width of ASCII output"
    )
    parser.add_argument(
        "-H",
        "--height",
        type=int,
        default=None,
        help="Height of ASCII output (overrides auto height)",
    )
    parser.add_argument(
        "-c", "--color", action="store_true", help="Output colored ASCII art"
    )
