...
def geohashinspect(res):
    ...
    geohash_gpd["ipq"] = (
        4 * np.pi * geohash_gpd["cell_area"] / (geohash_gpd["cell_perimeter"] ** 2)
    )
    # Calculate zonal standardized compactness
    geohash_gpd["zsc"] = (
        np.sqrt(
            4 * np.pi * geohash_gpd["cell_area"]
            - np.power(geohash_gpd["cell_area"], 2) / np.power(6378137, 2)
        )
        / geohash_gpd["cell_perimeter"]
    )
    return geohash_gpd
...
def geohash_norm_area(geohash_gpd):
    ...
    vmin, vmax, vcenter = (
        geohash_gpd["norm_area"].min(),
        geohash_gpd["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
    geohash_gpd = geohash_gpd[
        ~geohash_gpd["crossed"]
    ]  # remove cells that cross the dateline
    geohash_gpd.to_crs("proj=moll").plot(
        column="norm_area",
        ax=ax,
        norm=norm,
        legend=True,
...
def geohash_norm_area_hist(geohash_gpd):
    ...
    counts, bins, patches = ax.hist(
        geohash_gpd_filtered["norm_area"], bins=50, alpha=0.7, edgecolor="black"
    )

    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = (
        geohash_gpd_filtered["norm_area"].min(),
        geohash_gpd_filtered["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def geohash_compactness_hist(geohash_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(
        geohash_gpd_filtered["ipq"], bins=50, alpha=0.7, edgecolor="black"
    )
