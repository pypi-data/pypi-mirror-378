...
def georefinspect(res):
    ...
    georef_gpd["ipq"] = (
        4 * np.pi * georef_gpd["cell_area"] / (georef_gpd["cell_perimeter"] ** 2)
    )
    # Calculate zonal standardized compactness
    georef_gpd["zsc"] = (
        np.sqrt(
            4 * np.pi * georef_gpd["cell_area"]
            - np.power(georef_gpd["cell_area"], 2) / np.power(6378137, 2)
        )
        / georef_gpd["cell_perimeter"]
    )
    return georef_gpd
...
def georef_norm_area(georef_gpd):
    ...
    vmin, vmax, vcenter = (
        georef_gpd["norm_area"].min(),
        georef_gpd["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
    georef_gpd = georef_gpd[
        ~georef_gpd["crossed"]
    ]  # remove cells that cross the dateline
    georef_gpd.to_crs("proj=moll").plot(
        column="norm_area",
        ax=ax,
        norm=norm,
        legend=True,
...
def georef_norm_area_hist(georef_gpd):
    ...
    counts, bins, patches = ax.hist(
        georef_gpd_filtered["norm_area"], bins=50, alpha=0.7, edgecolor="black"
    )

    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = (
        georef_gpd_filtered["norm_area"].min(),
        georef_gpd_filtered["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def georef_compactness_hist(georef_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(
        georef_gpd_filtered["ipq"], bins=50, alpha=0.7, edgecolor="black"
    )
