...
def isea3hinspect(res):
    ...
    isea3h_gpd["ipq"] = (
        4 * np.pi * isea3h_gpd["cell_area"] / (isea3h_gpd["cell_perimeter"] ** 2)
    )
    # Calculate zonal standardized compactness
    isea3h_gpd["zsc"] = (
        np.sqrt(
            4 * np.pi * isea3h_gpd["cell_area"]
            - np.power(isea3h_gpd["cell_area"], 2) / np.power(6378137, 2)
        )
        / isea3h_gpd["cell_perimeter"]
    )
    return isea3h_gpd
...
def isea3h_norm_area(isea3h_gpd):
    ...
    vmin, vmax, vcenter = (
        isea3h_gpd["norm_area"].min(),
        isea3h_gpd["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
    isea3h_gpd = isea3h_gpd[
        ~isea3h_gpd["crossed"]
    ]  # remove cells that cross the dateline
    isea3h_gpd.to_crs("proj=moll").plot(
        column="norm_area",
        ax=ax,
        norm=norm,
        legend=True,
...
def isea3h_norm_area_hist(isea3h_gpd):
    ...
    counts, bins, patches = ax.hist(
        isea3h_gpd_filtered["norm_area"], bins=50, alpha=0.7, edgecolor="black"
    )

    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = (
        isea3h_gpd_filtered["norm_area"].min(),
        isea3h_gpd_filtered["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def isea3h_compactness_hist(isea3h_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(
        isea3h_gpd_filtered["ipq"], bins=50, alpha=0.7, edgecolor="black"
    )
