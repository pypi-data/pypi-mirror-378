...
def olcinspect(resolution):
    ...
    """
    olc_gpd = olcgrid(resolution, output_format="gpd")
    ...
    olc_gpd["zsc"] = (
        np.sqrt(
            4 * np.pi * olc_gpd["cell_area"]
            - np.power(olc_gpd["cell_area"], 2) / np.power(6378137, 2)
        )
        / olc_gpd["cell_perimeter"]
    )
    return olc_gpd
...
def olc_norm_area_hist(olc_gpd):
    ...
    counts, bins, patches = ax.hist(
        olc_gpd_filtered["norm_area"], bins=50, alpha=0.7, edgecolor="black"
    )

    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = (
        olc_gpd_filtered["norm_area"].min(),
        olc_gpd_filtered["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def olc_compactness_hist(olc_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(
        olc_gpd_filtered["ipq"], bins=50, alpha=0.7, edgecolor="black"
    )
