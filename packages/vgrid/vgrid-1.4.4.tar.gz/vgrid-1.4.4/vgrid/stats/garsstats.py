...
def garsinspect(res):  # length unit is km, area unit is km2
    ...
    gars_gpd["ipq"] = (
        4 * np.pi * gars_gpd["cell_area"] / (gars_gpd["cell_perimeter"] ** 2)
    )
    # Calculate zonal standardized compactness
    gars_gpd["zsc"] = (
        np.sqrt(
            4 * np.pi * gars_gpd["cell_area"]
            - np.power(gars_gpd["cell_area"], 2) / np.power(6378137, 2)
        )
        / gars_gpd["cell_perimeter"]
    )
    return gars_gpd
...
def gars_norm_area(gars_gpd):
    ...
    gars_gpd.to_crs("proj=moll").plot(
        column="norm_area",
        ax=ax,
        norm=norm,
        legend=True,
...
def gars_norm_area_hist(gars_gpd):
    ...
    counts, bins, patches = ax.hist(
        gars_gpd_filtered["norm_area"], bins=50, alpha=0.7, edgecolor="black"
    )

    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = (
        gars_gpd_filtered["norm_area"].min(),
        gars_gpd_filtered["norm_area"].max(),
        1,
    )
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def gars_compactness_hist(gars_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(
        gars_gpd_filtered["ipq"], bins=50, alpha=0.7, edgecolor="black"
    )
