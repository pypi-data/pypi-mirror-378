...
Typical Usage:
    ```python
    from pyballistic import Calculator, Shot, DragModel
...
See Also:
    - pyballistic.interface: Calculator class to generate HitResults
    - pyballistic.unit: Unit system for all measurement values
    - pyballistic.vector: Vector mathematics for position/velocity
"""
...
from pyballistic.unit import Angular, Distance, Energy, Velocity, Weight, GenericDimension, Unit, PreferredUnits
from pyballistic.vector import Vector
from pyballistic.interpolation import (
    InterpolationMethod,
    interpolate_3_pt,
    interpolate_2_pt,
)
...
class DangerSpace(NamedTuple):
    ...
    def overlay(self, ax: Axes, label: Optional[str] = None) -> None:
        ...
        """
        try:
            from pyballistic.visualize.plot import add_danger_space_overlay  # type: ignore[attr-defined]
            add_danger_space_overlay(self, ax, label)
        except ImportError as err:
            raise ImportError(
                "Use `pip install pyballistic[charts]` to get results as a plot"
            ) from err
...
class HitResult:
    ...
    * Use base_data for interpolation if present
    ...
    def dataframe(self, formatted: bool = False) -> DataFrame:
        ...
        """
        try:
            from pyballistic.visualize.dataframe import hit_result_as_dataframe
            return hit_result_as_dataframe(self, formatted)
        except ImportError as err:
            raise ImportError(
                "Use `pip install pyballistic[charts]` to get trajectory as pandas.DataFrame"
            ) from err
    ...
    def plot(self, look_angle: Optional[Angular] = None) -> Axes:
        ...
        """
        try:
            from pyballistic.visualize.plot import hit_result_as_plot  # type: ignore[attr-defined]
