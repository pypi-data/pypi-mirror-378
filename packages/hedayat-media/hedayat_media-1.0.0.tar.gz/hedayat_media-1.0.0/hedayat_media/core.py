...
class HedayatMedia:
    ...
    def fetch_random_ayah(self, juz_number: int):
        ...
        if response.status_code == 200:
            ...
            return {"error": "خطا در دریافت داده‌ها از API"}
    ...
    def get_random_ayah(self):
        ...
        if response.status_code == 200:
            ...
            return {"error": "خطا در دریافت آیه"}

    def get_all_surahs(self):
        """دریافت لیست کامل سوره‌ها"""
        response = requests.get(API_SURAH_URL)
        if response.status_code == 200:
            return response.json()['data']
        return {"error": "خطا در دریافت سوره‌ها"}
    ...
    def get_surah(self, surah_number: int):
        ...
        if response.status_code == 200:
            ...
            return {"error": "خطا در دریافت سوره"}
