"""XNAT Custom Forms Data Extractor.

This module extracts custom form data from XNAT projects and saves it to
Excel files. It supports batch processing to handle large projects
efficiently while managing XNAT session limits.
"""
...
def setup_logging(logs_folder: str, project_id: str) -> None:
    ...
    log_file = os.path.join(
        logs_folder,
        f"{datetime.now(melbtz).strftime('%Y-%m-%d_%H-%M-%S')}_{project_id}_get.log"
    )
...
app = typer.Typer(help="Extract custom form data from XNAT projects")
...
def get_project_subjects_and_sessions_batch(
    session, project_id: str, subject_batch: List[str],
    server: str = None, username: str = None,
    password: str = None
) -> Dict[str, Dict[str, Any]]:
    """Get subjects and their sessions from a project batch.
    
    Args:
        session: XNAT session object
        project_id: XNAT project ID
        subject_batch: List of subject IDs to process
        server: XNAT server URL (optional, for API fallback)
        username: XNAT username (optional, for API fallback)
        password: XNAT password (optional, for API fallback)
        
    Returns:
        Dictionary mapping subject IDs to subject information including
        sessions
    """
    project_data = {}
    ...
    try:
        ...
        for subject_id in subject_batch:
            if subject_id not in project.subjects:
                logging.warning(f"Subject {subject_id} not found in "
            ...
            for session_id, exp_session in subject.experiments.items():
                session_label = exp_session.label
                
                # Try multiple ways to get the session type
                session_type = None
                
                # Method 1: Try xsitype attribute
                if hasattr(exp_session, 'xsitype'):
                    session_type = exp_session.xsitype
                
                # Method 2: Try xsi_type attribute
                elif hasattr(exp_session, 'xsi_type'):
                    session_type = exp_session.xsi_type
                
                # Method 3: Try accessing via data attribute
                elif (hasattr(exp_session, 'data') and
                      hasattr(exp_session.data, 'get')):
                    session_type = (exp_session.data.get('xsiType') or
                                   exp_session.data.get('xsi:type'))
                
                # Method 4: Try accessing via attrs
                elif hasattr(exp_session, 'attrs'):
                    session_type = (exp_session.attrs.get('xsi:type') or
                                   exp_session.attrs.get('xsiType'))
                
                # Method 5: Try via REST API if other methods failed and we
                # have API access
                if not session_type and server and username and password:
                    session_type = get_session_type_via_api(
                        server, username, password, project_id, subject_id,
                        session_id)
                
                # Method 6: Check the class name as fallback
                if not session_type:
                    class_name = exp_session.__class__.__name__
                    if 'MR' in class_name.upper():
                        session_type = 'xnat:mrSessionData'
                    elif 'CT' in class_name.upper():
                        session_type = 'xnat:ctSessionData'
                    elif 'PET' in class_name.upper():
                        session_type = 'xnat:petSessionData'
                    elif 'US' in class_name.upper():
                        session_type = 'xnat:usSessionData'
                    # Add other common types as needed
                
                project_data[subject_id]['sessions'][session_id] = {
                    'label': session_label,
                    'type': session_type
                }
                
        logging.info(f"Processed batch: {len(project_data)} subjects, "
                    f"{sum(len(subj['sessions']) for subj in project_data.values())} sessions")
        
    except Exception as e:
        logging.error(f"Error fetching project data batch: {e}")
...
# Testing XNAT connection function
...
def is_form_applicable_to_level(form: Dict, level: str) -> bool:
    """Check if a form is applicable to subject or session level.
    
    Args:
        form: Form definition dictionary
        level: "subject" or "session"
        
    Returns:
        True if form applies to the specified level
    """
    path = form.get("path", "")
    if level == "subject":
        return "xnat:subjectData" in path
    elif level == "session":
        return get_form_session_type_from_path(path) is not None
    return False


def get_session_type_via_api(server: str, username: str, password: str,
                            project_id: str, subject_id: str,
                            session_id: str) -> Optional[str]:
    """Fetch session type directly via XNAT REST API as fallback.
    ...
    Returns:
        Session type string or None if not found
    """
    try:
        api_url = (f"{server}data/projects/{project_id}/subjects/"
        ...
        if response.status_code == 200:
            ...
            # The session type is usually in the 'xsiType' field
            items = session_data.get('items', [])
            if items:
                return items[0].get('xsiType')
    except Exception as e:
        logging.debug(f"Failed to fetch session type via API for "
                     f"{session_id}: {e}")
...
def setup_folder_structure(base_path: str, server_name: str, project_id: str) -> Tuple[str, str]:
    ...
    """
    # Strip surrounding quotes from base_path if present
...
def save_form_data_to_excel(form_data_collection: Dict[str, List[Dict]],
                           form_definitions: List[Dict], excel_folder: str) -> None:
    """Save each form's data to separate Excel files.
    ...
    for form_uuid, data_rows in form_data_collection.items():
        ...
        filename = f"{safe_title}_{form_uuid[:8]}.xlsx"
        ...
        try:
            ...
            logging.error(f"Error saving Excel file for form {form_uuid}: "
                         f"{e}")


def display_and_select_forms(form_definitions: List[Dict]) -> List[Dict]:
    """Display forms and allow user to select which ones to process.
    
    Args:
        form_definitions: List of available form definitions
        
    Returns:
        List of selected form definitions
    """
    if not form_definitions:
        logging.error("No forms found that are applicable to this project.")
        return []
    
    print("\n" + "="*80)
    print("AVAILABLE CUSTOM FORMS (Applicable to this project)")
    print("="*80)
    
    for i, form in enumerate(form_definitions, 1):
        form_uuid = form["formUUID"]
        form_title = extract_form_title_from_contents(
            form.get("contents", ""))
        path = form.get("path", "Unknown path")
        
        # Determine form level and session type if applicable
        if "subjectData" in path:
            level = "Subject"
        else:
            session_type = get_form_session_type_from_path(form.get("path", ""))
            if session_type:
                level = f"Session ({session_type})"
            else:
                level = "Unknown"
        
        print(f"{i:2d}. {form_title}")
    ...
    while True:
        selection = input("Enter form numbers to process (e.g., '1,3,5' "
                         "or 'all' for all forms): ").strip()
        ...
        try:
            ...
            for idx in indices:
                if 0 <= idx < len(form_definitions):
                    selected_forms.append(form_definitions[idx])
                else:
                    print(f"Invalid index: {idx + 1}. Please try again.")
                    break
            else:
                if selected_forms:
                    print(f"\nSelected {len(selected_forms)} form(s) for "
                          f"processing.")
                    return selected_forms
                else:
                    print("No valid forms selected. Please try again.")
        except ValueError:
            print("Invalid input format. Please enter comma-separated "
                 "numbers or 'all'.")
...
def main(
    ...
    """Extract custom form data from entire XNAT project and save to Excel.
    
    Processes all subjects in the specified XNAT project, extracting custom
    form data in batches to prevent server overload. Data is saved to Excel
    files with proper session management and error handling.
    
    Args:
        server: XNAT server URL (can include or exclude http:// prefix).
        username: XNAT username for authentication.
        password: XNAT password for authentication.
        project_id: XNAT project identifier to process.
        form_uuid: Specific form UUID to process (empty string for all).
    ...
    Note:
        This function implements batch processing with session management
        to prevent XNAT server crashes during large data extractions.
    """
    # If called directly without parameters, get them from environment or prompt
    if not all([server, username, password, project_id]):
        typer.echo("Error: Missing required parameters when called as function")
        raise typer.Exit(1)
    ...
    try:
        ...
        # Note: We'll create fresh sessions per batch to avoid timeout issues
        
        # Fetch all form definitions and filter by project applicability (with caching)
        ...
        if not form_definitions:
            logging.error(
                "No form definitions found that are applicable to this "
                "project. Exiting."
            )
            return
        
        # Filter by specific UUID if provided
        if form_uuid:
            form_definitions = [
                form for form in form_definitions 
                if form["formUUID"] == form_uuid
            ]
            if not form_definitions:
                logging.error(
                    f"Form with UUID {form_uuid} not found or not "
                    f"applicable to project {project_id}."
                )
                return
            selected_forms = form_definitions
        else:
            # Interactive form selection
            selected_forms = display_and_select_forms(form_definitions)
            
        if not selected_forms:
            logging.info("No forms selected. Exiting.")
            return
        
        # OPTIMIZATION: Fetch ALL project data (subjects + sessions) at once
        logging.info("Fetching all project data (subjects and sessions) in one optimized call...")
        initial_session = get_optimized_session()
        ...
        if not all_project_data:
            logging.error("No subjects found in project. Exiting.")
            return
        
        # Get subject IDs for batch processing
        all_subject_ids = list(all_project_data.keys())
        
        # Check if sessions were found - if not, fall back to batch processing
        total_sessions = sum(len(subj['sessions']) for subj in all_project_data.values())
        if total_sessions == 0:
            logging.warning("No sessions found with optimized approach. Falling back to original batch processing for sessions...")
            # Keep the optimization for subjects but fall back to batch for sessions
            use_optimized_sessions = False
        else:
            use_optimized_sessions = True
            logging.info(f"Successfully optimized: {len(all_subject_ids)} subjects with {total_sessions} sessions")
        
        # OPTIMIZATION: Pre-process all form metadata once
        logging.info("Pre-processing form metadata for optimized processing...")
        form_metadata = {}
        for form in selected_forms:
            ...
            # Use cached metadata processing
            ...
            logging.debug(f"Pre-processed form: {form_title} ({form_uuid}) - {level}")
        
        # Initialize form data collection
        master_form_data_collection = {
            form["formUUID"]: [] for form in selected_forms
        }
        
        # Process subjects in batches
        total_batches = (len(all_subject_ids) + batch_size - 1) // batch_size
        logging.info(
            f"Processing {len(all_subject_ids)} subjects in "
        ...
        for batch_num in range(total_batches):
            ...
            logging.info(
                ...
                f"subjects {start_idx + 1}-{end_idx}"
            )
            
            try:
                if use_optimized_sessions:
                    # OPTIMIZATION: Use pre-fetched data instead of new XNAT connections
                    batch_project_data = {
                    ...
                    # Create fresh session for this batch to avoid auth timeouts
                    batch_session = get_optimized_session()
                    
                    # OPTIMIZATION: Use pre-processed form metadata
                    batch_form_data = collect_form_data_optimized(
                        server, username, password, project_id, 
                        batch_project_data, form_metadata, batch_session
                    )
                else:
                    # FALLBACK: Use original XNAT library approach for sessions
                    logging.debug(f"Using fallback approach for batch {batch_num + 1}")
                    with xnat.connect(server, username, password=password) as xnat_session:
                        # Get project data for this batch using original method
                        batch_project_data = get_project_subjects_and_sessions_batch(
                            xnat_session, project_id, subject_batch, server, username, password
                        )
                        ...
                        # Use optimized collection with pre-processed metadata
                        batch_form_data = collect_form_data_optimized(
                            server, username, password, project_id, 
                            batch_project_data, form_metadata, batch_session
                        )
                ...
                # CRITICAL: Close all active sessions after each batch to prevent XNAT crashes
                logging.debug(f"Closing all active sessions after batch {batch_num + 1} to prevent XNAT overload...")
                ...
                # Try to logout sessions even if batch failed
                try:
                    logging.info(f"Attempting to logout sessions after batch {batch_num + 1} failure...")
                    logout_all_sessions(server, username, password)
                except Exception:
                    logging.error(f"Failed to logout sessions after batch {batch_num + 1} error")
                continue
        
        # Save all collected data to Excel files
        logging.info("Saving all collected data to Excel files...")
        save_form_data_to_excel(master_form_data_collection, selected_forms, excel_folder)
        
        # Log final statistics
        total_records = sum(
            len(data) for data in master_form_data_collection.values()
        )
        logging.info(
            f"Total records collected across all forms: {total_records}"
        )
        ...
        for form in selected_forms:
            ...
            record_count = len(
                master_form_data_collection.get(form_uuid, [])
            )
            logging.info(f"  {form_title}: {record_count} records")
        
        # Final logout of all sessions
        logging.info("Logging out all sessions at completion...")
        logout_all_sessions(server, username, password)
        
        logging.info("Project-wide extraction completed successfully!")
        
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        # Attempt to logout sessions even if there was an error
        try:
            logout_all_sessions(server, username, password)
        except Exception:
...
def cli_main() -> None:
    """CLI entry point using Typer for command-line argument parsing.
    
    This function sets up the Typer CLI interface and calls the main
    processing function with the provided arguments.
    """
    app = typer.Typer(
        help="Extract custom form data from XNAT project and save to Excel"
    )
    ...
    def extract(
        server: str = typer.Option(
            ...
            "--server",
            "-x",
            help="XNAT server URL (e.g., http://localhost:8080/xnat)"
        ),
        ...
        project_id: str = typer.Option(
            ...
            "--project",
            "-pid",
            help="XNAT project ID to process"
        ),
        form_uuid: str = typer.Option(
            "",
            "--form-uuid",
            "-f",
            help="Specific form UUID to process (leave empty for all forms)"
        ),
        ...
        output_dir: str = typer.Option(
            ...
            "--output-dir",
            "-o",
            help="Base output directory for files"
        )
    ) -> None:
        """Extract custom form data from XNAT project.
        
        This command processes all subjects in the specified XNAT project,
        extracting custom form data in batches and saving to Excel files.
        
        Args:
            server: XNAT server URL.
            username: XNAT username.
            password: XNAT password (will be prompted securely).
            project_id: XNAT project identifier.
            form_uuid: Specific form UUID (optional).
            batch_size: Subjects per batch (1-100).
            session_delay: Delay between batches in seconds (0-60).
        """
