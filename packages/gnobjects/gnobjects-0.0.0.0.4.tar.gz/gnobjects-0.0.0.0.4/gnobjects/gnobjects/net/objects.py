...
from ._transport_pr_obj import GNProtocol
...
class Url:
    ...
    def build(self, parts: List[str]) -> str:
        ...
        if "hostname" in parts and self.hostname:
            if ":" in self.hostname:
                ...
                host = self.hostname
            if "port" in parts and self.port is not None:
    ...
    def toString(self) -> str:
        return self.build(["transport", "route", "scheme", "hostname", "port", "path", "params", "fragment"])
...
class CORSObject:
    def __init__(self,
                 ...
                 allow_transport_protocols: List[str] = ["*"],
                 allow_route_protocols: List[str] = ["*"],
                 allow_request_protocols: List[str] = ["*"]
                 ) -> None:
    ...
    def serialize(self) -> Dict[int, Union[str, bool, List[str]]]:
        return {
            0: self.allow_origins,
            1: self.allow_methods,
        
            2: self.allow_transport_protocols,
            3: self.allow_route_protocols,
            4: self.allow_request_protocols
        }

    @staticmethod
    def deserialize(data: Dict[int, Any]) -> 'CORSObject':
        return CORSObject(
            allow_origins=data.get(0, []),
            allow_methods=data.get(1, ['*']),

            allow_transport_protocols=data.get(2, ['*']),
            allow_route_protocols=data.get(3, ['*']),
            allow_request_protocols=data.get(4, ['*'])
        )
...
class GNRequest:
    ...
    @property
    def transportObject(self) -> Optional['GNProtocol']:
        """
        ...
        """
        return GNProtocol(self._transport) if self._transport else None
    ...
    def setRoute(self, route: Optional[str] = None):
        ...
        :param route: Маршрут запроса (например, `gn:proxy:origin`).

        Если не указан, используется `gn:proxy:origin`.
        """
        if route is None:
            route = 'gn:proxy:origin'
        self._route = route
...
class CommandObject(AllGNFastCommands):
    ...
    def __bool__(self):
        ...
        if isinstance(self.value, bool):
            ...
            return self.value in self._ok_values
