...
impl IOOptimizedStyleData {
    ...
    pub fn load_from_parquet_io_optimized(path: &str) -> PyResult<Self> {
        ...
        // 优化批处理大小 - 根据文件大小动态调整
        let optimal_batch_size = if file_size > 100 * 1024 * 1024 { // > 100MB
            32768  // 大文件使用更大的批处理
        ...
        println!("📊 读取完成: {}行数据, {}个批次", total_rows, all_data.len());

        // 预分配HashMap以减少重新哈希
        let estimated_dates = (total_rows / 1000).max(100); // 估算日期数量
        let mut data_by_date: HashMap<i64, Vec<(String, Vec<f64>)>> = HashMap::with_capacity(estimated_dates);
        ...
        println!("   📊 统计: {}个交易日, {}只股票", final_data_by_date.len(), total_stocks_processed);
        println!("   ⏱️  总耗时: {:.3}s", total_time.as_secs_f64());
        println!("   🚀 I/O速度: {:.1}MB/s", file_size as f64 / 1024.0 / 1024.0 / total_time.as_secs_f64());
...
fn load_factor_file_io_optimized(file_path: &Path, log_detailed: bool) -> PyResult<IOOptimizedFactorData> {
    ...
    // 创建股票索引映射
    let stock_index_map: HashMap<String, usize> = stocks.iter()
        .enumerate()
    ...
    if all_batches.len() > 4 {
        ...
        for batch in all_batches {
            let (batch_data, batch_dates, batch_has_nan) = process_factor_batch_optimized(&batch, date_col_idx, &stock_col_map, n_stocks)?;
            all_data.extend(batch_data);
            dates.extend(batch_dates);
            has_nan = has_nan || batch_has_nan;
        }
...
pub fn batch_factor_neutralization_io_optimized(
    ...
    let style_data = Arc::new(IOOptimizedStyleData::load_from_parquet_io_optimized(style_data_path)?);
    ...
    let factor_files: Vec<PathBuf> = factor_files_with_size.into_iter().map(|(path, _)| path).collect();
    ...
    let optimal_threads = if let Some(threads) = num_threads {
        ...
        let memory_gb = sys_info::mem_info().map(|info| info.total / 1024 / 1024).unwrap_or(8);
...
fn neutralize_single_factor_io_optimized(
    ...
    for (date_idx, &date) in factor_data.dates.iter().enumerate() {
        if let Some(day_data) = style_data.data_by_date.get(&date) {
            if let Ok(day_values) = process_single_date_io_optimized(
                date_idx, &factor_data, day_data, &union_stocks
            ) {
