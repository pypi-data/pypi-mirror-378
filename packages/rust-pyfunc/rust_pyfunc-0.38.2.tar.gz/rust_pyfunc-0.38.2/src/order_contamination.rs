...
pub fn order_contamination_bilateral(
    ...
    let bid_threshold = if bid_top_count > 0 && bid_top_count <= bid_volumes.len() {
        ...
        bid_volumes.get(bid_top_count.saturating_sub(1)).cloned().unwrap_or(0)
    };
    ...
    let ask_threshold = if ask_top_count > 0 && ask_top_count <= ask_volumes.len() {
        ...
        ask_volumes.get(ask_top_count.saturating_sub(1)).cloned().unwrap_or(0)
    };
    ...
    // 4. 大买单浸染卖单非大单的过程
    let large_bid_times: Vec<i64> = large_bid_positions.iter()
        .map(|&pos| exchtime[pos])
        .collect();
    ...
    for i in 0..n {
        ...
        // 使用二分查找找到窗口边界
        let left_idx = large_bid_times.binary_search(&window_start)
            .unwrap_or_else(|i| i);
        let right_idx = large_bid_times.binary_search(&window_end)
            .unwrap_or_else(|i| i);
    ...
    // 5. 大卖单浸染买单非大单的过程
    let large_ask_times: Vec<i64> = large_ask_positions.iter()
        .map(|&pos| exchtime[pos])
        .collect();
    ...
    for i in 0..n {
        ...
        // 使用二分查找找到窗口边界
        let left_idx = large_ask_times.binary_search(&window_start)
            .unwrap_or_else(|i| i);
        let right_idx = large_ask_times.binary_search(&window_end)
            .unwrap_or_else(|i| i);
