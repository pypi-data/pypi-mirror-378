...
pub fn trade_peak_analysis(
    ...
    let tier1_threshold = volume_sorted[((volume_sorted.len() as f64 * top_tier1) as usize).min(volume_sorted.len() - 1)];
    
    // 第二步：找到前top_tier2%的大成交量阈值
    let tier2_threshold = volume_sorted[((volume_sorted.len() as f64 * top_tier2) as usize).min(volume_sorted.len() - 1)];
...
/// 检查是否是局部高峰
fn is_local_peak(idx: usize, exchtime: &ArrayView1<i64>, volume: &ArrayView1<f64>, time_window_ns: i64) -> bool {
...
fn calculate_kurtosis(data: &[f64], mean: f64, std: f64) -> f64 {
    ...
    let n = data.len() as f64;
    let kurt: f64 = data.iter()
        .map(|&x| ((x - mean) / std).powi(4))
        .sum::<f64>() / n;
    
    // 使用Fisher的修正公式，与scipy.stats.kurtosis一致
    if data.len() > 3 {
        let adj_factor = ((n - 1.0) / ((n - 2.0) * (n - 3.0))) * 
