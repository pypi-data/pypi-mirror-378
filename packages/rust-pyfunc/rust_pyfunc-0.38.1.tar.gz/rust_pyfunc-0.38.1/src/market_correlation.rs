...
pub fn price_volume_orderbook_correlation(
    ...
    // 将纳秒转换为秒
    let trade_times: Vec<f64> = exchtime_trade.iter().map(|&t| t / 1_000_000_000.0).collect();
...
fn calculate_interval_correlation(
    ...
    // 构建对应的数量序列
    let series_t: Vec<f64> = common_prices.iter()
        .map(|&price| {
    ...
    let series_a: Vec<f64> = common_prices.iter()
        .map(|&price| {
            let key = (price * 1000.0) as i64;
    ...
    let series_b: Vec<f64> = common_prices.iter()
        .map(|&price| {
            let key = (price * 1000.0) as i64;
    ...
    let series_diff: Vec<f64> = series_a.iter().zip(series_b.iter())
        .map(|(a, b)| a - b)
        .collect();
