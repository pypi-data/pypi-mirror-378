...
pub fn analyze_sequence_permutations_v0816_fixed(
    ...
    for i in 0..n_windows {
        ...
        let indicators = compute_window_indicators_fixed(&window_data.to_owned(), window_size, n_clusters);
...
fn compute_window_indicators_fixed(window_data: &Array1<f64>, window_size: usize, n_clusters: usize) -> Vec<f64> {
    // 检查唯一值数量
    ...
    let cluster_size_entropy = compute_entropy_fixed(&cluster_sizes.iter().map(|&x| x as f64).collect::<Vec<_>>());
        ...
        for point_idx in 0..n_points {
            ...
            for cluster_idx in 0..k {
                ...
                for feature_idx in 0..n_features {
                    let diff = data[[point_idx, feature_idx]] - centroids[[cluster_idx, feature_idx]];
                    distance += diff * diff;
                }
...
fn compute_intra_cluster_distances_fixed(
    data: &Array2<f64>,
    assignments: &[usize],
) -> Vec<f64> {
    let n_clusters = assignments.iter().max().unwrap_or(&0) + 1;
        ...
        if cluster_points.len() > 1 {
            ...
            for i in 0..cluster_points.len() {
                for j in i + 1..cluster_points.len() {
                    let mut distance = 0.0;
                    for feature_idx in 0..n_features {
                        let diff = data[[cluster_points[i], feature_idx]] - data[[cluster_points[j], feature_idx]];
                        distance += diff * diff;
                    }
                    total_distance += distance.sqrt();
                    count += 1;
