...
pub fn fast_correlation_matrix(
    ...
    let compute_correlations_with_pool = |f: Box<dyn FnOnce() -> Vec<((usize, usize), f64)> + Send>| {
        if let Some(ref pool) = pool {
            pool.install(|| f())
        } else {
            f()
    ...
    let correlations = compute_correlations_with_pool(Box::new(move || {
        upper_triangle_indices
            ...
            .map(|(i, j)| {
                ...
                let corr = compute_correlation_fast(&col_i, &col_j, &col_stats_arc[i], &col_stats_arc[j], min_periods);
                ((i, j), corr)
            })
            .collect()
    }));
