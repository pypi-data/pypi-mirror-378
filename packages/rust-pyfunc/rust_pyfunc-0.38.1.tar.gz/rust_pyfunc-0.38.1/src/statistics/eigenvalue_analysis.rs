...
pub fn matrix_eigenvalue_analysis(
    ...
    if n_rows != 237 {
        return Err(pyo3::exceptions::PyValueError::new_err(
            format!("输入矩阵必须有237行，但得到了{}行", n_rows)
        ));
    }
...
/// 为单列数据计算差值矩阵的特征值
fn compute_column_eigenvalues(column: &ArrayView1<f64>) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let n = column.len();
...
/// - 我们可以利用这些性质来加速计算
fn compute_column_eigenvalues_optimized(column: &ArrayView1<f64>) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let n = column.len();
