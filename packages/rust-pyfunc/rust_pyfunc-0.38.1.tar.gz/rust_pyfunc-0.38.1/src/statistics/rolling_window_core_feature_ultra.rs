...
pub fn rolling_window_core_feature_ultra(
    ...
    for current_idx in (window_size - 1)..n {
        ...
        compute_base_correlations_ultra(
            &values, 
            window_start, 
            window_size, 
            &mut base_correlations
        );
        ...
        for mask_pos in 0..window_size {
            ...
            // 计算相关性变化程度
            let impact = calculate_correlation_impact_ultra(&base_correlations, &masked_correlations);
...
fn compute_base_correlations_ultra(
    ...
    for other_start in 0..num_windows {
        ...
        let corr = compute_correlation_ultra_fast(
            values, 
...
/// 改进稀疏数据下的稳健性
#[inline(always)]
fn calculate_correlation_impact_ultra(base_correlations: &[f64], masked_correlations: &[f64]) -> f64 {
    ...
    for (&base, &masked) in base_correlations.iter().zip(masked_correlations.iter()) {
        // 跳过NaN值或无意义的对比
        if base == base && masked == masked {  // 快速NaN检查
