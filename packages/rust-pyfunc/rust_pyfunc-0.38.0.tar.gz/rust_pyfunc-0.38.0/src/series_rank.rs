...
/// 单线程排名算法实现
fn rank_series_single_thread(data: Vec<f64>, method: &str, ascending: bool, na_option: &str) -> Vec<f64> {
    let n = data.len();
    if n == 0 {
        return vec![];
    }
...
/// 分配非NaN值的排名
fn assign_ranks(result: &mut Vec<f64>, valid_data: &[(usize, f64)], method: &str, rank_offset: f64) {
    match method {
        "average" => {
