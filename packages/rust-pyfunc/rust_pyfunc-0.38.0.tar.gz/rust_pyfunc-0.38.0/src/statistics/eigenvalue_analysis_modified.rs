...
impl FilterStats {
    ...
    fn print_summary(&self, total_cols: usize) {
        ...
        if self.timeout_errors > 0 {
            println!("  - 计算超时或跳过: {} ({:.1}%)", self.timeout_errors, self.timeout_errors as f64 / total_cols as f64 * 100.0);
        }
    ...
    for iteration in 0..MAX_ITERATIONS {
        ...
        let shift = if n > 1 {
            ...
            if discriminant >= 0.0 {
                ...
                // 选择更接近d的特征值
                if (lambda1 - d).abs() < (lambda2 - d).abs() { lambda1 } else { lambda2 }
            } else {
...
/// 快速近似特征值计算 - 用于中等大小矩阵
/// 使用采样和降维技术避免完整计算
fn compute_eigenvalues_fast_approximation(column: &ArrayView1<f64>) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let n = column.len();
...
fn compute_simplified_eigenvalues(column: &ArrayView1<f64>) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let n = column.len();
    let mut result = vec![0.0; n];
