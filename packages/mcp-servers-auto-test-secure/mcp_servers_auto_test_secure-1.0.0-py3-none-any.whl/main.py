...
async def run_once(quick_mode: bool = False, server_name: str = None, test_mode: str = "parallel", proxy_mode: bool = False, user_id: str = None, local_mode: bool = False):
    ...
    try:
        ...
        if local_mode:
            # Use local database - requires user to set MONGODB_URI environment variable
            config.MONGODB_URI = os.getenv("MONGODB_URI", "")
            if not config.MONGODB_URI:
                print("❌ Error: Local mode requires MONGODB_URI environment variable")
                print("   Please set: export MONGODB_URI='mongodb://user:pass@127.0.0.1:27017/mcpmarket?authSource=admin'")
                return
            print("🏠 Using local database")
        else:
            # Use remote database - requires user to set MONGODB_REMOTE_URI environment variable
            config.MONGODB_URI = os.getenv("MONGODB_REMOTE_URI", "")
            if not config.MONGODB_URI:
                print("❌ Error: Remote mode requires MONGODB_REMOTE_URI environment variable")
                print("   Please contact the administrator for database access credentials")
                print("   Then set: export MONGODB_REMOTE_URI='mongodb://user:pass@host:port/database'")
                return
...
def main():
    ...
    parser.add_argument(
        "--remote",
        action="store_true",
        help="Use remote database - this is the default"
    )
    parser.add_argument(
        "--local",
        action="store_true",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="mcp-servers-auto-test-secure 1.0.0"
    )
