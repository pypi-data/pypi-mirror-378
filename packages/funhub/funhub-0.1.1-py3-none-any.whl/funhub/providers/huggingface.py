...
class HuggingFaceProvider(BaseProvider):
    ...
    def __init__(self, drive=None):
        ...
        self.download_base = "https://huggingface.co"

    def sync_repo_to_drive(
        self, user: str, repo: str, branch: str = "main", force: bool = False
    ) -> SyncResult:
        """
        将HuggingFace仓库同步到fundrive

        Args:
            user: HuggingFace用户名/组织名
            repo: 仓库名
            branch: 分支名
            force: 是否强制重新同步
        ...
        try:
            ...
            if fid:
                self.logger.success(
                    f"HuggingFace仓库同步成功: {user}/{repo}, fid: {fid}"
                )
    ...
    def get_repo_info(self, user: str, repo: str) -> Dict:
        ...
        try:
            ...
            return {
                "name": data.get("id", "").split("/")[-1] if data.get("id") else repo,
