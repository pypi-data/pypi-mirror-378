...
class KaggleNotebookDownloader:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    cmd_str = f'kaggle kernels pull {kernel_slug}'
                    if self.dev_mode:
                    ...
                    if result.returncode == 0:
                        ...
                        if self.dev_mode:
                            logger.debug(f"Looking for .ipynb files in: {temp_dir}")
                        ...
                        if ipynb_files:
                            ...
                            ipynb_file = ipynb_file.absolute()
                            target_file = target_file.absolute()

                            if self.dev_mode:
                                logger.debug(f"Source file: {ipynb_file}")
                                logger.debug(f"Target file: {target_file}")
                                logger.debug(f"Source exists: {ipynb_file.exists()}")
                                logger.debug(f"Target dir exists: {target_file.parent.exists()}")
                            ...
                            if self.dev_mode:
                                logger.debug(f"About to copy from: {str(ipynb_file)}")
                                logger.debug(f"About to copy to: {str(target_file)}")

                            try:
                                shutil.copy2(str(ipynb_file), str(target_file))

                                if self.dev_mode:
                                    logger.debug(f"Copy operation completed")
                                    logger.debug(f"Checking if target exists immediately after copy: {target_file.exists()}")
                                    logger.debug(f"Target file size: {target_file.stat().st_size if target_file.exists() else 'N/A'}")
                                ...
                                if self.dev_mode:
                                    import traceback
                                    logger.debug(f"Copy error traceback: {traceback.format_exc()}")
                                return False
                        else:
                            # List all files in temp directory for debugging
                            if self.dev_mode:
                                all_files = list(Path(temp_dir).glob("*"))
                                logger.debug(f"All files in temp dir: {all_files}")
    ...
    def _convert_notebook_to_python_file(self, notebook: NotebookInfo, output_dir: Path) -> bool:
        ...
        try:
            ...
            if self.dev_mode:
                logger.debug(f"Looking for notebook file: {ipynb_file}")
                logger.debug(f"Absolute path: {ipynb_file.absolute()}")
                logger.debug(f"File exists: {ipynb_file.exists()}")
...
'''
    ...
    async def download_competition_notebooks(self, competition_url: str, limit: Optional[int] = None, output_dir: Optional[Path] = None) -> bool:
        ...
        if output_dir is None:
            output_dir = Path("kaggle_notebooks_downloaded").absolute()
