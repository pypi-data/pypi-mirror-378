...
class EmbeddingModelLoader:
    ...
    def start_loading(self):
        def load():
            # Check if HF_ENDPOINT environment variable is set and configure it
            hf_endpoint = os.environ.get('HF_ENDPOINT')
            if hf_endpoint:
                # Set the Hugging Face Hub endpoint for model downloads
                os.environ['HF_ENDPOINT'] = hf_endpoint
                logging.info(f"Using Hugging Face endpoint: {hf_endpoint}")
