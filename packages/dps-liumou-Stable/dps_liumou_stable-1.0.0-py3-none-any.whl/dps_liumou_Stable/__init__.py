...
class DockerPullSmart:
    ...
    def pull_image_with_mirror(self, image_name: str, mirror_url: str) -> bool:
        """使用镜像源拉取镜像"""
        # 构建镜像源地址
        mirror_image = f"{mirror_url}/{image_name}"
    ...
    def smart_pull(self, image_name: str, max_retries: int = 3, timeout: int = 300) -> bool:
        """智能拉取镜像"""
...
使用示例:
  ...
  %(prog)s -h                              # 显示帮助信息
    ...
    if args.list_mirrors:
        ...
        success = tool.smart_pull(args.image_name, max_retries=args.max_retries, timeout=args.timeout)
