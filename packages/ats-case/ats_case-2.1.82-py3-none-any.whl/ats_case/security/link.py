...
def _gw698(context: Context, protocol: str, retry_times=3):
    for i in range(retry_times):
        ...
        try:
            ...
            frame = base.write(context, frame=result.get('frame')).get("frame", None)
    ...
    try:
        ...
        mac = pro.encode(
            func.to_dict(protocol=protocol, comm_addr=context.meter.addr,
                         ...
                         element='',
                         session_key=context.test_sn))
        frame = base.write(context, frame=mac.get('frame')).get("frame", None)
    ...
    try:
        ...
        srnm = pro.encode(
            func.to_dict(protocol=protocol, comm_addr=context.meter.addr,
                         ...
                         element='',
                         session_key=context.test_sn))
        frame = base.write(context, frame=srnm.get('frame')).get("frame", None)
        pro.decode(func.to_dict(protocol=protocol, frame=frame, session_key=context.test_sn))
    ...
    try:
        ...
        disc = pro.encode(
            func.to_dict(protocol=protocol, comm_addr=context.meter.addr,
                         ...
                         element='',
                         session_key=context.test_sn))
        frame = base.write(context, frame=disc.get('frame')).get("frame", None)
        pro.decode(func.to_dict(protocol=protocol, frame=frame, session_key=context.test_sn))
    finally:
