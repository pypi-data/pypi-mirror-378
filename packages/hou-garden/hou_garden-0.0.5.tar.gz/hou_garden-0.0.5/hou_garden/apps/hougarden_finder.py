...
class HouGardenFinderService:
    ...
    async def _run_search(self, query: str, page: int, page_size: int, sort: int) -> Dict[str, Any]:
        def _blocking_call() -> Dict[str, Any]:
            return _query_hougarden_info_core(query=query, page=page, page_size=page_size, sort=sort)
    ...
    async def generate(self, payload: GenerateRequest) -> GenerateResponse:
        ...
        page = max(1, _safe_int(context.pop("page", self.default_page), self.default_page))
        page_size = max(1, _safe_int(context.pop("page_size", self.default_page_size), self.default_page_size))
        sort = _safe_int(context.pop("sort", self.default_sort), self.default_sort)
        surrounding = _safe_int(context.pop("surrounding", 0), 0)
        ...
        log_msg = (
            f"Incoming request user_id={payload.user_id} page={page} size={page_size} "
            f"sort={sort} surrounding={surrounding}"
        )
        self.logger.info(log_msg)
        SERVE_LOGGER.info(log_msg)
        try:
            data = await self._run_search(user_msg, page=page, page_size=page_size, sort=sort)
