...
from tashan_scispark.app.core.config import OUTPUT_PATH, get_proxies
...
def search_google_scholar(doi):
    ...
    response = requests.get(search_url, headers=headers, proxies=get_proxies())
...
def download_pdf_from_google(pdf_url, title, output_path):
    ...
    response = requests.get(pdf_url, stream=True, proxies=get_proxies())
...
def download_pdf_from_arxiv(doi, title, output_path):
    ...
    for result in search_engine.results():
        ...
        if pdf_doi == doi:
            print("ok")
            # 使用流式请求
            with requests.get(pdf_url, proxies=get_proxies(), stream=True) as r:
