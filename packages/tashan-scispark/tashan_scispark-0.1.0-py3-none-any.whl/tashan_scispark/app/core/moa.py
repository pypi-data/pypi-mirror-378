...
# 注意事项：
import agentscope
from agentscope.pipeline import MsgHub as msghub
from agentscope.agent import ReActAgent, UserAgent
from agentscope.message import Msg
from tashan_scispark.app.core.config import OUTPUT_PATH
import os
from tashan_scispark.app.core.tpl import tpl_env
...
def moa_idea_iteration(topic="", user_prompt="", user_id="", task=None):
    ...
    # 创建一个对话智能体和一个用户智能体
    dialogAgent_Gemini = ReActAgent(name="Gemini", model_config_name="glm-4-long", sys_prompt=system_prompt)
    dialogAgent_Qwen = ReActAgent(name="Qwen", model_config_name="qwen-plus", sys_prompt=system_prompt)
    dialogAgent_DeepSeek = ReActAgent(name="DeepSeek", model_config_name="deepseek-chat", sys_prompt=system_prompt)

    dialogAgent_AC = ReActAgent(name="AC", model_config_name="deepseek-chat", sys_prompt=system_prompt)

    dialogAgent_Reviewer = ReActAgent(name="Reviewer", model_config_name="deepseek-chat", sys_prompt=system_prompt)
...
def moa_model(model_configs, agent_list, topic, user_prompt, systeam_prompt, ac_prompt="", ac_systeam="", stage=""):
    ...
    for llm in agent_list['helper']:
        agent = ReActAgent(name=llm, model_config_name=llm, sys_prompt=system_prompt)
        agents[llm] = agent
    ...
    if ac_prompt != "" and agent_list['main'] != "":
        AC = ReActAgent(name="AC", model_config_name=agent_list['main'], sys_prompt=ac_systeam)
...
def moa_table(model_configs=model_configs, topic='', draft='', user_id='', task=None):
    ...
    dialogAgent_Qwen = ReActAgent(name="Reviewer 1", model_config_name="qwen-plus", sys_prompt=role)  #"qwen-max-0919"
    dialogAgent_Gemini = ReActAgent(name="Reviewer 2", model_config_name="glm-4-long",
                                     sys_prompt=role)  # "gemini-1.5-flash"
    dialogAgent_DeepSeek = ReActAgent(name="Reviewer 3", model_config_name="deepseek-chat", sys_prompt=role)
    # userAgent = UserAgent(name="Reviewer 4")

    dialogAgent_Viewer = ReActAgent(name="Viewer", model_config_name="deepseek-chat", sys_prompt=viewer)
