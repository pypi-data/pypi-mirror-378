...
from tashan_scispark.app.core.prompt import get_related_keyword_prompt, paper_compression_prompt, extract_entity_prompt, \
    extract_tec_entities_prompt, review_mechanism_prompt
from tashan_scispark.app.utils.llm_api import call_with_deepseek, call_with_deepseek_jsonout, call_with_qwenmax
from tashan_scispark.app.utils.arxiv_api import search_paper
from tashan_scispark.app.utils.scholar_download import download_all_pdfs
from tashan_scispark.app.utils.pdf_to_md import pdf2md_mineruapi
from tashan_scispark.app.utils.wiki_search import get_description, search
from tashan_scispark.app.core.config import OUTPUT_PATH, graph
import ast
...
def SearchKeyWordScore(Keywords):
    ...
    for item in Keywords:
        # 处理除零情况：当所有count值相同时，normalized_count设为0.5
        if max_count == min_count:
            normalized_count = 0.5  # 中性值，表示所有关键词的count权重相同
        else:
...
def search_releated_paper(topic, max_paper_num=5, compression=True, user_id="", task=None):
    ...
    try:
        ...
        print(f"\033[1;31m | ERROR    | Failed to extract keywords: {str(e)} \033[0m")
        Keywords = []
