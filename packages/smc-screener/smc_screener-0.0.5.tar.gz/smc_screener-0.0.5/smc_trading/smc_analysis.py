...
class Alerts:
    def __init__(self):
        ...
        # New attributes for current candle breaks
...
async def main(
    ...
    # Initialize CSV and Google Sheets setup
    summary_file, levels_file = None, None
    summary_file_exists, levels_file_exists = False, False
    filtered_file = None
    ...
    processed_summary, processed_levels, processed_filtered = set(), set(), set()  # Track processed data to avoid duplicates
        ...
        for attempt in range(max_retries):
            try:
                ...
                if success:
                    ...
                    # Save CSV for summaries and levels
                    ...
                    # Check for current candle CHoCH/BOS
                    if (smc.current_alerts.current_candle_swing_bullish_bos or
                        smc.current_alerts.current_candle_swing_bearish_bos or
                        smc.current_alerts.current_candle_swing_bullish_choch or
                        smc.current_alerts.current_candle_swing_bearish_choch or
                        smc.current_alerts.current_candle_internal_bullish_bos or
                        smc.current_alerts.current_candle_internal_bearish_bos or
                        smc.current_alerts.current_candle_internal_bullish_choch or
                    ...
                    # Prepare Google Sheets data (avoid duplicates)
                    if output_format in ["google_sheets", "both"] and gspread_client:
                        ...
                        if filtered_stocks:
                            if not filtered_headers:
                                filtered_headers = list(filtered_stocks[-1].keys())  # Use last stock's keys
