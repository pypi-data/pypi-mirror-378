...
class PyHook:
    ...
    def use(self, name: str, callback: Callable, **options) -> None:
        if asyncio.iscoroutinefunction(callback):
            asyncio.create_task(self._async_registry.register(name, callback, **options))
        else:
    ...
    def remove(self, name: str, callback: Optional[Callable] = None) -> None:
        try:
            ...
            pass
        try:
            asyncio.create_task(self._async_registry.unregister(name, callback))
        except:
            pass
    ...
    async def async_trigger(self, name: str, *args, **kwargs) -> List[Any]:
        ...
        if name in self._async_registry._hooks:
            return await self._async_registry.trigger(name, *args, _strategy=strategy, **kwargs)
        return []
...
__all__ = [
    "use", "use_once", "remove", "trigger", "async_trigger", "trigger_with_return",
