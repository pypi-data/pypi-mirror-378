...
def get_html_files(
    ...
    files: List[str] = natsorted(
        list(set(input_files) - set(exclude_files)), key=str
    )
    ...
    for i, filename in enumerate(html_files, start=1):
        ...
        async with aiofiles.open(filename, "rb") as file:
            ...
            if decoded_html is None:
                _logger.warning(
                    "Skipping empty or undecodable file: %s", filename
                )
                continue
            soup = BeautifulSoup(decoded_html, features="lxml")
            futures.append(extract_chapter(soup, config, filename))
...
async def extract_chapter(
    ...
    if config.html_replace:
        html_content = search_and_replace(html_content, config.html_replace)
...
def search_and_replace(content: str, regexs: list[tuple[str, str]]) -> str:
    ...
    except re.error as error:  # Catch regex compilation errors
        _logger.error("Invalid regex: %s", error)
        return content  # Return original content on error
    except (
        ValueError
    ) as error:  # Catch unpacking errors for malformed regexs list
        _logger.error("Invalid regex: %s", error)
        return content
...
def generate_single_txt(
    ...
    with open(filename, "w", newline="\n", encoding="utf8") as file:
        ...
        file.write("---\n")
        file.write(_("title:") + config.book_title + "\n")
        file.write(_("author:") + config.book_author + "\n")
