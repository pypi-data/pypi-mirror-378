class KubernetesController:
    ...
    def get_my_pod_spec(self):
        ...
        if pod.metadata.labels is not None:
            # Airflow에서 생성된 pod의 공통 라벨들을 확인
            is_airflow_pod = (
                pod.metadata.labels.get("airflow-worker") is not None or  # airflow worker 라벨
                pod.metadata.labels.get("dag_id") is not None or          # DAG ID 라벨  
                pod.metadata.labels.get("task_id") is not None or         # Task ID 라벨
                pod.metadata.labels.get("run_id") is not None             # Run ID 라벨
            )
            
            # 현재 실행 중이고 Airflow pod인 경우
            if (is_airflow_pod and 
                pod.status.phase in ["Running", "Pending"] and
