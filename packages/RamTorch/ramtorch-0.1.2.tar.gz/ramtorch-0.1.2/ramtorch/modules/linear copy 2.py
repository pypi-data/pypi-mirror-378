...
BWRD_GRAD_TRANSFER_STREAM = torch.cuda.Stream()
...
TRANSFER_GRAD_BACKWARD_FINISHED_EVENT = torch.cuda.Event()
COMPUTE_BACKWARD_START_EVENT = torch.cuda.Event()
COMPUTE_BACKWARD_DONE_EVENT = torch.cuda.Event()
...
class BouncingLinearFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        """
        global \
            BWRD_TRANSFER_STREAM, BWRD_GRAD_TRANSFER_STREAM, \
            TRANSFER_BACKWARD_FINISHED_EVENT, TRANSFER_GRAD_BACKWARD_FINISHED_EVENT, COMPUTE_BACKWARD_START_EVENT, COMPUTE_BACKWARD_DONE_EVENT, \
            BACKWARD_BUFFER_CLK, W_BWRD_BUFFERS, W_GRAD_BUFFERS, B_GRAD_BUFFERS
        ...
        torch.cuda.current_stream().wait_event(TRANSFER_GRAD_BACKWARD_FINISHED_EVENT)

        # mark the start of compute event
        COMPUTE_BACKWARD_START_EVENT.record()

        # Compute gradients
        grad_input = grad_out @ W_BWRD_BUFFERS[selected_buffer]
        ...
        COMPUTE_BACKWARD_DONE_EVENT.record()

        with torch.cuda.stream(BWRD_GRAD_TRANSFER_STREAM):
            # wait for the grad to finish before tossing it to ram
            BWRD_GRAD_TRANSFER_STREAM.wait_event(COMPUTE_BACKWARD_START_EVENT)
            ...
            # BWRD_GRAD_TRANSFER_STREAM.synchronize()
            TRANSFER_GRAD_BACKWARD_FINISHED_EVENT.record()
