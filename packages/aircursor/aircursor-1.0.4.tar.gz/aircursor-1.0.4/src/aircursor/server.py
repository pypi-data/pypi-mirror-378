...
def create_app() -> Flask:
    ...
    def root():
        ...
        for ip in ips:
            url = f"http://{ip}:{port}"
            img_data = _qr_data_url(url)
            entries.append((ip, url, img_data))
        ...
        html_parts = [
            ...
            "<style>body{font-family:system-ui,Segoe UI,Arial;margin:24px} .grid{display:grid;gap:16px;grid-template-columns:repeat(auto-fill,minmax(260px,1fr))} .card{border:1px solid #ddd;border-radius:10px;padding:16px} .ip{font-weight:600} img{width:220px;height:220px}</style>",
            "</head>",
            "<body>",
            "<h1>AirCursor</h1>",
            "<p>Scan a QR to connect to this server from another device on the same network.</p>",
            "<div class='grid'>",
        ]
        for ip, url, img in entries:
            html_parts.append(
                f"<div class='card'><div class='ip'>{ip}</div><div><a href='{url}'>{url}</a></div><img alt='QR {ip}' src='{img}' /></div>"
            )
