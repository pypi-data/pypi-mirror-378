...
class OutlookReaderManager:
    def __init__(self, user_name, user_password, token_name, service_url, verify_ssl=False):
    ...
    def _get_public_service_data(self, password_name):
        ...
        if is_login != 1 or is_auth != 1 or is_valid != 1:
            print("You do not have permission to access this service or invalid username or password!")
            return None
    ...
    def _update_public_service_data(self, password_name, password_value):
        ...
        is_valid = res_json['isValid']
        if is_login != 1 or is_auth != 1 or is_valid != 1:
            print("Update password failed! You do not have permission to access this service or invalid username or password!")
        else:
    ...
    def _initialize_app(self):
        ...
        if accounts:
            print("Found existing account:", accounts[0]['username'])
    ...
    def _authenticate(self):
        ...
        """
        if not self.result:
            print("No valid token in cache, need to log in via browser.")
