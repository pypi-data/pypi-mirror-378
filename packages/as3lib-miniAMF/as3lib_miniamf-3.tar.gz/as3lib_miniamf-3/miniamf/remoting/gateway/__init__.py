...
SERVER_NAME = 'MiniAMF/%s %s/%s' % (
    miniamf.version, impl,
    '.'.join(map(lambda x: str(x), sys.version_info[0:3]))
)


class BaseServiceError(miniamf.BaseError):
    """
    Base service error.
    """
...
class ServiceWrapper(object):
    ...
    def getAuthenticator(self, service_request=None):
        ...
        if service_request.method is None:
            if hasattr(self.service, '_miniamf_authenticator'):
                return self.service._miniamf_authenticator
        ...
        if hasattr(method, '_miniamf_authenticator'):
            return method._miniamf_authenticator
    ...
    def mustExposeRequest(self, service_request=None):
        ...
        if service_request.method is None:
            if hasattr(self.service, '_miniamf_expose_request'):
                return self.service._miniamf_expose_request
        ...
        if hasattr(method, '_miniamf_expose_request'):
            return method._miniamf_expose_request
    ...
    def getPreprocessor(self, service_request=None):
        ...
        if service_request.method is None:
            if hasattr(self.service, '_miniamf_preprocessor'):
                return self.service._miniamf_preprocessor
        ...
        if hasattr(method, '_miniamf_preprocessor'):
            return method._miniamf_preprocessor
...
class ServiceRequest(object):
    ...
    @type request: L{Envelope<miniamf.remoting.Envelope>}
...
class BaseGateway(object):
    ...
    def addService(self, service, name=None, description=None,
        ...
        @type name: C{str}
        @raise miniamf.remoting.RemotingError: Service already exists.
    ...
    def getServiceRequest(self, request, target):
        ...
        @type request: L{Request<miniamf.remoting.Request>}
    ...
    def getResponse(self, amf_request):
        ...
        @type amf_request: L{Envelope<miniamf.remoting.Envelope>}

        @return: The AMF response.
        @rtype: L{Envelope<miniamf.remoting.Envelope>}
        """
    ...
    def authenticateRequest(self, service_request, username, password,
        ...
        if hasattr(authenticator, '_miniamf_expose_request'):
    ...
    def preprocessRequest(self, service_request, *args, **kwargs):
        ...
        if hasattr(processor, '_miniamf_expose_request'):
            http_request = kwargs.get('http_request', None)
            args = (http_request,) + args
...
def authenticate(func, c, expose_request=False):
    ...
    setattr(attr, '_miniamf_authenticator', c)
...
def expose_request(func):
    ...
    if isinstance(func, types.MethodType):
        setattr(func.im_func, '_miniamf_expose_request', True)
    else:
        setattr(func, '_miniamf_expose_request', True)
...
def preprocess(func, c, expose_request=False):
    ...
    setattr(attr, '_miniamf_preprocessor', c)
