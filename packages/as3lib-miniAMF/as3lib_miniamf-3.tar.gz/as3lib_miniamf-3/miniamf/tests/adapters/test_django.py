...
class TypeMapTestCase(BaseTestCase):
    ...
    def test_objects_all(self):
        ...
        self.assertEqual(encoder.stream.getvalue(), b'\n\x00\x00\x00\x00')

        encoder = miniamf.get_encoder(miniamf.AMF3)
        encoder.writeElement(models.SimplestModel.objects.all())
        self.assertEqual(encoder.stream.getvalue(), b'\t\x01\x01')

    def test_NOT_PROVIDED(self):
        from django.db.models import fields

        self.assertEqual(
            miniamf.encode(fields.NOT_PROVIDED, encoding=miniamf.AMF0).getvalue(),
            b'\x06'
        )
...
class ClassAliasTestCase(BaseTestCase):
    ...
    def test_undefined(self):
        ...
        alias.applyAttributes(x, {
            'id': fields.NOT_PROVIDED
        })

        self.assertEqual(x.id, fields.NOT_PROVIDED)

        x.id = miniamf.Undefined

        attrs = alias.getEncodableAttributes(x)
        self.assertEqual(attrs, {'id': miniamf.Undefined})
...
class ForeignKeyTestCase(BaseTestCase):
    def test_one_to_many(self):
        ...
        self.assertEqual(
            ...
            b'publications\t\x01\x01\n\x04%s\x01' % chr(article_id).encode()
        )
    ...
    def test_static_relation(self):
        ...
        @see: #693
        """
        from miniamf import util  # noqa

        miniamf.register_class(models.StaticRelation)
...
class I18NTestCase(BaseTestCase):
    def test_encode(self):
        ...
        self.assertEqual(
            miniamf.encode(gettext_lazy('Hello')).getvalue(),
            b'\x06\x0bHello'
        )
...
class PKTestCase(BaseTestCase):
    ...
    def test_no_pk(self):
        ...
        encoded = miniamf.encode(instances, encoding=miniamf.AMF3).getvalue()

        decoded = next(miniamf.decode(encoded, encoding=miniamf.AMF3))
...
class AuthTestCase(BaseTestCase):
    ...
    def test_user(self):
        ...
        alias = miniamf.get_class_alias(models.User)
