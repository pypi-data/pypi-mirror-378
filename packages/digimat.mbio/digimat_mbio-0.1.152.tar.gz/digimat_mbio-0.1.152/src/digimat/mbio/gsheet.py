...
class MBIOTaskGSheet(MBIOTask):
    ...
    def loadCell(self, sheet, xml: XMLConfig, writable=True, digital=False):
        if sheet:
            ...
            name=xml.get('name')
            name=self.replaceVariables(name, myvariables)

            address=xml.get('cell') or xml.get('address')
            address=self.replaceVariables(address, myvariables)
            ...
            if name and not self.cell(name) and address:
                ...
                if writable:
                    ...
                    override=self.replaceVariables(override, sheet)
                ...
                if writable:
                    ...
                    self.loadValueExpression(value, xml, myvariables)
