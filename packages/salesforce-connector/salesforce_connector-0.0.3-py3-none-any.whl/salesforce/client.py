...
class SalesforceConnector:
    ...
    def create_lead(self, lead_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        This function sends a POST request to the Salesforce API to create a new lead.
        The lead details are passed as a dictionary (`lead_data`) containing the lead fields.

        Args:
            lead_data (dict): A dictionary containing the lead details.
                Commonly used fields:
                    - FirstName (str): Lead's first name
                    - LastName (str): Lead's last name **(Required)**
                    - Company (str): Lead's company name **(Required)**
                    - Email (str): Lead's email address
                    - Title (str): Job title of the lead
                    - LeadSource (str): Source of the lead (e.g., Web, Phone, Email)
                    - Status (str): Current status (e.g., Open, Working, Converted)
                    ...
                    - Description (str): Additional details about the lead

                ⚠️ Note: `LastName` and `Company` are **mandatory** fields for Salesforce leads.

        Returns:
            dict | None:
                - On success → A dictionary containing details of the newly created lead, including:
                    {
                        "id": "<lead_id>",
                        "success": true,
                        "errors": []
        ...
        Example:
            ...
            response = salesforce.create_lead(lead_data)
            if response:
                print("Lead created successfully!", response)
            else:
        ...
        try:
            ...
            data = response.json()
            # Try to get lead ID if response is a dict
            lead_id = data.get("id") if isinstance(data, dict) else None

            if lead_id:
                logger.info("✅ Lead created successfully! Lead ID: %s", lead_id)
            else:
                # Log whatever Salesforce returned (could be duplicates or any error)
                logger.error("⚠️ Salesforce returned response: %s", data)

            return data

        except requests.exceptions.HTTPError as e:
            # Log full Salesforce response if available
            logger.exception("❌ HTTP Error occurred while creating lead: %s", e)
            if e.response is not None:
                logger.error("📌 Salesforce Response: %s", e.response.text)
            return None

        except Exception as e:
            # Log any unexpected Python exception
            logger.exception("❌ Unexpected exception occurred while creating lead: %s", e)
            return None
