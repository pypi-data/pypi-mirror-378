...
class ParagraphProcessor(BaseProcessor):
    ...
    @classmethod
    def process_multi_p(cls, xml: str) -> str:
        """处理 xl-multi-p 标签，将其转换为多个 xl-p 标签"""
        def process_multi_p_match(match):
            ...
            # 使用 jinja2 模板生成多个 xl-p 标签
            template_str = '''
                {% for line in lines %}
                <xl-p{% if style_str %} style="{{ style_str }}"{% endif %}>{{ line }}</xl-p>
                {% endfor %}
            '''
            template = Template(template_str)
            result = template.render(lines=lines, style_str=style_str)
            return result
        
        return cls._process_tag(xml, cls.XL_MULTI_P_PATTERN, process_multi_p_match)
