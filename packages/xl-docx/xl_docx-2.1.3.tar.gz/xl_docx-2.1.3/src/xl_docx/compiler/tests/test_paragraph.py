...
class TestParagraphProcessor:
    ...
    def test_process_multi_p_simple(self):
        """测试处理简单的xl-multi-p标签"""
        xml = '<xl-multi-p text="1\n2\n3"></xl-multi-p>'
        result = ParagraphProcessor.process_multi_p(xml)
        assert '<xl-p>1</xl-p>' in result
        assert '<xl-p>2</xl-p>' in result
        assert '<xl-p>3</xl-p>' in result

    def test_process_multi_p_with_style(self):
        """测试处理带样式的xl-multi-p标签"""
        xml = '<xl-multi-p text="第一行\n第二行\n第三行" style="align:center;font-size:14px"></xl-multi-p>'
        result = ParagraphProcessor.process_multi_p(xml)
        assert '<xl-p style="align:center;font-size:14px">第一行</xl-p>' in result
        assert '<xl-p style="align:center;font-size:14px">第二行</xl-p>' in result
        assert '<xl-p style="align:center;font-size:14px">第三行</xl-p>' in result

    def test_process_multi_p_empty_lines(self):
        """测试处理包含空行的xl-multi-p标签"""
        xml = '<xl-multi-p text="第一行\n\n第三行"></xl-multi-p>'
        result = ParagraphProcessor.process_multi_p(xml)
    ...
    def test_process_multi_p_single_line(self):
        """测试处理单行的xl-multi-p标签"""
        xml = '<xl-multi-p text="只有一行"></xl-multi-p>'
        result = ParagraphProcessor.process_multi_p(xml)
        assert '<xl-p>只有一行</xl-p>' in result

    def test_process_multi_p_no_text(self):
        """测试处理没有text属性的xl-multi-p标签"""
        xml = '<xl-multi-p></xl-multi-p>'
        result = ParagraphProcessor.process_multi_p(xml)
        assert result == xml  # 没有text属性时应该返回原字符串

    def test_process_multi_p_whitespace_only(self):
        """测试处理只有空白字符的xl-multi-p标签"""
        xml = '<xl-multi-p text="   \n  \n   "></xl-multi-p>'
        result = ParagraphProcessor.process_multi_p(xml)
        assert '<xl-p>   </xl-p>' in result
        assert '<xl-p>  </xl-p>' in result
        assert '<xl-p>   </xl-p>' in result
