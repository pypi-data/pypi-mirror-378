...
class SimpleMCPServer:
    ...
    async def _search_tools(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        filters = ToolSearchFilter(
            ...
            min_tashan_score=args.get("min_tashan_score"),
            max_tashan_score=args.get("max_tashan_score"),
    ...
    async def _get_tool_evaluation(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not tool_identifier:
            return {"error": "缺少工具标识符"}
        
        tool = await self.db_manager.get_tool_by_name(tool_identifier)
        if not tool:
            return {"error": f"未找到工具: {tool_identifier}"}
        
        test_results = await self.db_manager.get_tool_test_results(tool_identifier, 5)
    ...
    async def _get_top_tools(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tools = await self.db_manager.get_top_tools(sort_by, limit)
    ...
    async def _get_tool_categories(self, args: Dict[str, Any]) -> Dict[str, Any]:
        """获取工具分类"""
        categories = await self.db_manager.get_category_stats()
