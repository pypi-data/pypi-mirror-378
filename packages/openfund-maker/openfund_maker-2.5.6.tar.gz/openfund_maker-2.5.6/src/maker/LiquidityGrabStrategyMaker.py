...
class LiquidityGrabStrategyMaker(StrategyMaker):
    ...
    def process_pair(self, symbol, pair_config):
        ...
        try:
            ...
            # Step 4: 验证首次触及条件
            if not self._validate_first_touch_condition(symbol, atf_result['target_level'], atf_result['equal_points_type'], liquidity_strategy):
    ...
    def _analyze_htf_trend_and_levels(self, symbol, htf, open_body_break, strategy, precision):
        ...
        if htf_cache_key not in self.htf_cache:
            ...
            profit_percent = self.toDecimal(abs((htf_resistance_price - htf_support_price) / htf_support_price * 100))
    ...
    def _validate_first_touch_condition(self, symbol, target_level, equal_points_type, strategy):
        """验证首次触及条件"""
        ...
        if level_key in touch_history:
            ...
            if (datetime.now() - last_touch_time).total_seconds() > 15*max_candle_interval:
