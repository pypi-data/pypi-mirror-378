...
class LiquidityGrabStrategyMaker(StrategyMaker):
    ...
    def process_pair(self, symbol, pair_config):
        ...
        try:
            ...
            # Step 4: 验证首次触及条件
            if not self._validate_first_touch_condition(symbol, atf_result['target_level'], atf_result['equal_points_type']):
    ...
    def _analyze_atf_liquidity_zones(self, symbol, atf, htf_result, open_body_break, strategy, precision):
        ...
        if atf_cache_key not in self.atf_cache:
            ...
            # 2.1 优先识别等高等低流动性
            liquidity_result = self._identify_equal_highs_lows_liquidity(symbol, atf_df, atf_trend, strategy)
            if not liquidity_result:
                self.logger.info(f"{symbol} : {step}.1. ATF {atf} 未找到有效的等高等低流动性")
                return None

            self.logger.info(f"{symbol} : {step}.1.1 ATF {atf} 发现{liquidity_result[self.EQUAL_POINTS_TYPE_COL]}流动性，价格={liquidity_result[self.EQUAL_POINTS_PRICE_COL]}")
    ...
    def _identify_equal_highs_lows_liquidity(self, symbol, data, trend, strategy) -> dict:
        """识别等高等低流动性"""
        atr_offset = strategy.get('liquidity_atr_offset', 0.1)
        
        # 使用SMCLiquidity查找等高等低
        df_equal_liquidity = self.find_EQH_EQL(symbol=symbol, data=data, trend=trend, atr_offset=atr_offset)
        # 找到流动性
    ...
    def _get_equal_liquidity(self, df_equal_liquidity, trend):
        # 筛选有等高点的行
        df_with_equal_points = df_equal_liquidity[df_equal_liquidity[self.HAS_EQUAL_POINTS_COL]]
        ...
        if trend == self.BULLISH_TREND:
            target_row = df_with_equal_points.loc[df_with_equal_points[self.EQUAL_POINTS_PRICE_COL].idxmax()]
        else:
            target_row = df_with_equal_points.loc[df_with_equal_points[self.EQUAL_POINTS_PRICE_COL].idxmin()]
    ...
    def _validate_support_resistance_confluence(self, symbol, htf_result, liquidity_result, atf_side, precision, strategy):
        ...
        if atf_side == self.BUY_SIDE:
            ...
            if liquidity_result[self.EQUAL_POINTS_TYPE_COL] != self.EQUAL_HIGH_TYPE:
                self.logger.info(f"{symbol} : {step}. 多头需要等高流动性，但找到的是{liquidity_result[self.EQUAL_POINTS_TYPE_COL]}")
                return None
            ...
            if liquidity_result[self.EQUAL_POINTS_TYPE_COL] != self.EQUAL_LOW_TYPE:
                self.logger.info(f"{symbol} : {step}. 空头需要等低流动性，但找到的是{liquidity_result[self.EQUAL_POINTS_TYPE_COL]}")
                return None
    ...
    def _validate_first_touch_condition(self, symbol, target_level, equal_points_type):
        """验证首次触及条件"""
