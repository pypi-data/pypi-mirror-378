...
class DTO10Helper(DtoErrorDescription):
    ...
    def set_connection_settings_to_wifi(self, ssid: str, password: str):
        logging.debug(f"Настройка для подключения к Wi-Fi (клиент)...\nСеть: {ssid}")
        ...
        self.fptr.setParam(268, 0)  # режим wi-fi (клиент)
        self.fptr.writeDeviceSetting()
        self.fptr.commitSettings()
        if self.request_error_code() != 0:
            logging.error("Ошибка при установке режима Wi-Fi (клиент)")
            return False

        self.fptr.setParam(296, 253)
        self.fptr.writeDeviceSetting()
        self.fptr.commitSettings()
        if self.request_error_code() != 0:
            logging.error("Ошибка при установке разрешения на подключение по Wi-Fi")
            return False

        self.fptr.setParam(269, ssid)
        self.fptr.writeDeviceSetting()
        self.fptr.commitSettings()
        if self.request_error_code() != 0:
            logging.error("Ошибка при установке SSID")
            return False

        self.fptr.setParam(270, password)
        self.fptr.writeDeviceSetting()
        self.fptr.commitSettings()
        if self.request_error_code() != 0:
            logging.error("Ошибка при установке пароля")
            return False

        logging.debug("Настройки Wi-Fi успешно записаны в ККТ.")
        logging.debug("Перезагрузка...")
        self.fptr.deviceReboot()

        self.connect_to_kkt_by_usb()
        self.fptr.setParam(self.IFptr.LIBFPTR_PARAM_DATA_TYPE, self.IFptr.LIBFPTR_DT_WIFI_INFO)
        self.fptr.queryData()
        ...
        self.fptr.setSingleSetting(self.IFptr.LIBFPTR_SETTING_PORT, str(self.IFptr.LIBFPTR_PORT_TCPIP))
        self.fptr.setSingleSetting(self.IFptr.LIBFPTR_SETTING_IPADDRESS, ip)
        self.fptr.setSingleSetting(self.IFptr.LIBFPTR_SETTING_IPPORT, port)
        self.fptr.applySingleSettings()

        return True

    def connect_to_kkt_by_wifi(self, ssid: str, password: str):
        logging.debug("Устанавливаем соединение по Wi-Fi")
        if not self.set_connection_settings_to_wifi(ssid, password):
            logging.debug("Не удалось установить настройки драйвера для подключения к ККТ")
            return False
        if self.open() != self.fptr.LIBFPTR_OK:
            logging.debug("Не удалось установить соединение с ККТ")
            return False
        if not self.is_connection_type_wifi():
            return False
        logging.debug(
            f"Соединение с ККТ установлено по Wi-Fi. SSID=[{ssid}], password=[{password}]"
        )
        return True
    ...
    def set_connection_settings_to_bluetooth(self):
    ...
    def connect_to_kkt_by_bluetooth(self):
        logging.debug("Устанавливаем соединение по Bluetooth")
        if not self.set_connection_settings_to_bluetooth():
            logging.debug("Не удалось установить настройки драйвера для подключения к ККТ")
            return False
        if self.open() != self.fptr.LIBFPTR_OK:
            logging.debug("Не удалось установить соединение с ККТ")
