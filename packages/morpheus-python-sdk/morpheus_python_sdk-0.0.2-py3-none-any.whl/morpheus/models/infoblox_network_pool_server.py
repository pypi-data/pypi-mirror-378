...
class InfobloxNetworkPoolServer(BaseModel):
    """
    InfobloxNetworkPoolServer
    """ # noqa: E501
    type: StrictStr = Field(description="Type Code (Infoblox)")
    ...
    service_mode: Optional[StrictStr] = Field(default='static', description="IP Mode", alias="serviceMode")
    config: Optional[InfobloxNetworkPoolServerConfig] = None
    credential: Optional[NSXNetworkServerCredential] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "name", "enabled", "serviceUrl", "serviceUsername", "servicePassword", "serviceThrottleRate", "ignoreSsl", "networkFilter", "zoneFilter", "tenantMatch", "serviceMode", "config", "credential"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['infoblox']):
            raise ValueError("must be one of enum values ('infoblox')")
        return value

    @field_validator('service_mode')
    def service_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of InfobloxNetworkPoolServer from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of InfobloxNetworkPoolServer from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "serviceMode": obj.get("serviceMode") if obj.get("serviceMode") is not None else 'static',
            "config": InfobloxNetworkPoolServerConfig.from_dict(obj["config"]) if obj.get("config") is not None else None,
