    ...
    def process_cat(self, category, dataValues):
        ...
        if category == "Product Enquiry":
            ...
            for product in products:
                ...
                if not is_make:
                    # Get most frequent make from PAST_SALES
                    query = '''
                            SELECT MAKE
                            FROM PAST_SALES
                            WHERE CUSTOMER_CODE = ?
                    ...
                    cursor.execute(query, (customer_code, item_no))
                    row = cursor.fetchone()
                    cursor.close()

                    if row and row[0]:  # if a frequent make is found
                        product["make"] = row[0]   # replace the make directly in products dict
                        is_make = True
                        logger.log(f"Replaced make with most frequent past sale make: {product['make']}")
                    else:
                        logger.log(f"No past sales found for item {item_no} and customer {customer_code}, keeping existing make: {product.get('make')}")
                ...
                # Check the stock availabilty of the make
        ...
        try:
            ...
            for product in products:
                ...
                if schemaName_Updated in schemaClasslist:
                    ...
                    response    = (
                        client.query
                        .get(schemaName_Updated, ["description", "answer","phy_attrib_2","phy_attrib_3","phy_attrib_4"]) #phy_attrib_5
                        .with_hybrid(
                                        alpha = alphaValue,
