...
def rd_get(connect, key, log, code='utf-8'):
    try:
        res = connect.get(key)
...
def dump_docs_redis_set(con, key, timeout, docs, log):
...
def dump_data_redis_set_list(con, key, timeout, data, log):
...
def dump_data_redis_set_zset(con, key, timeout, data, log):
    log.info('key:{},timeout:{},docs:{}'.format(key, timeout, data))
