...
class AMQPService(AbstractDomainEventsService):
    ...
    def publish(self, message: Any, event_type: DomainEvent, exchange: str = None):
        ...
        try:
            ...
            (logging.getLogger('api')
             .debug(f'Message sent to exchange {rabbit_exchange} using routing_key {rabbit_routing_key}'))
    ...
    def subscribe(self):
        try:
            ...
            logging.getLogger('api').error(f'Failed to subscribe to {self.queue_name}: {e}')