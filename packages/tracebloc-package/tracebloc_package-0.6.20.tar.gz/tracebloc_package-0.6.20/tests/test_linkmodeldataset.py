...
class TestLinkModelDataSet(unittest.TestCase):
    ...
    @patch("requests.post")
    def test_image_size_validation(self, mock_post):
        ...
        link_model.image_size(224)
        self.assertEqual(link_model._LinkModelDataSet__image_size, 224)
        self.assertTrue(link_model._LinkModelDataSet__eligibility_passed)

        # Test image size too small
        link_model.image_size(8 - 1)
        self.assertFalse(link_model._LinkModelDataSet__eligibility_passed)

        # Test image size too large
        link_model.image_size(224 + 1)
        self.assertFalse(link_model._LinkModelDataSet__eligibility_passed)

        # Test non-square image dimensions
        link_model.image_size((224, 256))
        self.assertFalse(link_model._LinkModelDataSet__eligibility_passed)

        # Test valid image shape
        captured_output = StringIO()
        sys.stdout = captured_output
        link_model.image_size((224, 224, 3))
        sys.stdout = sys.__stdout__
        self.assertIn(captured_output.getvalue(), "image_size:Invalid type \n\n")

        # Test invalid shape type
        captured_output = StringIO()
        sys.stdout = captured_output
        link_model.image_size("invalid")
        sys.stdout = sys.__stdout__
        self.assertIn("image_size:Invalid type \n\n", captured_output.getvalue())
        self.assertNotEqual(link_model._LinkModelDataSet__image_size, "invalid")
        ...
        sys.stdout = sys.__stdout__
        self.assertIn(
            "image_size:Image shape/size is fixed for each model for pytorch\n",
            captured_output.getvalue(),
        )
