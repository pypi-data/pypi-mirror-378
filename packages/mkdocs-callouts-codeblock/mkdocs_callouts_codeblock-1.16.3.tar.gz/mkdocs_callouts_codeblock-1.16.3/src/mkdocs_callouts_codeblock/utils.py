...
def convert_tagged_codeblocks_to_mkdocs_tabs(md_text: str) -> str:
    """
    将 Obsidian 带 tag 的代码块转为 mkdocs-material 的多标签代码块。
    ...
    while i < len(lines):
        ...
        if m:
            ...
            param_str = m.group(2)
            # 提取 tag（如有）
            tag_match = re.search(r'tag="([^"]+)"', param_str)
...
class CalloutParser:
    ...
    def parse(self, markdown: str) -> str:
        ...
        # 再处理代码块 tab
        markdown = convert_tagged_codeblocks_to_mkdocs_tabs(markdown)
        return markdown
