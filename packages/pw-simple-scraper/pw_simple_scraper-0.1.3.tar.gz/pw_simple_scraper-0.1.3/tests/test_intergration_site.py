...
async def test_index_list_extraction(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "index.html")) as page:
...
async def test_links_href_extraction(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "links.html")) as page:
            locator = page.locator("a.nav")
            hrefs = await locator.evaluate_all("els => els.map(el => el.getAttribute('href'))")
            assert set(hrefs) == {"/a.html", "/b.html"}

@pytest.mark.asyncio
async def test_dynamic_wait_visible_after_insert(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "dynamic.html")) as page:
...
async def test_dynamic_list_insert(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "dynamic_insert.html")) as page:
            items_locator = page.locator("li.item")
...
async def test_dynamic_href_added_later(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "dynamic_href.html")) as page:
...
async def test_attrs_mixed_href_filter(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "attrs_mixed.html")) as page:
            locator = page.locator(".nav")
            all_hrefs = await locator.evaluate_all("els => els.map(el => el.getAttribute('href'))")
...
async def test_empty_text_is_filtered_and_empty_href_ignored(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "empty.html")) as page:
            # 텍스트 필터링
            ...
            # href 필터링
            hrefs = await page.locator("a.maybe-href").evaluate_all("els => els.map(el => el.getAttribute('href')).filter(Boolean)")
            assert hrefs == []

@pytest.mark.asyncio
async def test_encoding_utf8_paragraphs(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "encoding_utf8.html")) as page:
...
async def test_longlist_count_and_edges(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "longlist.html")) as page:
...
async def test_nested_headlines_text_and_links(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "nested.html")) as page:
...
async def test_visibility_toggle(http_server):
    async with PlaywrightScraper(headless=True, timeout=5) as scraper:
        async with scraper.get_page(_u(http_server, "visibility.html")) as page:
