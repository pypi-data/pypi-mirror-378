...
if typing.TYPE_CHECKING:
    class FalsePerspectiveJson(typing_extensions.TypedDict):
        editor_properties: json_util.JsonObject
        percentage_of_depth_range: float
...
class FalsePerspective(BaseObjectType):
    ...
    percentage_of_depth_range: float = dataclasses.field(default=0.0, metadata={
        'reflection': FieldReflection[float](
            float, id=0x7f7065d3, original_name='PercentageOfDepthRange'
        ),
    })
    near_clip_plane: float = dataclasses.field(default=20.0, metadata={
        'reflection': FieldReflection[float](
            float, id=0xf4817f13, original_name='NearClipPlane'
        ),
    })
    far_clip_plane: float = dataclasses.field(default=8192.0, metadata={
        'reflection': FieldReflection[float](
    ...
    def _fast_decode(cls, data: typing.BinaryIO, property_count: int) -> typing_extensions.Self | None:
        ...
        return cls(editor_properties, percentage_of_depth_range, near_clip_plane, far_clip_plane)
    ...
    def from_json(cls, data: json_util.JsonValue) -> typing_extensions.Self:
        json_data = typing.cast("FalsePerspectiveJson", data)
        return cls(
            editor_properties=EditorProperties.from_json(json_data['editor_properties']),
            percentage_of_depth_range=json_data['percentage_of_depth_range'],
...
def _decode_percentage_of_depth_range(data: typing.BinaryIO, property_size: int) -> float:
    return struct.unpack('>f', data.read(4))[0]


def _decode_near_clip_plane(data: typing.BinaryIO, property_size: int) -> float:
    return struct.unpack('>f', data.read(4))[0]


def _decode_far_clip_plane(data: typing.BinaryIO, property_size: int) -> float:
    return struct.unpack('>f', data.read(4))[0]
