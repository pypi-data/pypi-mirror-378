...
class PlayerStateChange(BaseObjectType):
    ...
    item_to_change: int = dataclasses.field(default=0, metadata={
        'reflection': FieldReflection[int](
            int, id=0x4a3491bd, original_name='ItemToChange'
        ),
    })
    ...
    command_action: int = dataclasses.field(default=0, metadata={
        'reflection': FieldReflection[int](
            int, id=0x363d98a4, original_name='CommandAction'
        ),
    })
    ...
    def _fast_decode(cls, data: typing.BinaryIO, property_count: int) -> typing_extensions.Self | None:
        ...
        return cls(editor_properties, item_to_change, capacity_increase, amount, command, command_action)
    ...
    def from_json(cls, data: json_util.JsonValue) -> typing_extensions.Self:
        ...
        return cls(
            ...
            item_to_change=json_data['item_to_change'],
            capacity_increase=json_data['capacity_increase'],
            amount=json_data['amount'],
            command=json_data['command'],
            command_action=json_data['command_action'],
        )
...
def _decode_item_to_change(data: typing.BinaryIO, property_size: int) -> int:
...
def _decode_command_action(data: typing.BinaryIO, property_size: int) -> int:
    return struct.unpack('>l', data.read(4))[0]
