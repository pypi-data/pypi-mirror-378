...
class PathControl(BaseObjectType):
    ...
    path_curve_type: PathCurveType = dataclasses.field(default=PathCurveType.Unknown1, metadata={
        'reflection': FieldReflection[PathCurveType](
            PathCurveType, id=0xf25ffdff, original_name='PathCurveType', from_json=PathCurveType.from_json, to_json=PathCurveType.to_json
        ),
    })
    radius_control: Spline = dataclasses.field(default_factory=Spline, metadata={
        'reflection': FieldReflection[Spline](
            Spline, id=0xf8af858c, original_name='RadiusControl', from_json=Spline.from_json, to_json=Spline.to_json
        ),
    })
    height_control: Spline = dataclasses.field(default_factory=Spline, metadata={
        'reflection': FieldReflection[Spline](
    ...
    def _fast_decode(cls, data: typing.BinaryIO, property_count: int) -> typing_extensions.Self | None:
        ...
        path_curve_type = PathCurveType.from_stream(data)
...
def _decode_path_flags(data: typing.BinaryIO, property_size: int) -> int:
    return struct.unpack(">L", data.read(4))[0]


def _decode_path_curve_type(data: typing.BinaryIO, property_size: int) -> PathCurveType:
    return PathCurveType.from_stream(data)
...
_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, int], typing.Any]]] = {
    ...
    0xf8af858c: ('radius_control', Spline.from_stream),
    0x72cd60ce: ('height_control', Spline.from_stream),
}
