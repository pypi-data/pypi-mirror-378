...
class ConditionalRelay(BaseObjectType):
    ...
    def _fast_decode(cls, data: typing.BinaryIO, property_count: int) -> typing_extensions.Self | None:
        ...
        conditional2 = ConditionalTest.from_stream(data, property_size, default_override={'boolean': Boolean.Unknown})
    
        property_id, property_size = struct.unpack(">LH", data.read(6))
        assert property_id == 0x49614c51
        conditional3 = ConditionalTest.from_stream(data, property_size, default_override={'boolean': Boolean.Unknown})
    
        property_id, property_size = struct.unpack(">LH", data.read(6))
        assert property_id == 0xb498b424
        conditional4 = ConditionalTest.from_stream(data, property_size, default_override={'boolean': Boolean.Unknown})
    ...
    def to_stream(self, data: typing.BinaryIO, default_override: dict | None = None) -> None:
        ...
        before = data.tell()
        data.write(b'\x00\x00')  # size placeholder
        self.conditional2.to_stream(data, default_override={'boolean': Boolean.Unknown})
        after = data.tell()
        data.seek(before)
        data.write(struct.pack(">H", after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b'\x00\x00')  # size placeholder
        self.conditional3.to_stream(data, default_override={'boolean': Boolean.Unknown})
        after = data.tell()
        data.seek(before)
        data.write(struct.pack(">H", after - before - 2))
        data.seek(after)
        ...
        before = data.tell()
        data.write(b'\x00\x00')  # size placeholder
        self.conditional4.to_stream(data, default_override={'boolean': Boolean.Unknown})
        after = data.tell()
        data.seek(before)
        data.write(struct.pack(">H", after - before - 2))
        data.seek(after)
...
def _decode_conditional2(data: typing.BinaryIO, property_size: int) -> ConditionalTest:
    return ConditionalTest.from_stream(data, property_size, default_override={'boolean': Boolean.Unknown})
