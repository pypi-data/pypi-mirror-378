...
class DynamicLightSpotlight(BaseProperty):
    spotlight_angle: Spline = dataclasses.field(default_factory=Spline, metadata={
        'reflection': FieldReflection[Spline](
            Spline, id=0xfeeca0d6, original_name='SpotlightAngle', from_json=Spline.from_json, to_json=Spline.to_json
        ),
    })
    spotlight_angle_duration: float = dataclasses.field(default=0.0, metadata={
        'reflection': FieldReflection[float](
            float, id=0x79aa798b, original_name='SpotlightAngleDuration'
        ),
    })
    spotlight_angle_loops: bool = dataclasses.field(default=False, metadata={
        'reflection': FieldReflection[bool](
            bool, id=0x2fd08300, original_name='SpotlightAngleLoops'
        ),
    })
...
def _decode_spotlight_angle_duration(data: typing.BinaryIO, property_size: int) -> float:
    return struct.unpack('>f', data.read(4))[0]


def _decode_spotlight_angle_loops(data: typing.BinaryIO, property_size: int) -> bool:
    return struct.unpack('>?', data.read(1))[0]


_property_decoder: dict[int, tuple[str, typing.Callable[[typing.BinaryIO, int], typing.Any]]] = {
    0xfeeca0d6: ('spotlight_angle', Spline.from_stream),
