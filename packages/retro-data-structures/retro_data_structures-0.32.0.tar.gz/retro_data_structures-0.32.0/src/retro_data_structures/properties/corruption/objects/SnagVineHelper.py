...
class SnagVineHelper(BaseObjectType):
    ...
    hud_static_intensity: float = dataclasses.field(default=2.0, metadata={
        'reflection': FieldReflection[float](
            float, id=0xf5221f0d, original_name='HudStaticIntensity'
        ),
    })
    hud_static_duration: float = dataclasses.field(default=0.6000000238418579, metadata={
        'reflection': FieldReflection[float](
            float, id=0x9f3e7f56, original_name='HudStaticDuration'
        ),
    })
...
def _decode_hud_static_intensity(data: typing.BinaryIO, property_size: int) -> float:
    return struct.unpack('>f', data.read(4))[0]


def _decode_hud_static_duration(data: typing.BinaryIO, property_size: int) -> float:
    return struct.unpack('>f', data.read(4))[0]
