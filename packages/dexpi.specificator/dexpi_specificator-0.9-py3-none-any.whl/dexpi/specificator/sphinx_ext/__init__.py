...
def symbol_preview_column(element, auto_dir, directive):
    ...
    color_cls = directive.sp.model_by_name['Core'].search_unique('Color', metamodel.Type)
...
class InternalAutoelementDirective(SphinxDirective):
    ...
    def navigation_bar(self):
        ...
        for owner in owners:
            if owner is not owners[0]:
                owners_paragraph.append(nodes.inline(text='::'))
 ...
 #           return []
        ...
        out_dir = self.env.app.outdir / '_images'

        out_dir.mkdir(parents=True, exist_ok=True)
        etree.ElementTree(diagram).write(out_dir / diagram_name)
        ...
        link_paragraph.extend([
            nodes.inline(text='('),
            nodes.reference(refuri=nesting_level * '../' + '_images/' + diagram_name, text='Fullscreen Diagram'),
            nodes.inline(text=')'),])
        

        return [
            link_paragraph,
            nodes.image(uri=str(out_dir / diagram_name))]
    ...
    def base_info(self):
        ...
        if isinstance(self.element, metamodel.Model):
            ...
            code_paragraph.append(
                nodes.reference(refuri=f'../../{self.element.name} RC 1.xml',
                                text=f'{self.element.name} RC 1.xml in DEXPI XML'))
...
class SpeciPyDomain(Domain):
    ...
    def __init__(self, env: BuildEnvironment) -> None:
        super().__init__(env)
        self._model_by_name: None | dict[str, metamodel.Model] = None
        self._model_set: None | metamodel.ModelSet = None
        self._metadata = None
    ...
    def metadata(self) -> MetaData:
        ...
        if metadata is None:
            if _METADATA:
                ...
                metadata = MetaData.read(self.env.app.confdir / '_models' / 'metadata', self.model_by_name)
    ...
    def model_set(self) -> metamodel.ModelSet:
        ...
        if model_set is None:
            model_set = self._model_set = metamodel.ModelSet()
    ...
    @property
    def model_by_name(self) -> dict[str: metamodel.Model]:
        model_by_name = self._model_by_name
        if model_by_name is None:
            if _MODEL_BY_NAME:
                model_by_name = _MODEL_BY_NAME
            else:
                model_dir = self.env.app.confdir / '_models'
                model_by_name = read_xmls(model_dir.glob('*.xml'))
            self._model_by_name = model_by_name
    ...
    def get_element(self, qname: str):
        ...
        if qname == 'Builtin.AnyURI':
            a=1
        model_name, *sub_names = qname.split('.')
        element = self.model_by_name.get(model_name)
        if not element:
            return None
            TODO
    ...
    def create_automodel_documents(self):
        for doc_name in self.env.found_docs:
            ...
            for model_name in find_model_names_in_toc_trees(doc_raw):
                model = self.model_by_name.get(model_name)
                if not model:
                    continue
...
_CACHE_DIR: None | Path = None

@contextlib.contextmanager
def model_by_name_context(model_by_name: dict[str, metamodel.Model], metadata, config):
    global _MODEL_BY_NAME
    if _MODEL_BY_NAME is not None:
        # TODO
        raise Exception('model_by_name_context already set')
    ...
    if _CACHE_DIR:
        _CACHE_DIR = Path(_CACHE_DIR) / 'uml-diagrams'
