...
class LaTeXRenderer(Renderer):
    ...
    def draw_box(self, position: Point, width: float, height: float, frame: bool, background_color: BackgroundColor):
        ...
        self.code_parts.append(fr'\draw[fill={fill}] {self.point_to_tex(position)} rectangle {self.point_to_tex(opp_position)};')
        ...
        if frame and not any('rect' in line for line in self.code_parts):
            print('############', position, width, height)
        
            box = fr'\draw [fill={fill}]{self.point_to_tex(position)} rectangle ({self.length_to_tex(width)},{self.length_to_tex(height)});'
            self.code_parts.append(box)
    ...
    def draw_line(self, points: list[Point], source_marker=None, target_marker=None, line_style: LineStyle='solid'):
       # return
        ...
        line_parts = [fr'\draw {self.point_to_tex(points[0])}']
...
def make_diagram(
    ...
    diagram_specification = make_diagram_specification(elements, get_link=get_link)
    renderer_cls = {
        'svg': SvgRenderer,
