...
def make(model_by_name, metadata, out_dir, config):
    ...
    if 0:
        ...
        sphinx_process = subprocess.Popen([
             SPHINX_BUILD_PATH, '-M', 'html', sphinx_project_dir, out_dir, '-v'])
        sphinx_process.wait()
    else:
        from sphinx.cmd.build import main
        from ... import sphinx_ext
        with sphinx_ext.model_by_name_context(model_by_name, metadata, config):
         #   main([str(sphinx_project_dir), str(out_dir), '-v'])
            main(['-M', 'html', str(sphinx_project_dir), str(out_dir), '-v'])
...
def clean_html(out_dir):
    ...
    for html_path in out_dir.glob('**/*.html'):
        ...
        for img in html_tree.xpath('//img'):
            ...
            svg_element.attrib['style'] = 'max-width: 100%;'
