            ...
            if self.get_download:
                ...
                try:
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        basename = os.path.basename(temp_path)          # e.g. rLlWcvLBluI.webm
                        filename, ext = os.path.splitext(basename)

                        # Put it inside a directory named after the video_id
                        dirbase = os.path.join(self.download_directory, video_id)
                        os.makedirs(dirbase, exist_ok=True)

                        final_path = os.path.join(dirbase, basename)
                        ...
                        # Store file path in info
                        self.info["file_path"] = final_path
                        self.info["video_id"] = video_id

                        # Update registry *with the actual final path*
                        data = self.registry.edit_info(data=self.info, url=video_url, video_id=video_id)
                        data["video_path"] = final_path
                        self.registry.edit_info(data=data, url=video_url, video_id=video_id)

                        logger.info(f"[VideoDownloader] Saved info to registry: {final_path}")
                        if self.get_info:
                            self.stop()
                            return self.info
