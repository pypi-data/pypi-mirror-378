...
from agent_analytics.instrumentation.utils.langgraph_utils import (
    is_langgraph_task,
    get_graph_structure,
    get_compiled_graph,
)
from agent_analytics.instrumentation.utils.common import _get_runnable_from_func
...
class TraceloopCallbackHandler(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        if metadata.get("langgraph_node") == name:
            ...
            if name in graph.builder.nodes:
                node_func = graph.builder.nodes[name].runnable.func
                runnable = _get_runnable_from_func(node_func)
                _set_span_attribute(span, "gen_ai.runnable.code_id", runnable.code_id)
                _set_span_attribute(span, "gen_ai.runnable.input_schema", runnable.input_schema)
                _set_span_attribute(span, "gen_ai.runnable.output_schema", runnable.output_schema)
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        tool_func = extract_tool_func()
        if tool_func:
            runnable = _get_runnable_from_func(tool_func)
            _set_span_attribute(span, "gen_ai.runnable.code_id", runnable.code_id)
            _set_span_attribute(span, "gen_ai.runnable.input_schema", runnable.input_schema)
            _set_span_attribute(span, "gen_ai.runnable.output_schema", runnable.output_schema)
