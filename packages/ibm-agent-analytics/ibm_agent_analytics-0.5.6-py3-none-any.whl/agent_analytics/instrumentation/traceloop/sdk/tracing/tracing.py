...
def _set_association_properties_attributes(span, properties: dict) -> None:
    for key, value in properties.items():
        # openai metrics fix
        # See https://github.com/traceloop/openllmetry/pull/2273
...
def init_instrumentations(
    ...
    for instrument in instruments:
        if instrument == Instruments.ALEPHALPHA:
            ...
            if init_crewai_instrumentor(new_trace_on_workflow):
                instrument_set = True
            ...
            if init_langchain_instrumentor(new_trace_on_workflow):
                instrument_set = True
        # elif instrument == Instruments.LANGGRAPH:
        #     if init_langgraph_instrumentor():
        #         instrument_set = True
        elif instrument == Instruments.LITELLM:
            if init_litellm_instrumentor():
                instrument_set = True
        # elif instrument == Instruments.LOGGER:
        #     if init_logger_instrumentor():
        #         instrument_set = True
...
def init_langchain_instrumentor(new_trace_on_workflow: bool = False):
    try:
...
# Add CrewAI
def init_crewai_instrumentor(new_trace_on_workflow: bool = False):
    try:
...
def init_logger_instrumentor():
    try:
        Telemetry().capture("instrumentation:logger:init")
        from .opentelemetry_instrumentation_logger import LoggerInstrumentation

        instrumentor = LoggerInstrumentation()
        if not instrumentor.is_instrumented_by_opentelemetry:
            instrumentor.instrument()
        return True
    except Exception as e:
