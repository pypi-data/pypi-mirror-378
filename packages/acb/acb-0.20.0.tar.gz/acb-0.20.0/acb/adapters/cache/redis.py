...
class Cache(CacheBase, SSLConfigMixin):
    ...
    async def init(self, *args: t.Any, **kwargs: t.Any) -> None:
        ...
        if self.config.cache.connection_string:
            masked_connection_string = self.config.cache.connection_string
            if "@" in masked_connection_string:
                parts = masked_connection_string.split("@")
                if ":" in parts[0]:
                    ...
                    if len(auth_part) >= 3:
                        ...
                        auth_part[1] = "***"
                        parts[0] = ":".join(auth_part)
                masked_connection_string = "@".join(parts)
            self.logger.info(
