...
def _query_account_usage(cli: SnowCLI, database: Optional[str], schema: Optional[str]) -> List[DependencyEdge]:
    filters: List[str] = []
    if database:
...
def _query_information_schema(cli: SnowCLI, database: Optional[str], schema: Optional[str]) -> List[DependencyEdge]:
    ...
    for row in out.rows:
        src = _fq(row.get("VIEW_CATALOG"), row.get("VIEW_SCHEMA"), row.get("VIEW_NAME") or "")
        tgt = _fq(
