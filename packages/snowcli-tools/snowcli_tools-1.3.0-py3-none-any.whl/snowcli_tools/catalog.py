...
def _list_databases(cli: SnowCLI, include_account: bool, only_database: Optional[str]) -> List[str]:
    if only_database:
...
def _get_ddl(cli: SnowCLI, object_type: str, fq_name: str, timeout: int = 60) -> Optional[str]:
    try:
        out = cli.run_query(
...
def _extract_args_from_arguments_field(name: str, arguments: Optional[str], signature: Optional[str]) -> Optional[str]:
...
def build_catalog(
    ...
    *,
    ...
    def _sig_for(rec: Dict, name: Optional[str]) -> Optional[str]:
        return _extract_args_from_arguments_field(str(name) if name else "", rec.get("arguments"), rec.get("signature"))

    def _identity(category: str, rec: Dict, name_key: str, sig: Optional[str]) -> Optional[str]:
        db = rec.get("DATABASE_NAME") or rec.get("TABLE_CATALOG") or rec.get("CATALOG_NAME") or rec.get("database_name")
        sch = rec.get("SCHEMA_NAME") or rec.get("TABLE_SCHEMA") or rec.get("schema_name")
    ...
    if incremental and include_ddl:
        ...
        def load_prev(base: str, category: str, name_key: str) -> None:
            ...
            for r in _load_rows(p):
                ...
                sig = _sig_for(r, name) if category in ("FUNCTION", "PROCEDURE") else None
    ...
    if include_ddl:
        ...
        def add_job(
            ...
            sch = rec.get("schema_name") or rec.get("SCHEMA_NAME") or rec.get("TABLE_SCHEMA") or rec.get("table_schema")
            name = rec.get(name_key) or rec.get(name_key.upper()) or rec.get("name")
        ...
        for r in all_functions:
            ...
            name = r.get("name")
            sig = _extract_args_from_arguments_field(str(name) if name else "", r.get("arguments"), r.get("signature"))
    ...
    _write_json(out_path / "catalog_summary.json", [{"totals": totals, "databases": databases}])
...
def export_sql_from_catalog(
    ...
    if summary_path.exists():
        try:
            ...
            totals = (summary[0] if isinstance(summary, list) and summary else {}).get("totals", {})
    ...
    if path:
        for r in _load_rows(path):
            ...
            if not (dbn and schn and name):
                continue
            fq = f"{_quote_ident(dbn)}.{_quote_ident(schn)}.{_quote_ident(name)}" + (sig or "")
            jobs.append(
                (
                    "functions",
                    "FUNCTION",
