...
class TestMarginAnalysisClient(BaseClientTest):
    ...
    def test_list(self):
        """测试按目标代码不分页获取融资融券分析"""
        print("\n" + "=" * 50)
        print("测试融资融券分析客户端 - 按目标代码不分页获取")
        print("=" * 50)
        
        try:
            result = self.client.margin_analysis.list(analysis_type="index", target_code="000300.SH", start_date="20250101", end_date="20250922")   
            print(f"状态: {result.get('status')}")
            self.print_list_info(result)
        except Exception as e:
            print(f"测试按目标代码不分页获取时出错: {e}")
