...
from .data import AtomNotFound, AtomImpl
...
class AtomTable:
    ...
    def __init__(self):
        self._atoms: Dict[str, AtomImpl] = {}       # name -> Atom object
        self._by_id: Dict[int, AtomImpl] = {}       # id -> Atom object
        self._counter = 0
        
        # Only need lock for the creation path
        self._creation_lock = threading.Lock()
    
    def ensure(self, name: str) -> AtomImpl:
        ...
        with self._creation_lock:
            ...
            atom_obj = AtomImpl(atom_id, name)
    ...
    def get_by_name(self, name: str) -> Optional[AtomImpl]:
        """Get atom by name, None if not exists."""
        return self._atoms.get(name)
    
    def get_by_id(self, atom_id: int) -> AtomImpl:
        """Get atom by ID."""
    ...
    def ensure_many(self, names: Iterable[str]) -> List[AtomImpl]:
    ...
    def all_atoms(self) -> Dict[str, AtomImpl]:
