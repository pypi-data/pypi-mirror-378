...
class Evaluator:
    ...
    def meta_simulate(self, dg_model_real: DGModel, n_learning: int, n_train: int,n_test: int, n_true_repetitions: int, n_practitioner_repetitions: int, n_sl_repetitions: int, kfolds: int):
        ...
        pd.DataFrame(list_of_ntrue_accuracies).to_csv(
            os.path.join(figuredirname, "ntrue_accuracies.csv"), index=False
        )
        pd.DataFrame(list_of_ntrue_cv_accuracies).to_csv(
            os.path.join(figuredirname, "ntrue_cv_accuracies.csv"), index=False
        )
        pd.DataFrame(list_of_npractitioner_accuracies).to_csv(
            os.path.join(figuredirname, "npractitioner_accuracies.csv"), index=False
        )
        pd.DataFrame(list_of_npractitioner_cv_accuracies).to_csv(
            os.path.join(figuredirname, "npractitioner_cv_accuracies.csv"), index=False
        )
        pd.DataFrame(list_of_nsl_accuracies).to_csv(
            os.path.join(figuredirname, "nsl_accuracies.csv"), index=False
        )
        pd.DataFrame(list_of_nsl_cv_accuracies).to_csv(
            os.path.join(figuredirname, "nsl_cv_accuracies.csv"), index=False
        )
        dag_rows = []
        for sl, records in list_of_dag_fidelity.items():
            for rep_idx, rec in enumerate(records):
                ...
                row.update(rec)
                dag_rows.append(row)
        pd.DataFrame(dag_rows).to_csv(
            os.path.join(figuredirname, "dag_fidelity.csv"), index=False
        )
        dist_rows = []
        ...
        pd.DataFrame(dist_rows).to_csv(
            os.path.join(figuredirname, "distribution_fidelity.csv"), index=False
        )
