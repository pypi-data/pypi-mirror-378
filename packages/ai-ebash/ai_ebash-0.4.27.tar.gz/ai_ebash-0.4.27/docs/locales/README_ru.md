...
> **Интеграция искусственного интеллекта в ваш терминал.** Задавайте вопросы ***ChatGPT***, ***Deep Seek***, ***Grok*** и многим другим большим языковым моделям (LLM) и выполняйте предложенные скрипты и команды прямо из командной строки. Идеально подходит для новичков в администрировании Linux и Windows.
...
При первом запуске программа использует нейросеть от Microsoft - **DeepSeek-R1-Lite-Preview** с публичным токеном. Это не самый лучший выбор, так как при множестве запросов может периодически появляться сообщение о превышении квоты запросов, но для тестового запуска сойдет. 

Для полноценной работы смените провайдера в [настройках](#конфигурация). По умолчанию в программу уже добавлено несколько популярных моделей, таких как **DeepSeek**, **Grok 4 Fast**, **Qwen3 Coder**. 

**Для того чтобы эти нейросети заработали необходимо [получить](#получение-токена-api_key-и-подключение-к-предустановленной-нейросети) личный токен и добавить его в настройки выбранной нейросети в [настройках](#установка) программы.**

> [!NOTE]
> Ai-eBash может работать с любой нейросетью поддерживающей подключение по API. На сегодняшний день это практически все большие языковые модели (LLM) представленные на рынке.

## Подключение к нейросетям
В Ai-eBash уже предустановлено несколько популярных нейросетей, однако политика провайдеров не позволяет полноценно работать без авторизации. Вам необходимо получить личный токен (или по другому - API_KEY) на сайте провайдера.
