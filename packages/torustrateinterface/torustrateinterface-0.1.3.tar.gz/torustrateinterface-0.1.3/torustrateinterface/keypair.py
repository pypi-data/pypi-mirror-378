...
class Keypair:
    ...
    def create_from_mnemonic(
        ...
        if crypto_type == KeypairType.ECDSA:
            ...
            keypair = cls.create_from_private_key(
                private_key, ss58_format=ss58_format, crypto_type=crypto_type
            )
    ...
    def create_from_uri(
        ...
        if crypto_type == KeypairType.ECDSA:
            ...
            derived_keypair = cls.create_from_private_key(
                private_key, ss58_format=ss58_format, crypto_type=crypto_type
            )
        else:
    ...
    def create_from_encrypted_json(
        ...
        private_key, public_key = decode_pair_from_encrypted_json(
            json_data, passphrase, maxmem
        )
        ...
        return cls.create_from_private_key(
            private_key, public_key, ss58_format=ss58_format, crypto_type=crypto_type
        )
    ...
    def export_to_encrypted_json(self, passphrase: str, name: str = None) -> dict:
        ...
        if self.crypto_type != KeypairType.SR25519:
            raise NotImplementedError(
                f"Cannot create JSON for crypto_type '{self.crypto_type}'"
            )
    ...
    def verify(
        ...
        if not verified:
            ...
            verified = crypto_verify_fn(
                signature, b"<Bytes>" + data + b"</Bytes>", self.public_key
            )
