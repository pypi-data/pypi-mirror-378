...
class SubstrateInterface:
    ...
    def compose_call(
        ...
        PATCHED VERSION: Includes fix for BoundedBTreeMap encoding issues.
        ...
        # Check if this is a call that uses BoundedBTreeMap parameters
        if call_module == "Permission0" and call_function == "update_stream_permission":
            from scalecodec.types import BoundedBTreeMap, Map, BTreeMap
            from scalecodec.base import ScaleBytes
            from scalecodec.types import CompactU32
            
            original_btreemap_encode = BoundedBTreeMap.process_encode
            original_map_encode = Map.process_encode
            
            def fixed_btreemap_encode(self, value):
                """Fixed encoding for BoundedBTreeMap that handles various input formats."""
                ...
                if isinstance(value, list):
                    ...
                    map_data = list(value.items())
                else:
                    map_data = value
                
                if not hasattr(self, 'map_key') or self.map_key is None:
                    self.map_key = 'AccountId'
                    self.map_value = 'u16'
                
                try:
                    element_count_compact = CompactU32()
                    element_count_compact.encode(len(map_data))
                    data = element_count_compact.data
                    
                    for item_key, item_value in map_data:
                    ...
                    return original_btreemap_encode(self, value)
            
            def fixed_map_encode(self, value):
                """Fixed encoding for Map that ensures map_key and map_value are set."""
                if not hasattr(self, 'map_key') or self.map_key is None:
                    # Set default types for Permission0 BoundedBTreeMap
                    self.map_key = 'AccountId'
                    self.map_value = 'u16'
                return original_map_encode(self, value)
            
            BoundedBTreeMap.process_encode = fixed_btreemap_encode
            BTreeMap.process_encode = fixed_btreemap_encode
            Map.process_encode = fixed_map_encode
            ...
            try:
                ...
                try:
                    ...
                    if "Element count of value" in str(e) and "doesn't match type_definition" in str(e):
                        if "new_recipients" in call_params and call_params["new_recipients"]:
                            wrapped_params = call_params.copy()
                            wrapped_params["new_recipients"] = [call_params["new_recipients"]]
