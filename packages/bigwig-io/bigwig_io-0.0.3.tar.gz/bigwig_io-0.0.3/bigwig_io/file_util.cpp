...
public:
    ...
    std::future<ByteArray> read(int64_t size, int64_t offset, bool ensure_size = false) {
        ...
        if (!buffer) {
            ...
            std::async(std::launch::async, [buffer, read_future = std::move(read_future)]() mutable {
                try {
