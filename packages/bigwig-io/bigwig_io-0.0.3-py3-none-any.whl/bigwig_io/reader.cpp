...
public:
    ...
    BigwigReader(const std::string& p, uint64_t pa = 0) : path(p), parallel(pa) {
        file = std::make_shared<BufferedFilePool>(path, "r");
    ...
    std::vector<float> read_values(
        ...
        if (bin_averaging == "none") {
            ...
            throw std::runtime_error("bin_averaging " + bin_averaging + " invalid");
        }
