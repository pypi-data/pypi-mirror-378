...
std::vector<float> read_values_at_locs(
    ...
    while (!(result = tree_nodes.next()).done) {
        ...
        auto future = std::async(std::launch::async, [&file, uncompress_buffer_size, &locs, result, resolution, &values, zoom_level]() {
            if (zoom_level < 0) {
                fill_values_at_locs(
                    file,
...
std::vector<float> read_values_stats_at_locs(
    ...
    while (!(result = tree_nodes.next()).done) {
        tracker.update(tree_nodes.coverage);
        auto future = std::async(std::launch::async, [&file, uncompress_buffer_size, &locs, result, resolution, &values_stats, zoom_level, &lock_chrs]() {
...
std::vector<float> read_stats_at_locs(
    ...
    while (!(result = tree_nodes.next()).done) {
        tracker.update(tree_nodes.coverage);
        auto future = std::async(std::launch::async, [&file, uncompress_buffer_size, &locs, result, &values_stats, zoom_level, &lock_locs]() {
