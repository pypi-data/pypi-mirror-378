...
class MetadataDict(TypedDict):
    """
    A typed dictionary to represent metadata for a message in the AgentChatbot component. An
...
class ChatMessage:
    """
    A dataclass that represents a message in the AgentChatbot component (with type="messages"). The only required field is `content`. The value of `gr.Chatbot` is a list of these dataclasses.
    Parameters:
...
class AgentChatbot(Component):
    ...
    def __init__(
        ...
        *,
        ...
        editable: Literal["llm"] | None = None,
        latex_delimiters: list[dict[str, str | bool]] | None = None,
        rtl: bool = False,
        show_share_button: bool | None = None,
        show_copy_button: bool = False,
        watermark: str | None = None,
        avatar_images: tuple[str | Path | None, str | Path | None, str | Path | None] | None = None,
        ...
        Parameters:
            ...
            examples: A list of example messages to display in the chatbot before any user/assistant messages are shown. Each example should be a dictionary with an optional "text" key representing the message that should be populated in the AgentChatbot when clicked, an optional "files" key, whose value should be a list of files to populate in the AgentChatbot, an optional "icon" key, whose value should be a filepath or URL to an image to display in the example box, and an optional "display_text" key, whose value should be the text to display in the example box. If "display_text" is not provided, the value of "text" will be displayed.
        ...
        """
        self.type: Literal["tuples", "messages"] = "messages"
        ...
        self.avatar_images: list[dict | None] = [None, None, None]
        if avatar_images is None:
            ...
            self.avatar_images = [
                ...
                self.serve_static_file(avatar_images[2]),
            ]
    ...
    def _preprocess_content(
        ...
        if chat_message is None:
            ...
            if component is not None:
                ...
                raise ValueError(
                    f"Invalid component for AgentChatbot component: {chat_message.component}"
                )
        else:
    ...
    def _create_file_message(chat_message, filepath):
        ...
        return FileMessage(
            ...
            alt_text=AgentChatbot._get_alt_text(chat_message),
        )
    ...
    def _postprocess_message_messages(
        ...
        if isinstance(message, dict):
            ...
            raise Error(
                f"Invalid message for AgentChatbot component: {message}", visible=False
            )
    ...
    def example_payload(self) -> Any:
        if self.type == "messages":
            return [
                Message(name="user", role="user", content="Hello!").model_dump(),
                Message(name="llm", role="assistant", content="How can I help you?").model_dump(),
            ]
        return [["Hello!", None]]
    ...
    def example_value(self) -> Any:
        if self.type == "messages":
            return [
                Message(name="user", role="user", content="Help me search for weather in Singapore today.").model_dump(),
                Message(name="terminal", role="assistant", content="$root : ").model_dump(),
                Message(name="llm", role="assistant", content="Sure! \n ```shell\n$root : ddgr xxx\n```").model_dump(),
            ]
        return [["Hello!", None]]
