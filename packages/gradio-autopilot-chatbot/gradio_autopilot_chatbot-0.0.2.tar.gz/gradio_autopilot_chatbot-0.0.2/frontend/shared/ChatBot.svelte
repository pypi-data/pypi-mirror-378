<script lang="ts">
  ...
  import { available_names } from "../constants";
  ...
  export let editable: "llm" | null = null;
  export let show_share_button = false;
  export let show_copy_all_button = false;
  export let rtl = false;
  export let show_copy_button = false;
  export let avatar_images: [
    FileData | null,
    FileData | null,
    FileData | null,
  ] = [null, null, null];
  ...
  {#if value !== null && value.length > 0 && groupedMessages !== null}
    <div class="message-wrap" use:copy>
      {#each groupedMessages as messages, i}
        {@const name = messages[0].name}
        {@const role = messages[0].role === "user" ? "user" : "bot"}
        {@const avatar_img =
          avatar_images[available_names.indexOf(messages[0].name)]}
