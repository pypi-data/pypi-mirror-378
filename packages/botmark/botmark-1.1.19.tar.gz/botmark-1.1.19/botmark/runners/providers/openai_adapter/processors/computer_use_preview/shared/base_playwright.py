...
# Only for type checking – avoids runtime dependency on Playwright
if TYPE_CHECKING:  # pragma: no cover
    from playwright.sync_api import Browser, Page  # noqa: F401
...
class BasePlaywrightComputer:
    ...
    Abstract base for Playwright-based computers:
      ...
      - Subclasses override `_get_browser_and_page()` to create a local or remote
        connection, returning (Browser, Page).
      - This base class handles context creation (`__enter__`/`__exit__`),
        plus common "computer-like" actions such as click, scroll, type, etc.
      - Also provides extra browser actions: `goto(url)`, `back()`, `forward()`.
    """
    ...
    def __enter__(self):
        # Lazy import ensures module can be imported without Playwright installed
        try:
        ...
        # Start Playwright and call the subclass hook to get browser/page
        self._playwright = sync_playwright().start()
        self._browser, self._page = self._get_browser_and_page()

        # Example: set up route interception (can be extended with blocking logic)
