            ...
            if ident:
                if not "disabled" in attrs["class"]:
                    ...
                    if src:
                        ...
                        ROOT = Path(botmark.__file__).resolve().parent
                        src = str( Path(src.replace("$PROJECT_ROOT", str(ROOT))))
                    ...
                    if "info" in attrs["class"]:
                        info_content = re.sub(r'<!--.*?-->', '', content, flags=re.DOTALL)
                        info_tokens += md.parse( info_content )
