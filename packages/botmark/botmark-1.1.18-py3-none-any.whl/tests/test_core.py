...
load_dotenv(os.path.dirname(os.path.realpath(__file__)) + "/.env.test")
...
# ──────────────────────────────────────────────────────────────────────────────
# BASIC SANITY: ensure local import
# ──────────────────────────────────────────────────────────────────────────────
...
def test_qa_block_all_sources(source_kind, label, model_id, test_name, qa_list):
    ...
    for i, qa in enumerate(qa_list):
        ...
        try:
            ...
            # regex: and contains:
            REGEX_PREFIX    = re.compile(r'^\s*regex\s*:\s*(.*)$', re.IGNORECASE)
            CONTAINS_PREFIX = re.compile(r'^\s*contains\s*:\s*(.*)$', re.IGNORECASE)

            if (
                actual == expected
                or (
                    (m := REGEX_PREFIX.match(expected))
                    and re.match(m.group(1), actual, flags=re.DOTALL)
                )
                or (
                    (m := CONTAINS_PREFIX.match(expected))
                    and m.group(1) in actual
                )
            ):
