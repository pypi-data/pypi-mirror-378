...
class HypeRate:
    ...
    async def join_channel(self, channel_name: str) -> None:
        ...
        This method sends a join packet for the specified channel and fires
        the 'channel_joined' event.
        ...
        try:
            ...
            self.logger.debug("Successfully joined channel: %s", channel_name)
        except websockets.exceptions.WebSocketException as e:
    ...
    async def leave_channel(self, channel_name: str) -> None:
        ...
        This method sends a leave packet for the specified channel and fires
        the 'channel_left' event.
        ...
        try:
            ...
            self.logger.debug("Successfully left channel: %s", channel_name)
        except websockets.exceptions.WebSocketException as e:
    ...
    def _handle_message(self, message: Union[str, bytes]) -> None:
        ...
        try:
            ...
            if topic.startswith("hr:"):
                ...
                hr = payload.get("hr")
                if hr is not None:
                    self.logger.debug("Heartbeat data received for topic %s: HR=%s",
                                      topic, hr)
                ...
                self.logger.debug("Received message for topic: %s", topic)
