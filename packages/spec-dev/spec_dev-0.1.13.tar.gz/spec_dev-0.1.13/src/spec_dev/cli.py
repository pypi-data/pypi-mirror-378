...
from .tasks_board import move_task
...
def build_parser() -> argparse.ArgumentParser:
    ...
    move_parser = subparsers.add_parser(
        "tasks-move",
        help="Move a task card into backlog, in-progress, or done.",
    )
    ...
    move_parser.add_argument(
        "--to",
        required=True,
        choices=["backlog", "in-progress", "done"],
        help="Target section for the task card.",
    )
    move_parser.add_argument(
        "destination",
        nargs="?",
    ...
    move_parser.set_defaults(handler=_handle_tasks_move)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        argv = ["init"]
    elif argv[0] not in {"init", "memory", "cycle", "tasks-manifest", "tasks-move"} and not argv[0].startswith("-"):
        argv = ["init", *argv]
...
def _handle_cycle(args: argparse.Namespace) -> int:
    ...
    print(f"Archived active artifacts to {archived} and reset active-dev")
    return 0
...
def _handle_tasks_move(args: argparse.Namespace) -> int:
    destination = Path(args.destination)
    move_task(destination, args.task_id, args.to)
    print(
        f"Moved {args.task_id} to {args.to}. Run spec-dev tasks-manifest --rewrite --check to refresh coverage."
    )
    return 0
