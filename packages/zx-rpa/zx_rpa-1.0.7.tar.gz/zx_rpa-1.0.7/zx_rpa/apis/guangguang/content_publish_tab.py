...
class ContentPublishTab:
    ...
    def input_video_description(self, description):
        """视频描述 输入视频描述"""
        self.tab.ele("t:div@@class:publish-content__description-input--inputWrap").ele("t:textarea").input(description, clear=True)
    ...
    def input_topic(self, topic: str):
        """输入话题"""
        self.tab.ele("t:input@@aria-label=输入关键词搜索").input(topic, clear=True)
