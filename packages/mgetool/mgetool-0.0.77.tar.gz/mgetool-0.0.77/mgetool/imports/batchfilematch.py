...
class BatchFileMatch:
    ...
    def __init__(self, path=".", suffix=None, patten=None, trans=True):
        ...
        if patten is not None:
            ...
            if suffix is not None:
                ...
                self.file_list = list(rglob_file(self.path,"*"))
    ...
    def filter_file_name_parent_folder(self, exclude=None,remove_parent=False):
        ...
        if exclude:
            ...
            if not any([i in exclude for i in ["*", "|", "?", ".", "\\", "[", "+"]]):  # 简单模式
                del_dirs = [self.file_list[r].parent for r, i in enumerate(files) if exclude in str(i)]
            else:
                pt2 = self._comp(exclude)
                del_dirs = [self.file_list[r].parent for r, i in enumerate(files) if re.search(pt2, str(i)) is not None]
    ...
    def filter_file_name(self, include: Union[List[str], str] = None, exclude: Union[List[str], str] = None,
        ...
        if patten:
            ...
            self.file_list = [i for i in self.file_list if re.search(pt, str(i)) is not None]
        else:
            if not include and not exclude:
                ...
                if include:
                    ...
                    if not any([i in include for i in ["*", "|", "?", ".", "\\", "[", "+"]]):  # 简单模式
                        self.file_list = [self.file_list[r] for r, i in enumerate(files) if include in str(i)]
                        files = [i for i in files if include in i] if exclude else []
                    else:
                        pt = self._comp(include)
                        index = [r for r, i in enumerate(files) if re.search(pt, str(i)) is not None]
                ...
                if exclude:
                    exclude = self._get_patten(exclude)
                    if not any([i in exclude for i in ["*", "|", "?", ".", "\\", "[", "+"]]):  # 简单模式
                        self.file_list = [self.file_list[r] for r, i in enumerate(files) if exclude not in str(i)]
                    else:
                        pt2 = self._comp(exclude)
                        self.file_list = [self.file_list[r] for r, i in enumerate(files) if re.search(pt2, str(i)) is None]
        ...
        if patten:
            ...
            if not include and not exclude:
                ...
                if include:
                    include = self._get_patten(include)
                    if not any([i in include for i in ["*", "|", "?", ".", "\\", "[", "+"]]):  # 简单模式
                        self.file_list = [self.file_list[r] for r, i in enumerate(file_dir) if include in str(i)]
                        file_dir = [i for i in file_dir if include in str(i)] if exclude else []
                    else:
                        pt = self._comp(include)
                        index = [r for r, i in enumerate(file_dir) if re.search(pt, str(i)) is not None]
                        self.file_list = [self.file_list[r] for r in index]
                ...
                if exclude:
                    exclude = self._get_patten(exclude)
                    if not any([i in exclude for i in ["*", "|", "?", ".", "\\", "[", "+"]]):  # 简单模式
                        self.file_list = [self.file_list[r] for r, i in enumerate(file_dir) if exclude not in str(i)]
                    else:
                        pt2 = self._comp(exclude)
                        self.file_list = [self.file_list[r] for r, i in enumerate(file_dir) if
                                          re.search(pt2, str(i)) is None]
