...
class TestFlowRegLiveBasics:
    ...
    def test_basic_initialization(self):
        ...
        assert flow_reg.options.save_w == True  # Always enabled for flow persistence
    ...
    def test_initialization_with_options(self):
        ...
        assert flow_reg.options.quality_setting == QualitySetting.FAST
        assert flow_reg.options.save_w == True
...
class TestFlowRegLiveReferenceUpdate:
    ...
    def test_reference_normalization_update(self):
        ...
        initial_norm_min = flow_reg.norm_min.copy() if isinstance(flow_reg.norm_min, list) else flow_reg.norm_min
        initial_norm_max = flow_reg.norm_max.copy() if isinstance(flow_reg.norm_max, list) else flow_reg.norm_max
        ...
        if isinstance(initial_norm_min, list):
            ...
            min_changed = any(not np.isclose(initial_norm_min[i], new_norm_min[i], rtol=5e-2) 
                             for i in range(len(initial_norm_min)))
            max_changed = any(not np.isclose(initial_norm_max[i], new_norm_max[i], rtol=5e-2) 
            ...
            assert min_changed or max_changed, \
                f"Expected normalization to change: min {initial_norm_min} -> {new_norm_min}, max {initial_norm_max} -> {new_norm_max}"
        else:
            assert not (np.isclose(initial_norm_min, new_norm_min, rtol=5e-2) and 
                       np.isclose(initial_norm_max, new_norm_max, rtol=5e-2))
