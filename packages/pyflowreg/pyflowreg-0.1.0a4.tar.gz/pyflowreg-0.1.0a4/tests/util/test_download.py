...
class TestDownloadData:
    ...
    def test_download_data_creates_folder(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('urllib.request.urlretrieve') as mock_retrieve:
                ...
                result = download_data("http://example.com/file.txt", "test.txt", output_folder)
    ...
    def test_download_data_skips_existing_file(self, capsys):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('urllib.request.urlretrieve') as mock_retrieve:
                result = download_data("http://example.com/file.txt", "existing.txt", output_folder)
...
class TestDownloadDemoData:
    ...
    def test_download_demo_data_default_folder(self):
        ...
        with patch('pyflowreg.util.download.download_data') as mock_download:
            ...
            result = download_demo_data("jupiter.tiff")
    ...
    def test_download_demo_data_custom_folder(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('pyflowreg.util.download.download_data') as mock_download:
                ...
                result = download_demo_data("jupiter.tiff", custom_folder)
...
class TestDemoDataUrls:
    ...
    def test_demo_url_accessible(self, demo_name, url):
        ...
        try:
            with urllib.request.urlopen(request, timeout=10) as response:
                ...
                assert response.status == 200, f"URL for {demo_name} returned status {response.status}"
            ...
            try:
                with urllib.request.urlopen(request, timeout=10) as response:
                    ...
                    assert response.status in [200, 206], f"URL for {demo_name} not accessible: {e}"
            except Exception as e2:
                pytest.skip(f"Could not verify {demo_name} URL (might be network issue): {e2}")
        except Exception as e:
            pytest.skip(f"Could not verify {demo_name} URL (might be network issue): {e}")
    ...
    def test_all_expected_demos_present(self):
        ...
        assert expected_demos == actual_demos, f"Missing or extra demos: {expected_demos ^ actual_demos}"
    ...
    def test_download_synth_data_integration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(result, 'rb') as f:
                ...
                assert magic[:4] == b'\x89HDF' or magic == b'\x89\x48\x44\x46\x0d\x0a\x1a\x0a'
