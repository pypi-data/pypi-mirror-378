...
class TestEstimateRigidXcorr2D:
    ...
    def test_basic_rigid_shift_detection(self):
        ...
        mov = ndi_shift(ref, shift=(true_dx_dy[1], true_dx_dy[0]), order=1, mode='constant')
    ...
    def test_subpixel_accuracy(self):
        ...
        mov = ndi_shift(ref, shift=(true_dx_dy[1], true_dx_dy[0]), order=3, mode='constant')
    ...
    def test_multichannel_input(self):
        ...
        for c in range(C):
            mov[..., c] = ndi_shift(ref[..., c], shift=(true_dx_dy[1], true_dx_dy[0]), order=1, mode='constant')
    ...
    def test_weighted_estimation(self):
        ...
        estimated_shift = estimate_rigid_xcorr_2d(ref, mov, weight=weight, target_hw=(64, 64), up=1)
...
class TestCCPrealignmentIntegration:
    ...
    def synthetic_rigid_data(self):
        ...
        y, x = np.ogrid[:H, :W]
        # Add gaussian blob
        reference += (10 * np.exp(-((y - 64) ** 2 + (x - 64) ** 2) / 200)).astype(np.float32)
        ...
        true_shifts = np.array([[10.0, -5.0], [-8.0, 7.0], [6.0, -4.0], [-5.0, -3.0], [4.0, 6.0]], dtype=np.float32)

        for t in range(T):
            # Create moved frame by shifting reference
            frames[t] = ndi_shift(reference, shift=(true_shifts[t, 1], true_shifts[t, 0]),  # (dy, dx)
    ...
    def test_cc_improves_registration(self, synthetic_rigid_data):
        ...
        options_no_cc = OFOptions(alpha=1.0, levels=5,  # Fewer levels
            eta=0.8, iterations=5,  # Very few iterations
            cc_initialization=False, quality_setting='fast')

        # Options with CC prealignment
        options_with_cc = OFOptions(alpha=1.0, levels=5,  # Same levels
            eta=0.8, iterations=5,  # Same few iterations
        ...
        assert error_with_cc <= error_no_cc * 1.05, f"CC error {error_with_cc:.4f} should be <= no-CC error {error_no_cc:.4f}"
        # Final error should be reasonable
        assert error_with_cc < 0.5

    @pytest.mark.parametrize("executor_name", ['sequential', 'threading', 'multiprocessing'])
    def test_cc_with_different_executors(self, synthetic_rigid_data, executor_name):
        ...
        options = OFOptions(
            alpha=1.0, levels=10, eta=0.8, iterations=20, 
            cc_initialization=True, cc_hw=64, cc_up=1,
...
if __name__ == "__main__":
    ...
    print(f"Error: dx={abs(estimated[0] - (-shift_x)):.2f}, dy={abs(estimated[1] - (-shift_y)):.2f}")
    ...
    options = OFOptions(cc_initialization=True, cc_hw=64, cc_up=2, quality_setting='fast')
