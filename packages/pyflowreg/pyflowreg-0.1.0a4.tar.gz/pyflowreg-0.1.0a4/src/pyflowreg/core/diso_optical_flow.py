...
class DisoOF:
    ...
    def _to_gray(self, img: np.ndarray, weight: Optional[np.ndarray] = None) -> np.ndarray:
        """
        ...
        if img.ndim == 3:
            ...
            if weight is not None:
                if weight.ndim == 1:
                    ...
                    weight = np.ones((img.shape[0], img.shape[1], img.shape[2])) * weight.reshape(1, 1, -1)
                elif weight.ndim == 2:
    ...
    def __call__(
        ...
        **kwargs : dict
        ...
        init = None
        if w is not None and isinstance(w, np.ndarray) and w.ndim == 3 and w.shape[2] == 2:
