    ...
    try:
        with tifffile.TiffFile(file_path) as tif:
                    ...
                    if 'hFastZ' in si:
                        ...
                        if 'enable' in fastz and fastz['enable']:
                            if 'numFramesPerVolume' in fastz:
                                metadata['slices_per_volume'] = int(fastz['numFramesPerVolume'])
            ...
            if not metadata['version'] and len(tif.pages) > 0:
                ...
                if hasattr(first_page, 'tags') and 'ImageDescription' in first_page.tags:
            ...
            if metadata['is_scanimage']:
                ...
                frames_from_metadata = (metadata['volumes'] * 
                ...
                metadata['total_frames'] = metadata['volumes'] * metadata['slices_per_volume']
            ...
            if match:
                if key == 'channels':
                    ...
                    if '[' in pattern:
                        ...
                        channels = [int(x) for x in channel_str.replace(',', ' ').split()]
                    ...
                    if '[' in pattern:
                        ...
                        z_str = match.group(1)
                        z_positions = [float(x) for x in z_str.replace(',', ' ').split()]
...
def interpret_scanimage_dimensions(tif_shape: Tuple[int, ...], 
                                  tif_axes: str,
                                  si_metadata: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if si_metadata.get('is_scanimage'):
        ...
        volumes = si_metadata.get('volumes', 1)
