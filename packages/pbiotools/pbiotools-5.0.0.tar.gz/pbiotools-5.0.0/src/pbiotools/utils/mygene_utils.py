...
def query_mygene(
    ...
    import pandas as pd
    import mygene
    import pbiotools.misc.pandas_utils as pd_utils
    ...
    res_df = functools.reduce(
        lambda df1, df2: pd.merge(df1, df2, how="inner", on="gene_id"), dfs
    )
...
def get_transcript_to_gene_mapping(transcript_ids, species="all"):
    ...
    id_mapping = parallel.apply_iter_simple(
        res, _parse_transcript_to_gene_query_result, progress_bar=True
    )
...
def _parse_gene_to_transcript_query_result(entry):
    ...
    if "ensembl.transcript" in entry:
        ...
        ret = [
            {"transcript_id": transcript, "gene_id": gene} for transcript in transcripts
        ]
...
def get_gene_to_transcript_mapping(
    ...
    id_mapping = parallel.apply_iter_simple(
        res, _parse_gene_to_transcript_query_result, progress_bar=True
    )
    id_mapping_flat = utils.flatten_lists(id_mapping)
    id_mapping_df = pd.DataFrame(id_mapping_flat)
    id_mapping_df = id_mapping_df.drop_duplicates()
