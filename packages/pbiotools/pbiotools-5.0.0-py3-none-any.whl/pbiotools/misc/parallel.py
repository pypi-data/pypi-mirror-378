...
def apply_parallel_groups(
    groups, num_procs, func, *args, progress_bar=False, backend="loky"
):
    ...
    if progress_bar:
        ...
        ret_list = joblib.Parallel(n_jobs=num_procs, backend=backend)(
            joblib.delayed(func)(group, *args)
            for name, group in tqdm.tqdm(
                groups, total=len(groups), leave=True, file=sys.stdout
            )
        )
    else:
        ret_list = joblib.Parallel(n_jobs=num_procs, backend=backend)(
            joblib.delayed(func)(group, *args) for name, group in groups
        )
    return ret_list
...
def apply_groups(groups, func, *args, progress_bar=False, backend="loky"):
    ...
    return apply_parallel_groups(
        groups, num_procs, func, *args, progress_bar=progress_bar, backend=backend
    )


def apply_parallel_split(
    data_frame,
    num_procs,
    func,
    *args,
    ...
    res = apply_parallel_groups(
        split_groups, num_procs, func, *args, progress_bar=progress_bar, backend=backend
    )
    return res


def apply_parallel(
    data_frame, num_procs, func, *args, progress_bar=False, backend="loky"
):
    """This function parallelizes applying a function to the rows of a data frame using the
    ...
    if progress_bar:
        ...
        ret_list = joblib.Parallel(n_jobs=num_procs, backend=backend)(
            joblib.delayed(func)(row[1], *args)
        ...
        ret_list = joblib.Parallel(n_jobs=num_procs, backend=backend)(
            joblib.delayed(func)(row[1], *args) for row in data_frame.iterrows()
        )
    return ret_list
...
def apply_df_simple(data_frame, func, *args, progress_bar=False, backend="loky"):
    ...
    return apply_parallel(
        data_frame, num_procs, func, *args, progress_bar=progress_bar, backend=backend
    )


def apply_iter_simple(
    items, func, *args, progress_bar=False, total=None, num_groups=None, backend="loky"
):
