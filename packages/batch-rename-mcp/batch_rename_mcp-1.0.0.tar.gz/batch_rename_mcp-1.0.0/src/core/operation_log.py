...
class OperationLog:
    ...
    def start_operation(self, operation_type: str, metadata: Dict[str, Any]) -> str:
        ...
        logger.info(f"开始操作: {operation_type} (ID: {operation_id})")
    ...
    def undo_operation(self, operation_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if operation["status"] != "completed":
            return {
                "success": False,
                "message": f"操作状态不允许撤销: {operation['status']}"
            }
        ...
        try:
            ...
            if undo_result["success"]:
                ...
                logger.info(f"操作撤销成功: {operation['id']}")
    ...
    def clean_old_logs(self, retention_days: int = 30) -> int:
        ...
        if cleaned_count > 0:
            self._write_log_file(log_data)
            logger.info(f"清理了 {cleaned_count} 个旧操作记录")
    ...
    def _execute_undo(self, operation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation["type"] != "batch_rename":
            return {
                "success": False,
                "message": f"不支持撤销操作类型: {operation['type']}"
            }
        ...
        for file_op in reversed(file_operations):
            try:
                ...
                if current_path.exists():
                    ...
                    success_count += 1
                    logger.info(f"撤销成功: {current_path.name} -> {original_path.name}")
                else:
                    failed_count += 1
                    failed_files.append(f"文件不存在: {current_path}")
                    
            except Exception as e:
                failed_count += 1
                failed_files.append(f"{file_op['new_path']}: {str(e)}")
                logger.error(f"撤销失败: {file_op['new_path']} - {e}")
        ...
        if failed_files:
            ...
            result["message"] += f", {failed_count} 个失败"
