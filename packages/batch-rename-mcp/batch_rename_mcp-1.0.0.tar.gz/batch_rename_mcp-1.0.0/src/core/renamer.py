...
class Renamer:
    ...
    def batch_rename(
        ...
        try:
            ...
            if len(files) > max_files:
                return {
                    "success": False,
                    "message": f"文件数量超过限制 ({len(files)} > {max_files})",
    ...
    def _validate_rename_plan(self, rename_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if duplicates:
            errors.append(f"发现重复的新文件名: {', '.join(duplicates)}")
        ...
        if existing_files:
            errors.append(f"目标文件已存在: {', '.join(existing_files[:5])}")
            if len(existing_files) > 5:
                errors.append(f"... 还有 {len(existing_files) - 5} 个文件")
    ...
    def _execute_rename(self, rename_plan: Dict[str, Any], options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if failed_operations:
                ...
                result["message"] += f", {len(failed_operations)} 个失败"
    ...
    def _resolve_conflict(self, path: Path, strategy: str) -> Path:
        """解决文件名冲突"""
        if strategy == "skip":
            raise ValueError(f"文件已存在，跳过: {path}")
