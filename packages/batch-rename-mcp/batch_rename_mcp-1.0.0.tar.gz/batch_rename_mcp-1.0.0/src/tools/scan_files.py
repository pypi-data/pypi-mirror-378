...
class ScanFilesHandler:
    ...
    async def handle(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"开始扫描文件: path={path}, recursive={recursive}, filter={filter_pattern}")
            ...
            logger.info(f"文件扫描完成: found={len(result.get('files', []))}")
            
            return result
            
        except Exception as e:
            logger.error(f"文件扫描处理失败: {e}")
            return {
                "success": False,
    ...
    def _scan_path(self, path: Path, recursive: bool, filter_pattern: str, max_files: int) -> tuple:
        ...
        try:
            ...
            logger.warning(f"扫描路径时遇到错误: {path} - {e}")
