...
class AiiApplication:
    ...
    def run(self, args: list[str] | None = None) -> int:
        ...
        command = arg_list[0].lower()

        if command == "run":
            return self._handle_run_alias(arg_list[1:])

        if command == "convo":
            return self._handle_convo_command(arg_list[1:])

        if command == "git":
            return self._handle_git_command(arg_list[1:])

        mode = MODE_COMMANDS.get(command)
        if mode:
            return self._run_mode_command(mode, arg_list[1:])

        return self._run_legacy(arg_list)

    def _handle_run_alias(self, args: list[str]) -> int:
        ...
        if mode:
            return self._run_mode_command(mode, args[1:])

        return self._run_legacy(args)

    def _run_mode_command(self, mode: str, remaining_args: list[str]) -> int:
        """Execute a specific AI mode command."""
        forwarded = ["--mode", mode]
        forwarded.extend(remaining_args)
        return self._run_legacy(forwarded)

    def _handle_convo_command(self, args: list[str]) -> int:
        ...
        if namespace.action == "continue":
            forwarded = ["--continue", namespace.conversation_id]
    ...
    def _handle_git_command(self, args: list[str]) -> int:
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--version", action="version", version=f"aii {__version__}")
