...
def align_procrustes_points_Rt01(p0, p1, weights=None):
    ...
    if not (p0.flags["C_CONTIGUOUS"] and p1.flags["C_CONTIGUOUS"]):
...
def align_procrustes_vectors_R01(v0, v1, weights=None):
    ...
    # Use the python implementation when the data isn't contiguous
    if not (v0.flags["C_CONTIGUOUS"] and v1.flags["C_CONTIGUOUS"]):
...
def hypothesis_board_corner_positions(
    icam_intrinsics=None, idx_inliers=None, **optimization_inputs
):
...
def _splined_stereographic_domain(lensmodel):
    ...
    if meta["order"] == 2:
        ...
        return nps.glue(
            ...
            (u[1:-2, -2] + u[1:-2, -1]) / 2.0,
            (u[-2, -2] + u[-1, -2] + u[-2, -1] + u[-1, -1]) / 4.0,
            (u[-2, -2:1:-1] + u[-1, -2:1:-1]) / 2.0,
            (u[-2, 1] + u[-1, 1] + u[-2, 0] + u[-1, 0]) / 4.0,
            (u[-2:0:-1, 0] + u[-2:0:-1, 1]) / 2.0,
            (u[0, 0] + u[0, 1] + u[1, 0] + u[1, 1]) / 4.0,
            (u[0, 1] + u[1, 1]) / 2.0,
            axis=-2,
        )
...
def polygon_difference(positive, negative):
    ...
    if isinstance(diff, (MultiPolygon, GeometryCollection)):
        ...
        raise Exception(
            f"I only know how to deal with MultiPolygon or Polygon, but instead got type '{type(diff)}"
        )
        ...
        for i in range(0, len(hole.coords)):
            ...
            if len(s) > 1:
                ...
                return [
                    subpiece
                    for piece in s
                    for subpiece in split_polygon_to_remove_holes(piece)
                ]
            # Split didn't work. Try the next vertex

        print("WARNING: Couldn't split the region. Ignoring", file=sys.stderr)
        return []

    return [subpiece for p in diff for subpiece in split_polygon_to_remove_holes(p)]
...
from the filename.
    ...
    def pull_framenumbers(files):
        ...
        for f in files:
            if not re.match("^[0-9]+$", f[Nleading:Itrailing]):
                raise Exception(
                    (
                        "Image filenames MUST be of the form 'something..number..something'\n"
                    ...
                    ).format(f, f[Nleading:Itrailing], leading, trailing)
                )
    ...
    for icamera in range(Ncameras):
        ...
        if framenumbers is not None:
            mapping.update(
                zip(
                    files_per_camera[icamera],
                    [(iframe, icamera) for iframe in framenumbers],
                )
            )
    return mapping
...
def _plot_options_state_meas_boundary(f, optimization_inputs, arg0):
    if arg0 is not None:
        i0 = f(arg0, **optimization_inputs)
    else:
        i0 = f(**optimization_inputs)
    if i0 is None:
        return
...
def ingest_packed_state(b_packed, **optimization_inputs):
    ...
    if b_packed.ravel().size != Nvars_expected:
        raise Exception(
            f"Mismatched array size: b_packed.size={b_packed.ravel().size} while the optimization problem expects {Nvars_expected}"
        )
...
def _plot_arg_covariance_ellipse(q_mean, Var, what):
    ...
    return (
        ...
        2 * major,
        2 * minor,
        180.0 / np.pi * np.arctan2(v0[1], v0[0]),
...
def _plot_args_points_and_covariance_ellipse(q, what):
    ...
    return (
        _plot_arg_covariance_ellipse(q_mean, Var, what),
        (q, dict(_with="dots", tuplesize=-2)),
    )


def measurements_board(
    optimization_inputs,
    *,
    ...
    if not return_observations:
        ...
        # shape (N,2), (N,2)
        return x_board[idx, ...], observations_board[idx, ..., :2]


def residuals_board(*args, residuals=None, **kwargs):
    ...
    """
    return measurements_board(*args, x=residuals, **kwargs)
...
def measurements_point(
    optimization_inputs, *, icam_intrinsics=None, x=None, return_observations=False
):
    ...
    indices_point_camera = optimization_inputs[
    ...
    if not return_observations:
        ...
        # shape (N,2), (N,2)
        return x_point[idx, ...], observations_point[idx, ..., :2]


def residuals_point(*args, residuals=None, **kwargs):
    ...
    """
    return measurements_point(*args, x=residuals, **kwargs)
...
def write_point_cloud_as_ply(filename, points, *, color=None, binary=True):
    ...
    if points.ndim != 2:
        raise Exception(
            f"points.shape must be (N,3) or (N,4) or (N,6), but points.ndim={points.ndim}"
        )
    N, Ntrailing = points.shape

    if not (Ntrailing == 3 or Ntrailing == 4 or Ntrailing == 6):
        raise Exception(
            f"points.shape[-1] must be one of (3,4,6) to indicate no-color or grayscale or bgr colors respectively. Instead got points.shape[-1] = {points.shape[-1]}"
        )

    if color is not None:
        if Ntrailing != 3:
            raise Exception(
                "Both 'points' and 'color' are specifying color information. At most one of those should provide it"
            )

        if color.ndim > 2:
            raise Exception(
                f"color.shape must be (N,) or (N,3), but color.ndim={color.ndim}"
            )
        if N != color.shape[0]:
            raise Exception(
                f"Inconsistent point counts: len(points)={len(points)} but len(color)={len(color)}"
            )

        if not (color.shape == (N,) or color.shape == (N, 3)):
            raise Exception(
                f"color.shape must be (N,) or (N,3), but color.shape={color.shape}"
            )
...
""".encode()
    ...
    with open(filename, "wb") as f:
        ...
        if binary:
            binary_ply = np.empty((N,), dtype=dtype)
