...
#define _P1(x, stride0, i0) (*(double*)((char*)(x) + (i0) * (stride0)))
#define _P2(x, stride0, stride1, i0, i1) \
    (*(double*)((char*)(x) + (i0) * (stride0) + (i1) * (stride1)))
...
#define P3(x, i0, i1, i2) \
    _P3(x, x##_stride0, x##_stride1, x##_stride2, i0, i1, i2)
...
#define init_stride_2D(x, d0, d1) \
    if (x##_stride1 <= 0)         \
        x##_stride1 = sizeof(*x); \
    if (x##_stride0 <= 0)         \
    x##_stride0 = d1 * x##_stride1
#define init_stride_3D(x, d0, d1, d2)   \
    if (x##_stride2 <= 0)               \
        x##_stride2 = sizeof(*x);       \
    if (x##_stride1 <= 0)               \
        x##_stride1 = d2 * x##_stride2; \
    if (x##_stride0 <= 0)               \
    x##_stride0 = d1 * x##_stride1
