...
template <int N>
static void rotate_point_r_core(
    // output
    ...
    //   xrot = x cos(th) + cross(r, x) sin(th)/th + r rt x (1 - cos(th)) /
    //   (th*th)
    ...
    const val_withgrad_t<N> th2 = rg[0] * rg[0] + rg[1] * rg[1] + rg[2] * rg[2];
    ...
    const val_withgrad_t<N> inner =
        rg[0] * x_ing[0] + rg[1] * x_ing[1] + rg[2] * x_ing[2];
    ...
    if (th2.x < 1e-10) {
        ...
        for (int i = 0; i < 3; i++) {
            x_outg[i] = x_ing[i] + cross[i] + rg[i] * inner / 2.;
        }
    } else {
        ...
        for (int i = 0; i < 3; i++) {
            x_outg[i] = x_ing[i] * sc.v[1] + cross[i] * sc.v[0] / th +
...
extern "C" void mrcal_rotate_point_r_full(
    ...
    if (J_r == NULL && J_x == NULL) {
        ...
        rotate_point_r_core<0>(x_outg.v, rg.v, x_ing.v, inverted);
        ...
        rotate_point_r_core<3>(x_outg.v, rg.v, x_ing.v, inverted);
        x_outg.extract_value(x_out, x_out_stride0);
        ...
        rotate_point_r_core<6>(x_outg.v, rg.v, x_ing.v, inverted);
        x_outg.extract_value(x_out, x_out_stride0);
        x_outg.extract_grad(J_r, 0, 3, 0, J_r_stride0, J_r_stride1);
...
extern "C" void mrcal_transform_point_rt_full(
    ...
    if (!inverted) {
        ...
        if (J_rt == NULL && J_x == NULL) {
            ...
            vec_withgrad_t<0, 3> x_outg;
            rotate_point_r_core<0>(x_outg.v, rg.v, x_minus_t.v, true);
            x_outg.extract_value(x_out, x_out_stride0);
            ...
            vec_withgrad_t<6, 3> x_outg;
            rotate_point_r_core<6>(x_outg.v, rg.v, x_minus_t.v, true);
            x_outg.extract_value(x_out, x_out_stride0);
            ...
            vec_withgrad_t<3, 3> x_outg;
            rotate_point_r_core<3>(x_outg.v, rg.v, x_minus_t.v, true);
            x_outg.extract_value(x_out, x_out_stride0);
            x_outg.extract_grad(J_x, 0, 3, 0, J_x_stride0, J_x_stride1);
        } else {
            ...
            rotate_point_r_core<9>(x_outg.v, rg.v, x_minus_t.v, true);
...
static void compose_r_core(
    ...
    */
    ...
    if (A.x < eps / 2.) {
        ...
        for (int i = 0; i < 3; i++) {
            (*r)[i] = (*r1)[i] * (val_withgrad_t<N>(1.0) -
                                  inner * (B_over_tanB - 1.) / (B * B * 4.)) +
                      (*r0)[i] * B_over_tanB + cross[i] / 2.;
        }
        return;
    } else if (B.x < eps) {
        // B ~ 0. I simplify
        ...
        for (int i = 0; i < 3; i++) {
            (*r)[i] = (*r0)[i] * (val_withgrad_t<N>(1.0) -
                                  inner * (A_over_tanA - 1.) / (A * A * 4.)) +
                      (*r1)[i] * A_over_tanA + cross[i] / 2.;
        }
        return;
    }
    ...
    val_withgrad_t<N> cosC =
        cosA * cosB - sinA_over_A * sinB_over_B * inner / 4.;

    for (int i = 0; i < 3; i++) {
        (*r)[i] = sinA_over_A * cosB * (*r0)[i] +
    ...
    // (smaller numbers) r01
        ...
        const val_withgrad_t<N> sinC =
            (val_withgrad_t<N>(1.) - cosC * cosC).sqrt();
...
extern "C" void mrcal_compose_r_full(
    ...
    if (dr_dr0 == NULL && dr_dr1 == NULL) {
        ...
        r0g.init_vars(r_0, 0, 3, -1, r_0_stride0);
        r1g.init_vars(r_1, 0, 3, -1, r_1_stride0);
        if (inverted0) {
            r0g *= -1;
        }
        if (inverted1) {
        ...
        compose_r_core<0>(&r01g, &r0g, &r1g);

        r01g.extract_value(r_out, r_out_stride0, 0, 3);
    } else if (dr_dr0 != NULL && dr_dr1 == NULL) {
        // r0 gradient only
        vec_withgrad_t<3, 3> r0g, r1g;

        r0g.init_vars(r_0, 0, 3, 0, r_0_stride0);
        r1g.init_vars(r_1, 0, 3, -1, r_1_stride0);
        if (inverted0) {
            r0g *= -1;
        }
        ...
        compose_r_core<3>(&r01g, &r0g, &r1g);

        r01g.extract_value(r_out, r_out_stride0, 0, 3);
        r01g.extract_grad(dr_dr0, 0, 3, 0, dr_dr0_stride0, dr_dr0_stride1, 3);
    } else if (dr_dr0 == NULL && dr_dr1 != NULL) {
        // r1 gradient only
        vec_withgrad_t<3, 3> r0g, r1g;

        r0g.init_vars(r_0, 0, 3, -1, r_0_stride0);
        r1g.init_vars(r_1, 0, 3, 0, r_1_stride0);
        if (inverted0) {
            r0g *= -1;
        }
        if (inverted1) {
        ...
        r01g.extract_value(r_out, r_out_stride0, 0, 3);
        r01g.extract_grad(dr_dr1, 0, 3, 0, dr_dr1_stride0, dr_dr1_stride1, 3);
    } else {
        // r0 AND r1 gradients
        vec_withgrad_t<6, 3> r0g, r1g;

        r0g.init_vars(r_0, 0, 3, 0, r_0_stride0);
        r1g.init_vars(r_1, 0, 3, 3, r_1_stride0);
        if (inverted0) {
            r0g *= -1;
        }
        if (inverted1) {
        ...
        compose_r_core<6>(&r01g, &r0g, &r1g);

        r01g.extract_value(r_out, r_out_stride0, 0, 3);
        r01g.extract_grad(dr_dr0, 0, 3, 0, dr_dr0_stride0, dr_dr0_stride1, 3);
        r01g.extract_grad(dr_dr1, 3, 3, 0, dr_dr1_stride0, dr_dr1_stride1, 3);
    }
}
