...
#define SAY_NONEWLINE(fmt, ...)     \
    fprintf(                        \
        stderr,                     \
        "libdogleg at %s:%d: " fmt, \
        __FILE__,                   \
        __LINE__,                   \
        ##__VA_ARGS__               \
    )
#define SAY(fmt, ...)                      \
    do {                                   \
        SAY_NONEWLINE(fmt, ##__VA_ARGS__); \
        fprintf(stderr, "\n");             \
    } while (0)

// This REQUIRES that a "dogleg_solverContext_t* ctx" be available
#define SAY_IF_VERBOSE(fmt, ...)                                           \
    do {                                                                   \
        if (ctx->parameters->dogleg_debug & DOGLEG_DEBUG_OTHER_THAN_VNLOG) \
            SAY(fmt, ##__VA_ARGS__);                                       \
    } while (0)

// I do this myself because I want this to be active in all build modes, not
// just !NDEBUG
#define ASSERT(x)                                        \
    do {                                                 \
        if (!(x)) {                                      \
            SAY("ASSERTION FAILED: " #x " is not true"); \
            exit(1);                                     \
        }                                                \
    } while (0)
...
typedef enum {
    VNLOG_DEBUG_STEP_TYPE_LIST(
        VNLOG_DEBUG_STEP_TYPE_NAME_COMMA
    )
    STEPTYPE_UNINITIALIZED
} vnlog_debug_step_type_t;
...
#define VNLOG_DEBUG_DECLARE_FIELD(type, name, initialvalue) type name;
    VNLOG_DEBUG_FIELDS(
        VNLOG_DEBUG_DECLARE_FIELD
    )
} vnlog_debug_data;
static void vnlog_debug_reset(
    void
) {
#define VNLOG_DEBUG_RESET_FIELD(type, name, initialvalue) \
    vnlog_debug_data.name = initialvalue;
    VNLOG_DEBUG_FIELDS(VNLOG_DEBUG_RESET_FIELD);
}
static void vnlog_debug_emit_legend(
    void
) {
#define VNLOG_DEBUG_SPACE_FIELD_NAME(type, name, initialvalue) " " #name

    vnlog_debug_reset();
    printf("# iteration step_accepted" VNLOG_DEBUG_FIELDS(
        VNLOG_DEBUG_SPACE_FIELD_NAME
    ) "\n");
}
__attribute__((unused)) static void vnlog_emit_double(
    double x
) {
    if (x == INFINITY) {
        printf("- ");
...
__attribute__((unused)) static void vnlog_emit_int(
    int x
) {
    if (x == -1) {
        printf("- ");
...
__attribute__((unused)) static void vnlog_emit_vnlog_debug_step_type_t(
    vnlog_debug_step_type_t x
) {
#define VNLOG_DEBUG_STEP_TYPE_SWITCH_EMIT(name, shortname) \
    case name:                                             \
        printf(shortname " ");                             \
        break;
    switch (x) {
...
static void vnlog_debug_emit_record(
    int iteration,
    int step_accepted
) {
...
void dogleg_getDefaultParameters(
...
void dogleg_setDebug(
    int debug
) {
    parameters_global.dogleg_debug = debug;
}
void dogleg_setInitialTrustregion(
    double t
) {
    parameters_global.trustregion0 = t;
}
void dogleg_setThresholds(
    double Jt_x,
    double update,
    double trustregion
) {
    if (Jt_x > 0.0) {
        parameters_global.Jt_x_threshold = Jt_x;
    }
    if (update > 0.0) {
        parameters_global.update_threshold = update;
    }
    if (trustregion > 0.0) {
        parameters_global.trustregion_threshold = trustregion;
    }
}

// these parameters likely should not be messed with.
void dogleg_setMaxIterations(
    int n
) {
    parameters_global.max_iterations = n;
}
void dogleg_setTrustregionUpdateParameters(
    double downFactor,
...
__attribute__((unused)) static double inner_withstride(
    const double* x,
    const double* y,
    unsigned int n,
    unsigned int stride
...
static void accum_outerproduct_packed(
    double* JtJ,
    const double* j,
    int n
) {
...
__attribute__((unused)) static void vec_add(
    double* dest,
    const double* v0,
    const double* v1,
    int n
...
__attribute__((unused)) static void vec_accum(
    double* dest,
    const double* v,
    int n
) {
...
static void mul_spmatrix_densevector(
    double* dest,
...
static double norm2_mul_spmatrix_t_densevector(
    const cholmod_sparse* At,
    const double* x
) {
...
// transpose(A)*x
static void mul_matrix_t_densevector(
    double* dest,
    const double* A,
    const double* x,
    int Nrows,
...
static double norm2_mul_matrix_vector(
    const double* A,
    const double* x,
    int Nrows,
    int Ncols
) {
    // computes norm2(A * x). For each row of A I compute that element of A*x,
    // and accumulate it immediately towards the norm
    double result = 0.0;
...
static void _dogleg_testGradient(
    unsigned int var,
    const double* p0,
    unsigned int Nstate,
    unsigned int Nmeas,
    ...
    cholmod_sparse* Jt0;
    double* J_dense =
        NULL;  // setting to NULL to pacify compiler's "uninitialized" warnings
    double* J_dense0 =
        NULL;  // setting to NULL to pacify compiler's "uninitialized" warnings

    // This is a plain text table, that can be easily parsed with "vnlog" tools
    printf(
        "# ivar imeasurement gradient_reported gradient_observed error "
        "error_relative\n"
    );
    ...
    for (unsigned int i = 0; i < Nmeas; i++) {
        ...
        if (is_sparse) {
            ...
            g_reported = (getGrad_dense(var, i, J_dense0, Nstate) +
                          getGrad_dense(var, i, J_dense, Nstate)) /
                         2.0;
        }
...
void dogleg_testGradient_dense(
    unsigned int var,
    const double* p0,
    unsigned int Nstate,
    unsigned int Nmeas,
    dogleg_callback_dense_t* f,
    void* cookie
) {
    return _dogleg_testGradient(
        var,
        p0,
        Nstate,
        Nmeas,
...
static void computeCauchyUpdate(
    ...
    if (!point->updateCauchy_valid) {
        ...
        // quadratic error function (Cauchy point). If this is past my trust
        // region, I move as far as the trust region allows along the steepest
        // descent direction. My error function is F=norm2(f(p)). dF/dP =
        // 2*ft*J. This is proportional to Jt_x, which is thus the steepest
        // ascent direction.
        //
        // Thus along this direction we have F(k) = norm2(f(p + k Jt_x)). The
        // Cauchy point is where F(k) is at a minimum: dF_dk = 2 f(p + k Jt_x)t
        // J Jt_x ~ (x + k J Jt_x)t J Jt_x = = xt J Jt x + k xt J Jt J Jt x =
        // norm2(Jt x) + k norm2(J Jt x) dF_dk = 0 -> k= -norm2(Jt x) / norm2(J
        // Jt x) Summary: the steepest direction is parallel to Jt*x. The Cauchy
        // point is at k*Jt*x where k = -norm2(Jt*x)/norm2(J*Jt*x)
        double norm2_Jt_x = norm2(point->Jt_x, ctx->Nstate);
        double norm2_J_Jt_x =
            ctx->is_sparse
                ? norm2_mul_spmatrix_t_densevector(point->Jt, point->Jt_x)
                : norm2_mul_matrix_vector(
                      point->J_dense,
                      point->Jt_x,
                      ctx->Nmeasurements,
                      ctx->Nstate
        ...
        vec_copy_scaled(point->updateCauchy, point->Jt_x, k, ctx->Nstate);
        SAY_IF_VERBOSE(
            "cauchy step size %.6g",
...
int dpptrf_(char* uplo, int* n, double* ap, int* info, int uplo_len);
int dpptrs_(
    char* uplo,
    int* n,
    int* nrhs,
...
void dogleg_computeJtJfactorization(
    dogleg_operatingPoint_t* point,
    ...
    if (ctx->is_sparse) {
        // I'm assuming the pattern of zeros will remain the same throughout, so
        // I analyze only once
        ...
        while (1) {
            ...
            SAY_IF_VERBOSE(
                "singular JtJ. Have rank/full rank: %zd/%d. Adding %g I from "
                "now on",
                ctx->factorization->minor,
                ctx->Nstate,
                ctx->lambda
            );
        }
        ...
        if (ctx->factorization_dense == NULL) {
            ...
            ctx->factorization_dense = malloc(
        ...
        while (1) {
            // I construct my JtJ. JtJ is packed and stored row-first. I have
            // two equivalent implementations. The one enabled here is maybe a
            // bit faster, but it's definitely clearer
#if 1
            memset(
...
#error this does not work. overwritten in the following loop
                ...
                for (int i0 = i1; i0 < ctx->Nstate; i0++, iJtJ++) {
                    ctx->factorization_dense[iJtJ] = inner_withstride(
                        &point->J_dense[i0],
                        &point->J_dense[i1],
                        ctx->Nmeasurements,
                        ctx->Nstate
            ...
            ASSERT(
                info >= 0
            );  // we MUST either succeed or see complain of singular
            // JtJ
            if (info == 0) {
                break;
            }
            ...
            SAY_IF_VERBOSE(
                "singular JtJ. Adding %g I from now on",
                ctx->lambda
            );
        }
    }
...
static void computeGaussNewtonUpdate(
    dogleg_operatingPoint_t* point,
    dogleg_solverContext_t* ctx
) {
    // I already have this data, so don't need to recompute
    if (!point->updateGN_valid) {
        ...
        if (ctx->is_sparse) {
            ...
            point->updateGN_cholmoddense = cholmod_solve(
                CHOLMOD_A,
                ctx->factorization,
                &Jt_x_dense,
                &ctx->common
            ...
            vec_negate(
                point->updateGN_cholmoddense->x,
                ctx->Nstate
            );  // should be more efficient than this later

            point->updateGN_lensq =
                norm2(point->updateGN_cholmoddense->x, ctx->Nstate);
        } else {
            memcpy(
                point->updateGN_dense,
                point->Jt_x,
                ctx->Nstate * sizeof(point->updateGN_dense[0])
            );
            int info;
            dpptrs_(
                &(char){'L'},
...
static void computeInterpolatedUpdate(
    double* update_dogleg,
    ...
    const double* b = ctx->is_sparse ? point->updateGN_cholmoddense->x
                                     : point->updateGN_dense;
...
static int computeCallbackOperatingPoint(
    dogleg_operatingPoint_t* point,
    dogleg_solverContext_t* ctx
) {
    if (ctx->is_sparse) {
...
static double computeExpectedImprovement(
    const double* step,
    ...
    if (ctx->is_sparse) {
        return -2.0 * inner(point->Jt_x, step, ctx->Nstate) -
...
static double takeStepFrom(
    ...
    if (pointFrom->updateCauchy_lensq >= trustregion * trustregion) {
        ...
        *step_len_sq = pointFrom->updateCauchy_lensq;
        ...
        // I'm not yet done. The cauchy point is within the trust region, so I
        // can go further. I look at the full Gauss-Newton step. If this is
        // within the trust region, I use it. Otherwise, I find the point at the
        // edge of my trust region that lies on a straight line between the
        // Cauchy point and the Gauss-Newton solution, and use that. This is the
        // heart of Powell's dog-leg algorithm.
        ...
        if (pointFrom->updateGN_lensq <= trustregion * trustregion) {
            ...
            if (ctx->parameters->dogleg_debug & DOGLEG_DEBUG_VNLOG) {
                ...
                vnlog_debug_data.step_len =
                    vnlog_debug_data.step_len_gauss_newton;
            }
            *step_len_sq = pointFrom->updateGN_lensq;

            // full Gauss-Newton step lies within my trust region. Take the full
            // step
            memcpy(
                step,
                ctx->is_sparse ? pointFrom->updateGN_cholmoddense->x
                               : pointFrom->updateGN_dense,
            ...
            // full Gauss-Newton step lies outside my trust region, so I
            // interpolate between the Cauchy-point step and the Gauss-Newton
            // step to find a step that takes me to the edge of my trust region.
            computeInterpolatedUpdate(
                step,
                step_len_sq,
                pointFrom,
                trustregion,
            ...
            if (ctx->parameters->dogleg_debug & DOGLEG_DEBUG_VNLOG) {
                ...
                vnlog_debug_data.step_len =
                    vnlog_debug_data.step_len_interpolated;
            }
        }
    }
    ...
    double expectedImprovement =
        computeExpectedImprovement(step, pointFrom, ctx);
    if (ctx->parameters->dogleg_debug & DOGLEG_DEBUG_VNLOG) {
        ...
        if (pointFrom->step_to_here_len_sq != INFINITY) {
            ...
            if (cos_direction_change >= 1.0) {
                ...
                vnlog_debug_data.step_direction_change_deg =
                    180.0 / M_PI * acos(cos_direction_change);
            }
        }
    }

    // are we done? For each state variable I look at the update step. If all
    // the elements fall below a threshold, I call myself done
    for (int i = 0; i < ctx->Nstate; i++) {
...
// accepted)
static int evaluateStep_adjustTrustRegion(
    ...
    SAY_IF_VERBOSE(
        "observed/expected improvement: %.6g/%.6g. rho = %.6g",
        observedImprovement,
        expectedImprovement,
        rho
    );
    ...
    if (rho < ctx->parameters->trustregion_decrease_threshold) {
        ...
        // Our model doesn't fit well. We should reduce the trust region size.
        // If the trust region size was affecting the attempted step, do this by
        // a constant factor. Otherwise, drop the trustregion to attempted step
        // size first
    ...
    } else if (rho > ctx->parameters->trustregion_increase_threshold &&
               before->didStepToEdgeOfTrustRegion) {
...
static int runOptimizer(
    ...
    SAY_IF_VERBOSE(
        "Initial operating point has norm2_x %.6g",
    ...
    while (stepCount < ctx->parameters->max_iterations) {
        ...
        while (1) {
            ...
            double expectedImprovement = takeStepFrom(
                ctx->beforeStep,
                ctx->afterStep->p,
            ...
            int afterStepZeroGradient =
                computeCallbackOperatingPoint(ctx->afterStep, ctx);
            SAY_IF_VERBOSE(
                "Evaluated operating point with norm2_x %.6g",
            ...
            if (evaluateStep_adjustTrustRegion(
                    ctx->beforeStep,
                    ctx->afterStep,
                    &trustregion,
                    expectedImprovement,
                ...
                // I accept this step, so the after-step operating point is the
                // before-step operating point of the next iteration. I exchange
                // the before- and after-step structures so that all the
                ...
                if (afterStepZeroGradient) {
                    SAY_IF_VERBOSE(
                        "Gradient low enough and we just improved. Done "
                        "iterating!"
                    );
            ...
            // This step was rejected. check if the new trust region size is
            // small enough to give up
            if (trustregion < ctx->parameters->trustregion_threshold) {
                SAY_IF_VERBOSE(
                    "trust region small enough. Giving up. Done iterating!"
                );
                return stepCount;
            }

            // I have rejected this step, so I try again with the new trust
            // region
        }
    }
...
static dogleg_operatingPoint_t* allocOperatingPoint(
    int Nstate,
    int numMeasurements,
    unsigned int NJnnz,
    ...
    int Npool = Nstate + numMeasurements + Nstate + Nstate + Nstate;
    if (!is_sparse) {
    ...
    point->p = &pool[0];
    point->x = &pool[Nstate];
    point->Jt_x = &pool[Nstate + numMeasurements];
    point->updateCauchy = &pool[Nstate + numMeasurements + Nstate];
    point->step_to_here = &pool[Nstate + numMeasurements + Nstate + Nstate];

    if (is_sparse) {
        point->Jt = cholmod_allocate_sparse(
            Nstate,
            numMeasurements,
            NJnnz,
            1,  // sorted
        ...
        point->updateGN_cholmoddense =
            NULL;  // This will be allocated as it is used
    } else {
        point->J_dense =
            &pool[Nstate + numMeasurements + Nstate + Nstate + Nstate];
        point->updateGN_dense =
            &pool
                [Nstate + numMeasurements + Nstate + Nstate + Nstate +
...
static void freeOperatingPoint(
    dogleg_operatingPoint_t** point,
    cholmod_common* common
) {
    // MUST match allocOperatingPoint(). It does a single malloc() and stores
    // the pointer into p
    free((*point)->p);
...
static void set_cholmod_options(
    cholmod_common* cc
) {
    // I want to use LGPL parts of CHOLMOD only, so I turn off the supernodal
    // routines. This gave me a 25% performance hit in the solver for a
    // particular set of optical calibration data.
    cc->supernodal = 0;
...
void dogleg_freeContext(
    dogleg_solverContext_t** ctx
) {
    freeOperatingPoint(
        &(*ctx)->beforeStep,
        (*ctx)->is_sparse ? &(*ctx)->common : NULL
    );
    freeOperatingPoint(
        &(*ctx)->afterStep,
        (*ctx)->is_sparse ? &(*ctx)->common : NULL
...
static double _dogleg_optimize(
    double* p,
    unsigned int Nstate,
    unsigned int Nmeas,
    unsigned int NJnnz,
    ...
    // runOptimizer places the most recent results into beforeStep in
...
static bool pseudoinverse_J_dense(
    // output
    double* out,
...
//
// This function is experimental, and subject to change
static cholmod_dense* pseudoinverse_J_sparse(
    // inputs
    const dogleg_operatingPoint_t* point,
    dogleg_solverContext_t* ctx,
    int i_meas0,
    ...
    // dense. cholmod doesn't have functions for this exact case. so I use the
    // dense-sparse-dense function (cholmod_solve), and densify the input.
    // Instead of sparse-sparse-sparse and the densifying the output
    // (cholmod_spsolve). This feels like it'd be more efficient
    ...
    for (int i_meas = 0; i_meas < NmeasInChunk; i_meas++) {
        ...
        for (unsigned int i0 = P(point->Jt, i_meas0 + i_meas);
             i0 < P(point->Jt, i_meas0 + i_meas + 1);
             i0++) {
    ...
    cholmod_dense* pinv =
        cholmod_solve(CHOLMOD_A, ctx->factorization, Jt_chunk, &ctx->common);
...
An article https://www.nature.com/articles/nmeth.3812 says
  ...
  D = norm2(x_io - x_i)^2 / ((Nstate+1) * norm2(x_io)/(Nmeasurements - Nstate
-1)) D_threshold = 4/Nmeasurements
...
static void accum_outlierness_factor(
    // output
    double* factor,
    ...
    if (featureSize <= 1) {
        ...
        if (fabs(det) < 1e-8) {
            *factor = DBL_MAX;  // definitely an outlier
            ...
            // inner(x, Bx)
            double xBx = (x[0] * x[0] * B00_det + x[0] * x[1] * B01_det * 2.0 +
                          x[1] * x[1] * B11_det) /
                         det;
            ...
            __attribute__((unused)) double xBBx =
                (v1 * v1 + v2 * v2) / (det * det);
        ...
        SAY("featureSize > 2 not implemented yet. Got featureSize=%d",
            featureSize);
        ASSERT(0);
    }
...
static void getOutliernessScale(
    // out
    ...
    *scale = (double)(Nmeasurements_nonoutliers) /
             (4. * ((double)(Nstate + 1) * norm2_x /
                    (double)(Nmeasurements_nonoutliers - Nstate - 1)));
}

static bool getOutliernessFactors_dense(
    // output
    ...
    // cholmod_spsolve() and cholmod_solve()) work in chunks of 4, so I do this
    // in chunks of 4 too. I pass it rows of J, 4 at a time. Note that if I have
    // measurement features, I don't want these to cross chunk boundaries, so I
    // set up chunk_size=lcm(N,4)
    int chunk_size = 4;
    if (featureSize <= 1) {
        featureSize = 1;
    }
    if (featureSize > 1) {
        // haven't implemented anything else yet. Don't have lcm() readily
        // available
    ...
    int i_measurement = 0;
    for (int i_feature = 0; i_feature < Nfeatures;
         i_feature++, i_measurement += featureSize) {
        if (i_measurement > i_measurement_valid_chunk_last) {
            bool pinvresult = pseudoinverse_J_dense(
                invJtJ_Jt,
                point,
                ctx,
                i_measurement,
        ...
        for (int i = 0; i < featureSize; i++) {
            for (int j = i; j < featureSize; j++, iA++) {
                ...
                for (int k = 0; k < Nstate; k++) {
                    A[iA] += invJtJ_Jt
                                 [Nstate * (i_measurement + i -
                                            i_measurement_valid_chunk_start) +
                                  k] *
                             point->J_dense[Nstate * i_measurement + j + k];
                }
            }
        }
        accum_outlierness_factor(
            &factors[i_feature],
...
static bool getOutliernessFactors_sparse(
    // output
    double* factors,  // Nfeatures factors
    ...
    cholmod_dense* Jt_chunk = cholmod_allocate_dense(
        Nstate,
        chunk_size,
        Nstate,
        CHOLMOD_REAL,
    ...
    for (int i_feature = 0; i_feature < Nfeatures;
        ...
        if (i_measurement > i_measurement_valid_chunk_last) {
            if (invJtJ_Jt) {
                cholmod_free_dense(&invJtJ_Jt, &ctx->common);
            }
            invJtJ_Jt = pseudoinverse_J_sparse(
                point,
                ctx,
                i_measurement,
                chunk_size,
    ...
    if (Jt_chunk) {
        cholmod_free_dense(&Jt_chunk, &ctx->common);
    }
    if (invJtJ_Jt) {
        cholmod_free_dense(&invJtJ_Jt, &ctx->common);
    }
...
bool dogleg_getOutliernessFactors(
    // output
    double* factors,  // Nfeatures factors
    ...
    bool result;
    if (ctx->is_sparse) {
        result = getOutliernessFactors_sparse(
            factors,
            scale,
            featureSize,
            Nfeatures,
        ...
        result = getOutliernessFactors_dense(
            factors,
            scale,
            featureSize,
            Nfeatures,
...
double dogleg_getOutliernessTrace_newFeature_sparse(
    const double* JqueryFeature,
    int istateActive,
    int NstateActive,
    int featureSize,
    int NoutlierFeatures,
    ...
    /*
      ...
      mean at project(q) (so that x has mean 0). I conceivably have a
      distribution for xref and I know its variance. For a quadratic form I can
      compute the expected value E(x*t C x) = trace(C Var(x*)). I'm going to
    ...
    */
    ...
    cholmod_sparse Jt_query_sparse = {
        .nrow = ctx->Nstate,
        .ncol = featureSize,
    ...
    // Really shouldn't need to do this every time. In fact I probably don't
    // need to do it at all, since this will have been done by the solver during
    // the last step
    dogleg_computeJtJfactorization(point, ctx);
    cholmod_sparse* invJtJ_Jp = cholmod_spsolve(
        CHOLMOD_A,
        ctx->factorization,
        &Jt_query_sparse,
        &ctx->common
    ...
    for (int i = 0; i < featureSize; i++) {
        for (unsigned int j = P(invJtJ_Jp, i); j < P(invJtJ_Jp, i + 1); j++) {
            ...
            if (row >= istateActive) {
                ...
                for (int k = i; k < featureSize;
                     k++) {  // computing one triangle
                    A[ic0 + k] +=
                        X(invJtJ_Jp, j) *
                        JqueryFeature[row - istateActive + k * NstateActive];
                }
            }
        }
    }
    ...
    __attribute__((unused)) double B01 = -invB01 * det_invB_recip;
    double traceB = B00 + B11;
    __attribute__((unused)) double traceBB =
        B00 * B00 + 2.0 * B01 * B01 + B11 * B11;
...
bool dogleg_markOutliers(
    ...
    if (!dogleg_getOutliernessFactors(
            factors,
            scale,
            featureSize,
            Nfeatures,
    ...
    *Noutliers = 0;
    for (int i = 0; i < Nfeatures; i++) {
        ...
        double confidence_drop_relative =
            1.0 - confidence_excluded / confidence0;
        if (confidence_drop_relative < OUTLIER_CONFIDENCE_DROP_THRESHOLD) {
            ...
            markedAny = true;
            SAY_IF_VERBOSE(
                "Feature %d has outlierness factor %f. Culling produces a "
                "confidence: %g. relative loss: %g... YES an outlier; "
                "confidence drops little",
                i,
                factors[i],
                confidence_excluded,
                confidence_drop_relative
            );
            (*Noutliers)++;
        } else {
            SAY_IF_VERBOSE(
                "Feature %d has outlierness factor %f. Culling produces a "
                "confidence: %g. relative loss: %g... NOT an outlier: "
                "confidence drops too much",
                i,
                factors[i],
                confidence_excluded,
                confidence_drop_relative
...
void dogleg_reportOutliers(
    ...
    for (int i = 0; i < Nfeatures; i++) {
        ...
        double rot_confidence_drop_relative =
            1.0 - confidence / confidence_full;

        SAY("%5d %9.3g %9.3g", i, factors[i], rot_confidence_drop_relative);
    }
