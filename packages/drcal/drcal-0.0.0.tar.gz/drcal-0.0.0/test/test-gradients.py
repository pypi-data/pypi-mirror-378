...
meastype_name = ("boards", "points", "points-triangulated", "regularization")
...
def get_variable_map(s):
    ...
    m = re.search(
        r"^## Intrinsics: (\d+) variables per camera \((\d+) for the core, (\d+) for the rest; (\d+) total\). Starts at variable (-?\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## Extrinsics: \d+ variables per camera for all cameras except camera 0 \((\d+) total\). Starts at variable (-?\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## Frames: \d+ variables per frame \((\d+) total\). Starts at variable (-?\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## Discrete points: \d+ variables per point \((\d+) total\). Starts at variable (-?\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## calobject_warp: (\d+) variables. Starts at variable (-?\d+)", s, re.M
    )
    ...
    frames = (
        f"ivar >= {ivar0_frames}         && ivar < {ivar0_frames}        +{Nvar_frames}"
    )
    points = (
        f"ivar >= {ivar0_points}         && ivar < {ivar0_points}        +{Nvar_points}"
    )
...
def get_measurement_map(s):
    ...
    m = re.search(
        r"^## Measurement calobjects: (\d+) measurements. Starts at measurement (\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## Measurement points: (\d+) measurements. Starts at measurement (\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## Measurement points-triangulated: (\d+) measurements. Starts at measurement (\d+)",
        s,
        re.M,
    )
    ...
    m = re.search(
        r"^## Measurement regularization: (\d+) measurements. Starts at measurement (\d+)",
        s,
        re.M,
    )
...
for test in tests:
    try:
        full = subprocess.check_output(
            [f"{testdir}/test-gradients"] + test.split(), shell=False, encoding="ascii"
        )
    except Exception:
    ...
    cut = subprocess.check_output(
        (
            ...
            "-p",
            "error_relative=error/((abs(gradient_reported)+abs(gradient_observed))/2. + 1e-3)",
            "-p",
            f"type={varmap},meastype={measmap}",
        ),
        input=full,
        encoding="ascii",
    )
    ...
    for vartype in range(Nvartypes):
        for meastype in range(Nmeastypes):
            ...
            err_relative_99percentile = testutils.percentile_compat(
                err, 99, interpolation="higher"
            )
            testutils.confirm(
                err_relative_99percentile < 1e-3,
                f"99%-percentile relative error={err_relative_99percentile} for vars {vartype_name[vartype]}, meas {meastype_name[meastype]} in {test}",
            )
