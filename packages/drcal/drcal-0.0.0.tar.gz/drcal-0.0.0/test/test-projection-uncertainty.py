...
def parse_args():
    ...
    parser.add_argument(
        "--observations-left-right-with-gap",
        action="store_true",
    ...
    if args.non_vanilla:
        if not (args.fixed == "cam0" and args.Ncameras == 1):
            print(
                "--non-vanilla works ONLY with --fixed cam0 --Ncameras 1",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if args.do_sample:
            print(
                "--non-vanilla incompatible with --do-sample. Instead I make sure that it produces the same results as the vanilla solves, and I --do-sample THOSE solves",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    for i in range(len(args.distances)):
        if args.distances[i] == "inf":
            ...
            if re.match("[0-9deDE.-]+", args.distances[i]):
                ...
                print(
                    '--distances is a comma-separated list of numbers or "inf"',
                    file=sys.stderr,
                )
                sys.exit(1)
            args.distances[i] = s
...
if args.Ncameras <= 0 or args.Ncameras > 4:
    print(
        f"Ncameras must be in [0,4], but got {args.Ncameras}. Giving up",
        file=sys.stderr,
    )
    sys.exit(1)
if args.points and not re.match("cross-reprojection", args.reproject_perturbed):
    print(
        "--points is currently implemented ONLY with --reproject-perturbed cross-reprojection-...",
        file=sys.stderr,
    )
    sys.exit(1)
...
from test_calibration_helpers import (
    calibration_baseline,
    calibration_make_non_vanilla,
    calibration_boards_to_points,
    calibration_sample,
)
...
pointscale = dict(pdf=1, svg=1, png=1, gp=1)
pointscale[""] = 1.0
...
if args.make_documentation_plots:
    print(
        f"Will write documentation plots to {args.make_documentation_plots}-xxxx.pdf and .svg"
    )

    if terminal["svg"] is None:
        terminal["svg"] = (
            'svg size 800,600       noenhanced solid dynamic    font ",14"'
        )
    if terminal["pdf"] is None:
        terminal["pdf"] = (
            'pdf size 8in,6in       noenhanced solid color      font ",16"'
        )
    if terminal["png"] is None:
        terminal["png"] = (
            'pngcairo size 1024,768 transparent noenhanced crop font ",12"'
        )
...
extrinsics_rt_fromref_true = np.array(
    (
        (0, 0, 0, 0, 0, 0),
        (0.08, 0.2, 0.02, 1.0, 0.9, 0.1),
...
if args.observations_left_right_with_gap:
    calibration_baseline_kwargs = dict(
        x_noiseradius=2.5 / 4.0, y_noiseradius=2.5, x_offset=2.5 / 2.0, x_mirror=True
    )
else:
    calibration_baseline_kwargs = dict()
...
# not exactly
optimization_inputs_baseline, models_true, frames_points_true = calibration_baseline(
    args.model,
    args.Ncameras,
    args.Nframes,
...
if args.non_vanilla:
    ...
    # will do that later
    optimization_inputs_baseline_moving_cameras_refcam0 = copy.deepcopy(
    ...
    optimization_inputs_baseline_moving_cameras_refframe0 = copy.deepcopy(
        optimization_inputs_baseline
    )
    calibration_make_non_vanilla(
...
if args.compare_baseline_against_mrcal_2_4:
    if (
        args.model == "opencv4"
        and args.Nframes == 50
        and args.extra_observation_at is None
        and object_width_n == 10
        and object_height_n == 9
        and object_spacing == 0.1
        and args.range_to_boards == 4.0
        and args.reproject_perturbed == "mean-pcam"
        and args.observed_pixel_uncertainty == 1.5
        and not fixedframes
        and not args.points
    ):
        ...
        if args.Ncameras == 1:
            ...
            Var_dq_inf_ref = np.array(
                [[[30.06831569, 14.20492251], [14.20492251, 16.75554809]]]
            )
        elif args.Ncameras == 4:
            Var_dq_ref = np.array(
                (
                    [[22.65023795, 7.20500655], [7.20500655, 17.29990464]],
                    [[37.51131869, 9.30598142], [9.30598142, 17.77739599]],
                    [[28.8054302, 10.66808841], [10.66808841, 20.76171949]],
                    [[36.16253686, 16.06114737], [16.06114737, 26.95796495]],
                )
            )
            Var_dq_inf_ref = np.array(
                (
                    [[1.19879461, 0.45313079], [0.45313079, 0.91451931]],
                    [[1.90196461, 0.53617757], [0.53617757, 0.76472281]],
                    [[1.65878182, 0.64492073], [0.64492073, 0.92189559]],
                    [[2.64985186, 1.16716666], [1.16716666, 1.30752352]],
                )
            )
        else:
            raise Exception(
...
if not args.points:
    ...
    indices_frame_camintrinsics_camextrinsics = optimization_inputs_baseline[
    ...
    indices_point_camintrinsics_camextrinsics = optimization_inputs_baseline[
...
if args.make_documentation_plots is not None:
    ...
    def observed_boards(icam):
        if indices_frame_camintrinsics_camextrinsics is not None:
            ...
            o = observations_board_true[
                indices_frame_camintrinsics_camextrinsics[:, 1] == icam, ..., :2
            ]
            # shape (Nframes,Nh*Nw,2)
    ...
    def observed_points(icam):
        if indices_point_camintrinsics_camextrinsics is not None:
            return observations_point_true[
                indices_point_camintrinsics_camextrinsics[:, 1] == icam, ..., :2
            ]
        return None
    ...
    if args.make_documentation_plots:
        for extension in ("pdf", "svg", "png", "gp"):
            obs_cam = [
                observed_pixels(icam)
                + [
                    (
                        q0_baseline,
                        dict(
                            _with=f'points pt 3 lw 2 lc "red" ps {2 * pointscale[extension]}'
                        ),
                    ),
                ]
                for icam in range(args.Ncameras)
            ...
            gp.add_plot_option(
                processoptions_output, "set", ("xtics 1000", "ytics 1000")
            )
...
def reproject_perturbed__common(
    q,
    distance,
    ...
    if not (baseline_points is None or baseline_points.size == 0) or not (
        query_points is None or query_points.size == 0
    ):
    ...
    if args.reproject_perturbed == "mean-pcam":
        ...
        if fixedframes:
            raise Exception(
                "meanq,bestq not implemented if fixedframes. MAYBE is possible, but not useful-enough to think about"
            )
        ...
        if args.reproject_perturbed == "bestq":
            ...
            if q_err.ndim < 3:
                ...
                q_reprojected = np.take_along_axis(
                    q_reprojected, nps.dummy(i, -1, 0), axis=-3
                )
            else:
                i = q_err.sum(axis=-3).argmin(axis=-2)
                q_reprojected = np.take_along_axis(
                    q_reprojected, nps.dummy(i, -1, 0, 0), axis=-3
                )
...
def reproject_perturbed__fit_boards_ref(
    ...
    calobject_height, calobject_width = baseline_optimization_inputs[
        "observations_board"
    ].shape[1:3]
...
def reproject_perturbed__cross_reprojection(
    ...
    if fixedframes:
        raise Exception(
            "reproject_perturbed__cross_reprojection(fixedframes = True) is not yet implemented. I would at least need to handle J_frames not existing when computing J_cross"
        )

    if not baseline_optimization_inputs["do_optimize_frames"]:
        raise Exception(
            "reproject_perturbed__cross_reprojection implementation expects the frames to be optimized"
        )
    ...
    slice_state_intrinsics = slice(
        istate_intrinsics0, istate_intrinsics0 + Nstates_intrinsics
    )

    if istate_calobject_warp0 is not None:
        slice_state_calobject_warp = slice(
            istate_calobject_warp0, istate_calobject_warp0 + Nstates_calobject_warp
        )

    every_observation_has_extrinsics = (
        baseline_optimization_inputs["do_optimize_extrinsics"]
        and Nstates_extrinsics > 0
        and istate_extrinsics0 is not None
    )
    # Not done yet. Will add to every_observation_has_extrinsics further down

    have_state = dict(board=istate_frame0 is not None, point=istate_point0 is not None)
    ...
    if have_state["board"] and have_state["point"]:
        if imeas_point0 != imeas_board0 + Nmeas_board:
            raise Exception(
                "Both point and board measurements are present. I expect the point measurements to immediately follow the board measurements, but I don't have that here. This and all the _all variables assumes this"
            )
    ...
    for what in have_state.keys():
        if have_state[what]:
            baseline_observations[what] = baseline_optimization_inputs[
                f"observations_{what}"
            ]
            query_observations[what] = np.array(
                [oi[f"observations_{what}"] for oi in query_optimization_inputs]
            )
            ...
            # set outliers to 0
            if what == "point":
                Nmeas_per_point = 2  # includes range normalization penalty
            else:
                Nmeas_per_point = 2
            x_baseline[
                imeas0_observations[what] : imeas0_observations[what]
                + Nmeas_observations[what]
            ].reshape(Nmeas_observations[what] // Nmeas_per_point, Nmeas_per_point)[
                (weight[what].ravel()) <= 0, :
            ] = 0
    ...
    if have_state["board"]:
        ...
        every_observation_has_extrinsics = every_observation_has_extrinsics and np.all(
    ...
    if have_state["point"]:
        ...
        every_observation_has_extrinsics = every_observation_has_extrinsics and np.all(
            baseline_optimization_inputs["indices_point_camintrinsics_camextrinsics"][
                :, 2
            ]
            >= 0
        )
    ...
    if istate_extrinsics0 is not None:
        slice_state_extrinsics = slice(
            istate_extrinsics0, istate_extrinsics0 + Nstates_extrinsics
        )
    else:
    ...
    if re.search("Je$", mode) and not every_observation_has_extrinsics:
        raise Exception(
            f"User asked for '{args.reproject_perturbed}', but Je is not available: not every observation has an extrinsics vector"
        )
    ...
    if have_state["board"]:
        W_delta_qref[
            ...,
            imeas0_observations["board"] : imeas0_observations["board"]
            + Nmeas_observations["board"],
        ] = np.array(nps.clump(query_q_noise_board, n=-(query_q_noise_board.ndim - 1)))
    if have_state["point"]:
        W_delta_qref[
            ...,
            imeas0_observations["point"] : imeas0_observations["point"]
            + Nmeas_observations["point"],
    ...
    J_observations = J_packed_baseline[
        imeas0_observations_all : imeas0_observations_all + Nmeas_observations_all, :
    ]
    ...
    for what in have_state.keys():
        if have_state[what]:
            ...
            W_delta_qref_xy_what = np.reshape(
                W_delta_qref[
                    ...,
                    imeas0_observations[what] : imeas0_observations[what]
                    + Nmeas_observations[what],
                ...
                (
                    len(W_delta_qref),
                    Nmeas_observations[what] // Nmeas_per_point,
                    Nmeas_per_point,
                ),
            )
            if not np.shares_memory(W_delta_qref_xy_what, W_delta_qref):
                raise Exception("clump() made new array. This is a bug")
    ...
    def get_rt_ref_refperturbed():
        def transform_point_rt3_withgrad_drt1(rt0, rt1, rt2, p):
            ...
            del (
                dpp_dp,
                dpp_drt012,
            )  # I'm running out of memory. Let's free stuff when we can
            return pp, dpp_drt1
        ...
        def transform_point_identity_gradient(p):
            ...
            if nps.norm2((dprot_drt - dprot_drt_reference).ravel()) > 1e-10:
                raise Exception(
                    "transform_point_identity_gradient() is computing the wrong thing. This is a bug"
                )

            del (
                dprot_drt_reference,
                _,
            )  # I'm running out of memory; free stuff when I can
        ...
        def get_cross_operating_point(
            ...
            *,
            ...
            for what in have_state.keys():
                if have_state[what]:
                    x_cross0_what = x_cross0[
                        imeas0_observations[what] : imeas0_observations[what]
                        + Nmeas_observations[what]
                    ]
                    x_cross0_what = x_cross0_what.reshape(pcam[what].shape[:-1] + (2,))
                    if not np.shares_memory(x_cross0, x_cross0_what):
                        raise Exception("reshape() made new array. This is a bug")

                    J_cross_what = J_cross[
                        ...,
                        imeas0_observations[what] : imeas0_observations[what]
                        + Nmeas_observations[what],
                        :,
                    ]
                    if not np.shares_memory(J_cross, J_cross_what):
                        raise Exception("reshape() made new array. This is a bug")
                    ...
                    if direction == "rt_ref_refperturbed":
                        ...
                        dx_drt_ref_refperturbed = nps.matmult(
                            dx_dpcam, dpcam_drt_ref_refperturbed[what]
                        )
                        ...
                        if what == "board":
                            ...
                            J_cross_what[:] = nps.mv(
                                nps.clump(
                                    nps.mv(dx_drt_ref_refperturbed, -1, -5), n=-4
                                ),
                                -2,
                                -1,
                            )
                            ...
                            J_cross_what[:] = nps.mv(
                                nps.clump(
                                    nps.mv(dx_drt_ref_refperturbed, -1, -3), n=-2
                                ),
                                -2,
                                -1,
                            )
                        ...
                        if what == "board":
                            ...
                            qcross, dq_dpcamperturbed, _ = mrcal.project(
                                ...
                                baseline_optimization_inputs["lensmodel"],
                                nps.dummy(
                                    query_intrinsics[:, idx_camintrinsics[what]], -2, -2
                                ),
                                get_gradients=True,
                            )
                        elif what == "point":
                        ...
                        x_cross0_what[:] = (qcross - qrefperturbed) * nps.dummy(
                            weight[what], -1
                        )
                        x_cross0_what[..., weight[what] <= 0, :] = 0  # outliers

                        dx_dpcamperturbed = dq_dpcamperturbed * nps.dummy(
                            weight[what], -1, -1
                        )
                        dx_dpcamperturbed[..., weight[what] <= 0, :, :] = 0  # outliers
                        dx_drt_ref_refperturbed = nps.matmult(
                            dx_dpcamperturbed, dpcamperturbed_drt_refperturbed_ref[what]
                        )
            ...
            return x_cross0 - x_baseline[
                imeas0_observations_all : imeas0_observations_all
                + Nmeas_observations_all
            ], J_cross
        ...
        def get_cross_operating_point__linearization(
            W_delta_qref,
            Jt_W_qref,
            query_b_unpacked,
            # not broadcasted
            ...
            *,
            ...
            if have_state["board"]:
                state_mask_fpcw[slice_state_frame] = 1
            if have_state["point"]:
                state_mask_fpcw[slice_state_point] = 1
            ...
            if every_observation_has_extrinsics:
                ...
                rt_cam_ref = b_baseline_unpacked[slice_state_extrinsics].reshape(
                    Nextrinsics, 6
                )
            ...
            if 1:
                ...
                if have_state["board"]:
                    rt_ref_frame = b_baseline_unpacked[slice_state_frame].reshape(
                        Nframes, 6
                    )
                    # shape (Nframes,6,6)
                ...
                if get_cross_operating_point__linearization.did_check_count < 5:
                    ...
                    testutils.confirm_equal(
                        ...
                        eps=1e-12,
                        worstcase=True,
                        msg="drt_ref_refperturbed__dbpacked() does the right thing",
                    )
                    ...
                    if 0:
                        ...
                        Kpackedp_noinv = np.fromfile(
                            "/tmp/Kpackedp_noinv", dtype=float
                        ).reshape(6, Nstates_point)
                        Kpackedp_noinv_ref = nps.matmult(J_cross_fp.T, Jpacked_fpcw)[
                            :, istate_point0:
                        ]
        ...
        if have_state["board"]:
            object_width_n = baseline_optimization_inputs["observations_board"].shape[
                -2
            ]
            object_height_n = baseline_optimization_inputs["observations_board"].shape[
                -3
            ]
        ...
        if 1:
                ...
                if have_state[what]:
                    ...
                    if what == "board":
                        ...
                        qq = mrcal.project(
                            ...
                            baseline_optimization_inputs["lensmodel"],
                            nps.dummy(
                                baseline_intrinsics[idx_camintrinsics[what], :], -2, -2
                            ),
                        )
                    elif what == "point":
                    ...
                    x = (qq - baseline_observations[what][..., idx, :2]) * nps.dummy(
                        weight[what], -1
                    )
            ...
            err_rms_baseline = np.sqrt(
                nps.norm2(err_sum_of_squares_baseline) / (N_sum_of_squares_baseline / 2)
            )

        dxcross_Jcross = dict(
            rt_ref_refperturbed=dict(),
        ...
        if 1:
            ...
            dxcross_Jcross["rt_ref_refperturbed"][method] = get_cross_operating_point(
                pcam, dpcam_drt_ref_refperturbed, direction="rt_ref_refperturbed"
            )
            ...
            dxcross_Jcross["rt_refperturbed_ref"][method] = get_cross_operating_point(
                pcamperturbed,
        ...
        if 1:
            ...
            if have_state["board"]:
                ...
                dpref_drt_ref_refperturbed = transform_point_identity_gradient(
                    prefperturbed
                )
                # shape (..., Nmeas_observations_all,Nh,Nw,3),
                #       (..., Nmeas_observations_all,Nh,Nw,3,6)
                ...
                dpcam_drt_ref_refperturbed["board"] = nps.matmult(
                    dpcam_dpref, dpref_drt_ref_refperturbed
                )
                del (
                    dpref_drt_ref_refperturbed,
                    dpcam_dpref,
                    _,
                )  # I'm running out of memory; free stuff when I can
            ...
            if have_state["point"]:
                ...
                dpcam_drt_ref_refperturbed["point"] = nps.matmult(
                    dpcam_dpref, dpref_drt_ref_refperturbed
                )
                del (
                    dpref_drt_ref_refperturbed,
            ...
            if have_state["board"]:
                dprefperturbed_drt_refperturbed_ref = transform_point_identity_gradient(
                    pref["board"]
                )
                # shape (..., Nmeas_observations_all,Nh,Nw,3),
                #       (..., Nmeas_observations_all,Nh,Nw,3,6)
                ...
                dpcamperturbed_drt_refperturbed_ref["board"] = nps.matmult(
                    dpcamperturbed_dprefperturbed, dprefperturbed_drt_refperturbed_ref
                )
                del (
                    dprefperturbed_drt_refperturbed_ref,
                    dpcamperturbed_dprefperturbed,
                    _,
                )  # I'm running out of memory; free stuff when I can
            ...
            if have_state["point"]:
                ...
                dpcamperturbed_drt_refperturbed_ref["point"] = nps.matmult(
                    dpcamperturbed_dprefperturbed, dprefperturbed_drt_refperturbed_ref
                )
                del (
                    dprefperturbed_drt_refperturbed_ref,
        ...
        if 1:
            ...
            if have_state["point"]:
                scale_points = b[istate_point0 : istate_point0 + 3]
            else:
                scale_points = None
            ...
            rrp_rpr__xcross__Jcross_e__Jcross_fp = np.empty(
                (args.Nsamples, 2, 3), dtype=object
            )
            get_cross_operating_point__linearization(
                W_delta_qref,
                Jt_W_qref,
        ...
        for direction in ("rt_ref_refperturbed", "rt_refperturbed_ref"):
            ...
            testutils.confirm_equal(
                dxcross_Jcross[direction]["compose-grad"][0],
            ...
            testutils.confirm_equal(
                dxcross_Jcross[direction]["compose-grad"][1],
        ...
        for method in rt_rr_all["rt_ref_refperturbed"].keys():
            ...
            th_deg = nps.mag(rt_error[:, :3]) * 180.0 / np.pi
        ...
        if 1:
            ...
            for what in have_state.keys():
                ...
                x_cross0 = (q_cross - baseline_observations[what][..., :2]) * nps.dummy(
                    weight[what], -1
                )
            ...
            if Nmeas_cross != dx_cross0.shape[-1]:
                raise Exception(
                    f"dx_cross0.shape[-1] mismatch. This is a bug. Nmeas_cross={Nmeas_cross}, dx_cross0.shape={dx_cross0.shape}"
                )

            err_rms_cross_solved = np.sqrt(
                err_sum_of_squares_cross_solved / (Nmeas_cross / 2)
            )
            testutils.confirm(
                err_rms_baseline * 10 < np.mean(err_rms_cross_ref0),
                msg=f"cross-reprojection uncertainty at distance={distance}: Unoptimized cross error is MUCH worse than the baseline",
            )
            testutils.confirm(
                np.mean(err_rms_cross_solved) * 2 < np.mean(err_rms_cross_ref0),
            ...
            if 0:
                ...
                print(
                    f"RMS error perturbed           = {np.mean(err_rms_cross_ref0)} pixels"
                )
                print(
    ...
    if 1:
        ...
        #
        # shape (Nsamples,6)
        rt_ref_refperturbed_predicted_from_samples = nps.transpose(
            nps.matmult(Kpacked_inv_JtJ, nps.transpose(Jt_W_qref))
        )
        ...
        rt_ref_refperturbed__mean0 = rt_ref_refperturbed - np.mean(
            rt_ref_refperturbed, axis=-2
        )
...
if args.reproject_perturbed == "fit-boards-ref":
    ...
    reproject_perturbed = reproject_perturbed__cross_reprojection
else:
...
for distance in args.distances:
    # shape (Ncameras, 2)
    q0_true_here = reproject_perturbed(
        q0_baseline,
...
for icam in (0, 3):
    ...
    if args.non_vanilla:
        for what, optimization_inputs_here in (
            (
                "moving-camera-ref-at-frame0",
                optimization_inputs_baseline_moving_cameras_refframe0,
            ),
            (
                "moving-camera-ref-at-cam0",
                optimization_inputs_baseline_moving_cameras_refcam0,
            ),
        ):
...
def check_uncertainties_at(q0_baseline, idistance):
    ...
    # shape (Ncameras, 2,2)
    Var_dq_observed = np.mean(
        nps.outer(q_sampled - q_sampled_mean, q_sampled - q_sampled_mean), axis=-4
    )
...
if not (
    args.explore or args.show_distribution or args.make_documentation_plots is not None
):
    testutils.finish()
    sys.exit()
...
def make_plot__distribution(icam, report_center_points=True, **kwargs):
    ...
    if report_center_points:
        ...
        data_tuples += (
            ...
            (
                q_sampled_mean,
                dict(tuplesize=-2, _with="points pt 3 ps 3", legend="Sampled mean"),
            ),
        ) + data_tuple_true_center_point
...
if args.show_distribution:
    ...
    for icam in range(args.Ncameras):
        ...
        if args.extra_observation_at is not None:
            plot_options["title"] += (
                f": boards at {args.range_to_boards}m, extra one at {args.extra_observation_at}m"
            )
    ...
    if args.make_documentation_plots:
        for extension in ("pdf", "svg", "png", "gp"):
            if extension == "png":
                continue
            data_tuples = [
                data_tuples_plot_options[icam][0]
                + [
                    (
                        q0_baseline[0],
                        q0_baseline[1],
            ...
            def subplots_noobservations(p):
                return [t for t in p if not is_datatuple_withdots(t)]
            ...
            for obs_yesno, dt in (
                ...
                if "_set" in processoptions_output:
                    gp.add_plot_option(
                        plot_options, "set", processoptions_output["_set"]
                    )
                    del processoptions_output["_set"]
                gp.plot(
                    *dt, **plot_options, multiplot="layout 2,2", **processoptions_output
                )
                if extension == "pdf":
                    os.system(f"pdfcrop {processoptions_output['hardcopy']}")
