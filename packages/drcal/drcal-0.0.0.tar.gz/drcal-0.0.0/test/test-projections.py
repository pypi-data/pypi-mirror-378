...
def check(intrinsics, p_ref, q_ref):
    ...
    if meta["has_gradients"]:
        ...
        testutils.confirm_equal(
            q_projected, q_ref, msg=f"Projecting {intrinsics[0]} with grad", eps=1e-2
        )
        testutils.confirm_equal(
            dq_dp, dq_dpi_ref[..., :3], msg=f"dq_dp {intrinsics[0]}", eps=1e-2
        )
        testutils.confirm_equal(
            dq_di, dq_dpi_ref[..., 3:], msg=f"dq_di {intrinsics[0]}", eps=1e-2
        )
        ...
        testutils.confirm_equal(
            dq_dp, dq_dpi_ref[..., :3], msg="dq_dp in-place", eps=1e-2
        )
        testutils.confirm_equal(
    ...
    if 1:
        ...
        testutils.confirm_equal(
            ...
            1,
            msg="Unprojected v are normalized",
            eps=1e-6,
        )
    ...
    testutils.confirm_equal(
        ...
        1,
        msg="Unprojected in-place v are normalized",
        eps=1e-6,
    )
    cos = nps.inner(v_unprojected, p_ref) / nps.mag(p_ref)
    cos = np.clip(cos, -1, 1)
    ...
    testutils.confirm_equal(
        nps.norm2(v_unprojected),
        1,
        msg="Unprojected v (with gradients) are normalized",
        eps=1e-6,
    )
    cos = nps.inner(v_unprojected, p_ref) / nps.mag(p_ref)
    cos = np.clip(cos, -1, 1)
    ...
    if 1:
        ...
        testutils.confirm_equal(
            nps.norm2(v_unprojected),
            1,
            msg="Unprojected v (with gradients, in-place) are normalized",
            eps=1e-6,
        )
        cos = nps.inner(v_unprojected, p_ref) / nps.mag(p_ref)
        cos = np.clip(cos, -1, 1)
...
check(
    (
        ...
        np.array(
            (
                (1512.0, 1112, 500.0, 333.0),
                (1512.0, 1112, 500.0, 433.0),
                (1512.0, 1112, 500.0, 533.0),
            )
        ),
    ),
    p,
    np.array([[651.2, 555.4], [-1163.2, 766.6], [-860.8, -1135.0]]),
)
...
check(
    (
        ...
        np.array(
            (
                (1512.0, 1112, 500.0, 333.0),
                (1502.0, 1112, 500.0, 433.0),
                (1522.0, 1112, 500.0, 533.0),
            )
        ),
    ),
    p,
...
check(
    (
        "LENSMODEL_OPENCV5",
        np.array((1512.0, 1112, 500.0, 333.0, -0.012, 0.035, -0.001, 0.002, 0.019)),
    ),
    p,
    np.array(
        [
            [651.2740691, 555.2309482],
...
check(
    ...
    np.array(
        [
            [-0.8479983, -0.52999894, -0.34690877],
