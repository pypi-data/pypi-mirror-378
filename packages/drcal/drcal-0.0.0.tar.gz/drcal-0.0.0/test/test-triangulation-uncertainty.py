...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        choices=("read", "write"),
        help="""A cache file stores the recalibration results;
    ...
    if args.cameras[0] < 0 or args.cameras[0] >= args.Ncameras:
        raise Exception(
            "--cameras must select two different cameras, each in [0,Ncameras-1]"
        )
    ...
    if args.q_calibration_stdev <= 0.0 and args.q_observation_stdev <= 0.0:
        raise Exception(
            "At least one of --q-calibration-stdev and --q-observation-stdev MUST be given as > 0"
        )
    return args
...
if args.make_documentation_plots:
    ...
    args.make_documentation_plots_path = os.path.join(
        d, re.sub(r"[^0-9a-zA-Z_\.\-]", "_", f)
    )

    print(
        f"Will write documentation plots to {args.make_documentation_plots_path}-xxxx.pdf and .png and .svg"
    )
...
# shape (Npoints,3)
p_triangulated_true0 = np.array((args.observed_point, args.observed_point), dtype=float)
# first point has x<0
...
if args.cache is None or args.cache == "write":
    optimization_inputs_baseline, models_true, frames_true = calibration_baseline(
        args.model,
        args.Ncameras,
        args.Nframes,
        None,
...
p_triangulated0 = triangulate_nograd(
    models_true[icam0].intrinsics()[1],
...
slices = (
    (q_true[0], (models_baseline[icam0], models_baseline[icam1])),
...
########## Gradient check
dp_triangulated_di0_empirical = grad(
    lambda i0: triangulate_nograd(
        i0,
        models_baseline[icam1].intrinsics()[1],
        models_baseline[icam0].extrinsics_rt_fromref(),
        models_baseline[icam0].extrinsics_rt_fromref(),
...
dp_triangulated_de1_empirical = grad(
    lambda e1: triangulate_nograd(
        models_baseline[icam0].intrinsics()[1],
        models_baseline[icam1].intrinsics()[1],
        models_baseline[icam0].extrinsics_rt_fromref(),
        models_baseline[icam0].extrinsics_rt_fromref(),
...
dp_triangulated_de0_empirical = grad(
    lambda e0: triangulate_nograd(
        models_baseline[icam0].intrinsics()[1],
        models_baseline[icam1].intrinsics()[1],
        e0,
        models_baseline[icam0].extrinsics_rt_fromref(),
...
dp_triangulated_drtrf_empirical = grad(
    lambda rtrf: triangulate_nograd(
        models_baseline[icam0].intrinsics()[1],
        models_baseline[icam1].intrinsics()[1],
        models_baseline[icam0].extrinsics_rt_fromref(),
        models_baseline[icam0].extrinsics_rt_fromref(),
...
dp_triangulated_dq_empirical = grad(
    lambda q: triangulate_nograd(
        models_baseline[icam0].intrinsics()[1],
        models_baseline[icam1].intrinsics()[1],
        models_baseline[icam0].extrinsics_rt_fromref(),
        models_baseline[icam0].extrinsics_rt_fromref(),
...
testutils.confirm_equal(
    dp_triangulated_dbstate[..., istate_i0 : istate_i0 + Nintrinsics],
...
testutils.confirm_equal(
    dp_triangulated_dbstate[..., istate_i1 : istate_i1 + Nintrinsics],
...
if istate_e0 is not None:
    testutils.confirm_equal(
        dp_triangulated_dbstate[..., istate_e0 : istate_e0 + 6],
...
if istate_e1 is not None:
    testutils.confirm_equal(
        dp_triangulated_dbstate[..., istate_e1 : istate_e1 + 6],
...
for ipt in range(Npoints):
    dp_triangulated_dq_empirical[ipt, ...] = dp_triangulated_dq_empirical_cross_only[
        ipt, :, ipt, :, :
    ]
...
testutils.confirm_equal(
    dp_triangulated_dq_empirical_cross_only,
    0,
    eps=1e-6,
...
testutils.confirm_equal(
    p_alone, p_calnoise, eps=1e-6, msg="triangulate(cal noise) returns the right point"
)
testutils.confirm_equal(
    p_alone, p_obsnoise, eps=1e-6, msg="triangulate(obs noise) returns the right point"
)
testutils.confirm_equal(
    p_alone, p, eps=1e-6, msg="triangulate(both noise) returns the right point"
)
testutils.confirm_equal(
...
np.random.seed(0)
qt_noise = np.random.multivariate_normal(
    mean=np.zeros(
        (Npoints * 2 * 2,),
    ),
    cov=var_qt,
    size=args.Nsamples,
).reshape(args.Nsamples, Npoints, 2, 2)
...
p_triangulated_sampled0 = triangulate_nograd(
    intrinsics_sampled[..., icam0, :],
    intrinsics_sampled[..., icam1, :],
...
for ipt in range(Npoints):
    Var_ranges_joint[ipt] = nps.matmult(
        p_triangulated0[ipt],
        Var_p_joint[ipt, :, ipt, :],
        nps.transpose(p_triangulated0[ipt]),
    )[0] / nps.norm2(p_triangulated0[ipt])
    Var_ranges_calibration[ipt] = nps.matmult(
        p_triangulated0[ipt],
        Var_p_calibration[ipt, :, ipt, :],
        nps.transpose(p_triangulated0[ipt]),
    )[0] / nps.norm2(p_triangulated0[ipt])
    Var_ranges_observations[ipt] = nps.matmult(
        p_triangulated0[ipt],
...
distance_sampled = nps.mag(
    p_triangulated_sampled0[:, 1, :] - p_triangulated_sampled0[:, 0, :]
)
...
Var_distance = nps.matmult(
    nps.glue(-diff, diff, axis=-1),
...
Var_p_sampled = (
    nps.matmult(nps.transpose(p_sampled - mean_p_sampled), p_sampled - mean_p_sampled)
    / args.Nsamples
)
...
testutils.confirm_equal(
    ...
    # reduce it
    eps=p_triangulated_true0[0, 2] / 150.0,
...
if not (args.explore or args.make_documentation_plots is not None):
    testutils.finish()
    sys.exit()
...
if args.make_documentation_plots is not None:
    ...
    Var_p_joint_diagonal = [
        Var_p_joint[ipt, :, ipt, :][(0, 2), :][:, (0, 2)] for ipt in range(Npoints)
    ]
    Var_p_calibration_diagonal = [
        Var_p_calibration[ipt, :, ipt, :][(0, 2), :][:, (0, 2)]
        for ipt in range(Npoints)
    ]
    Var_p_observation_diagonal = [
        Var_p_observation[ipt, :, :][(0, 2), :][:, (0, 2)] for ipt in range(Npoints)
    ]

    max_sigma_points = np.array(
        [np.max(np.sqrt(np.linalg.eig(V)[0])) for V in Var_p_joint_diagonal]
    )
    ...
    def makeplots(dohardcopy, processoptions_base):
        ...
        if dohardcopy:
            ...
            gp.plot(*subplots, multiplot="layout 1,2", unset="grid", **processoptions)
    ...
    if args.make_documentation_plots:
        for extension in ("pdf", "svg", "png", "gp"):
            makeplots(
                dohardcopy=True,
                processoptions_base=dict(
                    wait=False, terminal=terminal[extension], _set=extraset[extension]
                ),
            )
    else:
