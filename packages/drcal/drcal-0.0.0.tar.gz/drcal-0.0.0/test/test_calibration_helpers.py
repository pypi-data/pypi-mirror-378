...
def sample_dqref(observations, pixel_uncertainty_stdev, make_outliers=False):
    ...
    q_noise = (
        np.random.randn(*observations.shape[:-1], 2)
    ...
    if make_outliers:
        if not hasattr(sample_dqref, "idx_outliers_ref_flat"):
            ...
            sample_dqref.idx_outliers_ref_flat = np.random.choice(
                NobservedPoints,
...
def grad(f, x, *, switch=None, forward_differences=False, step=1e-6):
    ...
    def df_dxi(i, d, dflat):
        if not forward_differences:
            ...
            f0 = f(x)
            fplus = f(x + d)
            if switch is not None and nps.norm2(fplus - f0) > 1.0:
                fplus = switch(fplus)
            j = (fplus - f0) / step
            dflat[i] = 0
            return j
...
def calibration_baseline(
    ...
    if nps.norm2(extrinsics_rt_fromcam0_true[0]) > 0:
        raise Exception(
            "A non-identity cam0 transform was given. This is not supported"
        )

    if fixedframes and (moving_cameras or ref_frame0):
        raise Exception(
            "fixedframes only supported in the simple, vanilla case: not moving_cameras and not ref_frame0"
        )
    ...
    x_center = -(Ncameras - 1) / 2.0

    def synthesize(z, z_noiseradius, Nframes, x_offset):
        if avoid_oblique_views:
            kwargs = dict(
                pcamera_nominal_ref=np.array((x_center, 0, 0), dtype=float),
                max_oblique_angle_deg=30.0,
            )
        else:
            kwargs = dict()
        ...
        return mrcal.synthesize_board_observations(
            ...
            rt_ref_boardcenter=np.array((0.0, 0.0, 0.0, x_center + x_offset, 0, z)),
    ...
    if not x_mirror:
        ...
        #        (Nframes, 4,3)
        q_true, Rt_cam0_board_true = synthesize(
            z=range_to_boards,
        ...
        #        (Nframes//2, 4,3)
        q_true0, Rt_cam0_board_true0 = synthesize(
            z=range_to_boards,
            z_noiseradius=range_to_boards / 2.0,
            Nframes=Nframes // 2,
            x_offset=x_offset,
        )
        q_true1, Rt_cam0_board_true1 = synthesize(
            z=range_to_boards,
            z_noiseradius=range_to_boards / 2.0,
            Nframes=Nframes // 2,
            x_offset=-x_offset,
    ...
    if extra_observation_at is not None:
        q_true_extra, Rt_cam0_board_true_extra = synthesize(
            z=extra_observation_at,
        ...
        Rt_cam0_board_true = nps.glue(
            Rt_cam0_board_true, Rt_cam0_board_true_extra, axis=-3
        )
    ...
    if cull_left_of_center:
        ...
        weight[q_true[..., 0] < imagersize[0] / 2.0] /= 1000.0

    # I want observations of shape (Nframes*Ncameras, Nh, Nw, 3) where each row is
    # (x,y,weight)
    observations_board_true = nps.clump(
        nps.glue(q_true, nps.dummy(weight, -1), axis=-1), n=2
    )
    ...
    if not report_points:
        ...
        return optimization_inputs_baseline, models_true_refcam0, points_true


def calibration_make_non_vanilla(optimization_inputs, *, moving_cameras, ref_frame0):
    ...
    if moving_cameras and ref_frame0:
        ...
        if Ncameras > 1:
            raise Exception(
                "Scenario (moving-camera, stationary-frame, ref-at-frame) cannot work with multiple cameras: camera-rig implementation is required"
            )

        optimization_inputs["indices_frame_camintrinsics_camextrinsics"] = (
            np.ascontiguousarray(nps.transpose(nps.cat(idxce + 1, idxci, idxf)))
        )
    ...
    if moving_cameras and not ref_frame0:
        ...
        if Ncameras > 1:
            raise Exception(
                "Scenario (moving-camera, stationary-frame, ref-at-cam0) cannot work with multiple cameras: camera-rig implementation is required"
            )
    ...
    if not moving_cameras and ref_frame0:
...
def calibration_boards_to_points(optimization_inputs):
    ...
    indices_point_camintrinsics_camextrinsics = np.zeros(
        (Nobservations, object_height_n, object_width_n, 3), dtype=np.int32
    )
    ...
    ).reshape(object_height_n, object_width_n) + nps.dummy(
        iframe * object_height_n * object_width_n, -1, -1
    )

    # camintrinsics and camextrinsics
    # shape (Nobservations,H,W,2)
    indices_point_camintrinsics_camextrinsics[..., 1:] = nps.dummy(
        indices_frame_camintrinsics_camextrinsics[..., 1:], -2, -2
    )

    # shape (Nobservations*H*W, 3)
    indices_point_camintrinsics_camextrinsics = nps.clump(
        indices_point_camintrinsics_camextrinsics, n=3
    )
    ...
    # So I re-sort here
    iipoint = np.argsort(indices_point_camintrinsics_camextrinsics[:, 0], kind="stable")
    indices_point_camintrinsics_camextrinsics = (
    ...
    if Ncameras_extrinsics < 1:
        raise Exception(
            "Points-only solves need extra regularization, but currently that's only possible with Ncameras_extrinsics>=1"
        )
    ...
    s = 1.0 / nps.norm2(optimization_inputs["extrinsics_rt_fromref"][0, 3:])
...
def calibration_sample(
    ...
    def have(k):
        return (
            k in optimization_inputs_baseline
            and optimization_inputs_baseline[k] is not None
        )

    intrinsics_sampled = np.zeros(
        (Nsamples,) + optimization_inputs_baseline["intrinsics"].shape, dtype=float
    )

    if have("frames_rt_toref"):
        frames_sampled = np.zeros(
            (Nsamples,) + optimization_inputs_baseline["frames_rt_toref"].shape,
            dtype=float,
        )
        q_noise_board_sampled = np.zeros(
            (Nsamples,)
            + optimization_inputs_baseline["observations_board"].shape[:-1]
            + (2,),
            dtype=float,
        )
    else:
    ...
    if have("points"):
        points_sampled = np.zeros(
            (Nsamples,) + optimization_inputs_baseline["points"].shape, dtype=float
        )
        q_noise_point_sampled = np.zeros(
            (Nsamples,)
            + optimization_inputs_baseline["observations_point"].shape[:-1]
            + (2,),
            dtype=float,
        )
    else:
    ...
    if have("calobject_warp"):
        calobject_warp_sampled = np.zeros(
            (Nsamples,) + optimization_inputs_baseline["calobject_warp"].shape,
            dtype=float,
        )
    else:
    ...
    extrinsics_sampled_mounted = np.zeros(
        (Nsamples, Ncameras_extrinsics, 6), dtype=float
    )
    ...
    for isample in range(Nsamples):
        ...
        if have("observations_board"):
            (
                q_noise_board_sampled[isample],
                optimization_inputs["observations_board"],
            ) = sample_dqref(
                optimization_inputs["observations_board"], pixel_uncertainty_stdev
            )
        else:
            q_noise_board = None
        if have("observations_point"):
            (
                q_noise_point_sampled[isample],
        ...
        if fixedframes:
            extrinsics_sampled_mounted[isample, ...] = optimization_inputs[
                "extrinsics_rt_fromref"
            ]
        else:
            # the remaining row is already 0
            extrinsics_sampled_mounted[isample, 1:, ...] = optimization_inputs[
                "extrinsics_rt_fromref"
            ]
