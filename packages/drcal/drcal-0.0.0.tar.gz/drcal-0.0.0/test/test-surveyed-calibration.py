...
def parse_args():
    ...
    parser.add_argument(
        "--seed-rng", type=int, default=0, help="""Value to seed the rng with"""
    )
    parser.add_argument(
        "--do-sample",
        action="store_true",
    ...
    if args.range_board_center is None:
        args.range_board_center = args.range_board * 2.0
    return args
...
# shape (6,)
rt_cam_ref_true = np.array(
    (-0.04, 0.05, -0.1, 1.2, -0.1, 0.1),
)
...
if args.make_documentation_plots is not None:
    ...
    if args.make_documentation_plots:
        ...
        if terminal["pdf"] is None:
            terminal["pdf"] = (
                'pdf size 8in,6in       noenhanced solid color      font ",12"'
            )
...
model_true.intrinsics(
    (lensmodel, intrinsics_data_true),
)
...
q_center = nps.mv(
    nps.cat(
        *np.meshgrid(
            np.linspace(0.5, NboardgridX - 0.5, NboardgridX, endpoint=True)
            / NboardgridX
            * W,
            np.linspace(0.5, NboardgridY - 0.5, NboardgridY, endpoint=True)
            / NboardgridY
            * H,
        )
    ),
...
rt_cam_board_true[..., :3] += (
    np.random.rand(*rt_cam_board_true[..., :3].shape) * 2.0 - 1.0
) * random_radius__r_cam_board_true
rt_cam_board_true[..., 3:] += (
    np.random.rand(*rt_cam_board_true[..., 3:].shape) * 2.0 - 1.0
) * random_radius__t_cam_board_true
...
# shape (N,),
weight01 = (np.random.rand(*q_true.shape[:-1]) + 1.0) / 2.0  # in [0,1]
weight0 = 0.2
weight1 = 1.0
...
observations_point += nps.glue(q_true, nps.dummy(weight, -1), axis=-1)
# shape (Noversample*N, 3)
...
indices_point_camintrinsics_camextrinsics = np.zeros(
    (args.oversample, Npoints, 3), dtype=np.int32
)
indices_point_camintrinsics_camextrinsics[..., 0] += np.arange(Npoints)
indices_point_camintrinsics_camextrinsics = nps.clump(
    indices_point_camintrinsics_camextrinsics, n=2
)
...
_, optimization_inputs["observations_point"] = sample_dqref(
    optimization_inputs["observations_point"], pixel_uncertainty_stdev
)
...
if args.only_report_uncertainty:
    ...
    print(
        f"{args.range_board} {args.range_board_center} {np.sqrt(nps.trace(Var_q) / 2.0):.4f} {np.sqrt(Var_errz):.4f}"
    )
    sys.exit()
...
testutils.confirm_equal(rmserr_point, 0, eps=2.5, msg="Converged to a low RMS error")
...
testutils.confirm_equal(
    (np.trace(Rt_extrinsics_err[:3, :]) - 1) / 2.0,
    1.0,
    eps=np.cos(1.0 * np.pi / 180.0),  # 1 deg
...
testutils.confirm_equal(diff, 0, worstcase=True, eps=6.0, msg="Recovered intrinsics")
...
if True:
    ...
    if Nmeas > 0:
        ...
        testutils.confirm_equal(
            ...
            worstcase=True,
            eps=1e-8,
            msg="make_perfect_observations() works for points",
        )
    else:
        testutils.confirm(False, msg="Nmeasurements_points <= 0")
...
if 1:
    ...
    testutils.confirm_equal(
        np.mean(errz_sampled), 0, eps=0.01, msg="errz distribution has mean=0"
    )
    ...
    testutils.confirm_equal(
        ...
        eps=0.02,
        relative=True,
        msg="Predicted Var(errz) correct",
    )
    ...
    if args.make_documentation_plots is not None:
        ...
        def makeplot(**plotoptions):
            gp.plot(
                ...
                histogram=True,
                binwidth=binwidth,
                equation_above=(equation_observed, equation_predicted),
                xlabel="Error in z (m)",
                ylabel="Count",
