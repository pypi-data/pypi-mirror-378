...
testutils.confirm(np.all(np.diff(l) > 0), msg="Single matrix: monotonic eigenvalues")
testutils.confirm_equal(
    l, np.sort(l_ref[1, 2]), worstcase=True, eps=1e-6, msg="Single matrix: eigenvalues"
)
isorted = np.argsort(l_ref[1, 2])
for i in range(len(isorted)):
    # I check the abs(inner) to ignore sign differences
    testutils.confirm_equal(
        np.abs(nps.inner(v[:, i], v_ref[1, 2, :, isorted[i]])),
        1.0,
        worstcase=True,
        eps=1e-6,
...
all_shapes_passed = all(
    (
        ...
        for x in (
            testutils.confirm_equal(
                l.shape, X.shape[:-1], msg="Broadcasted matrix: l.shape"
            ),
            testutils.confirm_equal(
                v.shape, X.shape, msg="Broadcasted matrix: v.shape"
            ),
        )
    )
)
...
if all_shapes_passed:
    testutils.confirm(
        np.all(np.diff(l) > 0), msg="Broadcasted matrix: monotonic eigenvalues"
    )
    testutils.confirm_equal(
        l,
        np.sort(l_ref),
    ...
    testutils.confirm_equal(
        nps.matmult(v, diag(l), np.linalg.inv(v)),
        X,
        worstcase=True,
        eps=1e-6,
        msg="Broadcasted matrix: eigenvectors",
    )
