...
p_triangulated_true0[:, 1] = np.arange(Npoints) / 10.0 - 0.344
p_triangulated_true0[:, 2] = 100.0
...
# shape (Nmodelpairs,2)
M = [
    [models_baseline[i] for i in icameras[0]],
...
testutils.confirm_equal(
    p.shape, (Npoints, Nmodelpairs, 3), msg="point array has the right shape"
)
...
testutils.confirm_equal(
    Var_p0p1_observation_big.shape,
...
for ipt in range(Npoints):
    for imp in range(Nmodelpairs):
        ...
        testutils.confirm_equal(
            Var_p0p1_joint_big[ipt, imp, :, ipt, imp, :],
            Var_p0p1,
            msg=f"Covariance (joint) sub-matrix ipt={ipt} imp={imp}",
        )
        testutils.confirm_equal(
            Var_p0p1_calibration_big[ipt, imp, :, ipt, imp, :]
            + Var_p0p1_observation_big[ipt, imp, :, :],
            Var_p0p1,
