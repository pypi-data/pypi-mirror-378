...
def generate_world(Npoints_fixed=None):
    ...
    points_noise = np.array(
        [
            [-0.16415198, 0.10697666, 0.07137079],
    ...
    pref_noisy = pref_true * (1.0 + points_noise)
    ...
    observations_true = nps.glue(
        qcam_indexed_true,
        nps.transpose(np.ones((qcam_indexed_true.shape[0],))),
        axis=-1,
    )

    observations_noisy = nps.glue(
        qcam_indexed_noisy,
    ...
    rt_cam_ref_noise = np.array(
        [
            [-0.00781127, -0.04067386, -0.01039731, 0.02057068, -0.0461704, 0.02112582],
