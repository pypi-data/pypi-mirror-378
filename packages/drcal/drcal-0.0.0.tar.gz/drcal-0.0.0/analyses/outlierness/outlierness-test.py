...
def report_mismatch_relerr(a, b, what):
    relerr = np.abs(a - b) / ((a + b) / 2.0)
    if relerr > 1e-6:
        print(
            "MISMATCH for {}: relerr = {}, a = {}, b = {},".format(what, relerr, a, b)
        )
...
def report_mismatch_abserr(a, b, what):
    ...
    if abserr > 1e-6:
        print(
            "MISMATCH for {}: abserr = {}, a = {}, b = {},".format(what, abserr, a, b)
        )
...
def compute_outliernesses(J, x, jq, k_dima, k_cook):
    ...
    Aquery = nps.inner(
        jq,
        nps.transpose(
            np.linalg.solve(nps.matmult(nps.transpose(J), J), nps.transpose(jq))
        ),
    )
    ...
    def dima():
        ...
        def query():
            B = 1.0 / (Aquery + 1.0)
            return dict(self=k * (B * B), others=k * (B - B * B), self_others=k * (B))

        return dict(outliers=outliers(), query=query())
    ...
    def cook():
        ...
        def outliers():
            B = 1.0 / (Aoutliers - 1.0)
            return dict(
                self_others=k * x * x * (B + B * B), others=k * x * x * (-B - 1)
            )

        def query():
            B = 1.0 / (Aquery + 1.0)
            return dict(self_others=k * (1 - B), others=k * (B - B * B))

        return dict(outliers=outliers(), query=query())

    return dict(cook=cook(), dima=dima())
...
def CooksD_test(J, x, f, CooksD, k_dima, k_cook, i=0):
    ...
    report_mismatch_relerr(
        nps.inner(dx, dx) * k_cook,
        CooksD["self_others"][i],
        "self_others CooksD computed analytically, explicitly",
    )
    report_mismatch_relerr(
        (nps.inner(dx, dx) - dx[i] * dx[i]) * k_cook,
        CooksD["others"][i],
        "others CooksD computed analytically, explicitly",
    )


def outlierness_query_test(
    J, b, x, f, query, fquery_ref, outlierness_nox, k_dima, k_cook, i=0
):
...
def CooksD_query_test(J, b, x, f, query, fquery_ref, CooksD_nox, k_dima, k_cook, i=0):
    ...
    report_mismatch_relerr(
        nps.inner(dx_both, dx_both) * k_cook,
...
def Var_df(J, squery, stdev):
    ...
    return (
        nps.inner(
            squery,
            nps.transpose(
                np.linalg.solve(nps.matmult(nps.transpose(J), J), nps.transpose(squery))
            ),
        )
        * stdev
        * stdev
...
def show_outlierness(order, N, q, f, query, cooks_threshold, **stats):
    p = gp.gnuplotlib(
        ...
        title="Outlierness with order={} Npoints={} stdev={}".format(
            order, N, noise_stdev
        ),
    )
    p.plot(
        (
            q,
            stats["metrics"]["dima"]["outliers"]["self_others"]
            / stats["dimas_threshold"],
            dict(legend="Dima's self+others outlierness / threshold", _with="points"),
        ),
        (
            q,
            stats["metrics"]["dima"]["outliers"]["others"] / stats["dimas_threshold"],
            dict(legend="Dima's others-ONLY outlierness / threshold", _with="points"),
        ),
        (
            q,
            stats["metrics"]["cook"]["outliers"]["self_others"] / cooks_threshold,
            dict(legend="Cook's self+others outlierness / threshold", _with="points"),
        ),
        (
            q,
            stats["metrics"]["cook"]["outliers"]["others"] / cooks_threshold,
            dict(legend="Cook's others-ONLY outlierness / threshold", _with="points"),
        ),
    )
    return p
...
def show_uncertainty(order, N, q, f, query, cooks_threshold, **stats):
    coeffs = stats["b"]
    fitted_equation = "+".join(
        ["{} * x**{}".format(coeffs[i], i) for i in range(len(coeffs))]
    )

    p = gp.gnuplotlib(
        equation='({})-({}) title "Fit error off ground truth; y2 axis +- noise stdev" axis x1y2'.format(
            reference_equation, fitted_equation
        ),
        title="Uncertainty with order={} Npoints={} stdev={}".format(
            order, N, noise_stdev
        ),
        ymin=0,
    ...
    p.plot(
        ...
        (
            query,
            np.sqrt(stats["metrics"]["dima"]["query"]["self"]) * noise_stdev,
            dict(legend="Dima's self-ONLY; 1 point", _with="linespoints"),
        ),
        (
            query,
            np.sqrt(stats["metrics"]["dima"]["query"]["others"]) * noise_stdev,
            dict(
                legend="Dima's others-ONLY ALL {} points".format(Nquery),
                _with="linespoints",
            ),
        ),
...
def show_fit(order, N, q, f, query, cooks_threshold, **stats):
    ...
    p.plot(
        (q, f, dict(legend="input data", _with="points")),
        (
            query,
            stats["fquery"]
            + np.sqrt(stats["Vquery"])
            * np.array(
                (
                    (1,),
                    (0,),
            ...
            dict(legend="stdev_f", _with="lines"),
        ),
    )
    return p
...
plots = [
    show_outlierness(order, N, q, f, query, cooks_threshold, **stats),
