...
def parse_args():
    def positive_float(string):
        try:
            ...
            raise argparse.ArgumentTypeError(
                "argument MUST be a positive floating-point number. Got '{}'".format(
                    string
                )
            )
        if value <= 0:
    ...
    def positive_int(string):
        try:
            ...
            raise argparse.ArgumentTypeError(
                "argument MUST be a positive integer. Got '{}'".format(string)
            )
    ...
    parser.add_argument(
        "--scan-object-spacing-compensate-range-from",
        type=float,
        help="""Only applies if --scan object-spacing. By
    ...
    parser.add_argument(
        "--hardcopy",
        help="""Filename to plot into. If omitted, we make an interactive plot. This is
                        passed directly to gnuplotlib""",
    )
    parser.add_argument(
        "--terminal",
        help="""gnuplot terminal to use for the plots. This is passed directly to
                        gnuplotlib. Omit this unless you know what you're doing""",
    )
    parser.add_argument("--extratitle", help="""Extra title string to add to a plot""")
    parser.add_argument(
        "--title",
        help="""Full title string to use in a plot. Overrides
...
if args.object_width_n is None and args.object_height_n is None:
    args.object_width_n = "10"
    args.object_height_n = 10
elif (
    not (args.object_width_n is not None and args.object_height_n is not None)
    and args.scan != "object_width_n"
):
    raise Exception(
        "Either --object-width-n or --object-height-n are given: you must pass both or neither"
    )
...
controllable_args = dict(
    tilt_radius=dict(_type=float),
...
if any(controllable_args[a]["listlen"] > 2 for a in controllable_args.keys()):
    raise Exception(
        f"All controllable args must have either at most 2 values. {a} has {controllable_args[a]['listlen']}"
    )

controllable_arg_0values = [
    a for a in controllable_args.keys() if controllable_args[a]["listlen"] == 0
]
controllable_arg_2values = [
    a for a in controllable_args.keys() if controllable_args[a]["listlen"] == 2
]

if len(controllable_arg_2values) == 0:
    controllable_arg_2values = ""
elif len(controllable_arg_2values) == 1:
    controllable_arg_2values = controllable_arg_2values[0]
else:
    raise Exception(
        f"At most 1 controllable arg may have 2 values. Instead I saw: {controllable_arg_2values}"
    )

if re.match("num_far_constant_Nframes_", args.scan):
    if (
        args.x_radius is not None
        or args.y_radius is not None
        or args.z_radius is not None
    ):
        raise Exception(
            "--x-radius and --y-radius and --z-radius are exclusive with --scan num_far_..."
        )

    # special case
    if "Nframes" not in controllable_arg_0values:
        raise Exception(
            f"I'm scanning '{args.scan}', so --Nframes must not have been given"
        )
    ...
    if args.scan == "num_far_constant_Nframes_near":
        if args.Nframes_all is not None:
            raise Exception(
                f"I'm scanning '{args.scan}', so --Nframes-all must not have have been given"
            )
        if args.Nframes_near is None:
            raise Exception(
                f"I'm scanning '{args.scan}', so --Nframes-near must have have been given"
            )
    else:
        if args.Nframes_near is not None:
            raise Exception(
                f"I'm scanning '{args.scan}', so --Nframes-near must not have have been given"
            )
        if args.Nframes_all is None:
            raise Exception(
                f"I'm scanning '{args.scan}', so --Nframes-all must have have been given"
            )
    ...
    if args.scan != controllable_arg_2values:
        ...
        raise Exception(
            f"I'm scanning '{args.scan}', the arg given 2 values is '{controllable_arg_2values}'. They must match"
        )
    if len(controllable_arg_0values):
        raise Exception(
            f"I'm scanning '{args.scan}', so all controllable args should have some value. Missing: '{controllable_arg_0values}"
        )
    if args.scan == "":
...
if uncertainty_at_range_sampled_min is None:
    uncertainty_at_range_sampled_min = first(controllable_args["range"]["value"]) / 10.0
uncertainty_at_range_sampled_max = args.uncertainty_at_range_sampled_max
if uncertainty_at_range_sampled_max is None:
    uncertainty_at_range_sampled_max = last(controllable_args["range"]["value"]) * 10.0

uncertainty_at_range_samples = np.logspace(
...
def solve(
    ...
    if q_true_far is not None:
        ...
        q_true_far = np.zeros((0,) + q_true_near.shape[1:], dtype=q_true_near.dtype)
        Rt_ref_board_true_far = np.zeros(
            (0,) + Rt_ref_board_true_near.shape[1:], dtype=Rt_ref_board_true_near.dtype
        )
    ...
    Rt_ref_board_true = nps.glue(Rt_ref_board_true_near, Rt_ref_board_true_far, axis=-3)
    ...
    indices_frame.setfield(
        nps.outer(
            np.arange(Nframes_all, dtype=np.int32), np.ones((Ncameras,), dtype=np.int32)
        ),
        dtype=np.int32,
    )
    ...
    indices_camera.setfield(
        nps.outer(
            np.ones((Nframes_all,), dtype=np.int32), np.arange(Ncameras, dtype=np.int32)
        ),
        dtype=np.int32,
    )
    indices_frame_camintrinsics_camextrinsics = nps.glue(
        indices_frame_camera, indices_frame_camera[:, (1,)], axis=-1
    )
    ...
    q = nps.glue(q_true_near, q_true_far, axis=-5)
    ...
    observations = nps.glue(q, np.ones(q.shape[:-1] + (1,)), axis=-1)

    # shape (Ncameras, 1, 1, 2)
    imagersizes = nps.mv(nps.cat(*[m.imagersize() for m in models_true]), -2, -4)

    # mark the out-of-view observations as outliers
    observations[np.any(q < 0, axis=-1), 2] = -1.0
    observations[np.any(q - imagersizes >= 0, axis=-1), 2] = -1.0

    # shape (Nobservations, Nh, Nw, 2)
    observations = nps.clump(observations, n=2)
    ...
    if re.search("SPLINED", lensmodel):
        ...
        optimization_inputs["intrinsics"] = nps.glue(
            optimization_inputs["intrinsics"],
            np.zeros(
                (Ncameras, Nintrinsics - 4),
            ),
            axis=-1,
        )
        stats = mrcal.optimize(**optimization_inputs)
        ...
        if (
            ...
            raise Exception(
                "I'm assuming all the models here have a core. It's just lazy coding. If you see this, feel free to fix."
            )
...
def eval_one_rangenear_tilt(
    ...
    Ncameras,
    Nframes_near_samples,
    Nframes_far_samples,
):
    # I want the RNG to be deterministic
    np.random.seed(0)

    uncertainties = np.zeros(
        (len(Nframes_far_samples), len(uncertainty_at_range_samples)), dtype=float
    )

    radius_cameras = (args.camera_spacing * (Ncameras - 1)) / 2.0

    x_radius = (
        args.x_radius
        if args.x_radius is not None
        else range_near * 2.0 + radius_cameras
    )
    y_radius = args.y_radius if args.y_radius is not None else range_near * 2.0
    z_radius = args.z_radius if args.z_radius is not None else range_near / 10.0
    ...
    q_true_near, Rt_ref_board_true_near = mrcal.synthesize_board_observations(
        ...
        rt_ref_boardcenter__noiseradius=np.array(
            (
                np.pi / 180.0 * tilt_radius,
                np.pi / 180.0 * tilt_radius,
                np.pi / 180.0 * args.roll_radius,
                x_radius,
                y_radius,
                z_radius,
            )
    ...
    for i_Nframes_far in range(len(Nframes_far_samples)):
        ...
        if args.write_models_first_solve:
            for i in range(len(models_out)):
                ...
                if os.path.exists(f):
                    input(
                        f"File {f} already exists, and I want to overwrite it. Press enter to overwrite. Ctrl-c to exit"
                    )
...
output_table = np.zeros(
    (args.Nscan_samples, Nuncertainty_at_range_samples, output_table_Ncols), dtype=float
)

output_table[:, :, output_table_icol__range_uncertainty_sample] += (
    uncertainty_at_range_samples
)
...
if re.match("num_far_constant_Nframes_", args.scan):
    ...
    if args.scan == "num_far_constant_Nframes_near":
        Nframes_far_samples = np.linspace(
            0, args.Nframes_near // 4, Nfar_samples, dtype=int
        )
        Nframes_near_samples = Nframes_far_samples * 0 + args.Nframes_near

    else:
        Nframes_far_samples = np.linspace(0, args.Nframes_all, Nfar_samples, dtype=int)
    ...
    models_true = [
        mrcal.cameramodel(
            ...
            extrinsics_rt_toref=np.array(
                (0, 0, 0, i * args.camera_spacing, 0, 0), dtype=float
            ),
        )
    ...
    output_table[:, :, output_table_icol__uncertainty] = eval_one_rangenear_tilt(
        models_true,
    ...
    output_table[:, :, output_table_icol__Nframes_near] += nps.transpose(
        Nframes_near_samples
    )
    output_table[:, :, output_table_icol__Nframes_far] += nps.transpose(
        Nframes_far_samples
    )
    output_table[:, :, output_table_icol__Ncameras] = controllable_args["Ncameras"][
        "value"
    ]
    output_table[:, :, output_table_icol__range_near] = controllable_args["range"][
        "value"
    ][0]
    output_table[:, :, output_table_icol__range_far] = controllable_args["range"][
        "value"
    ][1]
    output_table[:, :, output_table_icol__tilt_radius] = controllable_args[
        "tilt_radius"
    ]["value"]
    output_table[:, :, output_table_icol__object_width_n] = controllable_args[
        "object_width_n"
    ]["value"]
    output_table[:, :, output_table_icol__object_spacing] = controllable_args[
        "object_spacing"
    ]["value"]
    ...
    range_samples = np.linspace(
        *controllable_args["range"]["value"], Nrange_samples, dtype=float
    )
    for i_range in range(Nrange_samples):
        output_table[i_range, :, output_table_icol__uncertainty] = (
            eval_one_rangenear_tilt(
                models_true,
                range_samples[i_range],
                None,
                controllable_args["tilt_radius"]["value"],
                controllable_args["object_width_n"]["value"],
                args.object_height_n,
    ...
    output_table[:, :, output_table_icol__Nframes_near] = controllable_args["Nframes"][
        "value"
    ]
    ...
    tilt_rad_samples = np.linspace(
        *controllable_args["tilt_radius"]["value"], Ntilt_rad_samples, dtype=float
    )
    ...
    output_table[:, :, output_table_icol__range_far] = -1
    output_table[:, :, output_table_icol__tilt_radius] += nps.transpose(
        tilt_rad_samples
    )
    output_table[:, :, output_table_icol__object_width_n] = controllable_args[
        "object_width_n"
    ...
    Ncameras_samples = np.linspace(
        *controllable_args["Ncameras"]["value"], N_Ncameras_samples, dtype=int
    )
    ...
    Nframes_samples = np.linspace(
        *controllable_args["Nframes"]["value"], N_Nframes_samples, dtype=int
    )
    ...
    samples = np.linspace(
        *controllable_args["object_width_n"]["value"], Nsamples, dtype=int
    )
    ...
    # the object_width_n.
    W = (controllable_args["object_width_n"]["value"][0] - 1) * controllable_args[
        "object_spacing"
    ]["value"]
    ...
    output_table[:, :, output_table_icol__object_spacing] += nps.transpose(
        object_spacing
    )
    ...
    Nsamples = args.Nscan_samples
    samples = np.linspace(
        *controllable_args["object_spacing"]["value"], Nsamples, dtype=float
    )
    ...
    if args.scan_object_spacing_compensate_range_from:
        output_table[:, :, output_table_icol__range_near] += controllable_args["range"][
            "value"
        ] * nps.transpose(samples / samples[0])
    else:
        output_table[:, :, output_table_icol__range_near] = controllable_args["range"][
            "value"
        ]
    ...
    output_table[0, :, output_table_icol__uncertainty] = eval_one_rangenear_tilt(
        models_true,
        controllable_args["range"]["value"],
        None,
        controllable_args["tilt_radius"]["value"],
...
if isinstance(controllable_args["range"]["value"], float):
    guides = [
        f"arrow nohead dashtype 3 from {controllable_args['range']['value']},graph 0 to {controllable_args['range']['value']},graph 1"
    ]
else:
    guides = [
        f"arrow nohead dashtype 3 from {r},graph 0 to {r},graph 1"
        for r in controllable_args["range"]["value"]
    ]
guides.append(
...
if args.set:
    gp.add_plot_option(plotoptions, _set=args.set)
if args.unset:
    gp.add_plot_option(plotoptions, _unset=args.unset)
