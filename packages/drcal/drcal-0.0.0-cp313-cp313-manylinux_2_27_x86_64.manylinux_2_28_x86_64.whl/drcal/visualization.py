...
plot
    ...
    if cameras_Rt_plot_ref is not None:
        ...
        if not isinstance(cameras_Rt_plot_ref, np.ndarray):
            ...
            try:
                ...
                raise Exception(
                    "cameras_Rt_plot_ref must be a transform or a list of transforms"
                )

        # cameras_Rt_plot_ref is now an array. Does it have the right shape?
        if cameras_Rt_plot_ref.shape[-2:] != (4, 3):
            raise Exception(
                "cameras_Rt_plot_ref must be a transform or a list of Rt transforms. Got an array that looks like neither"
            )
        # shape is (....,4,3)
        if cameras_Rt_plot_ref.ndim == 2:
            ...
            if len(cameras_Rt_plot_ref) != Ncameras:
                raise Exception(
                    "cameras_Rt_plot_ref must be a transform or a list of transforms, exactly one per camera. Got mismatched number of transforms"
                )
            # have an array of transforms with the right shape
            if frames_rt_toref is not None or points is not None:
                raise Exception(
                    "Got multiple transforms in cameras_Rt_plot_ref AND frames_rt_toref or points. This makes no sense: I can't tell which transform to use"
                )
            pass
        else:
            raise Exception(
    ...
    if frames_rt_toref is not None:
        if object_width_n is None or object_height_n is None or object_spacing is None:
            raise Exception(
                "frames_rt_toref is given, so object_width_n and object_height_n and object_spacing must be given as well"
            )
    ...
    def get_boards_to_plot():
        ...
        if not (
            show_calobjects is True
            or show_calobjects == "all"
            or show_calobjects == "thiscamera"
        ):
            raise Exception(
                "show_calobjects must be 'all' or 'thiscamera' or True or False"
            )
        ...
        if not (
            ...
            and optimization_inputs["observations_board"].size
        ):
            return None
    ...
    def get_points_to_plot():
        ...
        if not (
            show_points is True or show_points == "all" or show_points == "thiscamera"
        ):
            raise Exception(
                "show_points must be 'all' or 'thiscamera' or True or False"
            )
        ...
        if not (
            "points" in optimization_inputs
            and optimization_inputs["points"] is not None
            and optimization_inputs["points"].size
        ):
            return None
    ...
    def gen_curves_cameras():
        ...
        cam_axes = [
            gen_plot_axes(
                (camera_Rt_toplotcoords(i),), legend=camera_name(i), scale=axis_scale
            )
            for i in range(0, Ncameras)
        ]
        cam_axes_labels = [
            gen_plot_axes_labels((camera_Rt_toplotcoords(i),), scale=axis_scale)
            for i in range(0, Ncameras)
        ]
    ...
    def gen_curves_calobjects():
        ...
        if object_spacing <= 0 or object_width_n is None or object_height_n is None:
            raise Exception(
        ...
        if point_labels is not None:
            ...
            return [
                (
                    ...
                    dict(tuplesize=-3, _with="points", legend="points"),
                )
            ] + [
                (
                    points[ipoint],
                    dict(tuplesize=-3, _with="points", legend=point_labels[ipoint]),
                )
                for ipoint in point_labels.keys()
            ]

        else:
            return [(points, dict(tuplesize=-3, _with="points", legend="points"))]
    ...
    kwargs = dict(kwargs)
    gp.add_plot_option(kwargs, xlabel="x", ylabel="y", zlabel="z", overwrite=False)

    plot_options = dict(_3d=1, square=1, **kwargs)
...
def _options_heatmap_with_contours(
    plotoptions,  # we update this on output
    *,
    contour_min=0,
    contour_max,
    ...
    if do_contours:
        if contour_increment is None:
            ...
            base10_floor = np.power(10.0, np.floor(np.log10(increment)))

            # Look through the options, and pick the best one
            m = np.array((1.0, 2.0, 5.0, 10.0))
        ...
        if contour_labels_font is not None:
            gp.add_plot_option(
                plotoptions, "set", f'cntrlabel font "{contour_labels_font}"'
            )
        ...
        _with = np.array(
            ("image", "lines nosurface", f"labels {contour_labels_styles} nosurface")
        )
    else:
...
def fitted_gaussian_equation(
    *, binwidth, x=None, mean=None, sigma=None, N=None, legend=None
):
...
String passable to gnuplotlib in the 'equation' or 'equation_above' plot option
    ...
    if x is not None:
        if mean is not None or sigma is not None or N is not None:
            raise Exception(
                "x was given. So none of (mean,sigma,N) should have been given"
            )
        sigma = np.std(x)
        mean = np.mean(x)
        N = len(x)
        ...
        if mean is None or sigma is None or N is None:
            raise Exception(
                "x was not given. So all of (mean,sigma,N) should have been given"
            )

    var = sigma * sigma
    k = N * np.sqrt(2.0 * np.pi) * sigma * erf(binwidth / (2.0 * np.sqrt(2) * sigma))

    if legend is None:
        title = "notitle"
    else:
        title = f'title "{legend}"'
    return f"{k}*exp(-(x-{mean})*(x-{mean})/(2.*{var})) / sqrt(2.*pi*{var}) {title} with lines lw 2"


def _append_observation_visualizations(
    plot_data_args,
    *,
    model,
    legend_prefix="",
    ...
    for q, color, what in (
        ...
        if q is not None and len(q) > 0:
            ...
            if not _2d:
                _with += " nocontour"
            plot_data_args.append(
                (q[..., 0], q[..., 1])
                + (() if _2d else (np.zeros(q.shape[:-1]),))
                + (
                    dict(
                        tuplesize=2 if _2d else 3,
                        _with=_with,
...
def show_projection_diff(
    models,
    *,
    implied_Rt10=None,
    gridn_width: int | None = 60,
    ...
    **kwargs,
    ...
    if "title" not in kwargs:
        if intrinsics_only:
            ...
            using_uncertainties_string = (
                f"{'' if use_uncertainties else 'not '}using uncertainties"
            )
    ...
    if vectorfield:
        if len(models) > 2:
            raise Exception(
                "I can only plot a vectorfield when looking at exactly 2 models. Instead I have {}".format(
                    len(models)
                )
            )
    ...
    if directions and len(models) > 2:
        raise Exception(
            "I can only color-code by directions when looking at exactly 2 models. Instead I have {}".format(
                len(models)
            )
        )
    ...
    if directions:
        ...
        color = 180.0 / np.pi * np.arctan2(diff[..., 1], diff[..., 0])
    else:
    ...
    if vectorfield:
        ...
        gp.add_plot_option(plot_options, square=1, _xrange=[0, W], _yrange=[H, 0])

        curve_options = dict(_with="vectors filled palette", tuplesize=5)
        plot_data_args = [
            (
                q0[:, 0],
                q0[:, 1],
        ...
        curve_options = _options_heatmap_with_contours(
            ...
            if vectorfield:
                ...
                plot_data_args.append(
                    (
                        ...
                        valid_region0[:, 0] * 0,
                        dict(
                            _with='lines lw 4 lc "green" nocontour',
                            legend="valid region of 1st camera",
                        ),
                    )
                )
...
def show_stereo_pair_diff(
    ...
    **kwargs,
    ...
    if vectorfield:
        if len(model_pairs) > 2:
            raise Exception(
                "I can only plot a vectorfield when looking at exactly 2 model_pairs. Instead I have {}".format(
                    len(model_pairs)
                )
            )
...
def show_projection_uncertainty(
    model,
    *,
    gridn_width=60,
    gridn_height=None,
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    curveoptions = _options_heatmap_with_contours(
        # update these plot options
        kwargs,
        contour_max=cbmax,
        contour_increment=contour_increment,
...
def show_projection_uncertainty_vs_distance(
    model,
    *,
    where="centroid",
    observed_pixel_uncertainty=None,
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    if distance_min is None or distance_max is None:
        ...
        if distance_min is None:
            ...
            distance_min = np.min(distance_observed_at_calibration_time) / 5.0
        if distance_max is None:
            # * 10 for a bit of extra margin
            distance_max = np.max(distance_observed_at_calibration_time) * 10.0
...
def show_distortion_off_pinhole_radial(
    model,
    *,
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    if not m:
        raise Exception(
            "Radial distortion visualization implemented only for OpenCV distortions; for now"
        )
    N = int(m.group(1))
    ...
    if N >= 8:
        denominator = "1. + r*r * ({} + r*r * ({} + r*r * {}))".format(
            *distortions[5:8]
        )
    ...
    q_corners = np.array(((x0, y0), (x0, y1), (x1, y0), (x1, y1)), dtype=float)
    ...
    equations = [
        f'180./pi*atan(tan(x*pi/180.) * ({scale})) with lines lw 2 title "THIS model"',
        'x title "pinhole"',
    ]
    if show_fisheye_projections:
        equations += [
            '180./pi*atan(2. * tan( x*pi/180. / 2.)) title "stereographic"',
            '180./pi*atan(x*pi/180.) title "equidistant"',
            '180./pi*atan(2. * sin( x*pi/180. / 2.)) title "equisolid angle"',
            '180./pi*atan( sin( x*pi/180. )) title "orthogonal"',
        ]
    ...
    gp.add_plot_option(
        ...
        "set",
        [
            'arrow from {th}, graph 0 to {th}, graph 1 nohead lc "red"'.format(th=th)
            for th in th_centersy
        ]
        + [
            'arrow from {th}, graph 0 to {th}, graph 1 nohead lc "green"'.format(th=th)
            for th in th_centersx
        ]
        + [
            'arrow from {th}, graph 0 to {th}, graph 1 nohead lc "blue"'.format(th=th)
            for th in th_corners
        ],
    )
    ...
    if N >= 8:
        equations.extend(
            [
                numerator + ' axis x1y2 title "numerator (y2)"',
...
def show_distortion_off_pinhole(
    model,
    *,
    vectorfield=False,
    vectorscale=1.0,
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    if not vectorfield:
        ...
        kwargs["_set"] = [
            "object 1 rectangle from 0,0 to {},{} fillstyle empty".format(W, H)
        ]
        kwargs["square"] = True

        if "_set" in kwargs:
            if type(kwargs["_set"]) is list:
                kwargs["_set"].extend(kwargs["_set"])
            else:
                kwargs["_set"].append(kwargs["_set"])
            del kwargs["_set"]
...
def show_valid_intrinsics_region(
    models,
    *,
    cameranames=None,
    image=None,
    ...
    **kwargs,
    ...
    for m in models[1:]:
        ...
        if W != WH1[0] or H != WH1[1]:
            raise Exception(
                "All given models MUST have the same imagersize. Got {} and {}".format(
                    (W, H), WH1
                )
            )
    ...
    if image is not None:
        if isinstance(image, np.ndarray):
            plot_data_args.append((image, dict(_with="image", tuplesize=3)))
        else:
            kwargs["rgbimage"] = image

    plot_data_args.extend(
        (r[:, 0], r[:, 1], dict(_with='lines lw 4 lc "green"', legend=cameranames[i]))
        for i, r in enumerate(valid_regions)
    )

    if points is not None:
        plot_data_args.append((points, dict(tuplesize=-2, _with="points pt 7 ps 1")))

    plot_options = dict(square=1, _xrange=[0, W], _yrange=[H, 0], **kwargs)
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    if valid_intrinsics_region:
        ...
        if imager_boundary_sparse is None:
            raise Exception(
                "No valid-intrinsics region is available in this model. Pass valid_intrinsics_region=False"
            )
    else:
        # the imager boundary
        imager_boundary_sparse = np.array(
            ((0, 0), (W - 1, 0), (W - 1, H - 1), (0, H - 1), (0, 0)), dtype=float
        )
    ...
    if not vectorfield:
        gp.add_plot_option(plot_options, _set='cblabel "u correction (unitless)"')

        surface_curveoptions = dict(_with="image", tuplesize=3)
        if imager_domain:
            ...
            plot_data_tuples_surface = (
                (
                    *(
                    ...
                    dict(_with="vectors filled", tuplesize=4),
                ),
            )
        else:
            plot_data_tuples_surface = (
    ...
    plot_data_tuples_boundaries = (
        (
            imager_boundary,
            dict(
                _with='lines lw 2 lc "green"',
                tuplesize=-2,
                legend="Valid-intrinsics region"
                if valid_intrinsics_region
                else "Imager boundary",
            ),
        ),
        (
            domain_contour,
            dict(_with="lines lw 1", tuplesize=-2, legend="Spline-in-bounds"),
        ),
    )
    ...
    plot_data_tuples_knots = (
        (
            knots,
            dict(_with='points pt 2 ps 2 lc "green"', tuplesize=-2, legend="knots"),
        ),
    )
    ...
    # plot this area
    imager_boundary_nonan = imager_boundary[
        np.isfinite(imager_boundary[:, 0]) * np.isfinite(imager_boundary[:, 1]), :
    ]
    ...
    if len(invalid_regions) > 0:
        print(
            "WARNING: some parts of the imager cannot be projected from a region covered by the spline surface! You should increase the field-of-view of the model"
        )

        plot_data_tuples_invalid_regions = tuple(
            (
                r,
                dict(
                    tuplesize=-2,
...
def annotate_image__valid_intrinsics_region(image, model, *, color=(0, 255, 0)):
    ...
    if valid_intrinsics_region is None:
        ...
        print(
            "WARNING: annotate_image__valid_intrinsics_region(): valid-intrinsics region is undefined. Drawing a circle"
        )
    elif valid_intrinsics_region.size == 0:
        cv2.circle(image, tuple((model.imagersize() - 1) // 2), 10, color, -1)
        print(
            "WARNING: annotate_image__valid_intrinsics_region(): valid-intrinsics region is empty. Drawing a circle"
        )
    else:
...
The 'using' string.
    ...
    if gridn_height is None:
        gridn_height = int(round(H / W * gridn_width))
    return "($1*{}):($2*{}):3".format(
        float(W - 1) / (gridn_width - 1), float(H - 1) / (gridn_height - 1)
    )


def show_residuals_board_observation(
    optimization_inputs,
    i_observation,
    *,
    from_worst=False,
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    if "title" not in plot_options:
        title = "{}: i_observation={}{}, iframe={}, icam={}, {}RMS_error={:.2f}".format(
    ...
    gp.add_plot_option(plot_options, square=True, cbmin=0, overwrite=False)
    ...
    if imagepath is not None:
        ...
        gp.add_plot_option(plot_options, rgbimage=imagepath, overwrite=True)
        gp.add_plot_option(plot_options, "set", "autoscale noextend")
    else:
        icam = optimization_inputs["indices_frame_camintrinsics_camextrinsics"][
            i_observation, 1
        ]
        W, H = optimization_inputs["imagersizes"][icam]
        gp.add_plot_option(
            plot_options, xrange=[0, W - 1], yrange=[H - 1, 0], overwrite=False
        )

    gp.add_plot_option(plot_options, "unset", "key")

    if cbmax is not None:
        gp.add_plot_option(plot_options, cbmax=cbmax)
    ...
    data_tuples = (
        ...
        (
            ...
            vectorscale * x[:, 0],
            vectorscale * x[:, 1],
            dict(_with="vectors filled lw 2", tuplesize=4),
        ),
    )
...
def show_residuals_histogram(
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    equation = fitted_gaussian_equation(
    ...
    gp.add_plot_option(plot_options, equation_above=equation, overwrite=True)
    gp.add_plot_option(
        plot_options,
    ...
    data_tuples = [(x, dict(histogram=True, binwidth=binwidth))]
    if not return_plot_args:
        plot = gp.gnuplotlib(**plot_options)
        plot.plot(*data_tuples)
        return plot
...
def _get_show_residuals_data_onecam(
    ...
    if (
        ...
        err_chessboard, obs_chessboard = np.array(()), np.array(())
    ...
    if (
        ...
        err_point, obs_point = np.array(()), np.array(())
    ...
    if valid_intrinsics_region and icam_intrinsics is not None:
        ...
        valid_intrinsics_region_plotarg_3d = (
            ...
            dict(_with='lines lw 4 lc "green"', legend=legend),
        )
...
def show_residuals_vectorfield(
    model,
    x=None,
    # backwards-compatibility synonym of "x"
    residuals=None,
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    data_tuples = [
        (
            obs[:, 0],
            obs[:, 1],
            180.0 / np.pi * np.arctan2(err[..., 1], err[..., 0]),
            dict(_with="points pt 7 palette", tuplesize=3),
        )
    ]
    if valid_intrinsics_region_plotarg_2d is not None:
    ...
    **plot_options). Useful if we want to include this as a part of a more complex
    ...
    def mkplot(x, title, **plot_options):
        ...
        if "hardcopy" in plot_options and plot_options["hardcopy"] is not None:
            ...
            plot_options["hardcopy"] = re.sub(
                r"(\.[^\.]+$)", "." + what + r"\1", plot_options["hardcopy"]
            )
        ...
        gp.add_plot_option(plot_options, "unset", "grid")

        gp.add_plot_option(plot_options, _3d=True, ascii=True, overwrite=True)

        W, H = model.imagersize()
        gp.add_plot_option(
            plot_options, _xrange=[0, W], _yrange=[H, 0], overwrite=False
        )

        data_tuples = [(x, dict(tuplesize=3, _with="image", using=using))]
