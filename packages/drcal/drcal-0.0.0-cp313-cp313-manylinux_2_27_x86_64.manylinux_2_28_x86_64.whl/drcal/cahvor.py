...
def _construct_model(
    ...
    **rest,
    ...
    th = np.arccos(np.clip(nps.inner(Vp, Vp_expected), -1, 1)) * 180.0 / np.pi
    if th > 1e-3:
        print(
            f"WARNING: parsed .cahvor file has non-orthogonal Hp,Vp. Skew of {th:.3f} degrees. I'm using an orthogonal Vp, so the resulting model will work slightly differently",
            file=sys.stderr,
        )
    Vp = Vp_expected

    R_toref = nps.transpose(nps.cat(Hp, Vp, A))
    t_toref = C
    ...
    if is_cahvor_or_cahvore:
        ...
        if is_cahvore:
            ...
            if E is not None:
                raise Exception(
                    "Cahvor file {} LOOKS like a cahvor, but has an E".format(name)
                )
    ...
    return cameramodel(
        ...
        intrinsics=(
            lensmodel,
            nps.glue(np.array((fx, fy, cx, cy), dtype=float), distortions, axis=-1),
        ),
...
def _read(s, name):
    ...
    for k in ("Dimensions", "C", "A", "H", "V"):
        if k not in x:
            raise Exception(
                "Cahvor file '{}' incomplete. Missing values for: {}".format(name, k)
            )
    ...
    if "VALID_INTRINSICS_REGION" in x:
        x["VALID_INTRINSICS_REGION"] = x["VALID_INTRINSICS_REGION"].reshape(
            len(x["VALID_INTRINSICS_REGION"]) // 2, 2
        )
    ...
    if m:
        ...
        try:
            ...
            raise Exception(
                f"Cahvor file '{name}' looks like CAHVORE, but the CAHVORE declaration is unparseable: '{modelname}'"
            )
        ...
        if mtype == 1:
            if cahvore_linearity is not None:
                if cahvore_linearity != 1:
                    raise Exception(
                        f"Cahvor file '{name}' looks like CAHVORE, but has an unexpected linearity defined. mtype=1 so I expected no linearity at all or linearity=1, but got {cahvore_linearity}. CAHVORE declaration: '{modelname}'"
                    )
            cahvore_linearity = 1
        elif mtype == 2:
            if cahvore_linearity is not None:
                if cahvore_linearity != 0:
                    raise Exception(
                        f"Cahvor file '{name}' looks like CAHVORE, but has an unexpected linearity defined. mtype=2 so I expected no linearity at all or linearity=0, but got {cahvore_linearity}. CAHVORE declaration: '{modelname}'"
                    )
            cahvore_linearity = 0
        elif mtype == 3:
            if cahvore_linearity is None:
                raise Exception(
                    f"Cahvor file '{name}' looks like CAHVORE, but has a missing linearity. mtype=3 a linearity parameter MUST be defined. CAHVORE declaration: '{modelname}'"
                )
        else:
            raise Exception(
                f"Cahvor file '{name}' looks like CAHVORE, but has mtype={mtype}. I only know about types 1,2,3. CAHVORE declaration: '{modelname}'"
            )
    else:
        is_cahvore = False
...
def _deconstruct_model(model):
    ...
    if re.match("LENSMODEL_CAHVOR", lensmodel):
        ...
        x["O"] = nps.matmult(
            R_toref,
            nps.transpose(np.array((s_al * c_be, s_be, c_al * c_be), dtype=float)),
        ).ravel()
...
def _write(f, m, note=None):
    ...
    if re.match("LENSMODEL_CAHVOR", lensmodel):
        ...
        raise Exception(
            f"Cannot write lens model '{lensmodel}' to a .cahvor file. I only support PINHOLE, CAHVOR(E) and OPENCV model"
        )
    ...
    if re.match("LENSMODEL_CAHVORE", lensmodel):
        S_size = 21
    elif re.match("LENSMODEL_CAHVOR", lensmodel):
        S_size = 18
    f.write("S =\n" + ((" 0.0" * S_size) + "\n") * S_size)
...
def read_transforms(f):
    ...
    for l in f:
        ...
        m = re.match(
            r"\s*ins2veh\s*=\s*{p}\s*{q}\s*\n?$".format(p=re_pos, q=re_quat), l
        )
        if m:
        ...
        m = re.match(
            r"\s*cam2ins\s*\[({u})\]\s*=\s*{p}\s*{q}\s*\n?$".format(
                u=re_u, p=re_pos, q=re_quat
            ),
            l,
        )
        if m:
        ...
        raise Exception(
            "'transforms.txt': I only know about 'ins2veh' and 'cam2ins' lines. Got '{}'".format(
                l
            )
        )
