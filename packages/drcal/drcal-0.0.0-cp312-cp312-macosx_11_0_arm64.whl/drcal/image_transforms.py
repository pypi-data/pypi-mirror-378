...
A scalar scale_focal that can be passed to pinhole_model_for_reprojection()
    ...
    if type(fit) is np.ndarray:
        ...
        if fit == "corners":
            q_edges = np.array(
                ((0.0, 0.0), (0.0, H - 1.0), (W - 1.0, H - 1.0), (W - 1.0, 0.0))
            )
        elif fit == "centers-horizontal":
            q_edges = np.array(((0, (H - 1.0) / 2.0), (W - 1.0, (H - 1.0) / 2.0)))
    ...
    for p in normxy_edges:
        for ixy in range(2):
            if p[ixy] > 0:
                scale = np.min((scale, normxy_max[ixy] / p[ixy]))
            else:
                scale = np.min((scale, normxy_min[ixy] / p[ixy]))
    return scale


def pinhole_model_for_reprojection(
    model_from, fit=None, *, scale_focal=None, scale_image=None
):
    ...
    if scale_image is not None:
        ...
        kfxy = scale_image
        kcx = (kfxy + 2.0 * cx * kfxy - 1.0) / (2.0 * cx)
        kcy = (kfxy + 2.0 * cy * kfxy - 1.0) / (2.0 * cy)
...
This array can be passed to mrcal.transform_image()
    ...
    if plane_n is not None and intrinsics_only:
        raise Exception(
            "We're looking at remapping a plane (plane_d, plane_n are not None), so intrinsics_only should be False"
        )

    if distance is not None and (plane_n is not None or intrinsics_only):
        raise Exception(
    ...
    if (
        re.match("LENSMODEL_OPENCV", lensmodel_from)
        and lensmodel_to == "LENSMODEL_PINHOLE"
        and plane_n is None
        and not mask_valid_intrinsics_region_from
        ...
        cameraMatrix_from = np.array(
            ((fxy_from[0], 0, cxy_from[0]), (0, fxy_from[1], cxy_from[1]), (0, 0, 1))
        )
        ...
        if Rt_to_from is not None:
            ...
            if np.trace(R_to_from) > 3.0 - 1e-12:
...
def transform_image(
    image, mapxy, *, out=None, borderMode=None, borderValue=0, interpolation=None
):
