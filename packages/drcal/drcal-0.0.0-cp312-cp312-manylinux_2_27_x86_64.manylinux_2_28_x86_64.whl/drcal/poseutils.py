...
def compose_r(*r, get_gradients=False, out=None, inverted0=False, inverted1=False):
    ...
    if len(r) != 2:
        if get_gradients:
            raise Exception(
                "compose_r(..., get_gradients=True) is supported only if exactly 2 inputs are given"
            )
        if inverted0 or inverted1:
            raise Exception(
                "compose_r(..., inverted...=True) is supported only if exactly 2 inputs are given"
            )
...
def compose_rt(*rt, get_gradients=False, out=None, inverted0=False, inverted1=False):
    ...
    if len(rt) != 2:
        if get_gradients:
            raise Exception(
                "compose_rt(..., get_gradients=True) is supported only if exactly 2 inputs are given"
            )
        if inverted0 or inverted1:
            raise Exception(
                "compose_rt(..., inverted...=True) is supported only if exactly 2 inputs are given"
...
def compose_rt_tinyrt0_gradientrt0(rt1, out=None):
    ...
    if out is None:
        ...
        if not out.shape == out_shape:
            raise Exception(
                f"The given 'out' array has shape {out.shape} but rt1 has shape {rt1.shape}, so 'out' should have shape {out_shape}"
            )
        out[:] = 0
...
def compose_rt_tinyrt1_gradientrt1(rt0, out=None):
    ...
    if out is None:
        ...
        if not out.shape == out_shape:
            raise Exception(
                f"The given 'out' array has shape {out.shape} but rt0 has shape {rt0.shape}, so 'out' should have shape {out_shape}"
            )
        out[:] = 0
