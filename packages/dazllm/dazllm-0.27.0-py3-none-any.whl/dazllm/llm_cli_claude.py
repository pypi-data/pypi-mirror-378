...
class LlmClaudeCli(LlmCli):
    ...
    def chat(self, conversation: Conversation, force_json: bool = False) -> CliResponse:
        ...
        try:
            ...
            execution_log = []
            execution_log.append(f"Claude CLI Execution Log")
            execution_log.append(f"Command: claude --debug --print [options] [prompt]")  # Don't log full prompt
            ...
            log_output = "\n".join(execution_log)
            return CliResponse(value=response, output=log_output)

        except subprocess.TimeoutExpired:
            error_msg = "Claude CLI timeout after 120 seconds"
            log_output = f"Claude CLI Execution Log\nCommand: claude --debug --print [options] [prompt]\nError: {error_msg}"
    ...
    def chat_structured(
        ...
        try:
            ...
            try:
                ...
                enhanced_log = []
                enhanced_log.append(f"=== Claude CLI Structured Output ===")
                ...
                log_output = "\n".join(enhanced_log)
                return CliResponse(value=result, output=log_output)

            except Exception as parse_error:
                enhanced_log = []
                enhanced_log.append(f"=== Claude CLI Structured Output FAILED ===")
                enhanced_log.append(f"Schema: {schema.__name__}")
