...
class TestErrorHandling:
    ...
    def test_404_not_found_error(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                config.profile_url, status_code=404, json={"error": "Requested resource not found"}
            )
    ...
    def test_500_server_error(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, status_code=500, json={"error": "Internal Server Error"})
    ...
    def test_connection_timeout_error(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, exc=requests.exceptions.ConnectTimeout)
    ...
    def test_connection_error(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, exc=requests.exceptions.ConnectionError)
    ...
    def test_too_many_redirects_error(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, exc=requests.exceptions.TooManyRedirects)
    ...
    def test_ssl_verification_error(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, exc=requests.exceptions.SSLError)
    ...
    def test_extremely_long_api_key(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, json={"id": "user-123"}, status_code=200)
    ...
    def test_unicode_in_response(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, json=unicode_response, status_code=200)
    ...
    def test_nested_error_response(self, client, config):
        ...
        with requests_mock.Mocker() as m:
            m.get(config.profile_url, json=nested_error, status_code=400)
