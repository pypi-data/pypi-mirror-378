...
class TestLiveAPIIntegration(unittest.TestCase):
    ...
    def test_live_get_ingestion_logs(self):
        ...
        try:
            result = self.client.get_ingestion_logs(space_id=self.test_space_id, limit=10, offset=0)
    ...
    def test_live_get_spaces(self):
        ...
        try:
            ...
            print(
                f"✅ Spaces retrieved: {len(result) if isinstance(result, list) else 'dict format'}"
            )
        except HeySolError as e:
    ...
    def test_live_webhook_operations(self):
        ...
        try:
            ...
            webhook_result = self.client.register_webhook(
                webhook_url, events, "test-webhook-secret"
            )
            ...
            print(
                "⚠️  Skipping webhook details, update, and delete operations (API may not support them)"
            )
