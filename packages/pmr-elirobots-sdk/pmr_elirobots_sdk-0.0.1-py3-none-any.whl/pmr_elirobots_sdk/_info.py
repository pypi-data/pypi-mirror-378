...
class ECInfo(BaseEC):
    ...
    def servo_version(self) -> str:
        ...
        servo_versions = ""
        for i in range(6):
            servo_versions += "轴%i对应伺服版本为%i\n" % (
                i + 1,
                self.send_CMD("getJointVersion", {"axis": i}),
            )
    ...
    def get_tcp_pose(
        ...
        if unit_type is not None:
            ...
            return self.send_CMD(
                "get_tcp_pose", {"coordinate_num": frame_num, "tool_num": tool_num}
            )
    ...
    def _get_tool_coord(
        self, tool_num: int, unit_type: Optional[int] = None
    ) -> List[float]:
        """获取当前的位姿坐标数据
           Deprecated
        Args:
        ...
        if unit_type is not None:
            return self.send_CMD(
                "getTcpPos", {"tool_num": tool_num, "unit_type": unit_type}
            )
        else:
    ...
    def get_user_frame_data(
        self, user_num: int, unit_type: Optional[int] = None
    ) -> List[float]:
        """获取用户坐标系数据
        ...
        if unit_type is not None:
            return self.send_CMD(
                "getUserFrame", {"user_num": user_num, "unit_type": unit_type}
            )
        else:
            return self.send_CMD("getUserFrame", {"user_num": user_num})

    def set_user_frame_data(
        self, user_num: int, frame_value: List[float], unit_type: Optional[int] = None
    ) -> bool:
        """设置用户坐标系的数据
        ...
        if unit_type is not None:
            ...
            return self.send_CMD(
                "setUserFrame", {"user_num": user_num, "user_frame": frame_value}
            )
    ...
    def get_flange_pose_in_user_frame(
    ...
    def set_payload(self, tool_num: int, mass: float, barycenter: list) -> bool:
        ...
        return self.send_CMD(
            "cmd_set_payload", {"tool_num": tool_num, "m": mass, "cog": barycenter}
        )
    ...
    def set_safety_parameters(
        ...
        collision_sensitivity: Optional[int] = None,
    ) -> bool:
        """设置安全参数
    ...
    def blue_tool_btn_func(self) -> BaseEC.ToolBtnFunc:
        ...
        return self.ToolBtnFunc(
            self.send_CMD(
                "checkFlangeButton", {"button_num": self.ToolBtn.BLUE_BTN.value}
            )
        )
    ...
    def green_tool_btn_func(self) -> BaseEC.ToolBtnFunc:
        ...
        return self.ToolBtnFunc(
            self.send_CMD(
                "checkFlangeButton", {"button_num": self.ToolBtn.GREEN_BTN.value}
            )
        )
    ...
    def blue_tool_btn_func(self, func: int):
        ...
        """
        self.send_CMD(
            "setFlangeButton", {"button_num": self.ToolBtn.BLUE_BTN, "state": func}
        )
