...
class AsyncMCPAdmin:
    ...
    async def create_mcp(
        ...
        with open(zip_file_path, "rb") as zip_file:
            form_data.add_field("zip_file", zip_file, filename=zip_filename, content_type="application/zip")

        async with self.client.session.post(
            url, headers=headers, params=params, data=form_data
        ) as response:
            result = await self.client._handle_response(
                response=response, error_callable=raise_for_mcp_detail
            )
    ...
    async def update_mcp(
        ...
        async with self.client.session.put(
            url, headers=headers, params=params, json=mcp_update.model_dump(exclude_none=True)
        ) as response:
            result = await self.client._handle_response(
                response=response, error_callable=raise_for_mcp_detail
            )
