...
def run_command(
    ...
    try:
        ...
        if execution_mode == OutputMode.BATCH:
            # Batch mode: wait for completion, then return all output
            output_bytes, _ = process.communicate()
            output = output_bytes.decode("utf-8", errors="replace").rstrip()
            returncode = process.returncode
        else:
            # Streaming mode: process output line by line with ANSI buffering
            output_lines = []
            ...
            if process.stdout:  # Type guard to satisfy mypy
                while True:
                    ...
                    if line:
                        ...
                        if stream_callback:
                            processed_line = ansi_buffer.process_line(
                                decoded_line
                            )
