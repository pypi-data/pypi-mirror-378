"""ELF imported symbols dissector"""
...
def _dissect_impl(ctx: DissectionContext) -> RecordIterator:
    ...
    for symbol in elf.imported_symbols:
        yield {
            'elf_sha256': digest,
            'elf_isym_name': symbol.demangled_name,
            'elf_isym_type': str(symbol.type).rsplit('.', 1)[-1],
        }
...
DISSECTOR = Dissector(
    ...
    columns=[
        ...
        Column('elf_isym_name', DataType.STR),
        Column('elf_isym_type', DataType.STR),
    ],
    description="ELF binary imported symbols",
