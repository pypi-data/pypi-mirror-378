...
from ..exceptions import NetworkError, RubkaWebAPIException
...
class WebSocketClient:
    ...
    async def connect(self):
        try:
            ...
            await self.on_error(RubkaWebAPIException(f"An unexpected error occurred during WebSocket connection: {exc}") from exc)
    ...
    async def _run(self):
        try:
            ...
            await self.on_error(RubkaWebAPIException(f"Failed to decode WebSocket message: {exc}") from exc)
        except Exception as exc:
            await self.on_error(RubkaWebAPIException(f"An unexpected error occurred in WebSocket loop: {exc}") from exc)
        finally:
            await self.on_close()
