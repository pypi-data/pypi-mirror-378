...
class WorkerAgent(BaseAgent):
    ...
    async def _generate_final_response(
        ...
        try:
            ...
            # Check if we should use OpenAI SDK / compatible provider
            provider_name = (self.config.provider or "").lower()
            llm_base_url = getattr(self.config, "llm_api_base_url", None)
            agent_base_url = getattr(self.config, "base_url", None)
            use_openai = bool(
                llm_base_url or
                agent_base_url or
                ("openai" in provider_name) or
                ("openrouter" in provider_name)
            )
            ...
            try:
                ...
                generator = OpenAIResponseGenerator(
                    ...
                    model=self.config.model,
                    base_url=llm_base_url or agent_base_url
                )
