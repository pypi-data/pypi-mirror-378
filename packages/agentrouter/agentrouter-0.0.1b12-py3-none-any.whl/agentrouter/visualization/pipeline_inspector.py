...
class PipelineInspector:
    ...
    def visualize(
        ...
        Args:
            format: Visualization format ('console', 'mermaid', 'json', 'png')
            output: Optional file path to save output
            
        Returns:
            Visualization output (str for text formats, base64 string for PNG)
        """
        if format == 'console':
            ...
            png_bytes = visualizer.generate_mermaid_png(
            ...
            try:
                ...
                # Pillow is optional; skip preview when unavailable or display fails
                pass
    ...
    def _generate_mermaid_diagram(self) -> str:
        ...
        def get_node_id(raw_id: str) -> str:
            if raw_id not in node_id_map:
                node_id_map[raw_id] = sanitize(raw_id)
            return node_id_map[raw_id]

        # Add nodes
        for agent in self.pipeline_data['agents']:
            agent_id = get_node_id(agent['id'])
            label = f"{agent['name']}<br/>Type: {agent['type'].title()}<br/>Model: {agent['model']}"
        ...
        for tool in self.pipeline_data['tools']:
            tool_raw_id = f"tool_{tool['name']}"
            tool_id = get_node_id(tool_raw_id)
            if tool_id not in tool_ids:
                tool_ids.add(tool_id)
                lines.append(f"    {tool_id}[{tool['name']}<br/>Type: Tool]:::tool")

        # Add relationships
        for rel in self.pipeline_data['relationships']:
            from_id = get_node_id(rel['from'])
            to_id = get_node_id(rel['to'])
