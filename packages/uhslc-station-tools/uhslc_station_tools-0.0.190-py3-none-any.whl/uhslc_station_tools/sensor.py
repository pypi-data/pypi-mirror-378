...
class Station:
    ...
    def save_ts_files(self, text_collection, path=None, is_test_mode=False, callback: Callable = None):
        ...
        Args:
            ...
            path: Optional parent directory for saving. If ``None``, defaults are
                resolved by :mod:`uhslc_station_tools.utils`.
            is_test_mode: If ``True``, write to a sandbox/test location.
            callback: Optional function ``callback(success, failure)`` invoked
                after writing.
    ...
    def save_mat_high_fq(self, path: str, is_test_mode=False, callback: Callable = None):
        ...
        One file per sensor per month is written in the UHSLC convention
        expected by legacy tooling.
