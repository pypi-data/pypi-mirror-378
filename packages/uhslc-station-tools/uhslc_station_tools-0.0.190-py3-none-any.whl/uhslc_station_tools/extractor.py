...
def resolve_century(two_digit_year: int, anchor_year: Optional[int] = None) -> int:
    """
    Map a 2-digit year (00-99) to a 4-digit year assuming 1900-present.

    Rules:
    * If ``anchor_year`` is provided and ``<= 1935``, force the 1900s.
    * Otherwise, map 00..ceil -> 2000s where ``ceil = min(35, currentYY + 5)``,
    and everything else -> 1900s.

    This eliminates the classic 1950-2049 pivot bug (e.g., 49 -> 2049) while keeping
    modern data correct and future-proof by ~5 years.

    Args:
        two_digit_year: Year in the range 0-99.
        anchor_year: Optional hint from headers/metadata. If early (<=1935),
                     the function returns a year in the 1900s.

    Returns:
        The resolved 4-digit year.
    """

    # Honor an early-1900s hint from headers/metadata.
    if anchor_year is not None and anchor_year <= 1935:
        return 1900 + two_digit_year

    current_yy = date.today().year % 100
    ceiling = min(35, current_yy + 5)  # never past 2035, ~5-year headroom

    if 0 <= two_digit_year <= ceiling:
        return 2000 + two_digit_year

    return 1900 + two_digit_year

def determine_year(data_yy: str, anchor_year: Optional[int] = None) -> int:
    """
    Convert a 2-digit year string to a 4-digit year.

    This is a thin wrapper around :func:`resolve_century` that accepts a string
    and preserves backwards compatibility with previous callers.

    Args:
        data_yy: Two-digit year string (e.g., ``"03"`` for 2003).
        anchor_year: Optional hint from headers/metadata used by
        :func:`resolve_century`.

    Returns:
        The resolved 4-digit year.
    """

    return resolve_century(int(data_yy), anchor_year)
...
class DataExtractor(Month):
    ...
    Attributes:
        ...
        year: 4-digit year resolved from filename and header anchors.
    ...
    def __init__(self, filename):
        ...
        # Discover an anchor year from init_dates or headers (enables 1800s)
        anchor_year = None
        try:
            if getattr(self, "init_dates", None):
                for d in self.init_dates:
                    m = re.search(r"\b(18|19|20)\d{2}\b", str(d))
                    if m:
                        anchor_year = int(m.group(0))
                        break
        except Exception:
            pass

        # Extract the anchor year from the data file header if none provided.
        if anchor_year is None:
            try:
                if getattr(self, "headers", None):
                    m = re.search(r"\b(18|19|20)\d{2}\b", "\n".join(self.headers))
                    if m:
                        anchor_year = int(m.group(0))
            except Exception:
                pass

        # Resolve the 4-digit year using the (possibly None) anchor.
        self.year = determine_year(data_yy, anchor_year)
    ...
    def parse_file(self, filename):
        ...
        for sensor in range(len(self.sensor_ids)):
            ...
            try:
                ...
                anchor_from_headers = None
                try:
                    m = re.search(r"\b(18|19|20)\d{2}\b", "\n".join(self.headers))
                    if m:
                        anchor_from_headers = int(m.group(0))
                except Exception:
                    pass

                if len(year) == 2:
                    year = str(determine_year(year, anchor_year=anchor_from_headers))
                if len(year) == 1:
                    year = "200" + year
                ...
                if base.endswith(".dat"):
                    ...
                    yyyy = determine_year(yy, anchor_year=anchor_from_headers)
