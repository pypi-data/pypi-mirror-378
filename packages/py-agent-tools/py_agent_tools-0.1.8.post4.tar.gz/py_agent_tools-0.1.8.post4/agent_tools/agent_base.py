    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # 检查是否是postprocess_fn的异常
            if "postprocess_fn" in str(e) or "大模型提取" in str(e):
                # postprocess_fn异常，记录重试信息但不重试
                from agent_tools._log import log

                if hasattr(self, 'max_retries') and self.max_retries > 0:
                    current_retry = getattr(self, '_current_retry', 0)
                    total_retries = getattr(self, '_total_retries', 0)
                    log.warning(
                        f"🔄 后处理重试失败记录 - Agent: {self.__class__.__name__}, "
                    ...
                    log.error(f"后处理重试次数已用完，最终失败: {type(e).__name__}: {e}")
                # 直接重新抛出，不进行重试
                raise
