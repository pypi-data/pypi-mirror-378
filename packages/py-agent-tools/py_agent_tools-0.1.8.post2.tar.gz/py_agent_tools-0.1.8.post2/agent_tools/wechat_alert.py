    ...
    def format_exception_message(self, exception_info: ExceptionInfo) -> str:
        ...
        message_parts.extend(
            [
                ...
                f"• 消息: {exception_info.exception_message[:80]}{'...' if len(exception_info.exception_message) > 80 else ''}",  # noqa
            ]
        )
            ...
            try:
                ...
                if agent_instance and hasattr(agent_instance, 'max_retries'):
                    if agent_instance.max_retries > 0:
                        should_alert = False
                        log.info(
                            f"检测到 pydantic_ai 异常但还有重试次数({agent_instance.max_retries})，暂不发送预警"
                        )
                    else:
                        log.info(f"重试次数已用完，发送最终失败预警: {type(e).__name__}: {e}")
                ...
                if should_alert and agent_instance and hasattr(agent_instance, 'max_retries'):
                    if agent_instance.max_retries > 0:
                        should_alert = False
                        log.info(
                            f"检测到异常 {type(e).__name__} 但还有重试次数({agent_instance.max_retries})，暂不发送预警"
                        )
                    else:
                        log.info(f"重试次数已用完，发送最终失败预警: {type(e).__name__}: {e}")
