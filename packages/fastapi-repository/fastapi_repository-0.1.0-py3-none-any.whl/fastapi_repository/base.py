...
class BaseRepository:
    ...
    async def sum(
        ...
        **search_params,
        ...
        - Considers default_scope and search_params
        - Target is a numeric column on self.model (same table)
        - Returns 0 if no records exist or the result is NULL
        Example:
            await repo.sum("amount_cents", status="paid")
            await repo.sum("points", created_at__gte=since)
        """
        # Resolve column
        target_col = getattr(self.model, column, None)
        if target_col is None:
            raise AttributeError(f"{self.model.__name__} has no attribute '{column}'")
        ...
        # Use COALESCE to handle NULL (ensure result is 0 instead of None)
        query = (
            select(func.coalesce(func.sum(target_col), 0))
    ...
    def _apply_order_by(self, query, sorted_by: str, sorted_order: str):
        """
        Apply ORDER BY.
        - Column directly on the model: "email"
        - Relationship path: specify like "profile__company__name"
          When a path is specified, automatically add the necessary OUTER JOINs before sorting.
        """
        # No path specified: existing behavior (column directly on the model)
        ...
        *rel_parts, col_name = parts
        ...
        # Lightly check that the target is not itself a relationship (it should be a scalar column)
        ...
        # Add OUTER JOINs for the path (preserve rows even for one-to-many, etc.)
        for a in attrs:
    ...
    async def __get_conditions(self, **search_params):
        ...
        for key, value in search_params.items():
            if "__or__" in key:
                subkeys = [k for k in key.split("__or__") if k]
                if not subkeys:
                    continue
                subconds = [self._condition_from_key(k, value) for k in subkeys]
    ...
    def _condition_from_key(self, key: str, value):
        if "__" in key:
            parts = key.split("__")
            op = "exact"
            ...
            # 1ホップのリレーション: rel__field__op
            rel_attr = getattr(self.model, parts[0], None)
            if rel_attr is None or not hasattr(rel_attr, "property"):
                raise AttributeError(f"{self.model.__name__} has no relationship '{parts[0]}'")
            target_cls = rel_attr.property.mapper.class_
            target_col = getattr(target_cls, parts[1], None)
            if target_col is None:
                raise AttributeError(f"{target_cls.__name__} has no attribute '{parts[1]}'")

            pred = OPERATORS[op](target_col, value)
            # any()/has() を関係の種類で自動切替
            return rel_attr.any(pred) if getattr(rel_attr.property, "uselist", False) else rel_attr.has(pred)
        else:
            column = getattr(self.model, key, None)
            if column is None:
