...
class BulkModelViewSet:
    ...
    def _handle_array_upsert(self, request, unique_fields, update_fields, data_list):
        ...
        logger.info("=== ARRAY UPSERT STARTED ===")
        logger.info(f"Processing array with {len(data_list)} items")
        ...
        logger.info("Dispatching Celery task...")
        ...
        try:
            ...
            logger.info("Waiting for task completion (300s timeout)...")
            # Wait for the task to complete (synchronous behavior)
            task_result = task.get(
                timeout=300
            ...
            if all_ids:
                if prefer_minimal:
                    ...
                    logger.info("=== ARRAY UPSERT COMPLETED (MINIMAL) ===")
                    ...
                    logger.info(f"  - Response preparation: minimal")
                ...
                logger.info("=== ARRAY UPSERT COMPLETED ===")
                logger.info(f"TOTAL TIME: {total_time:.4f}s")
                logger.info("TIMING BREAKDOWN:")
                logger.info(
                    f"  - Celery dispatch: {celery_dispatch_time:.4f}s ({celery_dispatch_time / total_time * 100:.1f}%)"
    ...
    def _handle_array_upsert_direct(
        ...
        for batch_start in range(0, len(data_list), batch_size):
            ...
            for index, item_data in enumerate(batch_data):
                try:
                    # Validate using the project's serializer to ensure proper FK handling
                    serializer = serializer_class(data=item_data, context=base_context)
            ...
            try:
                ...
                try:
                    ...
                    if force_fallback or bulk_error:
                        logger.warning(
                            f"bulk_create with update_conflicts failed: {bulk_error}"
                        )
                        logger.info(
                    ...
                    if len(instances_to_upsert) >= timing_threshold:
                        logger.info(f"Database bulk_create completed in {db_time:.4f}s")

                    bulk_create_time = time.time() - bulk_operation_start
                    if len(instances_to_upsert) >= timing_threshold:
                        logger.info(f"Bulk upsert completed in {bulk_create_time:.4f}s")
                    ...
                    if len(instances_to_upsert) >= timing_threshold:
                        logger.info(f"Serialization completed in {serialize_time:.4f}s")

                    # Log comprehensive timing summary
                    if len(data_list) >= timing_threshold:
                        total_operation_time = time.time() - operation_start
                        logger.info("=== DIRECT PROCESSING TIMING SUMMARY ===")
                        logger.info(
                        ...
                        logger.info(f"Database operation time: {db_time:.4f}s")
                        logger.info(f"Serialization time: {serialize_time:.4f}s")
                        logger.info(f"Bulk upsert time: {bulk_create_time:.4f}s")
                        logger.info(f"Items processed: {len(data_list)}")
                        logger.info(
                    ...
                    if len(instances_to_upsert) >= timing_threshold:
                        logger.info(f"Fallback upsert completed in {total_time:.4f}s")
                    ...
                    if len(data_list) >= timing_threshold:
                        ...
                        logger.info(f"Fallback upsert time: {total_time:.4f}s")
                        logger.info(f"Items processed: {len(data_list)}")
                        logger.info(f"Instances created: {len(instances_to_create)}")
                        logger.info(f"Instances updated: {len(instances_to_update)}")
                        logger.info("=================================================")
