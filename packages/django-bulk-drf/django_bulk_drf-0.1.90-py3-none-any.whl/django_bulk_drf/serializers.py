...
class BulkModelSerializer(serializers.ModelSerializer):
    ...
    # Internal helper to fetch Meta options with fallback to base defaults
    def _get_meta_option(self, option_name: str, default):
        meta = getattr(self, "Meta", None)
        if meta and hasattr(meta, option_name):
            return getattr(meta, option_name)
        # Fallback to base class Meta defaults
        base_meta = getattr(BulkModelSerializer, "Meta", None)
        if base_meta and hasattr(base_meta, option_name):
            return getattr(base_meta, option_name)
        return default
    ...
    def get_fields(self):
        ...
        if not self._get_meta_option("mutate_fk_fields", False):
            return fields
        ...
        remove_fk = self._get_meta_option("remove_fk_relations", False)
    ...
    def to_representation(self, instance):
        ...
        if not self._get_meta_option("include_fk_ids", True):
            return data
        ...
        hide_output = self._get_meta_option("hide_fk_relations_output", False)
        if hide_output:
