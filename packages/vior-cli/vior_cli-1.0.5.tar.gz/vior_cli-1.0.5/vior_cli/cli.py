...
@click.group()
@click.version_option(version="1.0.5", prog_name="vior-cli", message="%(prog)s/%(version)s Python")
def cli():
    pass
...
@cli.command(name='sdk-verify-auth')
def sdk_verify_auth():
    status = config_manager.get_status()
...
def info():
    ...
    click.echo("  sdk-verify-auth  - Verify authentication for SDK")
