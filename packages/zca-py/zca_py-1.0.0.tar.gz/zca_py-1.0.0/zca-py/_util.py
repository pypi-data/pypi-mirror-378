...
def encodeAES(params, key, t=0):
	try:
		key = base64.b64decode(key)
		iv = bytes.fromhex("00000000000000000000000000000000")
		cipher = AES.new(key, AES.MODE_CBC, iv)
		if isinstance(params, (bytes, bytearray)):
			plaintext = bytes(params)
		elif isinstance(params, str):
			plaintext = params.encode()
		else:
		...
		return encodeAES(params, key, t + 1) if t < 3 else None
		
		
def decodeAES(params, key, t=0):
	try:
		...
		return decodeAES(params, key, t + 1) if t < 3 else None

# decodeEventData
def decodeZWS(parsed, key):
	payload = parsed.get("data")
	encrypt_type = parsed.get("encrypt")
	if payload is None or encrypt_type is None:
		return
	
	try:
		# encrypt_type 0: raw JSON
		if encrypt_type == 0:
			return json.loads(payload)
		
		# encrypt_type 1: Base64 + zlib(JSON) (pako.inflate)
		if encrypt_type == 1:
			decoded_bytes = base64.b64decode(payload)
			decompressed_bytes = zlib.decompress(decoded_bytes)
			decoded_data = decompressed_bytes.decode("utf-8")
			return json.loads(decoded_data)
		
		# encrypt_type 2 or 3: URL-decoded Base64 → AES-GCM(AAD)
		if encrypt_type in (2, 3):
			if not key:
				raise _exception.DecodePayloadError("Invalid data length or missing cipher key")
			...
			if len(data_source) < 16:
				raise _exception.DecodePayloadError("Invalid ciphertext/tag length")
			ciphertext = data_source[:-16]
			tag = data_source[-16:]
			...
			if encrypt_type == 2:
				# pako.inflate equivalent (zlib stream)
				decompressed_bytes = zlib.decompress(plaintext_bytes)
				decoded_data = decompressed_bytes.decode("utf-8")
			else:
				# encrypt_type 3: no decompression
				decoded_data = plaintext_bytes.decode("utf-8")
			
			return json.loads(decoded_data)
		
		# unsupported encrypt_type
		return
