...
async def generate_video_with_progress(
    ...
    try:
        ...
        if image_path and os.path.exists(image_path):
            ...
            logger.info(f"[{request_id}] Image encoded successfully")
            logger.info(f"[{request_id}] - MIME type: {mime_type}")
            logger.info(f"[{request_id}] - File size: {len(image_data)} bytes")
            logger.info(f"[{request_id}] - Base64 length: {len(image_base64)} chars")
