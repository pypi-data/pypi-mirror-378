...
async def generate_video_with_progress(
    ...
    try:
        ...
        if image_path and os.path.exists(image_path):
            ...
            # Upload image file to Gemini Files API first
            logger.info(f"[{request_id}] Uploading image to Gemini Files API")
            uploaded_file = gemini_client.files.upload(file=image_path)
            logger.info(f"[{request_id}] Image uploaded, file URI: {uploaded_file.uri}")
            
            # For image-to-video, we need to pass the uploaded file
            logger.info(f"[{request_id}] Calling Gemini API for image-to-video generation")
            logger.info(f"[{request_id}] API Request - Model: {model}, Prompt: {prompt}, Image: {uploaded_file.uri}")
