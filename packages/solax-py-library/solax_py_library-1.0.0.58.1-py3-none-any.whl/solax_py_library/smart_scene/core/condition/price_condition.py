...
class ElePriceCondition(BaseCondition):
    ...
    def meet_func_price(self, function_value: ConditionFunc, data_value, current_data) -> bool:
        """电价条件的判定"""
    ...
    def meet_func_highest_or_lowest_hours(self, data_value, current_timestamp, reverse) -> bool:
        start_time = data_value[0]
        end_time = data_value[1]
        hours = data_value[2]
        price = self.value["price"]
        start_timestamp = hour_to_today_timestamp(start_time)
        end_timestamp = hour_to_today_timestamp(end_time)
        ret = []
        for timestamp_key, price_value in price.items():
            if start_timestamp > int(timestamp_key) or int(timestamp_key) > end_timestamp:
                continue
        ...
        if current_timestamp in top_sorted_price:
            return True
        return False
