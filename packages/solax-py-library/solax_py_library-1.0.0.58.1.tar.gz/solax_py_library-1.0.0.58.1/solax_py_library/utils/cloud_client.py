...
class CloudClient:
    ...
    def get_electrovalence_data_from_cloud(self, ems_sn, token):
        try:
            ...
            for detail_info in chain(response_data["result"]["list"], response_data["result"].get("tomorrow", [])):
                start_timestamp = detail_info["startTimeStamp"]
                end_timestamp = detail_info["endTimeStamp"]
                # 处理欧分的情况
                buy_price = round(detail_info["buyPrice"], 5) if detail_info["buyPrice"] else None
                sale_price = round(detail_info["salePrice"], 5) if detail_info["salePrice"] else None
