...
def get_left_timestamp(now_time, minute=15):
    return now_time // (60 * minute) * (60 * minute)
...
def hour_to_today_timestamp(hour: int):
    today = datetime.now()
    today_zero = datetime(today.year, today.month, today.day, tzinfo=today.astimezone().tzinfo)
    # 加上目标小时
    target = today_zero + timedelta(hours=hour)
