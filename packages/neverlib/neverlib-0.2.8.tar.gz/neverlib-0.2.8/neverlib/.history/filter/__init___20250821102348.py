...
def __getattr__(name):
    ...
    if name in ['compute_frequency_eq']:
        ...
        try:
            from .auto_eq.freq_eq import compute_frequency_eq
        ...
        # 差分进化优化方法
        try:
            from .auto_eq.de_eq import get_filter_function, match_frequency_response, plot_spectra_comparison
        ...
        # 需要 deap 库的遗传算法基础版
        try:
            from .auto_eq.ga_eq_basic import (
        ...
        # 需要 deap 库的遗传算法高级版
        try:
            from .auto_eq.ga_eq_advanced import EQOptimizer
            return EQOptimizer
        except ImportError as e:
            raise ImportError(f"使用 {name} 需要安装 deap: pip install deap") from e
