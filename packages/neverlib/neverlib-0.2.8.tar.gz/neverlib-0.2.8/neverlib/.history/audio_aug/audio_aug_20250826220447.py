...
def snr_aug_changeNoise_dB(clean, noise, target_snr):
    """ 
    保持语音不变, 改变噪声的幅度
    snr = 10 * log10(signal_power / k*noise_power) 
    """
...
def snr_aug_changeClean(clean, noise, target_snr, clip_check=True):
    """ 
    ...
    # 计算纯净信号需要的幅度因子
    clean_scale = np.sqrt(noise_power * 10 ** (target_snr / 10) / (clean_power + 1e-8))
    clean_amp = clean * clean_scale
    noisy = clean_amp + noise
    # 防止削波
    if clip_check:
        if np.max(np.abs(noisy)) > 1:
            scale_factor = 1 / np.max(np.abs(noisy))
            noisy *= scale_factor
            clean_amp *= scale_factor 
    return noisy, clean_amp


def snr_aug_changeClean_dB(clean, noise, target_snr):
    """ 
    保持噪声不变, 改变纯净语音的幅度
    snr = 10 * log10(signal_power / k*noise_power) 
    """
...
def snr_aug_Interpolation(clean, noise, snr):
    ...
    noisy_tmp, clean_tmp = snr_aug_changeClean(clean, noise, snr, clip_check=False)
