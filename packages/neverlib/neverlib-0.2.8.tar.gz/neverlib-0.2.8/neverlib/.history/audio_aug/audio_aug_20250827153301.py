...
def volume_aug_dbrms(wav, dB_range, hpf=False, sr=16000, order=4, cutoff=100):
    wav_tmp=wav.copy()
    wav_rms = (wav_tmp ** 2).mean() ** 0.5
    target_level = random.uniform(dB_range[0], dB_range[1])
    scalar = 10 ** (target_level / 20) / (np.max(wav_rms) + EPS)
    wav_tmp *= scalar
    return wav_tmp
...
def volume_aug_linmax(wav, range):
    pass 
