...
def snr_aug_changeNoise_dB(clean, noise, target_snr, hp=False, sr=16000, order=4, cutoff=100):
    """ 
    保持语音不变, 改变噪声的幅度
    snr = 10 * log10(signal_power / k*noise_power) 
    """
...
def snr_aug_changeClean_dB(clean, noise, target_snr, clip_check=True, hp=False, sr=16000, order=4, cutoff=100):
    """ 
    保持噪声不变, 改变纯净语音的幅度
    snr = 10 * log10(signal_power / k*noise_power) 
    """
