...
def get_snr_vad(wav):
    ...
    wav = HPFilter(wav[:, 1], sr=sr, order=6, cutoff=100)
    wav = volume_norm(wav)
    wav_name = os.path.basename(wav_path)[:-4]
    wav_dir = os.path.dirname(wav_path)
    match = re.search(r"_vadstart(\d+)_vadend(\d+)", wav_name)
    vadstart, vadend = int(match.group(1)), int(match.group(2))
    ...
    snr = round(snr, 1)
    # 如果snr为nan
    assert not np.isnan(snr), f"snr为nan: {wav_path}"

    if (str(snr) in wav_name):
        return
    elif "snr" in wav_name:
        # -----(snr)11111.wav改名: 使用正则表达式进行替换
        new_filename = re.sub(r'(snr)[-+]?\d+(\.\d+)?', fr'\g<1>{snr}', wav_name)
        new_wav_path = os.path.join(wav_dir, f"{new_filename}.wav")
    else:
        # -----xxx.wav加snr
        new_wav_path = os.path.join(wav_dir, f"{wav_name}_snr{snr}.wav")
