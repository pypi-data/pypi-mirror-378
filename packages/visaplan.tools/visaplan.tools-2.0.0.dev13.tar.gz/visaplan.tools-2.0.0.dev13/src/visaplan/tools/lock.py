...
"""  # -------------------------------------------------- ] module docstring ]
# - removed version check for ls.lockfile
# TODO:
...
try:
    parse_version(pkg_version('zc.lockfile'))
...
__all__ = [
    ...
    'ConvenientLockOptions',  # namedtuple for internal use
...
DEFAULTS_KW = {
    ...
    'sep': ';',
    # for testability:
    # '_zclf_v': None,  # removed;
    #                     we don't check the zc.lockfile version anymore
...
ConvenientLockOptions = namedtuple('ConvenientLockOptions',
                             ['name',
...
def check_lockfile_options(name=None, *,  # -------------- [ skip docstring: [
    ...
    ConvenientLockOptions(name='.LOCK', mkdir=False,
                    tries=1,
                    delay=None,
                    autodelete=True,
                    logger=None,
    ...
    ConvenientLockOptions(name='my.lock', mkdir=False,
                    tries=1,
                    delay=None,
                    autodelete=True,
                    logger=None,
    ...
    ConvenientLockOptions(name='some/dir/.LOCK', mkdir=True,
                    tries=1,
                    delay=None,
                    autodelete=True,
                    logger=None,
    ...
    ConvenientLockOptions(name='some/dir/your.lock', mkdir=True,
                    tries=1,
                    delay=None,
                    autodelete=True,
                    logger=None,
    ...
    ConvenientLockOptions(name='my.lock',
                    mkdir=False,
                    tries=1,
                    delay=None,
                    autodelete=True,
    ...
    ConvenientLockOptions(... zclf_kw={'content_template': '{pid};{hostname}'}, ...)
    ...
    ...                                       # doctest: +NORMALIZE_WHITESPACE
    ConvenientLockOptions(name='.LOCK',
                    mkdir=False,
                    tries=3,
                    delay=0.5,
                    autodelete=False,
    ...
    return ConvenientLockOptions(name, mkdir,
                    tries, delay,
                    autodelete,
                    logger, verbose,
                    lockfile_kwargs(**lkw),
...
def lockfile_kwargs(**kwargs):  # ------------------------ [ skip docstring: [
    """
    ...
    >>> def lfkw_recent(**kw):
    ...     return lockfile_kwargs(**kw)
    >>> lfkw_recent(add_hostname=1)
    {'content_template': '{pid};{hostname}'}

    We don't expect pre-1.2.0 versions of zc.lockfile anymore
    since we require Python 3.6+ which zc.lockfile 1.4 was the first to
    support.
    ...
    >>> lfkw_recent(add_hostname=1, content_template='{pid}')
    Traceback (most recent call last):
      ...
    TypeError: With content_template given, no other arguments are allowed!
