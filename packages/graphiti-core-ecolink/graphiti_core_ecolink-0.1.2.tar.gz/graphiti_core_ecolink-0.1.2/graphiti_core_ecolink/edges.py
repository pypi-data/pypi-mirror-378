...
class Edge(BaseModel, ABC):
    ...
    agent_id: str | None = Field(default=None, description='agent identifier for this edge')
...
class EntityEdge(Edge):
    ...
    agent_id: str | None = Field(
        default=None, description='agent identifier for this edge'
    )
    ...
    async def save(self, driver: GraphDriver):
        edge_data: dict[str, Any] = {
            ...
            'agent_id': self.agent_id,  # 添加 agent_id 字段
...
def get_entity_edge_from_record(record: Any) -> EntityEdge:
    edge = EntityEdge(
        ...
        name=record['name'],
        group_id=record['group_id'],
        agent_id=record.get('agent_id'),  # 添加 agent_id 字段
