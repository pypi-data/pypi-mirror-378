...
# from tianxiadatong_qq_robot.app.log import local_logger
logging.basicConfig(
    level=logging.INFO,
...
#     encrypted = files("tianxiadatong_qq_robot").joinpath("resources.encrypted").read_bytes()
#     decrypted_zip = fernet.decrypt(encrypted)
#
#     # 2. 固定临时目录
#     temp_dir = Path(os.path.expandvars(r"%LOCALAPPDATA%\Temp\napcat"))
...
def start_fastapi(host,port):
    ...
    # 启动Uvicorn服务
    uvicorn.run(
        "tianxiadatong_qq_robot.main:app",
        host=host,
        port=port,
        reload=False,
        workers=1,
