...
def main() -> int:
    ...
    # Create logger
    logger = CLILogger.create_cli_logger(
        verbose=context.verbose,
        log_file="logs/cli.log" if Path("logs").exists() else None
    )
    ...
    try:
        ...
        if result.success:
            logger.command_end(args.command, success=True)
            output.print_result(result)
            return 0
        else:
            logger.command_end(args.command, success=False)
            output.print_result(result)
            return 1
        ...
        logger.command_end(args.command or "unknown", success=False)
        output.print_result(result)
        return 1
