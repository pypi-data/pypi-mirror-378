...
class Client(Singleton):
    ...
    def _build_direct_clients(self):
        for node in self.nodes:
            if node['connected'] and node['status']['minio'] and node['id'] not in self.direct_nodes_clients:
    ...
    def get_available_direct_client(self):
        if len(self.direct_nodes_clients) > 0:
            node_id = random.sample(list(self.direct_nodes_clients.keys()), 1)[0]
            return node_id, self.direct_nodes_clients[node_id]
        return None, None
    ...
    def _file_format(self, node_id, bucket, bucket_name):
        return '{}:{}/{}'.format(node_id, bucket, bucket_name)
    ...
    async def direct_put(self, path, bucket, bucket_name):
        node_id, client = self.get_available_direct_client()
        client: MinioAsync = self.direct_nodes_clients.get(node_id, None)
        if not node_id:
            return False, '没有可用的Direct Node'
        print(f'direct putting file -> {path} to {bucket}/{bucket_name} on node {node_id}...')
        ret, e = await client.upload_file(path, bucket, bucket_name, max_tries=2)
        if ret:
            file_url = self._file_format(node_id, bucket, bucket_name)
            return ret, file_url
        else:
            return False, e
    ...
    async def put_async(self, path, bucket, bucket_name, timeout=60, file_ttl=-1):
        async with self.sem:
            ...
            if ret:
                ret, e = await self.direct_put(path, bucket, bucket_name)
                if not ret:
