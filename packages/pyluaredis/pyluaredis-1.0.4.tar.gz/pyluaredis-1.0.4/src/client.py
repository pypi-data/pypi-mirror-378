...
class PyRedis:
    ...
    # Добавление и убавление 1 от числа или элемента множества/списка (если его удалось проверить, что это число)

    # Сортировка списка в Redis (ASC/DESC)

    # Получение значения из списка Redis по индексу

    # Получение и удаление первого/последнего элемента из списка/множества Redis
    # (первого и последнего работает только для списков, из множества берется любой)

    # Получение индекса списка Redis по значению

    # Добавить запись и конвертацию битовых строк и битовых массивов

    # Запись ключ-значение через словарь

    # Запись словаря по ключу (?)

    # expiretime (?)

    def r_get(self, key: str, default_value=None, convert_to_type: str | None = None):  # TODO - параметр sort для списков
        """
...
if __name__ == '__main__':
    # r = PyRedis(
    #     host='45.153.188.79', password='YrGtJoBpkUe4xhYNBNy8ouONU1URqFxT2gSv', port=6333
    # )
    # r.r_set('1', [1, 2, 3])
    # print(r.r_get('1'))
    # print(r.append_value_to_array('1', '4', top_of_list=True, get_old_value=True))
    # print(r.r_get('1'))
    # a: dict = {1: '1'}
    # print(a.get(2))

    def q_test_abc():
        return q_test_abc.__name__

    print(type(q_test_abc()))

    # memory_purge - ?
    # slowlog_get - запись в наши логи - ? | slowlog_len для записи последних логов после ошибки
    # time - ?
    # memory_stats - ?
    # object - получить время "простоя" для ключа - ?
