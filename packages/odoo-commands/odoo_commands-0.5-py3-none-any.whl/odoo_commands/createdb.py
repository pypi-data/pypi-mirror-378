...
def create_database_command(database: str, project_path='.', config=None, modules=None, populate=True):
    path = Path(project_path).resolve()

    if not modules:
        if not (path / 'odoo-project.py').exists():
            raise ValueError()
    ...
    if populate:
        ...
        populate_database(database, populate_path)
