...
class MiniRpaManager(MiniRpaFunction):
    ...
    def start_bot(self):
        ...
        if self.delivery_data:
            ...
            elif delivery_type == 'upload_file_to_sharepoint':
                # from_folder_path = self.replace_variables_in_string(self.delivery_dict.get('from_folder_path', ''))
                # from_file_name = self.prepare_file_name(self.delivery_dict.get('from_file_name', ''))
                sharepoint_site_url = self.delivery_dict.get('sharepoint_site_url', '')
                sharepoint_folder_url = self.delivery_dict.get('sharepoint_folder_url', '')
                sharepoint_file_name = self.prepare_file_name(self.delivery_dict.get('sharepoint_file_name', ''))
                self.upload_file_to_sharepoint(from_folder_path, from_file_name, sharepoint_site_url, sharepoint_folder_url, sharepoint_file_name)
