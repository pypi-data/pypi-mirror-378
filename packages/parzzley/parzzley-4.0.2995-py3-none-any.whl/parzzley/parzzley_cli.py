...
def parser(*, only_documentation: bool = True) -> argparse.ArgumentParser:
    ...
    p_cmd_sync.add_argument("volume_names", type=str, help="Names of the volumes to sync. If none are specified,"
                                                           " all volumes will be synced.", nargs="*")

    p_cmd_sync_loop = p_cmd.add_parser("sync_loop", help="Execute synchronization loop. Note: This should be done by a"
...
def _setup_logging(*, debug: bool = False):
    ...
    parzzley_logger.setLevel(logging.DEBUG if debug else logging.INFO)
