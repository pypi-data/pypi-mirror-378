    ...
    def generate_precommit_hooks(self, project_path: Path) -> str:
        ...
        config = {
            "repos": [
                ...
                {
                    "repo": "https://github.com/lgtkgtv/mcp_agent--code_quality_security_tools",
                    "rev": "v0.3.1",
                    "hooks": [
                        {
  ...
  security-scan:
    ...
    - name: Install dependencies
      run: |
        ...
        uv pip install --system mcp-for-code-quality-security
    ...
    - name: Upload SARIF to GitHub Security
      ...
      with:
        ...
        category: mcp-for-code-quality-security
...
before_script:
  ...
  - uv pip install -e .
  - uv pip install mcp-for-code-quality-security

security-scan:
  stage: security-scan
  script:
    - source .venv/bin/activate
    - mcpcqs scan . --config security --sarif-output security-results.sarif --json-output security-results.json --compliance-report compliance-report.json
  artifacts:
    when: always
    reports:
...
fastapi-security:
  ...
  script:
    - source .venv/bin/activate
    - mcpcqs scan . --fastapi-focus --critical-only --json-output fastapi-security.json
  artifacts:
    paths:
...
ai-security:
  ...
  script:
    - source .venv/bin/activate
    - mcpcqs scan . --ai-security --prompt-injection-check --json-output ai-security.json
  artifacts:
    paths:
      - ai-security.json
    expire_in: 7 days
...
install: $(VENV) ## Install dependencies and setup virtual environment
	$(VENV_PIP) install -e .
	$(VENV_PIP) install mcp-for-code-quality-security
...
docker-security: ## Run security scan in Docker
	docker run --rm -v $(PWD):/app -w /app python:3.12-slim bash -c "pip install mcp-for-code-quality-security && mcpcqs scan . --config security"
...
RUN pip install mcp-for-code-quality-security

# Copy source code
COPY . .

# Run security scan
RUN mcpcqs scan . --config security --sarif-output /tmp/security-results.sarif
...
LABEL security.scanner="mcp-for-code-quality-security" \\
