...
def download(services: Union[str, List[str]], force_download: bool = False):
    ...
    :param services: a string representing a service name or a list of them.
...
def _download(services: List[str]):
    ...
    if not _services_are_known(services, orkg_services.keys()):
        raise ORKGNLPValidationError(
            "Unknown model name(s) given {}. Please check the following known services: {}".format(
                services, list(orkg_services.keys())
            )
        )
        ...
        for repo in orkg_services[service]:
            for file_obj in repo["files"]:
                ...
                if not os.path.exists(os.path.join(service_dir, filename)):
                    snapshot_download(repo_id=repo["repo_id"], local_dir=service_dir)
                    already_found = False

        if already_found:
            msg.good(
                'Required files for "{}" service already found in {}. '
            ...
            msg.good(service_dir, show=orkgnlp_context.get("ORKG_NLP_VERBOSITY"))
