...
def test_account_get_not_found(runner, mock_boto3_session):
    ...
    with patch("ai_org.core.aws_client.boto3.Session") as mock_session:
        mock_session.return_value = mock_boto3_session["session"]
        mock_boto3_session["orgs"].describe_account.side_effect = Exception("Account not found")
...
def test_account_create_with_all_args(runner, mock_boto3_session):
    ...
    with patch("ai_org.core.aws_client.boto3.Session") as mock_session:
        ...
        # Mock the describe create account status
        mock_boto3_session["orgs"].describe_create_account_status.return_value = {
            "CreateAccountStatus": {
                "Id": "car-example123",
                "AccountName": "TestAccount",
                "State": "SUCCEEDED",
