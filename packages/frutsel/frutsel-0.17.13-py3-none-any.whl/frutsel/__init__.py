...
class Document:
    ...
    def _from_file(db, *args, **kwargs):
        ...
        try:
            ...
            raise FrutselError(f'Failed to get checksum for metadata file {metafile.path}')

        if not isinstance(document_dict, dict):
            if metafile.check_checksum(strip_name_prefix=os.path.join(db.root_path, 'meta')):
                raise FrutselError(f'Possibly incompatible metadata file {metafile.path} unexpectedly has {type(document_dict).__name__} contents instead of a dict')
            else:
                raise FrutselError(f'Corrupt metadata file {metafile.path} unexpectedly has {type(document_dict).__name__} contents instead of a dict')
        try:
            ...
            if metafile.check_checksum(strip_name_prefix=os.path.join(db.root_path, 'meta')):
                raise FrutselError(f'Failed to get tags from metadata file {metafile.path}')
            else:
                raise FrutselError(f'Failed to get tags from corrupt metadata file {metafile.path}')
        ...
        try:
            ...
            if metafile.check_checksum(strip_name_prefix=os.path.join(db.root_path, 'meta')):
                raise FrutselError(f'{ex} while loading incompatible metadata file {metafile.path}')
            else:
                raise FrutselError(f'{ex} while loading corrupt metadata file {metafile.path}')
...
class _FrutselFile:
    ...
    def get_name(self, strip=None):
        if self._name:
            ...
            decoded = codecs.decode(joined.encode(), 'quoted-printable').decode()
            return decoded
...
""",
    ...
    parser.add_argument('--git-autocommit', action='store_true', help='automatically do a git commit after each chenge to the database')
    ...
    if args.license:
        print("""frutsel.py Copyright (C) 2021-2025 maveobi path-fanfare-canon@duck.com
