...
class JamaUTManager:
    ...
    def validate_test_set_exists(self) -> Dict:
        ...
        try:
            ...
            search_results = client.get_abstract_items(
                project=self.project_id,
                contains=[JAMA_TEST_SET_ID]
            )

            # Look for exact match
            for item in search_results:
                if item.get('documentKey') == JAMA_TEST_SET_ID:
                    logging.info(f"Found {JAMA_TEST_SET_ID}: {item['fields']['name']}")
                    return item

            raise JamaRequiredItemNotFoundError(f"{JAMA_TEST_SET_ID} not found - cannot proceed with UT creation")
        except APIException as e:
            logging.error(f"Error validating {JAMA_TEST_SET_ID}: {e}")
            raise JamaRequiredItemNotFoundError(f"{JAMA_TEST_SET_ID} not found - cannot proceed with UT creation")
