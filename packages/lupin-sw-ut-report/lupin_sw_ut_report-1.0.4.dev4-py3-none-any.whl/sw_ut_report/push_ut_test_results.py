...
from sw_ut_report.jama_common import (
    ...
    draw_table, JAMA_UT_TEST_CASE_ID
)
...
def push_ut_test_results_to_jama(version: str, input_folder: str) -> int:
    ...
    try:
        ...
        target_group = find_test_group_containing_case(test_plan_id, JAMA_UT_TEST_CASE_ID)

        if not target_group:
            raise JamaConnectionError(f"Could not find test group containing {JAMA_UT_TEST_CASE_ID}")
