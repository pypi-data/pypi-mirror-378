...
JAMA_TEST_SET_ID = os.getenv("JAMA_TEST_SET_ID", "SmlPrep-SET-359")  # Default to SmlPrep-SET-359 if not specified
JAMA_UT_TEST_CASE_ID = os.getenv("JAMA_UT_TEST_CASE_ID", "SmlPrep-UT-1")  # Default to SmlPrep-UT-1 if not specified
JAMA_SANDBOX_SEARCH_AND_REPLACE = os.getenv("JAMA_SANDBOX_SEARCH_AND_REPLACE", "")  # Default to empty string if not specified
...
def get_jama_config(
    ...
    """
    Get Jama configuration with CLI parameters taking priority over environment variables.

    Priority order: CLI parameters > Environment variables > Default values
    ...
    Returns:
        Dict containing the final configuration values
    """
    return {
        'project_id': project_id or JAMA_DEFAULT_PROJECT_ID,
        'test_set_id': test_set_id or JAMA_TEST_SET_ID,
        'ut_test_case_id': ut_test_case_id or JAMA_UT_TEST_CASE_ID
    }
...
class JamaUTManager:
    ...
    def __init__(self, project_id: Optional[str] = None, test_set_id: Optional[str] = None, ut_test_case_id: Optional[str] = None):
        """Initialize the JamaUTManager with environment configuration and optional CLI parameters."""
        self.jama_client: Optional[JamaClient] = None

        # Get configuration with CLI parameters taking priority
        config = get_jama_config(project_id, test_set_id, ut_test_case_id)
        ...
        if not all([JAMA_URL, JAMA_CLIENT_ID, JAMA_CLIENT_PASSWORD, config['project_id']]):
    ...
    def find_or_create_module_folder(self, module_name: str, parent_item: Dict) -> Dict:
        ...
        try:
            ...
            print(f"    location: {parent_id} (test set)")
    ...
    def get_existing_relationships_for_unit_test(self, unit_test_id: int) -> List[Dict]:
        ...
        try:
            ...
            logging.info(f"Getting upstream relationships for unit test {unit_test_id}")
            ...
            if relationships:
                logging.info(f"Found {len(relationships)} upstream relationships for unit test {unit_test_id}")
                return relationships
            else:
                logging.info(f"No upstream relationships found for unit test {unit_test_id}")
                return []
    ...
    def create_verification_relationships(self, unit_test: Dict, covers_list: List[str]) -> Dict:
        ...
        # Apply search/replace rules to covers list
        processed_covers_list = apply_search_replace_to_cover_list(covers_list)
        ...
        try:
            ...
            if relationships_to_create:
                ...
                logging.info("No new relationships to create")
            ...
            logging.info(f"Relationship management summary:")
            logging.info(f"   Total requirements: {total_requirements}")
            logging.info(f"   Created relationships: {len(created_relationships)}")
            logging.info(f"   Existing relationships: {len(existing_relationships_in_covers)}")
            logging.info(f"   Orphaned relationships: {orphaned_count}")
            logging.info(f"   Failed requirements: {failed_count}")
    ...
    def get_test_cases_in_group_rest_api(self, test_plan_id: int, group_id: int) -> List[Dict]:
        """
        Get test cases in a specific test group using direct REST API call.
        ...
        try:
            ...
            # Access the underlying HTTP client from py_jama_rest_client
            core = client._JamaClient__core
            response = core.get(f"testplans/{test_plan_id}/testgroups/{group_id}/testcases")

            if response.status_code == 200:
                test_cases_data = response.json()
                return test_cases_data.get('data', [])
            else:
...
def parse_search_replace_rules() -> List[tuple]:
    """
    Parse the JAMA_SANDBOX_SEARCH_AND_REPLACE environment variable into a list of (search, replace) tuples.

    Expected format: "sourceString1,replacement1;sourceString2,replacement2"
    Multiple rules are separated by semicolons, each rule has source and replacement separated by comma.

    Returns:
        List[tuple]: List of (search_string, replacement_string) tuples
    """
    # Read environment variable at runtime
    search_replace_env = os.getenv("JAMA_SANDBOX_SEARCH_AND_REPLACE", "")

    if not search_replace_env:
        return []

    rules = []
    try:
        # Split by semicolon to get individual rules
        rule_strings = search_replace_env.split(';')
        ...
        for rule_string in rule_strings:
            ...
            # Split by comma to get source and replacement
            parts = rule_string.split(',', 1)  # Split on first comma only
            if len(parts) != 2:
                logging.warning(f"Invalid search/replace rule format: {rule_string}. Expected 'source,replacement'")
                continue

            search_string = parts[0].strip()
            replacement_string = parts[1].strip()

            if search_string and replacement_string:
                rules.append((search_string, replacement_string))
                logging.info(f"Parsed search/replace rule: '{search_string}' -> '{replacement_string}'")
            else:
                logging.warning(f"Empty search or replacement string in rule: {rule_string}")

    except Exception as e:
        logging.error(f"Error parsing search/replace rules: {e}")

    return rules


def apply_search_replace_to_cover_id(cover_id: str) -> str:
    """
    Apply search and replace rules to a cover ID.

    Args:
        cover_id: The original cover ID

    Returns:
        str: The cover ID after applying all search/replace rules
    """
    if not cover_id:
        return cover_id

    result = cover_id
    rules = parse_search_replace_rules()

    for search_string, replacement_string in rules:
        if search_string in result:
            original = result
            result = result.replace(search_string, replacement_string)
            if result != original:
                logging.info(f"Applied search/replace to cover ID: '{original}' -> '{result}' (rule: '{search_string}' -> '{replacement_string}')")

    return result


def apply_search_replace_to_cover_list(cover_list: List[str]) -> List[str]:
    """
    Apply search and replace rules to a list of cover IDs.

    Args:
        cover_list: List of cover IDs

    Returns:
        List[str]: List of cover IDs after applying all search/replace rules
    """
    if not cover_list:
        return []

    return [apply_search_replace_to_cover_id(cover_id) for cover_id in cover_list]
...
def update_test_run_status(test_run_id: int, status: str, description: str = "") -> bool:
    ...
    try:
        ...
        jama_status = status_mapping.get(status.upper(), status.upper())
