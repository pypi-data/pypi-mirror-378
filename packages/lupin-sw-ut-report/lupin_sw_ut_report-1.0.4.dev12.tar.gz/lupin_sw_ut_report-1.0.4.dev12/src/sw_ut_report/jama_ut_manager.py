...
def dry_run_unit_tests_creation(module_name: str, test_results: List[Dict]) -> int:
    ...
    try:
        ...
        try:
            ...
            print(f"FOUND: {JAMA_TEST_SET_ID} exists - {test_set['fields']['name']}")
...
def _dry_run_check_module_folder(jama_manager: JamaUTManager, module_name: str, parent_item: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        print(f"DEBUG: Using test set ID {parent_id} for module folder search")
        ...
        print(f"DEBUG: Found {len(children)} children under test set (ID: {parent_id})")
        ...
        print(f"WILL CREATE: Module folder '{module_name}' under test set")
