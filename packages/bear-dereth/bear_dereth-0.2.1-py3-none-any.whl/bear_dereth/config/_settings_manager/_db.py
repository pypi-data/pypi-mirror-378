...
from bear_dereth.config._settings_manager._base_classes import Document, Table
from bear_dereth.config._settings_manager._common import QueryCheck, ValueType
from bear_dereth.config._settings_manager._query import QueryLike
...
class JsonFileStorage(Table):  # FIXME: This is not semantically correct, this is a Storage, not a Table
    ...
    def __init__(self, file_path: str | Path, cache_size: int = 10, **kwargs) -> None:
        ...
        self._query_cache: LRUCache[QueryLike, list[Document]] = LRUCache[QueryLike, list[Document]](
            capacity=cache_size
        )
        self._load()
