...
class BatchStore:
    ...
    def split_dataset(
        ...
        """
        if train_proportion + eval_proportion + test_proportion != 1:
            raise ValueError('train_proportion + eval_proportion + test_proportion must be 1')
        ...
        for sample in samples:
            if sample.name in split_dict.samples.train:
                ...
                if random.random() <= train_proportion:
                    actual_train_samples.append(sample.name)
                    train_page_counts += len(sample.pages)
                elif random.random() <= train_proportion + eval_proportion:
                    actual_eval_samples.append(sample.name)
                    eval_page_counts += len(sample.pages)
                else:
                    actual_test_samples.append(sample.name)
    ...
    def get_split_dict(self, check_updated: bool = False) -> SplitDict:
    ...
    @staticmethod
    def split_page_name(page_name: str):
        ...
        def get_sheetname(stem: str, span: tuple[int, int]) -> str:
            splitted = re.split(r'-p\d+_', stem[span[0] :])
            if len(splitted) > 1:
                return splitted[-1]
            else:
                return splitted[0]
        ...
        if suffix in EXCEL_EXTS:
            ...
            if match:
                ...
                sheetname = get_sheetname(stem, span)
        else:
