...
class DetectionResults(InferenceResults):
    ...
    def _resize_mask(self, mask, bbox_model_input, bbox_image_input):
        ...
        tl_model_image = [int(round(i)) for i in inv_conversion(0, 0)]
        br_model_image = [
            int(round(i)) for i in inv_conversion(original_width, original_height)
        ]

        img = mask[
            max(tl_model_image[1], 0) : min(br_model_image[1], height),
            max(tl_model_image[0], 0) : min(br_model_image[0], width),
        ]

        # add padding to cropped image
        if (
            tl_model_image[0] < 0
            or tl_model_image[1] < 0
            or br_model_image[0] > width
            or br_model_image[1] > height
        ):
            background = numpy.zeros(
                (
                    br_model_image[1] - tl_model_image[1],
                    br_model_image[0] - tl_model_image[0],
                ),
                img.dtype,
            )
            background[
                abs(tl_model_image[1]) : (abs(tl_model_image[1]) + img.shape[0]),
                abs(tl_model_image[0]) : (abs(tl_model_image[0]) + img.shape[1]),
            ] = img
            img = background

        if bbox_model_input is None:
            # mask is not cropped, so use original image dimensions and proceed to resize
            bbox_image_mask_enclosed = [0, 0, original_width, original_height]
            bbox_image_resized_width = original_width
            bbox_image_resized_height = original_height
        else:
            # mask is cropped, so calculate bounding box and resize dimensions to preserve mask completeness for accurate resizing
            ...
            bbox_model_precise = [
                max(0.0, bbox_model_input[0] - tl_model_image[0]),
                max(0.0, bbox_model_input[1] - tl_model_image[1]),
                min(float(img.shape[1]), bbox_model_input[2] - tl_model_image[0]),
                min(float(img.shape[0]), bbox_model_input[3] - tl_model_image[1]),
            ]
            ...
            margin_model_x_int = [
                ...
                min(box_margin, img.shape[1] - bbox_model_ceil[2]),
            ]
            margin_model_y_int = [
                min(box_margin, bbox_model_ceil[1]),
                min(box_margin, img.shape[0] - bbox_model_ceil[3]),
            ]
        ...
        if bbox_image_resized_width == 0 or bbox_image_resized_height == 0:
            # conversion of bounding box resulted in one or both dimensions to be zero, so return empty mask with resulting dimensions
            mask = numpy.zeros(
