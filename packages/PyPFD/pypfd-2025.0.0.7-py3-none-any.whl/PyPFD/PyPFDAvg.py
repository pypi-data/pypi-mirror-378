...
#IEC61508
def Tce( λ_du,λ_dd,T1_month,MTTR):
    λ_d =  λ_du + λ_dd
...
def Tce_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR):

    λ_d =  λ_du + λ_dd
    T1_hour = T1_month*730
    T2_hour = T2_month*730
    val = (λ_du*(1-PDC)/λ_d)*((T1_hour/2)+MTTR)+(λ_du*(PDC)/λ_d)*((T2_hour/2)+MTTR)+(λ_dd/λ_d)*MTTR
    return val


#IEC61508
def Tge(λ_du,λ_dd,T1_month,MTTR):
    λ_d =  λ_du + λ_dd
    T1_hour = T1_month*730
    val = (λ_du/λ_d)*((T1_hour/3)+MTTR)+λ_dd/λ_d*MTTR
    return val

def Tge_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR):
    

    λ_d =  λ_du + λ_dd
    T1_hour = T1_month*730
    T2_hour = T2_month*730
    val = (λ_du*(1-PDC)/λ_d)*((T1_hour/3)+MTTR)+(λ_du*(PDC)/λ_d)*((T2_hour/3)+MTTR)+(λ_dd/λ_d)*MTTR
    return val
...
#IEC61508
def Tg2e(λ_du,λ_dd,T1_month,MTTR):
    λ_d =  λ_du + λ_dd
    T1_hour = T1_month*730
    val = (λ_du/λ_d)*((T1_hour/4)+MTTR)+λ_dd/λ_d*MTTR
    return val

def Tne(λ_du,λ_dd,n,T1_month,MTTR):
    λ_d =  λ_du + λ_dd
    T1_hour = T1_month*730
    val = 1 
...
def pfd_avg_1oo1_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR):
    val = (λ_du+λ_dd)*Tce_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR)
    return  val
...
def pfd_avg_1oo2_pt(λ_du,λ_dd,β,βd,T1_month,T2_month,PDC,MTTR):
    ...
    val = 2 * (((1-βd)*λ_dd + (1- β)*λ_du)**2)*Tce_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR)*Tge_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR) + \
          βd*λ_dd*MTTR + β*λ_du*(1-PDC)*(T1_hour/2+MTTR)+β*λ_du*(PDC)*(T2_hour/2+MTTR)
...
def pfd_avg_2oo2(λ_du,λ_dd,T1_month,MTTR):
    return  pfd_avg_1oo1(λ_du,λ_dd,T1_month,MTTR)*2

def pfd_avg_2oo2_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR):
    return  pfd_avg_1oo1_pt(λ_du,λ_dd,T1_month,T2_month,PDC,MTTR)*2
...
#in development
def pfd_avg_1oo2_dif(λ_du1,λ_dd1,T1_month1,MTTR1,β1,βd1,λ_du2,λ_dd2,T1_month2,MTTR2,β2,βd2):
