            ...
            try:
                ...
                for attr in ['content', 'text', 'message', 'data', 'response', 'body']:
                    if hasattr(x, attr):
                        val = getattr(x, attr)
                        if val:
                            important_data[attr] = str(val)  # Capture first 1000 chars
                if important_data:
    ...
    def wrapped_invoke_model(**kwargs):  # type: ignore[no-untyped-def]
        ...
        try:
            ...
            try:
                ...
                # Handle Bedrock response body safely - capture content and restore it
                body = result.get('body', None)
                if body is not None:
                    try:
                        # For Bedrock responses, body is usually a StreamingBody
                        if hasattr(body, 'read'):
                            # Read the streaming body content for tracing
                            body_content = body.read()
                            if isinstance(body_content, bytes):
                                body_str = body_content.decode('utf-8')
                                try:
                                    # Try to parse as JSON for tracing
                                    response_data["body"] = json.loads(body_str)
                                except json.JSONDecodeError:
                                    # If not JSON, keep as string
                                    response_data["body"] = body_str  # Limit length
                                
                                # Restore the StreamingBody so the application can still read it
                                from io import BytesIO
                                result['body'] = BytesIO(body_content)
                            else:
                        ...
                        response_data["body"] = f"<body-parse-error: {str(body_error)}>"
                else:
                ...
                try:
                    result_json = f"{{\"statusCode\": {getattr(result, 'ResponseMetadata', {}).get('HTTPStatusCode', 'unknown')}, \"extraction_method\": \"fallback\", \"error\": \"{str(serialize_error)}\"}}"
                except Exception:
    ...
    try:
        ...
        if error:
            ...
            try:
                ...
                result_json = f"{{\"content\": \"{str(result)}\"}}"
