...
def query_rows(
    ...
    *,
    ...
    1) Raw SQL mode:
       ...
       In this mode we append/wrap a LIMIT safely without conflicting with existing placeholders.
    ...
    Args:
        ...
        where: Trusted SQL WHERE clause (bind values via `params`) (table mode only).
        order_by: Trusted SQL ORDER BY clause (table mode only).
    ...
    if sql is not None:
        ...
        # WHERE / ORDER BY (trusted text, bind values via params)
        if where:
...
def stream_to_parquet(
    ...
    *,
    ...
    Args:
        manager: ConnectionManager instance.
        sql: SQL SELECT to execute (raw SQL mode).
