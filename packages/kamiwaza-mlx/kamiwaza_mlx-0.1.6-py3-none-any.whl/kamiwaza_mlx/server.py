...
parser.add_argument("--kv-cache-max-tokens", type=int, default=1024, help="Upper bound for KV cache tokens (to cap memory usage).")
parser.add_argument("--kv-cache-min-tokens", type=int, default=256, help="Lower bound for KV cache tokens when building caches.")
...
if IS_VISION:  # ──────────── VISION PATH ────────────
    ...
    def sync_gen(prompt: str, _imgs, req: ChatReq) -> str:  # noqa: C901, D401
        ...
        if cache_to_use is None and not IS_VISION:
            suffix_len = (len(suffix_ids_val) if suffix_ids_val.ndim == 1 else int(suffix_ids_val.shape[-1]))
            desired = max(int(args.kv_cache_min_tokens), min(int(args.kv_cache_max_tokens), suffix_len + int(args.prefix_cache_headroom)))
            cache_to_use = _make_bounded_kv_cache_for_model(desired, keep=args.kv_cache_keep)
            log.info(f"Allocating bounded KV cache for generation: max_size={desired}, keep={args.kv_cache_keep}")
    ...
    def stream_chunks(prompt: str, _imgs, req: ChatReq):  # noqa: C901
        ...
        if cache_to_use is None and not IS_VISION:
            ...
            desired = max(int(args.kv_cache_min_tokens), min(int(args.kv_cache_max_tokens), suffix_len + int(args.prefix_cache_headroom)))
            cache_to_use = _make_bounded_kv_cache_for_model(desired, keep=args.kv_cache_keep)
            log.info(f"Allocating bounded KV cache for streaming: max_size={desired}, keep={args.kv_cache_keep}")
...
# ─────────────────── KVCache Builder (bounded) ───────────────────
def _make_bounded_kv_cache_for_model(max_size: int, keep: int) -> List[Any]:
    """Build a KV cache structure matching the model's expected shape,
    but with bounded per-slot caches (RotatingKVCache) to cap memory.

    This uses MODEL.make_cache() when available to preserve per-layer
    sub-structures (e.g., multiple attn modules), replacing concrete
    cache objects recursively with RotatingKVCache(max_size, keep).
    """
    ...
    if template is None:
        ...
        # Fallback: a flat list per layer
        return [RotatingKVCache(max_size=max_size, keep=int(keep)) for _ in range(num_layers)]

    def _bounded_like(x):
        if x is None:
            return None
        if isinstance(x, (list, tuple)):
            return type(x)(_bounded_like(xx) for xx in x)
        # Replace any known cache type with a fresh RotatingKVCache
        if isinstance(x, (RotatingKVCache, KVCache, QuantizedKVCache)):
            return RotatingKVCache(max_size=max_size, keep=int(keep))
        # Unknown object – leave as-is (should be rare)
        return x

    return _bounded_like(template)
...
async def completions(req: ChatReq, request: Request):  # noqa: C901 – same as original
    ...
    if conv_id and not req.reset_conversation:
        ...
        if rec is not None:
            try:
                ...
                if rec.boundary_offsets and k > 0:
                    ...
                    # Direct current record (string-based)
        ...
        if precomp_cache is None:
            # no existing record or mismatch → allocate fresh bounded cache for this conversation
            desired = int(args.conversation_kv_max_tokens)
            ctx = _model_ctx_len(MODEL)
            if isinstance(ctx, int) and ctx > 0:
                desired = min(desired, ctx)
            conv_cache = _make_bounded_kv_cache_for_model(desired, keep=args.kv_cache_keep)
            ...
            log.info(f"🧵 Conversation KV allocate for '{conv_id}' (max_size={desired})")
    ...
    if (args.enable_prefix_caching and not IS_VISION and GLOBAL_PROMPT_CACHE is None and 
        ...
        try:
            ...
            desired = sys_len + int(args.prefix_cache_headroom)
            desired = max(desired, int(args.kv_cache_min_tokens))
            desired = min(desired, int(args.kv_cache_max_tokens))
            temp_cache = _make_bounded_kv_cache_for_model(desired, keep=args.kv_cache_keep)
            log.info(f"Allocating bounded prefix KV cache: max_size={desired}, keep={args.kv_cache_keep}")
