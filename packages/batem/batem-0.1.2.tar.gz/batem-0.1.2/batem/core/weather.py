...
The module provides:
- ElevationRetriever: Elevation data retrieval and caching from web APIs
- SWDbuilder: Historical weather data download and management from Open-Meteo API
- SiteWeatherData: Weather time series data storage and processing for building energy analysis
- Utility functions: Weather variable conversion and atmospheric calculations
...
from batem.core.timemg import REGULAR_DATETIME_FORMAT, datetime_with_day_delta, epochtimems_to_datetime, epochtimems_to_stringdate, stringdate_to_epochtimems, TimezoneFinder, local_timezone
...
class SWDbuilder:
    ...
    @staticmethod
    def _download_weather_data_by_decade(server_url: str, site_latitude_north_deg: float, site_longitude_east_deg: float,
                                       start_year: int, end_year: int, json_file_path: str) -> None:
        ...
        :raises ValueError: If the download fails after all retry attempts
        """
        print(f'Downloading data from {start_year} to {end_year} in 10-year chunks')
        ...
        decade_count = 0
        chunk_size = 10  # Start with 10-year chunks
    ...
    @staticmethod
    def build(location: str, latitude_north_deg: float, longitude_east_deg: float, from_requested_stringdate: str = None, to_requested_stringdate: str = None, albedo: float = 0.1, pollution: float = 0.1, from_to_naming: dict[str, str] = DEFAULT_FROM_TO_NAMING) -> SiteWeatherData:
        """
...
class SiteWeatherData:
    ...
    def long_wave_radiation_sky(self) -> np.ndarray:
        ...
        E_clear_W_per_m2 = 0.711 + 0.56 * dew_point_norm + 0.73 * (dew_point_norm ** 2)
        E_cloud_W_m2 = 0.96 * Stefan_Boltzmann * ((ground_temperatures_deg + 273.15 - 5) ** 4)
    ...
    def surface_out_radiative_exchange(self, slope_deg: float, surface_temperature_deg: list[float], ground_temperature_deg: list[float], surface_m2: float = 1) -> tuple[np.ndarray, np.ndarray]:
        ...
        dew_point_norm = dew_point_temperatures_deg / 100.0
        clear_sky_irradiance_W_per_m2 = 0.711 + 0.56 * dew_point_norm + 0.73 * (dew_point_norm ** 2)
        cloud_irradiance_W_per_m2 = 0.96 * Stefan_Boltzmann * ((outdoor_temperatures_deg + 273.15 - 5) ** 4)
    ...
    def plot(self):
        TimeSeriesPlotter(self._variable_name_data, self._datetimes, self.variable_units)
