...
class AxesConfigurator(ABC):
    ...
    @abstractmethod
    def configure_axes(self, fig: Any, config: Any):
        raise NotImplementedError

    @abstractmethod
    def create_figure(self, config: Any) -> Any:
        raise NotImplementedError
...
class FigureSaver(ABC):
    ...
    @abstractmethod
    def save_figure(self, fig: Any, config: Any):
        raise NotImplementedError
...
class Plotter:
    ...
    def plot(self, data: Any):
        ...
        self._renderer.render_plot(fig, processed_data)
        self._axes_configurator.configure_axes(fig, self._config)
        self._figure_saver.save_figure(fig[0], self._config)
...
def plot_weather(location: str):
    ...
    plotter = Plotter(WeatherPlotConfig(as_png=True,
                                        file_path=file_path,
                                        size=(10, 5),
                                        variables=[
