...
def map_sqlalchemy_type_to_pydantic(
    ...
    if isinstance(sqlalchemy_type, String):
        ...
        try:
            return sqlalchemy_type.python_type.__name__, {}  # type: ignore  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        except Exception:
            return "Any", {}


def is_nullable(sqlalchemy_type: type) -> bool:
    return sqlalchemy_type.nullable if hasattr(sqlalchemy_type, "nullable") else False  # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType]


def get_default_value(sqlalchemy_type: type) -> Any:
    return sqlalchemy_type.default if hasattr(sqlalchemy_type, "default") else None  # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType]
