    ...
    def create_chat_completion_with_fallback(
        ...
        try:
            ...
            if len(available_providers) > 1:
                alternative_model = self._get_cross_provider_fallback(
                    current_provider)
                if alternative_model:
                    log_warning(f"Switching to {alternative_model}", logger)
                    alt_response, alt_success = self._try_model_completion(
                        alternative_model, messages, max_tokens, system_prompt, max_retries, retry_base_delay
                    )
                    if alt_response is not None:
                        log_success(
                            f"Successfully switched to {alternative_model}", logger)
                        return alt_response

            alternative_model = self._get_same_provider_fallback(
                model_name, current_provider)
            if alternative_model:
                log_warning(f"Switching to {alternative_model}", logger)
                alt_response, alt_success = self._try_model_completion(
                    alternative_model, messages, max_tokens, system_prompt, max_retries, retry_base_delay
            ...
            log_error(
                Exception(f"All fallback attempts exhausted for {model_name}"), "", logger)
            return None
