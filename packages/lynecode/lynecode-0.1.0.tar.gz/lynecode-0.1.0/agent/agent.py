...
class Agent:
    ...
    def _extract_json_from_response(self, response: str) -> Optional[Dict[str, Any]]:
        ...
        double_brace_match = re.search(r'\{\{\s*({[\s\S]*?})\s*\}\}', text)
        ...
        try:
            parsed = self._parse_with_fallbacks(text)
            if isinstance(parsed, dict) and "action" in parsed:
                if parsed["action"] in ["call_tool", "summaries", "plan"]:
                    return self._validate_and_fix_response_structure(parsed)
                else:
            ...
            if len(response_list) == 0:
                ...
                if original_response and original_response.strip():
                    ...
                    if len(clean_content) > 500:
                        ...
                        if summary_lines:
                            clean_content = ' '.join(summary_lines[:3])
    ...
    def run(self) -> str:
        ...
        try:
            ...
            final_response = f"Maximum iterations reached. Session history: {len(self.session_history)} entries"
            self.save_to_conversation_history(final_response)
