...
# Get package version
try:
    __version__ = version("jira-mcp-standalone")
except Exception:
    __version__ = "1.14.3"

server = Server("jira-mcp-server")
# Override the server info with correct version
server._server_info = {"name": "jira-mcp-server", "version": __version__}
...
async def run_server():
    ...
    try:
        ...
        if any(x in str(e) for x in ["ClosedResourceError", "Connection", "TaskGroup", "BrokenResourceError"]):
