...
class SyftClient:
    ...
    def __repr__(self) -> str:
        ...
        lines = []
        lines.append("╭─ SyftClient ────────────────────────────────────────────╮")
        lines.append(f"│ Email: {self.email:<49}│")
        lines.append("├─────────────────────────────────────────────────────────┤")
        
        if self.platforms:
            lines.append("│ Platforms & Transport Layers:                           │")
            for platform_name, platform in self.platforms.items():
                ...
                # Platform header
                lines.append(f"│                                                         │")
                lines.append(f"│ {platform_name.upper():<55}│")
                
                # Transport layers in columns
                if transports:
                    # Show up to 2 transports per line
                    for i in range(0, len(transports), 2):
                        transport1 = transports[i]
                        transport2 = transports[i+1] if i+1 < len(transports) else ""
                        line = f"│   {transport1:<25} {transport2:<25}│"
                        lines.append(line)
        else:
            lines.append("│ No authenticated platforms                              │")
