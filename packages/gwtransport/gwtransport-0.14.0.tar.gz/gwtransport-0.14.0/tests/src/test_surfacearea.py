...
def test_rectangle_no_clipping():
    ...
    # No clipping bounds
    avg_heights = compute_average_heights(x_edges, y_edges, -1, 4)
...
def test_rectangle_full_clipping():
    ...
    avg_heights = compute_average_heights(x_edges, y_edges, 4, 5)
    assert_almost_equal(avg_heights[0, 0], 0.0)

    # Clip completely below
    avg_heights = compute_average_heights(x_edges, y_edges, -2, -1)
    assert_almost_equal(avg_heights[0, 0], 0.0)
...
def test_rectangle_partial_clipping():
    ...
    avg_heights = compute_average_heights(x_edges, y_edges, 2, 5)
    ...
    avg_heights = compute_average_heights(x_edges, y_edges, -1, 2)
    assert_almost_equal(avg_heights[0, 0], 2.0)
...
def test_multiple_quads():
    ...
    avg_heights = compute_average_heights(x_edges, y_edges, 0.5, 1.5)
...
def test_zero_width_handling():
    ...
    avg_heights = compute_average_heights(x_edges, y_edges, -1, 3)
...
def test_edge_case_bounds():
    ...
    avg_heights = compute_average_heights(x_edges, y_edges, 0, 2)
    assert_almost_equal(avg_heights[0, 0], 2.0)

    # Lower bound exactly at bottom
    avg_heights = compute_average_heights(x_edges, y_edges, 0, 3)
    assert_almost_equal(avg_heights[0, 0], 2.0)
...
def test_conservation_property():
    ...
    # Full area
    full_heights = compute_average_heights(x_edges, y_edges, 0, 4)

    # Split into two parts
    lower_heights = compute_average_heights(x_edges, y_edges, 0, 2)
    upper_heights = compute_average_heights(x_edges, y_edges, 2, 4)
