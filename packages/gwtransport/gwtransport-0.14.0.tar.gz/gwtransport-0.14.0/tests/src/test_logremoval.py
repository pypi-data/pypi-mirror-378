...
def test_weight_sum_behavior():
    ...
    result1 = parallel_mean(log_removals, [0.7, 0.4])  # Sum > 1
...
def test_length_mismatch_behavior():
    ...
    try:
        result1 = parallel_mean([3.0, 4.0], [1.0])
    ...
    try:
        result2 = parallel_mean([3.0], [0.5, 0.5])
        assert isinstance(result2, (int, float))
    except (ValueError, IndexError):
        pass
...
def test_axis_parameter_2d_arrays():
    ...
    result_axis1 = parallel_mean(log_removals_2d, axis=1)

    # Expected results: parallel_mean for each row
    expected_row0 = parallel_mean([3.0, 4.0, 5.0])
    expected_row1 = parallel_mean([2.0, 3.0, 4.0])
    ...
    result_axis0 = parallel_mean(log_removals_2d, axis=0)

    # Expected results: parallel_mean for each column
    expected_col0 = parallel_mean([3.0, 2.0])
    expected_col1 = parallel_mean([4.0, 3.0])
    expected_col2 = parallel_mean([5.0, 4.0])
...
def test_axis_parameter_3d_arrays():
    ...
    result_axis2 = parallel_mean(log_removals_3d, axis=2)

    # Expected results: parallel_mean for each pair along axis 2
    expected_00 = parallel_mean([3.0, 4.0])
    expected_01 = parallel_mean([5.0, 2.0])
    expected_10 = parallel_mean([1.0, 6.0])
    expected_11 = parallel_mean([2.0, 3.0])
...
def test_axis_parameter_with_flow_fractions():
    ...
    # Test with axis=1
    result = parallel_mean(log_removals_2d, flow_fractions_2d, axis=1)

    # Expected results: weighted parallel_mean for each row
    expected_row0 = parallel_mean([3.0, 4.0, 5.0], [0.5, 0.3, 0.2])
    expected_row1 = parallel_mean([2.0, 3.0, 4.0], [0.6, 0.2, 0.2])
...
def test_axis_parameter_behavior():
    ...
    try:
        result = parallel_mean(log_removals_1d, axis=0)
...
def test_negative_axis():
    ...
    result_neg1 = parallel_mean(log_removals_2d, axis=-1)
    result_pos1 = parallel_mean(log_removals_2d, axis=1)

    assert_allclose(result_neg1, result_pos1, rtol=1e-10)

    # axis=-2 should be equivalent to axis=0 for 2D array
    result_neg2 = parallel_mean(log_removals_2d, axis=-2)
    result_pos0 = parallel_mean(log_removals_2d, axis=0)
...
def test_empty_array_with_axis():
    ...
    try:
        result = parallel_mean(empty_2d, axis=1)
