...
def test_config_valid():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/valid.yaml"]
        )
        == 0
    )
...
def test_config_invalid():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/invalid.yaml"]
        )
        == 1
    )
...
def test_config_skip():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/skip.yaml"]
        )
        == 1
    )
...
def test_config_invalid_css_only():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/invalid_css_only.yaml",
            ]
        )
        == 1
    )
...
def test_config_warning():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/warning.yaml"]
        )
        == 1
    )
...
def test_config_warning_but_pass():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/warning_pass.yaml",
            ]
        )
        == 0
    )
...
def test_config_return_value():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/return_254.yaml",
            ]
        )
        == 254
    )
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/return_255.yaml",
            ]
        )
        == 255
    )
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/return_255_256.yaml",
            ]
        )
        == 255
    )
...
def test_config_angularjs():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/angularjs.yaml",
            ]
        )
        == 0
    )
...
def test_config_angularjs_normal_quotes():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/angularjs_normal.yaml",
            ]
        )
        == 0
    )
...
def test_config_multiple_ignoreres():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/multiple_ignores.yaml",
            ]
        )
        == 0
    )
...
def test_config_stack_size():
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                f"--config={HTML_TEST_FILES}/config_files/stack_size.yaml",
            ]
        )
        == 0
    )
...
def test_config_log_file():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/log_file.yaml"]
        )
        == 0
    )
...
def test_config_extra():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/extra.yaml"]
        )
        == 0
    )
...
def test_config_nofiles():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--config={HTML_TEST_FILES}/config_files/no_files.yaml"]
        )
        == 1
    )
