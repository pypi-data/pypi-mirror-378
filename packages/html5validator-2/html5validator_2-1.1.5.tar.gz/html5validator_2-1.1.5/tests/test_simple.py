...
def test_valid():
    ...
    assert subprocess.call(["html5validator", f"--root={HTML_TEST_FILES}/valid/"]) == 0


def test_invalid():
    """Command line test for invalid files"""
    assert (
        subprocess.call(["html5validator", f"--root={HTML_TEST_FILES}/invalid/"]) == 1
    )
...
def test_warning():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/warning/", "--show-warnings"]
        )
        == 1
    )
...
def test_warning_but_pass():
    ...
    assert (
        subprocess.call(["html5validator", f"--root={HTML_TEST_FILES}/warning/"]) == 0
    )
...
def test_stack_size():
    ...
    assert (
        subprocess.call(["html5validator", f"--root={HTML_TEST_FILES}/valid/", "-lll"])
        == 0
    )
...
def test_valid_format_flags():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/valid/", "--format", "text"]
        )
        == 0
    )
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/valid/", "--format", "gnu"]
        )
        == 0
    )
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/valid/", "--format", "json"]
        )
        == 0
    )
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/valid/", "--format", "xml"]
        )
        == 0
    )
...
def test_invalid_format_flags():
    ...
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/invalid/", "--format", "text"]
        )
        == 3
    )
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/invalid/", "--format", "gnu"]
        )
        == 1
    )
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/invalid/", "--format", "json"]
        )
        == 1
    )
    assert (
        subprocess.call(
            ["html5validator", f"--root={HTML_TEST_FILES}/invalid/", "--format", "xml"]
        )
        == 8
    )
...
def test_nofiles():
    ...
    assert subprocess.call(["html5validator", "--root=MISSING"]) == 1
