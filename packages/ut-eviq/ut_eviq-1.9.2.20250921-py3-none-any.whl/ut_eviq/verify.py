...
class EvinVfyAdm:
    ...
    def vfy_duns(
        ...
        if not _val:
            ...
            return
        if len(_val) > 9:
            DoA.append_unique_by_key(doaod_vfy, cls.duns_length_is_invalid, d_evin)
            d_sw['duns'] = False
            return
        if not _val.isdigit():
    ...
    def vfy_coynmdis(cls, d_sw: TyDoB, d_evin: TyDic, doaod_vfy: TyDoAoD) -> None:
        ...
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.coynmdis_is_empty, d_evin)
            d_sw['coynmdis'] = False
            return
        if len(_val) > 500:
            DoA.append_unique_by_key(doaod_vfy, cls.coynmdis_length_is_invalid, d_evin)
            d_sw['coynmdis'] = False
            return
        d_sw['coynmdis'] = True
        return
    ...
    def vfy_coynmoff(cls, d_sw: TyDoB, d_evin: TyDic, doaod_vfy: TyDoAoD) -> None:
        ...
        _val = d_evin.get(_key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.coynmoff_is_empty, d_evin)
            d_sw['coynmoff'] = False
            return
        if len(_val) > 500:
            DoA.append_unique_by_key(doaod_vfy, cls.coynmoff_length_is_invalid, d_evin)
            d_sw['coynmoff'] = False
            return
        d_sw['coynmoff'] = True
        return
    ...
    def vfy_ctryco(cls, d_sw: TyDoB, d_evin: TyDic, doaod_vfy: TyDoAoD) -> None:
        ...
        _val = d_evin.get(_key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.ctryco_is_empty, d_evin)
            d_sw['ctryco'] = False
            return
        if len(_val) > 2:
            DoA.append_unique_by_key(doaod_vfy, cls.ctryco_length_is_invalid, d_evin)
            d_sw['ctryco'] = False
            return
        import pycountry
        try:
            ...
            DoA.append_unique_by_key(doaod_vfy, cls.ctryco_is_invalid, d_evin)
            d_sw['ctryco'] = False
            return
        d_sw['ctryco'] = True
        return
    ...
    def vfy_uniqueid(
        ...
        _val = d_evin.get(_key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.uniqueid_is_empty, d_evin)
            d_sw['uniqueid'] = False
            return
        if len(_val) > 50:
            DoA.append_unique_by_key(doaod_vfy, cls.uniqueid_length_is_invalid, d_evin)
            d_sw['uniqueid'] = False
            return
    ...
    def vfy_postco(cls, d_sw: TyDoB, d_evin: TyDic, doaod_vfy: TyDoAoD) -> None:
        ...
        if not _val_postco:
            DoA.append_unique_by_key(doaod_vfy, cls.postco_is_empty, d_evin)
            d_sw['postco'] = False
            return
        if len(_val_postco) > 30:
            DoA.append_unique_by_key(doaod_vfy, cls.postco_length_is_invalid, d_evin)
            d_sw['postco'] = False
            return
        _key_ctryco: TyStr = Cfg.Utils.evin_key_ctryco
        _val_ctryco: TnStr = d_evin.get(_key_ctryco)
        ...
        if not verify_postal_code_format(postal_code=_val_postco, country_iso2=_val_ctryco):
            DoA.append_unique_by_key(doaod_vfy, cls.postco_is_invalid, d_evin)
            d_sw['postco'] = False
            return
        d_sw['postco'] = True
        return
