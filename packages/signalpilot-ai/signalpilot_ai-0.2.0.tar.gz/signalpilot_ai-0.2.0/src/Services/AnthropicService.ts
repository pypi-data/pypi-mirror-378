...
export class AnthropicService implements IChatService {
  ...
  async sendEphemeralMessage(
    ...
    if (!jwtToken) {
      ...
      return { cancelled: true, reason: 'Authentication required' } as CancelledRequest;
    }
  ...
  private async handleEphemeralRequestError(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!retryClient) {
          throw new Error('Anthropic client not available for retry. Please check your API key configuration.');
        }
  ...
  private async validateClientInitialized(
    ...
    if (!client) {
      const errMsg = {
        message: 'Anthropic client not available. Please check your API key configuration.',
        variables: {
  ...
  private async shouldSkipRetry(
    ...
    if (errorMessage.includes('authentication_error') || errorMessage.includes('Invalid API key')) {
      this.requestStatus = ChatRequestStatus.ERROR;
      await errorLogger?.({
