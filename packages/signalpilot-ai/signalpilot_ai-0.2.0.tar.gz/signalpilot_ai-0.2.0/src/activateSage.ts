...
export async function activateSage(
  ...
  try {
    ...
    if (jwtInitialized) {
      console.log('[Plugin] JWT authentication initialized successfully on startup');
    } else {
  ...
  notebooks.currentChanged.connect(async (_, notebook) => {
    if (notebook) {
      ...
      (window as any).debugLoginSuccess = {
        show: async () => {
          try {
            const { LoginSuccessModalService } = await import('./Services/LoginSuccessModalService');
