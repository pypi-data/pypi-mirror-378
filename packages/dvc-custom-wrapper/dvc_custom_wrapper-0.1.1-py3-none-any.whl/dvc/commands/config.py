...
NAME_REGEX = r"^(?:(?P<top>(remote|db|global_log))\.)?(?P<section>[^\.]+)\.(?P<option>[^\.]+)$"
...
def _name_type(value):
    ...
    if not match:
        raise argparse.ArgumentTypeError(
            "name argument should look like remote.name.option, "
            "db.name.option, global_log.option or section.option"
        )
    top = match.group("top")
    return (
