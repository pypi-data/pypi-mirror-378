...
class PDGConductorScheduler:
    ...
    def prepare_batch_job(self, work_item_chain, topnet_path, wrapper_script_source):
        ...
        if not self.render_dir:
            self.render_dir = os.path.abspath(
                os.path.join(str(self.context.workingDir(False)), "execute", "pdg_render"))
        ...
        if current_hip_path:
            # Use the same directory and base name as the current HIP file
            hip_dir = os.path.dirname(current_hip_path)
            hip_basename = os.path.splitext(os.path.basename(current_hip_path))[0]
            base_hip_path = os.path.join(hip_dir, f"{hip_basename}.hip")
        else:
            # Fallback if no HIP file is loaded (shouldn't happen in practice)
            hip_dir = str(self.context.workingDir(False))
            base_hip_path = os.path.join(hip_dir, f"pdg_graph_{work_item_chain[0].node.name}.hip")
    ...
    def prepare_single_job(self, work_item, topnet_path, wrapper_script_source):
        ...
        if current_hip_path:
            ...
            hip_dir = str(self.context.workingDir(False))
            base_hip_path = os.path.join(hip_dir, f"pdg_graph_{work_item.node.name}.hip")
