...
from typing import Any, Generator, overload, TYPE_CHECKING
...
class Branch(Token):
    ...
    def __repr__(self):
        return f"<{self.xml_label} phrase={self.phrase.id!r}>{str().join(repr(i) for i in self.stack)!r}</{self.xml_label}>"
...
class Phrase:
    ...
    suffix_phrases: set[Phrase]

    def __init__(self, id: Any = ''):
        self.id = id
        self.sub_phrases = set()
        self.suffix_phrases = set()
    ...
    def add_suffix_phrases(self, *nodes: Phrase | Iterable[Phrase]) -> Self:
        self.suffix_phrases.update(nodes)
        return self
...
class RootPhrase(Phrase):
    ...
    class _Crawler:
        ...
        def __call__(self):
            ...
            active_stop = self.branch.ends(self.remain_row_content, self.row_n, self.row_viewpoint, self.abs_viewpoint)

            sub_starts = list()
            for sp in self.branch.phrase.sub_phrases:
                if start_node := sp.starts(self.remain_row_content, self.row_n, self.row_viewpoint, self.abs_viewpoint, self.branch):
            ...
            def _branch_start(start):
                if start.start_node.match_rel_start:
                    # remain token
                    self.branch.extend_branch(
                        0, start.start_node.match_rel_start,
                        self.remain_row_content[:start.start_node.match_rel_start],
                        self.row_n, self.row_viewpoint, self.abs_viewpoint
                    ).call_branch_extend()
                self.branch.stack.append(start)  # end node

                self.branch = start
                self.remain_row_content = self.remain_row_content[start.start_node.match_rel_end:]
                self.abs_viewpoint += start.start_node.match_rel_end
                start.start_node.call_branch_start()
                if not self.remain_row_content:
                    _nextrow()
                else:
                    self.row_viewpoint += start.start_node.match_rel_end

            def _search_suffix():
                x_starts = list()
                for xp in self.branch.phrase.suffix_phrases:
                    if x_node := xp.starts(self.remain_row_content, self.row_n, self.row_viewpoint, self.abs_viewpoint,
                                           # passes the parent branch of the branch belonging to the suffix
                                           # to return to it at the end of the suffix
                                           self.branch.branch):
                        x_starts.append(x_node)
                return x_starts

            def _stop_current_branch():
                ...
                self.branch.stack.append(active_stop)  # end node
                ...
                if suffix_starts := _search_suffix():
                    suffix_start = min(suffix_starts)
                    _branch_start(suffix_start)
                else:
            ...
            if sub_starts:
                ...
                if active_stop and active_stop < sub_start.start_node:
                    _stop_current_branch()
                else:
                    _branch_start(sub_start)
            elif active_stop:
                _stop_current_branch()
            else:
                self.branch.extend_branch(
