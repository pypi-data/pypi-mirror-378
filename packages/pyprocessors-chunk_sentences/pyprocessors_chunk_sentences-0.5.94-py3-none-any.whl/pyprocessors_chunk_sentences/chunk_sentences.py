...
class TokenBasedSentenceAnalysis(SentenceAnalysis):
    ...
    def split(self, max_len) -> List[MarkedSentence]:
        ...
        start_token = 0
        start = self.sentence.start
        while start_token < len(self.tokens):
            end_token = min(start_token + max_len, len(self.tokens))
            s_text = self.decode(self.tokens[start_token:end_token])
            nb_tokens = end_token - start_token
            start_token = end_token
            end = start + len(s_text)
            covered_s_text = self.text[start:end]
            if covered_s_text != s_text:
                # we have an issue... try to find individual words...
                words = s_text.split()
                sentence_start = -1
                sentence_end = -1
                for word in words:
