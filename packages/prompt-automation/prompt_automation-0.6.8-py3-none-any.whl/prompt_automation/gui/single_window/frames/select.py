...
class SearchFocusController:
    ...
    def decide_focus(self, current: str, event: Any) -> FocusAction:
        ...
        if printable and not control:
            ...
            return FocusAction(target="search", consume=True)
...
def build(app) -> Any:  # pragma: no cover - Tk runtime
    ...
    expanded: set[str] = {_canon_rel(rel) for rel in load_expanded()} if hier_mode else set()
    ...
    def _activate_from_entry(event=None):
        if listbox.size() == 0:
            status.set("No templates available")
            return "break"
        sel = listbox.curselection()
        if not sel:
            idx = 0
