...
@service_registry.tool(
        name="search_assets:search_asset",
...
async def search_asset(request: SearchAssetRequest, ctx=None) -> List[SearchAssetResponse]:
    ...
    try:
        response = await client.post(settings.base_url + ASSISTANT_BASE_ENDPOINT, data=payload, headers=headers)

        search_response = response.get("assets")
        li = list(
            map(lambda asset: SearchAssetResponse.model_validate(asset), search_response)
        )
