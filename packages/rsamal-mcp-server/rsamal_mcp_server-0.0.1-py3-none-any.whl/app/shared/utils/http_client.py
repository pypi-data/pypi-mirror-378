...
class AsyncHttpClient:
    ...
    async def client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            verify_setting = get_ssl_verify_setting(settings.ssl_config, settings.ssl_verify)
    ...
    async def post(
        ...
        try:
            ...
            if content_type == "application/x-www-form-urlencoded":
                response = await client.post(url, data=data, params=params, headers=headers or {})
            else: 
                response = await client.post(url, json=data, params=params, headers=headers or {})
            response.raise_for_status()
            return response.json()
        except httpx.HTTPError as e:
            raise ExternalAPIError(f"HTTP request failed: {str(e)}")
