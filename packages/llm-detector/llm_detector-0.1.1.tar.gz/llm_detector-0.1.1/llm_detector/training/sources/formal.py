...
class LegalDocumentsSource(BaseDataSource):
    ...
    def stream_samples(self, limit: int | None = None) -> Iterator[TextSample]:
        ...
        if dataset is None:
            # Fallback to Multi Legal Pile
            alt_dataset = "joelniklaus/Multi_Legal_Pile"
            logger.info(f"Trying alternative: {alt_dataset}")
            try:
                dataset = load_dataset(
                    alt_dataset,
                    "en_contracts",  # English contracts
                    split="train",
                    streaming=True,
                )
                self._dataset_name = alt_dataset
                self._current_config = "en_contracts"
            except Exception as e2:
                raise RuntimeError(f"Could not load any legal dataset") from e2
    ...
    def stream_samples(self, limit: int | None = None) -> Iterator[TextSample]:
        ...
        for config in ["arxiv", "pubmed"]:
            try:
                ...
                if config == "pubmed":
                    # Both failed
                    raise RuntimeError(f"Could not load scientific papers dataset") from e
                continue
