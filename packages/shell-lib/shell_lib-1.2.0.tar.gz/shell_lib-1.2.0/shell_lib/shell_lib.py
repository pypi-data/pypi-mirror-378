...
if sys.flags.utf8_mode:
    msg = ("Python UTF-8 Mode is enabled. shell-lib doesn't support "
           "running in this mode, many operations may be unreliable, "
           "due to the mandatory use of UTF-8 encoding.")
...
class SetEnvContextManager:
    ...
    #    os.putenv() / os.unsetenv(), the changes will not be reflected in it.
    ...
    def reliable_getenv(key: str) -> Union[str, None]:
        ...
        try:
            if IS_WINDOWS:
                import ctypes
                n = ctypes.windll.kernel32.GetEnvironmentVariableW(key, None, 0)
                if n == 0:
                ...
                ctypes.windll.kernel32.GetEnvironmentVariableW(key, buf, n)
                return buf.value
            else:
            ...
            print(f"reliable_getenv() error occured: {e}")
