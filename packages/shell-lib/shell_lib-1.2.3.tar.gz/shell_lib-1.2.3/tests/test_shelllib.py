...
from shell_lib.shell_lib import FS_ENCODING, Shell, PathInfo, SetEnvContextManager
...
class TestShellOtherOperations(unittest.TestCase):
    ...
    def test_get_set_env_non_ascii(self):
        sh = self.sh
        NON_ASCII_K = '环境变量shell-lib'
        NON_ASCII_V = '环境变量值'
        orig = sh.get_env(NON_ASCII_K)
        self.assertEqual(orig, os.getenv(NON_ASCII_K))
        with sh.set_env(NON_ASCII_K, NON_ASCII_V):
            self.assertEqual(sh.get_env(NON_ASCII_K), NON_ASCII_V)
            self.assertEqual(os.getenv(NON_ASCII_K), NON_ASCII_V)
        self.assertEqual(sh.get_env(NON_ASCII_K), orig)
        self.assertEqual(os.getenv(NON_ASCII_K), orig)
    ...
    def test_z3_get_filesystem_encoding(self):
