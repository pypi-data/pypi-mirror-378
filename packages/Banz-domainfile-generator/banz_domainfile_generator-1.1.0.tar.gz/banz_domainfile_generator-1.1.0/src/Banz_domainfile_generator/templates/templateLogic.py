...
def generateBaseLogicImpl(object, logicPath, logicPackage, basePackage, modelPackage):
    ...
    with open('.' + logicPath + object.package + '\\impl\\Base' + object.name + 'LogicImpl.java', 'w') as file:
        ...
        for imp in sorted([f'{modelPackage}{ref.fieldType.package if hasattr(ref.fieldType,'package') else object.package}.transfer.{ref.fieldType.target}Bto' for ref in oneToNReferences]
                          +[f'{modelPackage + object.package}.{object.name}']
                          +[f'{modelPackage + object.package}.transfer.{object.name}Bto']
                          +[f'{modelPackage + object.package}.transfer.{object.name}BtoMapper']):
            file.write(f'import {imp};\n')
        for imp in sorted([f'{logicPackage}{ref.fieldType.package if hasattr(ref.fieldType,'package') else object.package}.{ref.fieldType.target}Logic' for ref in oneToNReferences]
                           + [f'{logicPackage}{object.package}.Base{object.name}Logic']):
            file.write(f'import {imp};\n')
