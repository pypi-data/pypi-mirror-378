...
use crate::repository::{
    ...
    repo_store_vfs, spawner::RepoProcessSpawnerTemplate, RepoSpecError, RequestWithBytesChunk,
    RequestWithRepo,
};
use crate::sidecar;
use crate::streaming::{
...
impl RepositoryService for RepositoryServiceImpl {
    ...
    async fn write_ref(
        ...
        sidecar::unary!(self, request, RepositoryServiceClient, write_ref)
    }
...
impl RepositoryServiceImpl {
    ...
    #[instrument(name = "search_files_by_name", skip(self, request, metadata))]
    async fn inner_search_files_by_name(
        ...
        tracing_span_id!();
        fallback_if_feature_disabled(metadata, "rhgitaly-search-files-by-name", true)?;
        info!(
            "Processing, request={:?}",
