...
class AoA:
    ...
    @staticmethod
    def union_distinct_set(aoa: TyAoA) -> TyArr:
        """
    ...
    @staticmethod
    def union_distinct_cmpr(aoa: TyAoA) -> TyArr:
        """
        Union of all arrays of Array of Arrays to an Array of distinct elements
        using comprehension
        """
        _arr_union: TyArr = []
        for _arr in aoa:
            _arr_new: TyArr = [item for item in _arr if item not in _arr_union]
            _arr_union = _arr_union + _arr_new
        return _arr_union
