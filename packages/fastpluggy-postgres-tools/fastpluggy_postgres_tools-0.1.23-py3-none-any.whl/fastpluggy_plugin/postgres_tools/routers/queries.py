...
def get_query_performance_statistics(db_manager: Session, limit: int = 10, include_p95: bool = False):
    ...
    # Prefer new pg_stat_statements columns (Postgres 13+), fall back to legacy names
    def build_query(total_col: str, mean_col: str):
        p95_column = ""
        if include_p95:
            p95_column = f", (SELECT percentile_cont(0.95) WITHIN GROUP (ORDER BY {total_col}/GREATEST(calls,1)) FROM pg_stat_statements) AS p95_time"
        return f"""
            SELECT
              query,
              calls,
              {total_col} AS total_time,
              {mean_col} AS mean_time,
              ROUND({total_col} / GREATEST(calls, 1), 2) AS avg_time,
    ...
    query_new = build_query("total_exec_time", "mean_exec_time")
    try:
        result = execute_query(db_manager, query_new, {"limit": limit})
        return result, True
    except Exception:
        # Ensure we rollback the failed transaction before retrying with legacy columns
        try:
            db_manager.rollback()
        except Exception:
            pass
        # Fallback to legacy columns
        query_legacy = build_query("total_time", "mean_time")
        result = execute_query(db_manager, query_legacy, {"limit": limit})
        return result, True
