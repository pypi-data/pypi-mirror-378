...
class Serialization:
    ...
    def _resolve_structure(x: Any,
                        ...
                        *,
                        ...
                        _cache: dict[Any, Any] | None = None) -> Any:
        """
        Deep-resolve Resource/URI inside containers AND inside model objects' fields.
        - Uses SCHEMA to traverse your model objects
        - Updates objects in-place; returns transformed container copies
        - Caches resolutions of the same Resource/URI (by resourceId/value) to avoid duplicates
        - Cycle-safe via _seen
        """
        with hub.use(deserial_log):
            #if hub.logEnabled: log.debug(f"Resolving structure of a '%s'",type(x).__name__)
            if _seen is None:
                _seen = set()
            if _cache is None:
                _cache = {}
            ...
            if Serialization._is_reference(x):
                if hub.logEnabled: log.debug(f"Found reference with type '%s'",type(x).__name__)
                if hub.logEnabled: log.debug(f"{x}")
                key = getattr(x, "resource", None) or getattr(x, "value", None)
                
                if key in _cache:
                    if hub.logEnabled: log.debug(f"{key} found in cache")
                    return _cache[key]
                if hub.logEnabled: log.debug(f"looking up: {key} from {type(x).__name__}")
                resolved = resolver(x)
            ...
            if isinstance(x, list):
                return [Serialization._resolve_structure(v, resolver, _seen=_seen, _cache=_cache) for v in x]
            if isinstance(x, tuple):
                return tuple(Serialization._resolve_structure(v, resolver, _seen=_seen, _cache=_cache) for v in x)
            if isinstance(x, set):
                return {Serialization._resolve_structure(v, resolver, _seen=_seen, _cache=_cache) for v in x}
            if isinstance(x, dict):
                return {k: Serialization._resolve_structure(v, resolver, _seen=_seen, _cache=_cache) for k, v in x.items()}
            ...
            if isinstance(x, TypeCollection):
                ...
                for v in x:
                    ...
                    if new_coll is not None:
                        new_coll.append(nv)
                return new_coll if new_coll is not None else [Serialization._resolve_structure(v, resolver, _seen=_seen, _cache=_cache) for v in x]
            ...
            if fields:
                for fname in fields.keys():
                    if not hasattr(x, fname):
                        continue
                    #if hub.logEnabled: log.debug(f"Resolving '%s'.'%s'",type(x).__name__,fname)
                    cur = getattr(x, fname)
                    ...
                    if new is not cur:
                        try:
                            ...
                            if hub.logEnabled: log.debug(f"'%s'.'%s' did not resolve",type(x).__name__,fname)
                            # Read-only property / frozen dataclass, etc. — ignore silently or log if you prefer
                            pass
                return x

            # Anything else: leave as-is
            if hub.logEnabled: log.debug(f"Do not know how to resolve the structure of a '%s'",type(x).__name__)
            return x
