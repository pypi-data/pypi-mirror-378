...
class DataverseClient(metaclass=SingletonMeta):
    ...
    def download_tables_as_df(self):
        ...
        for table in tables["value"]:
            if "iony" in table["SchemaName"] and table["DisplayName"]["UserLocalizedLabel"]:
    ...
    # === HELPER METHODS
    def get_guid_by_casual_name(self, table_name, data) -> uuid.UUID | None:
        """
        ...
        """
        if table_name in ["iony_recipe"]:
            name_column = "iony_namecasual"
        elif table_name in ["iony_article"]:
            name_column = "iony_name"
        elif table_name in ["iony_r2rsession"]:
            name_column = "iony_sessionid"
        else:
            raise EntityError(f"Get guid not implemented for {table_name}")
    ...
    def name_to_guid(self, table_name, name=None) -> uuid.UUID | None:
        """
        ...
        """
        if table_name in ["iony_article", "iony_batch"]:
            name_column = table_name + "nr"
        elif table_name == "iony_r2rsession":
            name_column = "iony_sessionid"
        else:
            name_column = "iony_name"
        guid_column = table_name + "id"
...
class DataverseTable:
    ...
    def _generate_properties(self):
        ...
        for column_logical_name, metadata in self.col_metadata.items():
            ...
            if not column_logical_name.startswith("iony_"):
                continue
            ...
            if column_logical_name.startswith("iony_"):
                property_name = column_logical_name.split("_", 1)[1]  # Remove 'iony_' prefix
            else:
            ...
            if column_logical_name == "iony_livetaggingmetajson":
                logger.info(
                    f"Found column {column_logical_name}: AttributeType={field_type}, IsValidForCreate={metadata.get('IsValidForCreate')}, IsValidForUpdate={metadata.get('IsValidForUpdate')}"
                )
    ...
    def get_relationships(self):
        ...
        for rel in result.get("value", []):
            if (
                "iony" in rel.get("ReferencingEntity", "")
                and rel["ReferencingEntity"] == self.logical_name
            ):
        ...
        if not value and self.guid:
            try:
                ...
                for field, fresh_value in record.items():
                    if field.startswith("iony_") or field.endswith("id"):
    ...
    def write_to_dataverse(self):
        ...
        if self.logical_name in ["iony_article", "iony_recipe"]:
