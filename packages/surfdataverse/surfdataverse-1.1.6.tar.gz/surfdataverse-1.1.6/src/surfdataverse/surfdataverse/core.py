...
class DataverseClient(metaclass=SingletonMeta):
    ...
    def download_tables_as_df(self, schema_filter=None):
        ...
        Args:
            schema_filter: Optional string to filter tables by schema name (e.g., 'iony')
        """
    ...
    def get_guid_by_casual_name(self, table_name, data, name_column=None) -> uuid.UUID | None:
        ...
        if not name_column:
            ...
            if "_" in table_name:
                ...
                raise EntityError(f"Cannot infer name column for {table_name}. Please provide name_column parameter.")
...
class DataverseTable:
    ...
    def __init__(self, table_logical_name, table_prefix="iony_"):
        """
        Args:
            table_logical_name: The pluralized name of the Dataverse table
            table_prefix: The prefix used for custom table/column names (default: "iony_")
        """
        ...
        self.choices = {
            name: options for name, options in all_choices.items()
            if prefix_filter in name
        }
    ...
    def _generate_properties(self):
        ...
        for column_logical_name, metadata in self.col_metadata.items():
            ...
            if column_logical_name.startswith(self.table_prefix):
                property_name = column_logical_name.replace(self.table_prefix, "", 1)  # Remove prefix
            else:
