...
class Tape:
    ...
    def __init__(self, tape: list[Any] | str, process_as = int) -> None:
        self.tape = list(map(process_as, tape)) if type(tape) == str else tape
        self.lambdas_to = 0
    ...
    def run(self, turing: Turing, index: int) -> None:
        current = index
        iterations = 0
        turing.state = turing.initial_state
    ...
    def run_after(self, turing: Turing, offset: int = 0):
        self.run(turing, self.lambdas_from + offset)

    def run_before(self, turing: Turing, offset: int = -1):
        self.run(turing, self.lambdas_to + offset)
    ...
    def __iter__(self):
        return ''.join(map(str, self.tape)).__iter__()
