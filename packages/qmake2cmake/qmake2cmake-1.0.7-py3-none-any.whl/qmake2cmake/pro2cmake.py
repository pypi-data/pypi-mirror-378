...
def map_condition(condition: str) -> str:
    ...
    condition = re.sub(
        r"(?:contains|equals)\(QMAKE_HOST\.os\s*,\s*Linux\)",
        r"CMAKE_HOST_UNIX AND NOT CMAKE_HOST_APPLE",
        condition,
    )
    condition = re.sub(
        r"(?:contains|equals)\(QMAKE_HOST\.os\s*,\s*Windows\)", r"CMAKE_HOST_WIN32", condition
    )
    condition = re.sub(
        r"(?:contains|equals)\(QMAKE_HOST\.os\s*,\s*Darwin\)", r"CMAKE_HOST_APPLE", condition
    )
    condition = re.sub(
        r"(?:contains|equals)\(QMAKE_HOST\.arch\s*,\s*([^)]+)\)",
        r'CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "\1"',
        condition,
    )
...
def _map_libraries_to_cmake(libraries: List[str], known_libraries: Set[str]) -> List[str]:
    ...
    for lib in libraries:
        ...
        if lib.startswith("-L"):
            # handled in _map_lib_dirs_to_cmake
            continue
        elif lib.startswith("-"):
            lib = f"# Remove flag: {lib}"
        else:
...
def _map_lib_dirs_to_cmake(libraries: List[str], known_libraries: Set[str]) -> List[str]:
    result = []  # type: List[str]
    for lib in libraries:
        if lib.startswith("-L"):
            lib = lib[2:]
...
def extract_cmake_libraries(
    ...
    return (
        ...
        _map_lib_dirs_to_cmake(public_dirs, known_libraries),
        _map_lib_dirs_to_cmake(private_dirs, known_libraries),
    )
...
def extract_library_dependencies(
    ...
    (public_libs, private_libs, _, _) = extract_cmake_libraries(top_scope)
    ...
    for inner_scope in merged_scopes:
        if handling_first_scope:
            ...
            continue
        (public_libs, private_libs, _, _) = extract_cmake_libraries(inner_scope)
...
def write_main_part(
    ...
    **kwargs: Any,
    ...
    has_test_data = False
    if typename == "Test":
        cm_fh.write(f"{indent_0}if (NOT QT_BUILD_STANDALONE_TESTS AND NOT QT_BUILDING_QT)\n")
        cm_fh.write(f"{indent_1}cmake_minimum_required(VERSION 3.16)\n")
        cm_fh.write(f"{indent_1}project({name} LANGUAGES C CXX ASM)\n")
        cm_fh.write(f"{indent_1}find_package(Qt6BuildInternals COMPONENTS STANDALONE_TEST)\n")
        cm_fh.write(f"{indent_0}endif()\n\n")
            ...
            for data in test_data:
                if "*" in data:
                    cm_fh.write(
                        dedent(
                            f"""\
                        {indent_0}file(GLOB_RECURSE test_data_glob
                        {indent_1}RELATIVE ${{CMAKE_CURRENT_SOURCE_DIR}}
                        {indent_1}{data})
                        """
                        )
                    )
                    cm_fh.write(f"{indent_0}list(APPEND test_data ${{test_data_glob}})\n")
                else:
                    cm_fh.write(f'{indent_0}list(APPEND test_data "{data}")\n')
            cm_fh.write("\n")

    target_ref = name
    if typename == "Tool":
        target_ref = "${target_name}"
        cm_fh.write(f"{indent_0}qt_get_tool_target_name(target_name {name})\n")
    ...
    cm_fh.write(f"{indent_0}{cmake_function}({target_ref}\n")
    for extra_line in extra_lines:
        cm_fh.write(f"{indent_0}    {extra_line}\n")

    write_sources_section(cm_fh, scopes[0], indent=indent, **kwargs)

    if has_test_data:
        cm_fh.write(f"{indent_0}    TESTDATA ${{test_data}}\n")
    # Footer:
    cm_fh.write(f"{indent_0})\n")

    if typename == "Tool":
        cm_fh.write(f"{indent_0}qt_internal_return_unless_building_tools()\n")
    ...
    ignored_keys_report = write_ignored_keys(scopes[0], indent_0)
    ...
    for c in scopes[1:]:
        ...
        ignored_keys_report = write_ignored_keys(c, indent_0)
        if ignored_keys_report:
            cm_fh.write(ignored_keys_report)
...
def write_binary(cm_fh: IO[str], scope: Scope, gui: bool = False, *, indent: int = 0) -> str:
    ...
    extra_keys: List[str] = ["target.path", "INSTALLS"]
...
def write_jar(cm_fh: IO[str], scope: Scope, *, indent: int = 0) -> str:
    ...
    indent_0 = spaces(indent)
    indent_1 = spaces(indent + 1)
    cm_fh.write(f"{indent_0}qt_internal_add_jar({target}\n")
    cm_fh.write(f"{indent_1}INCLUDE_JARS {android_sdk_jar}\n")
    cm_fh.write(f"{indent_1}SOURCES ${{java_sources}}\n")
    cm_fh.write(f'{indent_1}OUTPUT_DIR "${{QT_BUILD_DIR}}/{install_dir}"\n')
    cm_fh.write(f"{indent_0})\n\n")

    cm_fh.write(f"{indent_0}install_jar({target}\n")
    cm_fh.write(f"{indent_1}DESTINATION {install_dir}\n")
    cm_fh.write(f"{indent_1}COMPONENT Devel\n")
    cm_fh.write(f"{indent_0})\n\n")
...
"""
    ...
    for scope in scopes:
        ...
        (
            scope_public_libs,
            scope_private_libs,
            scope_public_lib_dirs,
            scope_private_lib_dirs,
        ) = extract_cmake_libraries(scope)
        ...
        if prefer_private_visibility:
            ...
            scope_private_lib_dirs = scope_public_lib_dirs + scope_private_lib_dirs
            scope_public_lib_dirs.clear()
        ...
        write_list(
            ...
            "",
            indent=indent,
            header=f"target_link_directories({binary_name} PRIVATE\n",
            footer=")\n",
        )
        write_list(
            io_string,
...
def main(command_line_args: Optional[List[str]] = None) -> None:
    ...
    for file in args.files:
        ...
        if os.path.isdir(file):
            print(
                f"Error: '{file}' is a directory. Please provide a .pro file path.", file=sys.stderr
            )
            sys.exit(1)
