...
class QmakeParser:
    ...
    def _generate_grammar(self):
        ...
        def process_escape_sequences(s, loc, tokens):
            """Process escape sequences in unquoted values"""
            result = tokens[0].replace("\\\\", "\\")  # Double backslash -> single backslash
            result = result.replace("\\'", "'")  # Backslash + single quote -> single quote
            result = result.replace('\\"', '"')  # Backslash + double quote -> double quote
            return result

        LiteralValuePart = add_element(
            "LiteralValuePart",
            pp.Word(pp.unicode.printables, excludeChars="$#()").setParseAction(
    ...
    def parseFile(self, file: str) -> Tuple[pp.ParseResults, str]:
        ...
        with open(file, "r", errors="replace") as file_fd:
