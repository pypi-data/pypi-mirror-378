...
def test_linker_flags():
    '''Test the conversion of LIBS assignments to target_link_libraries calls.'''
    output = convert("linker_flags")

    assert("target_link_libraries(linker_flags PRIVATE" in output)

    lines = output.split('\n')
    in_target_link_libraries = False
    libs_content = []

    for line in lines:
        if "target_link_libraries(linker_flags PRIVATE" in line:
            in_target_link_libraries = True
            continue
        elif in_target_link_libraries and line.strip() == ')':
            break
        elif in_target_link_libraries:
            libs_content.append(line.strip())

    libs_text = ' '.join(libs_content)
    assert("foo" in libs_text)
    assert("bar" in libs_text)

    assert("target_link_directories(linker_flags PRIVATE" in output)

    in_target_link_directories = False
    dirs_content = []

    for line in lines:
        if "target_link_directories(linker_flags PRIVATE" in line:
            in_target_link_directories = True
            continue
        elif in_target_link_directories and line.strip() == ')':
            break
        elif in_target_link_directories:
            dirs_content.append(line.strip())

    dirs_text = ' '.join(dirs_content)
    assert("bardir" in dirs_text)
    assert("foodir" in dirs_text)


def test_qmake_host_os_conditions():
    output = convert("qmake_host")

    assert("if(CMAKE_HOST_UNIX AND NOT CMAKE_HOST_APPLE)" in output)
    assert("if(CMAKE_HOST_WIN32)" in output)
    assert("if(CMAKE_HOST_APPLE)" in output)
    assert('if(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "x86_64")' in output)
    assert('if(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "arm64")' in output)
    ...
    for line in lines:
        line = line.strip()
        if "if(CMAKE_HOST_UNIX AND NOT CMAKE_HOST_APPLE)" in line:
            state = ConditionState.LINUX
        elif "if(CMAKE_HOST_WIN32)" in line:
            state = ConditionState.WINDOWS
        elif "if(CMAKE_HOST_APPLE)" in line:
            state = ConditionState.DARWIN
        elif 'if(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "x86_64")' in line:
            state = ConditionState.X86_64
        elif 'if(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "arm64")' in line:
            state = ConditionState.ARM64
        elif "endif()" in line:
            state = ConditionState.NONE
        elif "linux_host.cpp" in line:
            assert(state == ConditionState.LINUX)
        elif "windows_host.cpp" in line or "windows_host_contains.cpp" in line:
            assert(state == ConditionState.WINDOWS)
        elif "darwin_host.cpp" in line or "darwin_host_contains.cpp" in line:
            assert(state == ConditionState.DARWIN)
        elif "x86_64_host.cpp" in line:
            assert(state == ConditionState.X86_64)
        elif "arm64_host.cpp" in line:
            assert(state == ConditionState.ARM64)
