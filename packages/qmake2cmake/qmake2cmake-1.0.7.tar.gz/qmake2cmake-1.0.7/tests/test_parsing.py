...
from qmake2cmake.qmake_parser import get_qmake_parser
...
def test_quoted_string():
    result = parse_file(_tests_path + '/data/quoted_string.pro')
    assert len(result) == 2
    validate_op('X', '=', ['foo \'a\' "b" bar', 'bla'], result[0])
    validate_op('Y', '=', ['foo \'a\' "b" bar', 'bla'], result[1])


def test_escape_sequences():
    result = parse_file(_tests_path + '/data/escape_sequences.pro')
    assert len(result) == 3
    validate_op('SINGLE_QUOTE', '=', ["'"], result[0])
    validate_op('DOUBLE_QUOTE', '=', ['"'], result[1])
    validate_op('ESCAPED_BACKSLASH', '=', ["\\"], result[2])
...
def test_assignment_with_braces():
    result = parse_file(_tests_path + '/data/assignment_with_braces.pro')
    assert len(result) == 5

    validate_op('A', '=', ['{}', 'foo{bar}'], result[0])
    validate_op('B', '=', ['{', '}'], result[1])
    validate_op('C', '=', ['{', '{', '{', '}'], result[2])

    assert 'condition' in result[3]
    assert result[3]['condition'] == 'true'
    true_statements = result[3]['statements']
    assert len(true_statements) == 3

    validate_op('D', '=', ['{', 'foo', '}'], true_statements[0])
    validate_op('E', '=', ['{', 'one', '{', 'two', '}', 'three', '}'], true_statements[1])
    validate_op('F', '=', ['zero', '{', 'one', '{', 'two', '}', 'three', '}', 'four'], true_statements[2])
