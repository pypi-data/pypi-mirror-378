...
class DatasetItemClient:
    """Class for managing dataset items in Barcable.
    ...
    Args:
        ...
        Barcable (Barcable): Instance of Barcable client for API interactions.
    ...
    Example:
        ...
        dataset = Barcable.get_dataset("<dataset_name>")
    ...
    def __init__(self, dataset_item: DatasetItem, Barcable: "Barcable"):
    ...
    def run(
        ...
        *,
    ...
    ) -> Generator[BarcableSpan, None, None]:
        """Create a context manager for the dataset item run that links the execution to a Barcable trace.
        ...
        Yields:
            span: A BarcableSpan that can be used to trace the execution of the run.
        """
        trace_name = f"Dataset run: {run_name}"

        with self.Barcable.start_as_current_span(name=trace_name) as span:
            span.update_trace(
                name=trace_name,
                metadata={
            ...
            self.Barcable.api.dataset_run_items.create(
...
class DatasetClient:
    """Class for managing datasets in Barcable.
    ...
    def __init__(self, dataset: Dataset, items: List[DatasetItemClient]):
        ...
        self.items = items
        self._barcable: Optional["Barcable"] = None

    def _get_Barcable_client(self) -> Optional["Barcable"]:
        """Get the Barcable client from the first item."""
        if self._barcable is None and self.items:
            self._barcable = self.items[0].Barcable
        return self._barcable
    ...
    def run_experiment(
        ...
        *,
        ...
        """Run an experiment on this Barcable dataset with automatic tracking.

        This is a convenience method that runs an experiment using all items in this
        dataset. It automatically creates a dataset run in Barcable for tracking and
        comparison purposes, linking all experiment results to the dataset.

        Key benefits of using dataset.run_experiment():
        - Automatic dataset run creation and linking in Barcable UI
        ...
        - Automatic URL generation for viewing results in Barcable dashboard

        Args:
            name: Human-readable name for the experiment run. This will be used as
                the dataset run name in Barcable for tracking and identification.
            run_name: Optional exact name for the dataset run. If provided, this will be
                used as the exact dataset run name in Barcable. If not provided, this will
            ...
            description: Optional description of the experiment's purpose, methodology,
                or what you're testing. Appears in the Barcable UI for context.
        ...
        Returns:
            ...
            - dataset_run_id: ID of the created dataset run in Barcable.
            - dataset_run_url: Direct URL to view the experiment results in Barcable UI.
        ...
        Raises:
            ValueError: If the dataset has no items or no Barcable client is available.

        Examples:
            Basic dataset experiment:
            ```python
            dataset = Barcable.get_dataset("qa-evaluation-set")
            ...
            ```python
            dataset = Barcable.get_dataset("content-generation-benchmark")
            ...
            # Results are automatically linked to dataset in Barcable
            print(f"Experiment completed! View in Barcable: {result['dataset_run_url']}")
            ...
            dataset = Barcable.get_dataset("model-benchmark")
            ...
            # Both experiments are now visible in Barcable for easy comparison
            print("Compare results in Barcable:")
            print(f"GPT-4: {result_gpt4.dataset_run_url}")
            print(f"Custom: {result_custom.dataset_run_url}")
            ```

        Note:
            - All experiment results are automatically tracked in Barcable as dataset runs
        ...
        """
        Barcable_client = self._get_Barcable_client()
        if not Barcable_client:
            raise ValueError("No Barcable client available. Dataset items are empty.")

        return Barcable_client.run_experiment(
            name=name,
            run_name=run_name,
