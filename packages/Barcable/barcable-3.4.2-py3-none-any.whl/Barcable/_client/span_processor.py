"""Span processor for Barcable OpenTelemetry integration.

This module defines the BarcableSpanProcessor class, which extends OpenTelemetry's
BatchSpanProcessor with Barcable-specific functionality. It handles exporting
spans to the Barcable API with proper authentication and filtering.

Key features:
- HTTP-based span export to Barcable API
- Basic authentication with Barcable API keys
...
from Barcable._client.environment_variables import (
    BARCABLE_FLUSH_AT,
    BARCABLE_FLUSH_INTERVAL,
    BARCABLE_OTEL_TRACES_EXPORT_PATH,
)
from Barcable._client.utils import span_formatter
from Barcable.logger import Barcable_logger
from Barcable.version import __version__ as Barcable_version


class BarcableSpanProcessor(BatchSpanProcessor):
    """OpenTelemetry span processor that exports spans to the Barcable API.

    This processor extends OpenTelemetry's BatchSpanProcessor with Barcable-specific functionality:
    ...
    4. HTTP-based span export to the Barcable OTLP endpoint
    5. Debug logging for span processing operations
    6. Authentication with Barcable API using Basic Auth
    ...
    It integrates with OpenTelemetry's standard span lifecycle, adding Barcable-specific
    ...
    def __init__(
        ...
        *,
        ...
        env_flush_at = os.environ.get(BARCABLE_FLUSH_AT, None)
        flush_at = flush_at or int(env_flush_at) if env_flush_at is not None else None

        env_flush_interval = os.environ.get(BARCABLE_FLUSH_INTERVAL, None)
        flush_interval = (
        ...
        default_headers = {
            ...
            "x_Barcable_public_key": public_key,
        }

        # Merge additional headers if provided
        headers = {**default_headers, **(additional_headers or {})}

        traces_export_path = os.environ.get(BARCABLE_OTEL_TRACES_EXPORT_PATH, None)
        ...
        Barcable_span_exporter = OTLPSpanExporter(
            endpoint=endpoint,
            headers=headers,
            timeout=timeout,
        )
        ...
        super().__init__(
            span_exporter=Barcable_span_exporter,
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        if self._is_Barcable_span(span) and not self._is_Barcable_project_span(span):
    ...
    @staticmethod
    def _is_Barcable_span(span: ReadableSpan) -> bool:
        return (
            span.instrumentation_scope is not None
            and span.instrumentation_scope.name == BARCABLE_TRACER_NAME
        )
    ...
    def _is_Barcable_project_span(self, span: ReadableSpan) -> bool:
        if not BarcableSpanProcessor._is_Barcable_span(span):
            return False
