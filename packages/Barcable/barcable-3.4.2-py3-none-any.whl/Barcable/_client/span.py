"""OTEL span wrapper for Barcable.

This module defines custom span classes that extend OpenTelemetry spans with
Barcable-specific functionality. These wrapper classes provide methods for
creating, updating, and scoring various types of spans used in AI application tracing.

Classes:
- BarcableObservationWrapper: Abstract base class for all Barcable spans
- BarcableSpan: Implementation for general-purpose spans
- BarcableGeneration: Specialized span implementation for LLM generations

All span classes provide methods for media processing, attribute management,
and scoring integration specific to Barcable's observability platform.
"""
...
from Barcable._client.attributes import (
...
from Barcable.types import MapValue, ScoreDataType, SpanLevel
...
_OBSERVATION_CLASS_MAP: Dict[str, Type["BarcableObservationWrapper"]] = {}


class BarcableObservationWrapper:
    """Abstract base class for all Barcable span types.

    This class provides common functionality for all Barcable span types, including
    media processing, attribute management, and scoring. It wraps an OpenTelemetry
    span and extends it with Barcable-specific features.

    Attributes:
        _otel_span: The underlying OpenTelemetry span
        _Barcable_client: Reference to the parent Barcable client
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new Barcable span wrapper.

        Args:
            otel_span: The OpenTelemetry span to wrap
            Barcable_client: Reference to the parent Barcable client
        ...
        self._otel_span.set_attribute(
            BarcableOtelSpanAttributes.OBSERVATION_TYPE, as_type
        )
        self._Barcable_client = Barcable_client
        self._observation_type = as_type

        self.trace_id = self._Barcable_client._get_otel_trace_id(otel_span)
        self.id = self._Barcable_client._get_otel_span_id(otel_span)

        self._environment = environment or self._Barcable_client._environment
        if self._environment is not None:
            self._otel_span.set_attribute(
                BarcableOtelSpanAttributes.ENVIRONMENT, self._environment
            )
        ...
        if self._otel_span.is_recording():
            ...
            attributes.pop(BarcableOtelSpanAttributes.OBSERVATION_TYPE, None)

            self._otel_span.set_attributes(
                {k: v for k, v in attributes.items() if v is not None}
            )

    def end(self, *, end_time: Optional[int] = None) -> "BarcableObservationWrapper":
    ...
    def score(
        ...
        *,
        ...
        Example:
            ...
            ```
        """
        self._Barcable_client.create_score(
            name=name,
    ...
    def _mask_attribute(self, *, data: Any) -> Any:
        ...
        """
        if not self._Barcable_client._mask:
            return data

        try:
            return self._Barcable_client._mask(data=data)
    ...
    def _process_media_in_attribute(
        ...
        *,
        ...
        """
        if self._Barcable_client._resources is not None:
            return (
                self._Barcable_client._resources._media_manager._find_and_process_media(
                    data=data,
                    field=field,
                    trace_id=self.trace_id,
                    observation_id=self.id,
    ...
    def start_observation(
        ...
        *,
        ...
        if as_type == "event":
            timestamp = time_ns()
            event_span = self._Barcable_client._otel_tracer.start_span(
            ...
            return cast(
                ...
                BarcableEvent(
                    ...
                    Barcable_client=self._Barcable_client,
                    input=input,
                    output=output,
                    metadata=metadata,
        ...
        if not observation_class:
            Barcable_logger.warning(
                f"Unknown observation type: {as_type}, falling back to BarcableSpan"
            )
            observation_class = BarcableSpan

        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._Barcable_client._otel_tracer.start_span(name=name)
    ...
    def start_as_current_observation(  # type: ignore[misc]
        ...
        *,
        ...
        """
        return self._Barcable_client._create_span_with_parent_context(
            name=name,
            as_type=as_type,
...
class BarcableSpan(BarcableObservationWrapper):
    """Standard span implementation for general operations in Barcable.

    This class represents a general-purpose span that can be used to trace
    any operation in your application. It extends the base BarcableObservationWrapper
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            otel_span=otel_span,
            as_type="span",
            Barcable_client=Barcable_client,
            input=input,
            output=output,
            metadata=metadata,
    ...
    def start_span(
        ...
        Returns:
            A new BarcableSpan that must be ended with .end() when complete

        Example:
            ```python
            parent_span = Barcable.start_span(name="process-request")
            try:
                # Create a child span
    ...
    def start_as_current_span(
        ...
        *,
        ...
        Returns:
            A context manager that yields a new BarcableSpan

        Example:
            ```python
            with Barcable.start_as_current_span(name="process-request") as parent_span:
    ...
    def start_generation(
        ...
        *,
        ...
        Returns:
            A new BarcableGeneration that must be ended with .end() when complete

        Example:
            ```python
            span = Barcable.start_span(name="process-query")
            try:
    ...
    def start_as_current_generation(
        ...
        *,
        ...
        Returns:
            A context manager that yields a new BarcableGeneration
    ...
    def create_event(
        ...
        *,
        ...
        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._Barcable_client._otel_tracer.start_span(
                name=name, start_time=timestamp
            )
...
class BarcableGeneration(BarcableObservationWrapper):
    """Specialized span implementation for AI model generations in Barcable.

    This class represents a generation span specifically designed for tracking
    AI/LLM operations. It extends the base BarcableObservationWrapper with specialized
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new BarcableGeneration span.
...
class BarcableEvent(BarcableObservationWrapper):
    """Specialized span implementation for Barcable Events."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new BarcableEvent span.
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        """Update is not allowed for BarcableEvent because events cannot be updated.

        This method logs a warning and returns self without making changes.

        Returns:
            self: Returns the unchanged BarcableEvent instance
        """
        Barcable_logger.warning(
            "Attempted to update BarcableEvent observation. Events cannot be updated after creation."
        )
        return self


class BarcableAgent(BarcableObservationWrapper):
    """Agent observation for reasoning blocks that act on tools using LLM guidance."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableAgent span."""
        kwargs["as_type"] = "agent"
        super().__init__(**kwargs)


class BarcableTool(BarcableObservationWrapper):
    """Tool observation representing external tool calls, e.g., calling a weather API."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableTool span."""
        kwargs["as_type"] = "tool"
        super().__init__(**kwargs)


class BarcableChain(BarcableObservationWrapper):
    """Chain observation for connecting LLM application steps, e.g. passing context from retriever to LLM."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableChain span."""
        kwargs["as_type"] = "chain"
        super().__init__(**kwargs)


class BarcableRetriever(BarcableObservationWrapper):
    """Retriever observation for data retrieval steps, e.g. vector store or database queries."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableRetriever span."""
        kwargs["as_type"] = "retriever"
        super().__init__(**kwargs)


class BarcableEmbedding(BarcableObservationWrapper):
    """Embedding observation for LLM embedding calls, typically used before retrieval."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableEmbedding span."""
        kwargs["as_type"] = "embedding"
        super().__init__(**kwargs)


class BarcableEvaluator(BarcableObservationWrapper):
    """Evaluator observation for assessing relevance, correctness, or helpfulness of LLM outputs."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableEvaluator span."""
        kwargs["as_type"] = "evaluator"
        super().__init__(**kwargs)


class BarcableGuardrail(BarcableObservationWrapper):
    """Guardrail observation for protection e.g. against jailbreaks or offensive content."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new BarcableGuardrail span."""
...
_OBSERVATION_CLASS_MAP.update(
    {
        ...
        "tool": BarcableTool,
        "chain": BarcableChain,
        "retriever": BarcableRetriever,
        "evaluator": BarcableEvaluator,
        "embedding": BarcableEmbedding,
        "guardrail": BarcableGuardrail,
    }
)
