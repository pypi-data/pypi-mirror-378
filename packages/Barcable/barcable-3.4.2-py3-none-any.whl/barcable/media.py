"""This module contains the BarcableMedia class, which is used to wrap media objects for upload to Barcable."""
...
from Barcable.api import MediaContentType
from Barcable.types import ParsedMediaReference

T = TypeVar("T")


class BarcableMedia:
    """A class for wrapping media objects for upload to Barcable.

    This class handles the preparation and formatting of media content for Barcable,
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a BarcableMedia object.
        ...
        if base64_data_uri is not None:
            ...
            self._log.error(
                "base64_data_uri, or content_bytes and content_type, or file_path must be provided to BarcableMedia"
            )
    ...
    def _reference_string(self) -> Optional[str]:
        ...
        return f"@@@BarcableMedia:type={self._content_type}|id={self._media_id}|source={self._source}@@@"
    ...
    def parse_reference_string(reference_string: str) -> ParsedMediaReference:
        ...
        Example reference string:
            "@@@BarcableMedia:type=image/jpeg|id=some-uuid|source=base64_data_uri@@@"
        ...
        Raises:
            ...
            ValueError: If the reference string does not start with "@@@BarcableMedia:type=".
        ...
        if not reference_string.startswith("@@@BarcableMedia:type="):
            raise ValueError(
                "Reference string does not start with '@@@BarcableMedia:type='"
            )

        if not reference_string.endswith("@@@"):
            raise ValueError("Reference string does not end with '@@@'")

        content = reference_string[len("@@@BarcableMedia:") :].rstrip("@@@")
    ...
    def resolve_media_references(
        *,
        ...
        Args:
            ...
            Barcable_client: Barcable client instance used to fetch media content.
        ...
        def traverse(obj: Any, depth: int) -> Any:
            ...
            if isinstance(obj, str):
                ...
                for reference_string in reference_string_matches:
                    try:
                        parsed_media_reference = BarcableMedia.parse_reference_string(
                            reference_string
                        )
                        media_data = Barcable_client.api.media.get(
