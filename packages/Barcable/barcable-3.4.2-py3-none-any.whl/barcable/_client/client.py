"""Barcable OpenTelemetry integration module.

This module implements Barcable's core observability functionality on top of the OpenTelemetry (OTel) standard.
"""
...
from Barcable._client.attributes import BarcableOtelSpanAttributes
...
from Barcable._client.datasets import DatasetClient, DatasetItemClient
from Barcable._client.environment_variables import (
    BARCABLE_DEBUG,
    BARCABLE_HOST,
    BARCABLE_OTEL_TRACES_EXPORT_PATH,
    BARCABLE_PUBLIC_KEY,
    BARCABLE_SAMPLE_RATE,
    BARCABLE_SECRET_KEY,
    BARCABLE_TIMEOUT,
    BARCABLE_TRACING_ENABLED,
    BARCABLE_TRACING_ENVIRONMENT,
)
from Barcable._client.resource_manager import BarcableResourceManager
from Barcable._client.span import (
    BarcableAgent,
    BarcableChain,
    BarcableEmbedding,
    BarcableEvaluator,
...
from Barcable._client.utils import run_async_safely
from Barcable._utils import _get_timestamp
from Barcable._utils.parse_error import handle_fern_exception
from Barcable._utils.prompt_cache import PromptCache
from Barcable.api.resources.commons.errors.error import Error
from Barcable.api.resources.ingestion.types.score_body import ScoreBody
from Barcable.api.resources.prompts.types import (
    CreatePromptRequest_Chat,
    CreatePromptRequest_Text,
    Prompt_Chat,
    Prompt_Text,
)
from Barcable.experiment import (
    Evaluation,
    EvaluatorFunction,
    ExperimentData,
    ExperimentItem,
...
from Barcable.media import BarcableMedia
from Barcable.model import (
    ChatMessageDict,
...
from Barcable.types import MaskFunction, ScoreDataType, SpanLevel, TraceContext


class Barcable:
    """Main client for Barcable tracing and platform features.

    This class provides an interface for creating and managing traces, spans,
    and generations in Barcable as well as interacting with the Barcable API.
    ...
    Attributes:
        api: Synchronous API client for Barcable backend communication
        async_api: Asynchronous API client for Barcable backend communication
        _otel_tracer: Internal BarcableTracer instance managing OpenTelemetry components

    Parameters:
        public_key (Optional[str]): Your Barcable public API key. Can also be set via BARCABLE_PUBLIC_KEY environment variable.
        secret_key (Optional[str]): Your Barcable secret API key. Can also be set via BARCABLE_SECRET_KEY environment variable.
        host (Optional[str]): The Barcable API host URL. Defaults to "https://cloud.barcable.com". Can also be set via BARCABLE_HOST environment variable.
        timeout (Optional[int]): Timeout in seconds for API requests. Defaults to 5 seconds.
        httpx_client (Optional[httpx.Client]): Custom httpx client for making non-tracing HTTP requests. If not provided, a default client will be created.
        debug (bool): Enable debug logging. Defaults to False. Can also be set via BARCABLE_DEBUG environment variable.
        tracing_enabled (Optional[bool]): Enable or disable tracing. Defaults to True. Can also be set via BARCABLE_TRACING_ENABLED environment variable.
        flush_at (Optional[int]): Number of spans to batch before sending to the API. Defaults to 512. Can also be set via BARCABLE_FLUSH_AT environment variable.
        flush_interval (Optional[float]): Time in seconds between batch flushes. Defaults to 5 seconds. Can also be set via BARCABLE_FLUSH_INTERVAL environment variable.
        environment (Optional[str]): Environment name for tracing. Default is 'default'. Can also be set via BARCABLE_TRACING_ENVIRONMENT environment variable. Can be any lowercase alphanumeric string with hyphens and underscores that does not start with 'barcable'.
        release (Optional[str]): Release version/hash of your application. Used for grouping analytics by release.
        media_upload_thread_count (Optional[int]): Number of background threads for handling media uploads. Defaults to 1. Can also be set via BARCABLE_MEDIA_UPLOAD_THREAD_COUNT environment variable.
        sample_rate (Optional[float]): Sampling rate for traces (0.0 to 1.0). Defaults to 1.0 (100% of traces are sampled). Can also be set via BARCABLE_SAMPLE_RATE environment variable.
        mask (Optional[MaskFunction]): Function to mask sensitive data in traces before sending to the API.
        blocked_instrumentation_scopes (Optional[List[str]]): List of instrumentation scope names to block from being exported to Barcable. Spans from these scopes will be filtered out before being sent to the API. Useful for filtering out spans from specific libraries or frameworks. For exported spans, you can see the instrumentation scope name in the span metadata in Barcable (`metadata.scope.name`)
        ...
        tracer_provider(Optional[TracerProvider]): OpenTelemetry TracerProvider to use for Barcable. This can be useful to set to have disconnected tracing between Barcable and other OpenTelemetry-span emitting libraries. Note: To track active spans, the context is still shared between TracerProviders. This may lead to broken trace trees.

    Example:
        ```python
        from Barcable.otel import Barcable

        # Initialize the client (reads from env vars if not provided)
        barcable = Barcable(
            public_key="your-public-key",
            secret_key="your-secret-key",
            host="https://cloud.barcable.com",  # Optional, default shown
        )

        # Create a trace span
        with barcable.start_as_current_span(name="process-query") as span:
    ...
    _resources: Optional[BarcableResourceManager] = None
    ...
    def __init__(
        ...
        *,
        ...
        self._host = host or cast(
            str, os.environ.get(BARCABLE_HOST, "https://cloud.barcable.com")
        )
        self._environment = environment or cast(
            str, os.environ.get(BARCABLE_TRACING_ENVIRONMENT)
        )
        self._project_id: Optional[str] = None
        sample_rate = sample_rate or float(os.environ.get(BARCABLE_SAMPLE_RATE, 1.0))
        if not 0.0 <= sample_rate <= 1.0:
            raise ValueError(
                f"Sample rate must be between 0.0 and 1.0, got {sample_rate}"
            )

        timeout = timeout or int(os.environ.get(BARCABLE_TIMEOUT, 5))

        self._tracing_enabled = (
            tracing_enabled
            and os.environ.get(BARCABLE_TRACING_ENABLED, "true").lower() != "false"
        )
        if not self._tracing_enabled:
            barcable_logger.info(
                "Configuration: Barcable tracing is explicitly disabled. No data will be sent to the Barcable API."
            )

        debug = (
            debug if debug else (os.getenv(BARCABLE_DEBUG, "false").lower() == "true")
        )
        if debug:
            logging.basicConfig(
                format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
            )
            barcable_logger.setLevel(logging.DEBUG)

        public_key = public_key or os.environ.get(BARCABLE_PUBLIC_KEY)
        if public_key is None:
            barcable_logger.warning(
                "Authentication error: Barcable client initialized without public_key. Client will be disabled. "
                "Provide a public_key parameter or set BARCABLE_PUBLIC_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return

        secret_key = secret_key or os.environ.get(BARCABLE_SECRET_KEY)
        if secret_key is None:
            barcable_logger.warning(
                "Authentication error: Barcable client initialized without secret_key. Client will be disabled. "
                "Provide a secret_key parameter or set BARCABLE_SECRET_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return
        ...
        if os.environ.get("OTEL_SDK_DISABLED", "false").lower() == "true":
            barcable_logger.warning(
                "OTEL_SDK_DISABLED is set. Barcable tracing will be disabled and no traces will appear in the UI."
            )

        # Initialize api and tracer if requirements are met
        self._resources = BarcableResourceManager(
    ...
    def start_span(
        ...
        *,
        ...
        Returns:
            A BarcableSpan object that must be ended with .end() when the operation completes

        Example:
            ```python
            span = barcable.start_span(name="process-data")
            try:
                # Do work
    ...
    def start_as_current_span(
        ...
        *,
    ...
    ) -> _AgnosticContextManager[BarcableSpan]:
        ...
        Returns:
            A context manager that yields a BarcableSpan
        ...
        if trace_context:
            ...
            if trace_id:
                ...
                with otel_trace_api.use_span(
                    ...
                    otel_span.set_attribute(BarcableOtelSpanAttributes.AS_ROOT, True)
    ...
    def _create_observation_from_otel_span(
        ...
        *,
        ...
        if as_type in get_observation_types_list(ObservationTypeGenerationLike):
            ...
            # issue is that BarcableEvent could be returned and that classes have diff. args
            return observation_class(  # type: ignore[return-value,call-arg]
                otel_span=otel_span,
                barcable_client=self,
                environment=self._environment,
                input=input,
                output=output,
                metadata=metadata,
            ...
            # span._otel_span.set_attribute("barcable.observation.type", as_type)
            # return span
    ...
    def start_generation(
        ...
        *,
        ...
        Args:
            ...
            prompt: Associated prompt template from Barcable prompt management

        Returns:
            A BarcableGeneration object that must be ended with .end() when complete

        Example:
            ```python
            generation = Barcable.start_generation(
    ...
    def start_as_current_generation(
        ...
        *,
        ...
        prompt: Optional[PromptClient] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableGeneration]:
        ...
        Returns:
            A context manager that yields a BarcableGeneration

        Example:
            ```python
            with Barcable.start_as_current_generation(
                name="answer-generation",
                model="gpt-4",
                input={"prompt": "Explain quantum computing"}
            ) as generation:
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableAgent]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableTool]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableChain]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableRetriever]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableEvaluator]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        prompt: Optional[PromptClient] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableEmbedding]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[BarcableGuardrail]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        Example:
            ```python
            # Create a span
            with Barcable.start_as_current_observation(name="process-query", as_type="span") as span:
                # Do work
                result = process_data()
                span.update(output=result)
            ...
            with Barcable.start_as_current_observation(name="web-search", as_type="tool") as tool:
                # Do tool work
                results = search_web(query)
                tool.update(output=results)

            # Create a generation observation
            with Barcable.start_as_current_observation(
                name="answer-generation",
                as_type="generation",
                model="gpt-4"
            ) as generation:
    ...
    def _get_span_class(
        ...
        Type[BarcableTool],
        Type[BarcableChain],
        Type[BarcableRetriever],
        Type[BarcableEvaluator],
        Type[BarcableEmbedding],
        Type[BarcableGuardrail],
        Type[BarcableGeneration],
        Type[BarcableEvent],
        Type[BarcableSpan],
    ]:
        ...
        with otel_trace_api.use_span(parent_span):
            with self._start_as_current_otel_span_with_processed_media(
                ...
                if remote_parent_span is not None:
                    Barcable_span._otel_span.set_attribute(
                        BarcableOtelSpanAttributes.AS_ROOT, True
                    )
    ...
    def update_current_generation(
        ...
        *,
        ...
        Example:
            ```python
            with Barcable.start_as_current_generation(name="answer-query") as generation:
                # Initial setup and API call
                response = llm.generate(...)

                # Update with results that weren't available at creation time
                Barcable.update_current_generation(
                    output=response.text,
                    usage_details={
                        "prompt_tokens": response.usage.prompt_tokens,
                        "completion_tokens": response.usage.completion_tokens
        ...
        if current_otel_span is not None:
            generation = BarcableGeneration(
                otel_span=current_otel_span, Barcable_client=self
            )
    ...
    def update_current_span(
        ...
        *,
        ...
        Example:
            ```python
            with Barcable.start_as_current_span(name="process-data") as span:
                # Initial processing
                result = process_first_part()

                # Update with intermediate results
                Barcable.update_current_span(metadata={"intermediate_result": result})

                # Continue processing
                final_result = process_second_part(result)

                # Final update
                Barcable.update_current_span(output=final_result)
            ```
        """
        if not self._tracing_enabled:
            Barcable_logger.debug(
    ...
    def update_current_trace(
        ...
        *,
        ...
        This method updates the Barcable trace that the current span belongs to. It's useful for
        adding trace-level metadata like user ID, session ID, or tags that apply to
        the entire Barcable trace rather than just a single observation.

        Args:
            name: Updated name for the Barcable trace
            user_id: ID of the user who initiated the Barcable trace
            session_id: Session identifier for grouping related Barcable traces
            version: Version identifier for the application or service
            input: Input data for the overall Barcable trace
            output: Output data from the overall Barcable trace
            metadata: Additional metadata to associate with the Barcable trace
            tags: List of tags to categorize the Barcable trace
            public: Whether the Barcable trace should be publicly accessible

        Example:
            ```python
            with Barcable.start_as_current_span(name="handle-request") as span:
                # Get user information
                user = authenticate_user(request)

                # Update trace with user context
                Barcable.update_current_trace(
                    user_id=user.id,
        ...
        if current_otel_span is not None:
            existing_observation_type = current_otel_span.attributes.get(  # type: ignore[attr-defined]
                BarcableOtelSpanAttributes.OBSERVATION_TYPE, "span"
            )
    ...
    def create_event(
        ...
        *,
        ...
        """Create a new Barcable observation of type 'EVENT'.

        The created Barcable Event observation will be the child of the current span in the context.
        ...
        Returns:
            The Barcable Event object

        Example:
            ```python
            event = Barcable.create_event(name="process-event")
            ```
        """
        timestamp = time_ns()
    ...
    def _create_remote_parent_span(
        ...
        if not self._is_valid_trace_id(trace_id):
            Barcable_logger.warning(
                f"Passed trace ID '{trace_id}' is not a valid 32 lowercase hex char Barcable trace id. Ignoring trace ID."
            )

        if parent_span_id and not self._is_valid_span_id(parent_span_id):
            Barcable_logger.warning(
                f"Passed span ID '{parent_span_id}' is not a valid 16 lowercase hex char Barcable span id. Ignoring parent span ID."
            )
    ...
    def _create_observation_id(self, *, seed: Optional[str] = None) -> str:
        """Create a unique observation ID for use with Barcable.

        This method generates a unique observation ID (span ID in OpenTelemetry terms)
        for use with various Barcable APIs. It can either generate a random ID or
        ...
        correlate an external ID with a Barcable observation ID, use the external ID as
        ...
        Example:
            ...
            obs_id = Barcable.create_observation_id()

            # Generate a deterministic ID based on a seed
            user_obs_id = Barcable.create_observation_id(seed="user-123-feedback")

            # Correlate an external item ID with a Barcable observation ID
            item_id = "item-789012"
            correlated_obs_id = Barcable.create_observation_id(seed=item_id)

            # Use the ID with Barcable APIs
            Barcable.create_score(
                name="relevance",
                value=0.95,
                trace_id=trace_id,
    ...
    def create_trace_id(*, seed: Optional[str] = None) -> str:
        """Create a unique trace ID for use with Barcable.

        This method generates a unique trace ID for use with various Barcable APIs.
        ...
        This method ensures the generated ID meets this requirement. If you need to
        correlate an external ID with a Barcable trace ID, use the external ID as the
        seed to get a valid, deterministic Barcable trace ID.
        ...
        Returns:
            A 32-character lowercase hexadecimal string representing the Barcable trace ID.

        Example:
            ```python
            # Generate a random trace ID
            trace_id = Barcable.create_trace_id()

            # Generate a deterministic ID based on a seed
            session_trace_id = Barcable.create_trace_id(seed="session-456")

            # Correlate an external ID with a Barcable trace ID
            external_id = "external-system-123456"
            correlated_trace_id = Barcable.create_trace_id(seed=external_id)

            # Use the ID with trace context
            with Barcable.start_as_current_span(
        ...
        if not seed:
            ...
            return Barcable._format_otel_trace_id(trace_id_int)
    ...
    def create_score(
        ...
        *,
        ...
        This method creates a score for evaluating a Barcable trace or observation. Scores can be
        ...
        Args:
            ...
            session_id: ID of the Barcable session to associate the score with
            dataset_run_id: ID of the Barcable dataset run to associate the score with
            trace_id: ID of the Barcable trace to associate the score with
            ...
            config_id: Optional ID of a score config defined in Barcable
    ...
    def score_current_trace(
        ...
        *,
        ...
        Example:
            ```python
            with Barcable.start_as_current_span(name="process-user-request") as span:
                # Process request
                result = process_complete_request()
                span.update(output=result)

                # Score the overall trace
                Barcable.score_current_trace(
    ...
    def flush(self) -> None:
        """Force flush all pending spans and events to the Barcable API.

        This method manually flushes any pending spans, scores, and other events to the
        Barcable API. It's useful in scenarios where you want to ensure all data is sent
        before proceeding, without waiting for the automatic flush interval.

        Example:
            ```python
            # Record some spans and scores
            with Barcable.start_as_current_span(name="operation") as span:
                # Do work...
                pass

            # Ensure all data is sent to Barcable before proceeding
            Barcable.flush()
    ...
    def shutdown(self) -> None:
        """Shut down the Barcable client and flush all pending data.

        This method cleanly shuts down the Barcable client, ensuring all pending data
        ...
        Example:
            ```python
            # Initialize Barcable
            Barcable = Barcable(public_key="...", secret_key="...")

            # Use Barcable throughout your application
            # ...
    ...
    def get_current_trace_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with Barcable.start_as_current_span(name="process-request") as span:
                # Get the current trace ID for reference
                trace_id = Barcable.get_current_trace_id()
    ...
    def get_current_observation_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with Barcable.start_as_current_span(name="process-user-query") as span:
                # Get the current observation ID
                observation_id = Barcable.get_current_observation_id()
    ...
    def get_trace_url(self, *, trace_id: Optional[str] = None) -> Optional[str]:
        """Get the URL to view a trace in the Barcable UI.

        This method generates a URL that links directly to a trace in the Barcable UI.
        ...
        Returns:
            A URL string pointing to the trace in the Barcable UI,
        ...
        Example:
            ...
            with Barcable.start_as_current_span(name="process-request") as span:
                trace_url = Barcable.get_trace_url()
                log.info(f"Processing trace: {trace_url}")

            # Get URL for a specific trace
            specific_trace_url = Barcable.get_trace_url(trace_id="1234567890abcdef1234567890abcdef")
    ...
    def get_dataset(
        ...
        """
        try:
            Barcable_logger.debug(f"Getting datasets {name}")
            ...
            items = [DatasetItemClient(i, Barcable=self) for i in dataset_items]
    ...
    def run_experiment(
        ...
        *,
        ...
        automatically traces all executions with Barcable for observability, runs
        ...
        - Integration with Barcable datasets for experiment tracking
        ...
        Args:
            ...
            run_name: Optional exact name for the experiment run. If provided, this will be
                used as the exact dataset run name if the `data` contains Barcable dataset items.
            ...
            data: Array of data items to process. Can be either:
                ...
                - List of Barcable DatasetItem objects from dataset.items
            ...
            metadata: Optional metadata dictionary to attach to all experiment traces.
                ...
                If `data` are Barcable dataset items, the metadata will be attached to the dataset run, too.

        Returns:
            ExperimentResult containing:
            - run_name: The experiment run name. This is equal to the dataset run name if experiment was on Barcable dataset.
            - item_results: List of results for each processed item with outputs and evaluations
            - run_evaluations: List of aggregate evaluation results for the entire run
            - dataset_run_id: ID of the dataset run (if using Barcable datasets)
            - dataset_run_url: Direct URL to view results in Barcable UI (if applicable)
        ...
        Examples:
            ...
            result = Barcable.run_experiment(
            ...
            Using with Barcable datasets:
            ```python
            # Get dataset from Barcable
            dataset = Barcable.get_dataset("my-eval-dataset")
            ...
            # Results automatically linked to dataset in Barcable UI
        ...
        Note:
            ...
            - All executions are automatically traced and visible in Barcable UI
            - When using Barcable datasets, results are automatically linked for easy comparison
        ...
        *,
        ...
        for i, result in enumerate(item_results):
            if isinstance(result, Exception):
                Barcable_logger.error(f"Item {i} failed: {result}")
        ...
        for run_evaluator in run_evaluators:
            try:
                ...
                Barcable_logger.error(f"Run evaluator failed: {e}")
        ...
        for evaluation in run_evaluations:
            try:
                ...
                Barcable_logger.error(f"Failed to store run evaluation: {e}")
            ...
            try:
                ...
                if hasattr(item, "id") and hasattr(item, "dataset_id"):
                    try:
                        ...
                        Barcable_logger.error(f"Failed to create dataset run item: {e}")
                ...
                for evaluator in evaluators:
                    try:
                        ...
                        Barcable_logger.error(f"Evaluator failed: {e}")
    ...
    def create_dataset(
        ...
        *,
        ...
        """Create a dataset with the given name on Barcable.
        ...
        Returns:
            Dataset: The created dataset as returned by the Barcable API.
        """
        try:
            body = CreateDatasetRequest(
                name=name, description=description, metadata=metadata
            )
            Barcable_logger.debug(f"Creating datasets {body}")
    ...
    def create_dataset_item(
        ...
        *,
        ...
        Returns:
            DatasetItem: The created dataset item as returned by the Barcable API.

        Example:
            ```python
            from Barcable import Barcable

            Barcable = Barcable()

            # Uploading items to the Barcable dataset named "capital_cities"
            Barcable.create_dataset_item(
        ...
        try:
            ...
            barcable_logger.debug(f"Creating dataset item {body}")
    ...
    def resolve_media_references(
        ...
        *,
        ...
        in the format "@@@BarcableMedia:...@@@". When found, it (synchronously) fetches the actual media content using
        the provided Barcable client and replaces the reference string with a base64 data URI.
        ...
        Example:
            obj = {
                "image": "@@@BarcableMedia:type=image/jpeg|id=123|source=bytes@@@",
                "nested": {
                    "pdf": "@@@BarcableMedia:type=application/pdf|id=456|source=bytes@@@"
                }
            }

            result = await BarcableMedia.resolve_media_references(obj, Barcable_client)
            ...
            # }
        """
        return BarcableMedia.resolve_media_references(
            Barcable_client=self,
            obj=obj,
    ...
    def get_prompt(
        ...
        *,
        ...
        Keyword Args:
            ...
            fallback: Union[Optional[List[ChatMessageDict]], Optional[str]]: The prompt string to return if fetching the prompt fails. Important on the first call where no cached prompt is available. Follows Barcable prompt formatting with double curly braces for variables. Defaults to None.
        ...
        Barcable_logger.debug(f"Getting prompt '{cache_key}'")
        ...
        if cached_prompt.is_expired():
            Barcable_logger.debug(f"Stale prompt '{cache_key}' found in cache.")
            try:
                # refresh prompt in background thread, refresh_prompt deduplicates tasks
                Barcable_logger.debug(f"Refreshing prompt '{cache_key}' in background.")
    ...
    def _fetch_prompt_and_update_cache(
        ...
        *,
        ...
        cache_key = PromptCache.generate_cache_key(name, version=version, label=label)
        Barcable_logger.debug(f"Fetching prompt '{cache_key}' from server...")
    ...
    def create_prompt(
        ...
        *,
        ...
        """Create a new prompt in Barcable.
        ...
        """
        try:
            Barcable_logger.debug(f"Creating prompt {name=}, {labels=}")
    ...
    def update_prompt(
        ...
        *,
        ...
        """Update an existing prompt version in Barcable. The Barcable SDK prompt cache is invalidated for all prompts witht he specified name.

        Args:
            name (str): The name of the prompt to update.
            version (int): The version number of the prompt to update.
            new_labels (List[str], optional): New labels to assign to the prompt version. Labels are unique across versions. The "latest" label is reserved and managed by Barcable. Defaults to [].

        Returns:
            Prompt: The updated prompt from the Barcable API.
