"""If you use the OpenAI Python SDK, you can use the Barcable drop-in replacement to get full logging by changing only the import.

```diff
- import openai
+ from Barcable.openai import openai
```

Barcable automatically tracks:
...
See docs for more details: https://Barcable.com/docs/integrations/openai
"""
...
from Barcable._client.get_client import get_client
from Barcable._client.span import BarcableGeneration
...
log = logging.getLogger("Barcable")
...
class OpenAiArgsExtractor:
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.args["name"] = name
        self.args["Barcable_public_key"] = Barcable_public_key
        self.args["Barcable_prompt"] = Barcable_prompt
        self.args["trace_id"] = trace_id
        self.args["parent_observation_id"] = parent_observation_id

        self.kwargs = kwargs

    def get_Barcable_args(self) -> Any:
...
def _Barcable_wrapper(func: Any) -> Any:
...
def _extract_chat_prompt(kwargs: Any) -> Any:
    ...
    if prompt:
        # uf user provided functions, we need to send these together with messages to Barcable
        prompt.update(
            {
                "messages": [
        ...
        # vanilla case, only send messages in openai format to Barcable
                ...
                processed_content.append(
                    {
                        "type": "input_audio",
                        "input_audio": {
                            "data": BarcableMedia(base64_data_uri=base64_data_uri),
                            "format": format,
                        },
                    }
                )
...
def _extract_chat_response(kwargs: Any) -> Any:
    ...
    if kwargs.get("audio") is not None:
        ...
        if "data" in audio and audio["data"] is not None:
            ...
            audio["data"] = BarcableMedia(base64_data_uri=base64_data_uri)
...
def _get_Barcable_data_from_kwargs(resource: OpenAiDefinition, kwargs: Any) -> Any:
    ...
    Barcable_public_key = kwargs.get("Barcable_public_key", None)
    if Barcable_public_key is not None and not isinstance(Barcable_public_key, str):
        raise TypeError("Barcable_public_key must be a string")
    ...
    Barcable_prompt = kwargs.get("Barcable_prompt", None)
    ...
    return {
        ...
        "model": model or None,
        "prompt": Barcable_prompt,
    }


def _create_Barcable_update(
    completion: Any,
...
def _get_Barcable_data_from_default_response(
...
def _wrap(
    ...
    Barcable_args = arg_extractor.get_Barcable_args()

    Barcable_data = _get_Barcable_data_from_kwargs(open_ai_resource, Barcable_args)
    Barcable_client = get_client(public_key=Barcable_args["Barcable_public_key"])

    generation = Barcable_client.start_observation(
        as_type="generation",
        name=Barcable_data["name"],
        input=Barcable_data.get("input", None),
        metadata=Barcable_data.get("metadata", None),
        model_parameters=Barcable_data.get("model_parameters", None),
        trace_context={
            "trace_id": cast(str, Barcable_data.get("trace_id", None)),
            "parent_span_id": cast(
                str, Barcable_data.get("parent_observation_id", None)
            ),
        },
        model=Barcable_data.get("model", None),
        prompt=Barcable_data.get("prompt", None),
    )
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return BarcableResponseGeneratorSync(
            ...
            model, completion, usage = _get_Barcable_data_from_default_response(
                open_ai_resource,
...
async def _wrap_async(
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return BarcableResponseGeneratorAsync(
                resource=open_ai_resource,
                response=openai_response,
                generation=generation,
            )
...
class BarcableResponseGeneratorSync:
    def __init__(
        self,
        *,
        resource: Any,
...
class BarcableResponseGeneratorAsync:
    def __init__(
        self,
        *,
        resource: Any,
