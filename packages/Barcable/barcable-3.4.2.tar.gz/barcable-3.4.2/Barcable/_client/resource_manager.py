"""Tracer implementation for Barcable OpenTelemetry integration.

This module provides the BarcableTracer class, a thread-safe singleton that manages OpenTelemetry
tracing infrastructure for Barcable. It handles tracer initialization, span processors,
API clients, and coordinates background tasks for efficient data processing and media handling.

Key features:
- Thread-safe OpenTelemetry tracer with Barcable-specific span processors and sampling
...
from Barcable._client.constants import BARCABLE_TRACER_NAME
from Barcable._client.environment_variables import (
    BARCABLE_MEDIA_UPLOAD_ENABLED,
    BARCABLE_MEDIA_UPLOAD_THREAD_COUNT,
    BARCABLE_RELEASE,
    BARCABLE_TRACING_ENVIRONMENT,
)
from Barcable._client.span_processor import BarcableSpanProcessor
from Barcable._task_manager.media_manager import MediaManager
from Barcable._task_manager.media_upload_consumer import MediaUploadConsumer
from Barcable._task_manager.score_ingestion_consumer import ScoreIngestionConsumer
from Barcable._utils.environment import get_common_release_envs
from Barcable._utils.prompt_cache import PromptCache
from Barcable._utils.request import BarcableClient
from Barcable.api.client import AsyncFernBarcable, FernBarcable
from Barcable.logger import Barcable_logger
from Barcable.types import MaskFunction

from ..version import __version__ as Barcable_version


class BarcableResourceManager:
    ...
    2. Managing the span processor for exporting spans to the Barcable API
    3. Creating and managing Barcable API clients (both synchronous and asynchronous)
    ...
    _instances: Dict[str, "BarcableResourceManager"] = {}
    ...
    def _initialize_instance(
        ...
        *,
        ...
        if tracing_enabled:
            ...
            Barcable_processor = BarcableSpanProcessor(
            ...
            tracer_provider.add_span_processor(Barcable_processor)
        ...
        self.api = FernBarcable(
            base_url=host,
            username=self.public_key,
            password=secret_key,
            x_Barcable_sdk_name="python",
            x_Barcable_sdk_version=Barcable_version,
            x_Barcable_public_key=self.public_key,
            httpx_client=self.httpx_client,
            timeout=timeout,
        )
        self.async_api = AsyncFernBarcable(
            base_url=host,
            username=self.public_key,
            password=secret_key,
            x_Barcable_sdk_name="python",
        ...
        score_ingestion_client = BarcableClient(
            public_key=self.public_key,
            secret_key=secret_key,
            base_url=host,
            version=Barcable_version,
        ...
        self._media_upload_enabled = os.environ.get(
            BARCABLE_MEDIA_UPLOAD_ENABLED, "True"
        ...
        media_upload_thread_count = media_upload_thread_count or max(
            int(os.getenv(BARCABLE_MEDIA_UPLOAD_THREAD_COUNT, 1)), 1
        )
        ...
        Barcable_logger.info(
            f"Startup: Barcable tracer successfully initialized | "
    ...
    def flush(self) -> None:
        ...
        Barcable_logger.debug("Successfully flushed OTEL tracer provider")

        self._score_ingestion_queue.join()
        Barcable_logger.debug("Successfully flushed score ingestion queue")

        self._media_upload_queue.join()
        Barcable_logger.debug("Successfully flushed media upload queue")
...
def _init_tracer_provider(
    *,
    ...
    environment = environment or os.environ.get(BARCABLE_TRACING_ENVIRONMENT)
    release = release or os.environ.get(BARCABLE_RELEASE) or get_common_release_envs()

    resource_attributes = {
        BarcableOtelSpanAttributes.ENVIRONMENT: environment,
