...
from Barcable.langchain.utils import _extract_model_name
...
try:
    ...
    raise ModuleNotFoundError(
        "Please install langchain to use the Barcable langchain integration: 'pip install langchain'"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    def __init__(
        ...
        Args:
            public_key: Optional Barcable public key. If not provided, will use the default client configuration.
            update_trace: Whether to update the Barcable trace with the chains input / output / metadata / name. Defaults to False.
        """
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        if (
            run_id in self.runs
            and isinstance(self.runs[run_id], BarcableGeneration)
            and run_id not in self.updated_completion_start_time_memo
        ):
            current_generation = cast(BarcableGeneration, self.runs[run_id])
    ...
    def _get_observation_type_from_serialized(
        ...
        """Determine Barcable observation type from LangChain component.
        ...
        Returns:
            The appropriate Barcable observation type string
        """
    ...
    def on_retriever_error(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            Barcable_logger.exception(e)

    def _parse_Barcable_trace_attributes_from_metadata(
        self,
        ...
        if "Barcable_session_id" in metadata and isinstance(
            metadata["Barcable_session_id"], str
        ):
            attributes["session_id"] = metadata["Barcable_session_id"]

        if "Barcable_user_id" in metadata and isinstance(
            metadata["Barcable_user_id"], str
        ):
            attributes["user_id"] = metadata["Barcable_user_id"]

        if "Barcable_tags" in metadata and isinstance(metadata["Barcable_tags"], list):
            attributes["tags"] = [str(tag) for tag in metadata["Barcable_tags"]]
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self._register_Barcable_prompt(
            ...
            if parent_run_id is None:
                span.update_trace(
                    ...
                    **self._parse_Barcable_trace_attributes_from_metadata(metadata),
                )
    ...
    def _register_Barcable_prompt(
        ...
        *,
        ...
        """We need to register any passed Barcable prompt to the parent_run_id so that we can link following generations with that prompt.
        ...
        Barcable_prompt = metadata and metadata.get("Barcable_prompt", None)

        if Barcable_prompt:
            self.prompt_to_parent_run_map[parent_run_id] = Barcable_prompt
    ...
    def _deregister_Barcable_prompt(self, run_id: Optional[UUID]) -> None:
        ...
        try:
            ...
            if agent_run is not None:
                agent_run._otel_span.set_attribute(
                    BarcableOtelSpanAttributes.OBSERVATION_TYPE, "agent"
                )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if span is not None:
                ...
                self._deregister_Barcable_prompt(run_id)
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if registered_prompt:
                self._deregister_Barcable_prompt(parent_run_id)
            ...
            content = {
                ...
                "metadata": self.__join_tags_and_metadata(
                    ...
                    keep_Barcable_trace_attributes=True
                    if parent_run_id is None
                    else False,
                ),
                "model": model_name,
    ...
    def _log_model_parse_warning(self) -> None:
        if not hasattr(self, "_model_parse_warning_logged"):
            Barcable_logger.warning(
                "Barcable was not able to parse the LLM model. The LLM call will be recorded without model name. Please create an issue: https://github.com/Barcable/Barcable/issues/new/choose"
            )
    ...
    def __join_tags_and_metadata(
        ...
        tags: Optional[List[str]] = None,
        metadata: Optional[Dict[str, Any]] = None,
        keep_Barcable_trace_attributes: bool = False,
        ...
        return (
            _strip_Barcable_keys_from_dict(final_dict, keep_Barcable_trace_attributes)
            if final_dict != {}
            else None
        )
...
def _parse_usage_model(usage: typing.Union[pydantic.BaseModel, dict]) -> Any:
    ...
    for model_key, Barcable_key in conversion_list:
        if model_key in usage_model:
            ...
            usage_model[Barcable_key] = final_count  # Translate key and keep the value
...
def _strip_Barcable_keys_from_dict(
    metadata: Optional[Dict[str, Any]], keep_Barcable_trace_attributes: bool
) -> Any:
    if metadata is None or not isinstance(metadata, dict):
        return metadata

    Barcable_metadata_keys = [
        "Barcable_prompt",
    ]

    Barcable_trace_attribute_keys = [
        "Barcable_session_id",
        "Barcable_user_id",
        "Barcable_tags",
    ]
    ...
    for key in Barcable_metadata_keys:
        metadata_copy.pop(key, None)

    if not keep_Barcable_trace_attributes:
        for key in Barcable_trace_attribute_keys:
            metadata_copy.pop(key, None)
