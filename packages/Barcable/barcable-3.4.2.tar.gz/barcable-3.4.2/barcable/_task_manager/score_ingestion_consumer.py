...
from ..version import __version__ as Barcable_version
...
from Barcable._utils.parse_error import handle_exception
from Barcable._utils.request import APIError, BarcableClient
from Barcable._utils.serializer import EventSerializer

MAX_EVENT_SIZE_BYTES = int(os.environ.get("BARCABLE_MAX_EVENT_SIZE_BYTES", 1_000_000))
MAX_BATCH_SIZE_BYTES = int(os.environ.get("BARCABLE_MAX_BATCH_SIZE_BYTES", 2_500_000))
...
class ScoreIngestionConsumer(threading.Thread):
    _log = logging.getLogger("Barcable")
    ...
    def _upload_batch(self, batch: List[Any]) -> None:
        self._log.debug(
            f"API: Uploading batch of {len(batch)} score events to Barcable API"
        )

        metadata = ScoreIngestionMetadata(
            batch_size=len(batch),
            sdk_name="python",
            sdk_version=Barcable_version,
        ...
        self._log.debug(
            f"API: Successfully sent {len(batch)} score events to Barcable API in batch mode"
        )
