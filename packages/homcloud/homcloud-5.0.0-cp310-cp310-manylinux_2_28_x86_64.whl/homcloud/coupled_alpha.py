...
from homcloud.pdgm_format import PDGMWriter, BinaryChunk, CoupledAlphaRelativeInformationChunk
...
class RelativePHFiltration:
    ...
    def build_phat_matrix(self):
        ...
        for simplex in self.simplices:
            boundary_on = sorted([self.simplex_to_index[b] for b in boundary(simplex) if b in self.simplex_to_index])
            matrix.set_dim_col(self.simplex_to_index[simplex], len(simplex) - 1, boundary_on)

        return matrix

    def boundary_map_style(self):
        return "simplicial-relative" if self.save_boundary_map else "none"
