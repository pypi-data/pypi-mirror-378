        ...
        try:
            for i in range(60):
                ...
                if server_detail is None:
                    log.info(f"server_id: {server_id} deleted")
                    return True
                log.warn(f"server_id: {server_id} is deleting")
                time.sleep(5)
            log.warn(f"Failed to wait for server {server_id} is deleted: Error.\nserver_detail is {server_detail}")
            return False
        except Exception as e:
