...
class Modbus(Protocol):
    ...
    def _raise_if_error(self, response : bytes, exception_codes: ExceptionCodesType) -> None:
        if response == b"":
    ...
    def read_coils(self, start_address: int, number_of_coils: int) -> list[bool]:
        ...
        pdu : bytes = self._adapter.query(
            ...
            stop_condition=Length(self._length(n_coil_bytes + 2)), # TODO : convert to multiple stop conditions here
            full_output=False
        )
    ...
    def read_discrete_inputs(self, start_address: int, number_of_inputs: int) -> list[bool]:
        """
    ...
    def read_holding_registers(self, start_address: int, number_of_registers: int) -> list[int]:
        """
    ...
    def read_multi_register_value(
        ...
        data : Union[bytes, int, float] = struct.unpack(
            endian_symbol[_word_order] + struct_format(type_cast, n_registers * 2), buffer
        )[0]

        # If data is a string, do additionnal processing
        output : Union[bytes, int, float, str]
    ...
    def read_input_registers(self, start_address: int, number_of_registers: int) -> List[int]:
        """
        ...
        response = self._parse_pdu(
            self._adapter.query(self._make_pdu(self._make_pdu(query)), full_output=False)
        )
        self._raise_if_error(response, exception_codes=EXCEPTIONS)
        _, _, registers_data = struct.unpack(
    ...
    def write_single_coil(self, address: int, enabled: bool) -> None:
        ...
        response = self._parse_pdu(
            self._adapter.query(
                self._make_pdu(query), stop_condition=Length(self._length(len(query))), full_output=False
            )
        )
    ...
    def write_single_register(self, address: int, value: int) -> None:
        ...
        self._raise_if_error(response, EXCEPTIONS)
        assert query == response, f"Response ({response!r}) should match query ({query!r})"
    ...
    def read_exception_status(self) -> int: # TODO : Check the return type
        ...
        response = self._parse_pdu(self._adapter.query(self._make_pdu(query), full_output=False))
        self._raise_if_error(response, EXCEPTIONS)
        _, exception_status = cast(Tuple[int, int], struct.unpack(ENDIAN + "BB", response)) # TODO : Check this
    ...
    def _diagnostics(
        ...
        returned_function, returned_subfunction, subfunction_returned_data = (
            cast(Tuple[int, int, bytes], struct.unpack(ENDIAN + f"BH{return_subfunction_bytes}s", response))
        )
        if check_response:
            assert (
    ...
    def diagnostics_return_diagnostic_register(self) -> int:
        ...
        returned_data = self._diagnostics(
            DiagnosticsCode.RETURN_DIAGNOSTIC_REGISTER, 0, 2
        )
    ...
    def diagnostics_force_listen_only_mode(self) -> None:
        ...
        """
        self._diagnostics(
            DiagnosticsCode.FORCE_LISTEN_ONLY_MODE, 0, 0, check_response=False
        )
    ...
    def diagnostics_return_bus_communication_error_count(self) -> int:
        ...
        """
        response = self._diagnostics(
            DiagnosticsCode.RETURN_BUS_COMMUNICATION_ERROR_COUNT, 0, 2
        )
        count = struct.unpack("H", response)
        return count
    ...
    def diagnostics_return_bus_exception_error_count(self) -> int:
        ...
        """
        response = self._diagnostics(
            DiagnosticsCode.RETURN_BUS_EXCEPTION_ERROR_COUNT, 0, 2
        )
        count = struct.unpack("H", response)
        return count
    ...
    def diagnostics_return_server_no_response_count(self) -> int:
        ...
        """
        response = self._diagnostics(
            DiagnosticsCode.RETURN_SERVER_NO_RESPONSE_COUNT, 0, 2
        )
        count = struct.unpack("H", response)
        return count
    ...
    def diagnostics_return_bus_character_overrun_count(self) -> int:
        ...
        """
        response = self._diagnostics(
            DiagnosticsCode.RETURN_BUS_CHARACTER_OVERRUN_COUNT, 0, 2
        )
        count = struct.unpack("H", response)
        return count
    ...
    def write_multiple_registers(self, start_address: int, values: list[int]) -> None:
        ...
        response = self._parse_pdu(
            self._adapter.query(
                self._make_pdu(query), stop_condition=Length(self._length(5)), full_output=False
            )
        )
        self._raise_if_error(response, EXCEPTIONS)
    ...
    def read_file_record(self, records: list):
        ...
        if response_size > SIZE_LIMIT:
            raise ValueError(
                f"Sum of records lenghts is too high : {sum([r[3] for r in records])}"
            )
    ...
    def write_file_record(self, records: list) -> None:
        """
