...
class Action(Enum):
    ...
    GET_PREVIOUS_BUFFER = "get_previous_buffer"  # (descriptor,) -> (data,)
    FLUSHREAD = "flushread"
    ...
    ENUMERATE_LOGGER_CONNECTIONS = "enumerate_logger_connections"
    GET_PID = "get_pid"
...
def backend_request(conn : Connection, action : Action, *args : Any, timeout : float = STANDARD_RESPONSE_TIMEOUT) -> BackendResponse:
    try:
        conn.send((action.value, *args))
    except (BrokenPipeError, OSError):
        raise BackendCommunicationError("Failed to communicate with backend")
    else:
        ...
        try:
            ...
            raise BackendCommunicationError(f"Failed to receive backend response to {action.value}")                
        
        # Check if the response is correctly formatted
        if not (isinstance(raw_response, tuple) and isinstance(raw_response[0], str)):
            raise BackendCommunicationError(f"Invalid response received from backend : {raw_response}")
