...
from syndesi.tools.backend_api import LOCALHOST, BACKEND_PORT
...
class BackendWrapper:
    ...
    def run(self) -> None:
        console = Console()
        with (
            console.status(f"[bold green]Syndesi backend running on {self.address}", spinner="dots")
            if not self.quiet
            else nullcontext()
        ):
            backend = Backend(
                host=self.address, port=self.port, backend_shutdown_delay=self.shutdown_delay
            )
            try:
                backend.start()
