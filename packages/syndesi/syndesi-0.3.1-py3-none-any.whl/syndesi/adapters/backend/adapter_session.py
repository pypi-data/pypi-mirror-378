...
from .descriptors import adapter_descriptor_by_string, Descriptor, SerialPortDescriptor, IPDescriptor, VisaDescriptor
from .stop_condition_backend import stop_condition_from_json
...
class AdapterSession(threading.Thread):
    ...
    daemon = True
    _shutdown_counter_top : Optional[int]
    _shutdown_counter : Optional[int]

    def __init__(self, adapter_descriptor: str, shutdown_delay: Optional[NumberLike]):
        ...
        if self._shutdown_delay is not None: 
            self._shutdown_counter_top = int(round(self._shutdown_delay / self.MONITORING_DELAY))
    ...
    def _pop_next_timeout_event(self) -> Tuple[Optional[TimeoutEvent], Optional[float]]:
    ...
    def send(self, conn: NamedConnection, action: Action, *args : Any) -> None:
        if not frontend_send(conn.conn, action, *args):
            self._logger.warning(
                f"Failed to send to {conn.remote()}"
            )
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                self._logger.critical(f'Error in {self._adapter.descriptor} session loop : {e}')
                try:
                    for conn in self.connections:
    ...
    def loop(self) -> bool:
        ...
        wait_list : List[Selectable] = [conn.conn for conn in self.connections]  # Copy connections
        ...
        if not self._has_timeout_event(TimeoutEvent.ADAPTER) and next_adapter_timeout is not None:
            self._add_timeout_event(TimeoutEvent.ADAPTER, next_adapter_timeout)
        ...
        if len(ready) == 0:
            ...
            if event == TimeoutEvent.MONITORING:
                self._add_timeout_event(TimeoutEvent.MONITORING, time.time() + self.MONITORING_DELAY)
                stop = self._monitor()
                if stop:
                    return True
    ...
    def manage_conn(self, conn: NamedConnection) -> None:
        ...
        if not conn.conn.poll():
            ...
            self._logger.warning(
                f"Client {conn.remote()} closed unexpectedly"
            )
            self._remove_connection(conn)
            return
        try:
            ...
            if not (isinstance(request, tuple) and len(request) >= 1):
                ...
                try:
                    match action:
                        case Action.OPEN:
                            self._adapter.set_stop_condition(
                                stop_condition_from_json(request[1])
                            )
                        ...
                        case Action.SET_STOP_CONDITION:
                            stop_condition = stop_condition_from_json(request[1])
                            self._adapter.set_stop_condition(stop_condition)
                            response_action, extra_arguments = Action.SET_STOP_CONDITION, ()
                        ...
                        case _:
                            response_action, extra_arguments = Action.ERROR_UNKNOWN_ACTION, (f'{action}',)
                except Exception as e:
                    tb = e.__traceback__
                    if tb is None:
                        error_message = ""
            ...
            if remove_after_response:
                self._logger.info(
                    f"Closing client {conn.remote()} connection"
                )
                self._remove_connection(conn)
