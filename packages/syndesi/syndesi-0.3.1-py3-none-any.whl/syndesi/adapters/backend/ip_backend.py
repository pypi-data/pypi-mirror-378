...
from .adapter_backend import AdapterBackend, HasFileno, Selectable
    ...
    def open(self) -> bool:
        ...
        if self._status == AdapterBackendStatus.DISCONNECTED:
            ...
            if self._socket is None:
                if self.descriptor.transport == IPDescriptor.Protocol.TCP:
                    self._socket = cast(_socket.socket, socket.socket(socket.AF_INET, socket.SOCK_STREAM))
                elif self.descriptor.transport == IPDescriptor.Protocol.UDP:
                    self._socket = cast(_socket.socket, socket.socket(socket.AF_INET, socket.SOCK_DGRAM))
            try:
                self._socket.settimeout(0.5) # TODO : Configure this cleanly, it has to be less than the receive timeout of the frontend 
