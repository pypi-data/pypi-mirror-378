...
class LogManager:
    ...
    def __init__(self) -> None:
        ...
        self._stream_handler : Optional[logging.StreamHandler[TextIO]] = None
    ...
    def update_loggers(self) -> None:
        ...
        for l in LoggerAlias:
            if self._all_loggers or l.value in self._loggers:
...
def log(
    ...
    loggers: Optional[list[str] | str] =None,
) -> None:
    """
    ...
    """
    debug_manager = LogManager()
    update = False

    if level is not None:
        debug_manager.set_log_level(level)
        update = True
    if console is not None:
        debug_manager.set_console_log(console)
        update = True
    if file is not None:
        debug_manager.set_log_file(file)
        update = True
    if loggers is not None:
        debug_manager.set_logger_filter(loggers)
        update = True

    if update:
        debug_manager.update_loggers()
