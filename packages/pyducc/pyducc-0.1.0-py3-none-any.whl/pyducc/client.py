...
class DuccClient:
    ...
    :param large_file: 是否启用大文件模式，默认值为 True
    ...
    def __init__(
        ...
        self.headers = {
            ...
            "User-Agent": "ducc-sdk-python/1.0.0",
            "SDK-Language": "python"
        }
    ...
    def start_polling(
        ...
        def _long_polling():
            ...
            if self.env == 'pro' or self.env == 'pre':
                url = 'http://longpolling.ducc.jd.local.ducc.jd.local' + long_polling_api
            else:
                url = 'http://test.ducc.jd.local' + long_polling_api
                ...
                try:
                    ...
                    res = requests.get(
                        ...
                        params={
                            ...
                            "propertiesFileForceFlag": self.large_file
                        },
                        headers=headers,
                        timeout=timeout
                    )
                    ...
                    if res.status_code == 200:
                        ...
                        if real_status != 200:
                            self.logger.error(f"DUCC配置变更, 调用DUCC长轮询接口失败，status={real_status}, message={res_body.get('message')}")
                        ...
                        self.logger.debug(f"DUCC配置变更, 读取ducc更新配置用时：{(time.time() - t_start)}")
                        ...
                        if last_modified != headers['If-Modified-Since']:
                            ...
                            self.logger.debug(
                                f"DUCC配置变更, 更新配置完成用时：(time.time() - t_start)s,版本 {last_modified} -> { headers['If-Modified-Since']}"
                            )
                            last_modified = headers['If-Modified-Since']
                            
                            # 标记首次成功拉取数据
                            if not first_poll_success.is_set():
                                self.logger.info("DUCC配置变更, 首次成功拉取数据")
                        ...
                        self.logger.debug(
                            f'DUCC配置变更, 长轮询返回304，继续... {(time.time() - t_start)}s',

                        )
                        time.sleep(timeout[1])
                        continue
                except requests.exceptions.ConnectionError as e:
                    self.logger.error(f"DUCC配置变更, 长轮询过程中连接异常, 请检查网络或服务 url: {url}")
                    time.sleep(timeout[1])
                except requests.exceptions.Timeout as e:
                    self.logger.error(f"DUCC配置变更, 长轮询过程中请求超时,请修改配置或检查网络 url: {url} 当前最大超时时间 {polling_timeout_ms}ms")
                except Exception as e:
                    self.logger.error(f"DUCC配置变更, 长轮询过程中发生异常,等待下次重试, url: {url}: {str(e)}")
                    time.sleep(timeout[1])
        ...
        # 等待首次成功拉取数据
        self.logger.info("等待首次成功拉取DUCC配置...")
        first_poll_success.wait()
        self.logger.info("首次DUCC配置拉取成功，继续执行")
