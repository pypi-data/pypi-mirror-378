...
def main():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="If set, overwrite existing config file at out_path",
        default=False,
    )
...
""".lstrip()
    ...
    if out_file.exists() and args.force is not True:
        print(
            f"Refusing to overwrite existing file: {out_file}. Use --force to override.",
            file=sys.stderr,
        )
        sys.exit(1)
