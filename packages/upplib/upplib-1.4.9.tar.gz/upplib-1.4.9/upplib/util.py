...
def get_log_msg(contents: dict,
    ...
    __time__0 = None
    if '__time__0' in contents and contents['__time__0'] != 'null':
        __time__0 = contents['__time__0']
        if tz is not None and type(__time__0) is int:
            __time__0 = to_datetime_str(__time__0, tz=tz)
    # content
    content = None
    ...
    if content is not None:
        ...
        if len(str(content).split(' ')) >= 2:
            ...
            _time_ = None
    return ' '.join(filter(lambda s: s is not None, [__time__0, _time_, level, content]))
