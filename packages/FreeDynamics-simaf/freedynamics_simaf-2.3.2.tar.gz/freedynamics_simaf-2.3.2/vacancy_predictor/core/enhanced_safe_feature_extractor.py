...
class EnhancedSafeFeatureExtractor:
    ...
    def extract_features(self, df: pd.DataFrame, n_atoms: int, 
        ...
        # 13. Filtrar features según modo de seguridad
    ...
    # ============== MÉTODOS DE CONTROL CORREGIDOS ==============
    
    def _filter_features_by_mode(self, features: Dict[str, Any]) -> Dict[str, Any]:
        """Filtrar features según el modo de seguridad (CORREGIDO)"""
        allowed_features = FeatureCategories.get_allowed_features(self.config.feature_mode)
        
        if allowed_features is None:  # Modo FULL
            return features
        
        # Features avanzadas permitidas según el modo
        advanced_features_low_risk = {
            'pe_skew_robust', 'pe_kurt_robust', 'thermal_fluctuation_index',
            'pe_gradient_magnitude', 'atomic_density_local', 'spatial_anisotropy',
            'energy_stress_coupling', 'structural_cohesion'
        }
        
        advanced_features_medium_risk = {
            'pe_disorder_metric', 'stress_instability', 'stress_anisotropy',
            'hydrostatic_pressure_ratio', 'volume_packing_efficiency',
            'coordination_disorder', 'effective_density_normalized',
            'voro_vol_cv_enhanced', 'thermodynamic_imbalance',
            'multiscale_disorder', 'energy_asymmetry', 'pe_variability_ratio',
            'stress_I1_I2_ratio', 'stress_I2_I3_ratio', 'spatial_energy_proxy',
            'volume_energy_density'
        }
        
        advanced_features_high_risk = {
            'vacancy_density_proxy'
        }
        
        # Determinar features avanzadas permitidas
        if self.config.feature_mode == FeatureMode.CONSERVATIVE:
            allowed_advanced = advanced_features_low_risk
        elif self.config.feature_mode == FeatureMode.STANDARD:
            allowed_advanced = advanced_features_low_risk | advanced_features_medium_risk
        else:  # FULL
            allowed_advanced = (advanced_features_low_risk | 
                              advanced_features_medium_risk | 
                              advanced_features_high_risk)
        
        # Filtrar solo features permitidas
        filtered = {}
        for key, value in features.items():
            # Preservar target, metadata y features permitidas
            if (key in ['vacancies', 'file_hash'] or 
                key.startswith('_') or
                key in allowed_features or
                key in allowed_advanced):
                filtered[key] = value
