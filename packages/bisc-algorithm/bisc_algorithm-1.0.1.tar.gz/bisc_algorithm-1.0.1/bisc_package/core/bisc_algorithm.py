...
def mine_algorithm(permutations: List[Permutation], m: int) -> Dict[Tuple[int, ...], Set[FrozenSet]]:
    """
    ...
    Args:
        ...
        m: Upper bound on pattern length to search for
    ...
    """
    # Initialize S with all classical patterns of length at most m
    S = {}

    # Generate all classical patterns of length 1 to m
    ...
    for perm in permutations:
        # Get all subwords of length at most m
        subwords = perm.subwords(m)
...
def bisc_algorithm(permutations: List[Permutation], m: int) -> List[MeshPattern]:
    """
    ...
    Examples:
        ...
        >>> forbidden = bisc_algorithm(stack_sortable, 3)
        >>> # Should find pattern 231 as forbidden
    """
    # Step 1: MINE - find allowed patterns
    print(f"MINE: Analyzing {len(permutations)} permutations for patterns of length ≤ {m}")
    allowed_patterns = mine_algorithm(permutations, m)
...
def bisc_with_pruning(permutations: List[Permutation], m: int) -> List[MeshPattern]:
    """
    ...
    """
    # Run basic BiSC
    forbidden_patterns = bisc_algorithm(permutations, m)
