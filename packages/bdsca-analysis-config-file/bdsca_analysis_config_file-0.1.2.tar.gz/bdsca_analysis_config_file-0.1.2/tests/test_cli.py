...
def test_validate_example_yaml(tmp_path: Path) -> None:
    ...
    assert cp.returncode == 0
    assert "Valid YAML (schema v3)" in cp.stdout
...
def test_change_target_project_requires_name_version(tmp_path: Path) -> None:
    ...
    assert (
        "version is missing" in cp.stderr
        or "'version' is missing" in cp.stderr
        or "'version' is a required property" in cp.stderr
        or "Must include one of" in cp.stderr
    )
