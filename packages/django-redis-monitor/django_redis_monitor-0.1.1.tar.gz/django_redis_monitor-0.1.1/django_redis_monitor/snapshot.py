...
class SnapshotScheduler(BaseScheduler):
    ...
    def schedule(self, now):
        ...
        errors = {}
        for task in tasks:
            signals.snapshot_started.send(sender=self, instance=task, now=now)
            ...
            try:
                ...
                signals.snapshot_error.send(sender=self, task=task, now=now, exception=e)
            ...
            signals.snapshot_ended.send(sender=self, task=task, snapshot=snapshot, now=now)
        if len(tasks):
