...
class Timeline:
    ...
    def add_video(
        ...
        elif PythonValidator.is_instance_of(video, VideoFileMedia):
            if video.size != self.size:
                # Force timeline.size if needed
                video.add_video_filter(GraphFilters.video.scale(self.size[0], self.size[1]))
            # TODO: Maybe force fps also if needed (?)
