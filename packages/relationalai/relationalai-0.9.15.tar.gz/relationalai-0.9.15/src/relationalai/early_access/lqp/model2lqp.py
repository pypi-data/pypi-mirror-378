...
from relationalai.early_access.lqp.types import meta_type_to_lqp, is_numeric
    ...
    if rel_builtins.is_builtin(task.relation):
        if task.relation.name == "construct_datetime" and len(terms) == 7:
            ...
            assert monotype or numeric, \
                f"Expected types of `=`to be comparable but got `{term_types[0].type_name}` and `{term_types[1].type_name}`"
            lqp_name = relname_to_lqp_name(task.relation.name)
