...
class OAuthSSLMismatchException(RAIException):
    def __init__(self, account):
        self.message = "SSL certificate verification failed during OAuth token exchange."
        self.name = "OAuth Token Exchange SSL Mismatch Error"
    ...
    def format_message(self):
        ...
        This usually happens when the provided 'account' is not the Snowflake account locator but is instead based on the organization name.
        You can fix the issue by changing your current account ({self.account}) to the Snowflake account locator in your configuration.
        You can retrieve the Snowflake account location via 'SELECT CURRENT_ACCOUNT();'.
        """)
