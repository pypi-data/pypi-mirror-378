...
class FocusedWave(AcousticField):
    ...
    def _apply_delay(self, dx=None):
        ...
        try:
            ...
            # 5. Calculate distances for active elements
            active_element_positions = element_positions[active_elements]
            distances = np.array([
                np.sqrt(
                    (active_element_positions[i] - self.focal_line)**2 +
                    self.params['Foc']**2
                )
                for i in range(len(active_elements))
            ])

            # 6. Calculate max and min distances
            max_distance = np.max(distances)
            min_distance = np.min(distances)

            # 7. Calculate max delay
            max_delay_samples = int(np.ceil((max_distance - min_distance) / self.params['c0'] / self.kgrid.dt))
            ...
            # 9. Calculate and apply delays for active elements only
            for elem_idx in active_elements:
                ...
                for grid_idx in range(start_grid, end_grid):
                    ...
                    # Time delay = (distance - min_distance) / speed_of_sound
                    delay_samples = int(np.round((distance - min_distance) / self.params['c0'] / self.kgrid.dt))
