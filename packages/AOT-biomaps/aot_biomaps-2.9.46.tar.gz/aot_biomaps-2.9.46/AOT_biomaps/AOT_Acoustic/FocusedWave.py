...
class FocusedWave(AcousticField):
    ...
    def _generate_2Dacoustic_field_KWAVE(self, isGPU=True if config.get_process() == 'gpu' else False, show_log=True):
        ...
        try:
            ...
            # Initialize active grid points mask (size = total_grid_points)
            active_grid_points = np.zeros(total_grid_points, dtype=bool)

            # Calculate starting position to center the transducer in the grid
            current_position = (Nx - total_grid_points) // 2

            # Set active elements in source mask and mark active grid points
            for i in range(self.params['num_elements']):
                x_start = current_position
                x_end = current_position + element_width_grid_points

                if i in selected_indices:  # If this element is active
                    source.p_mask[x_start:x_end, 0] = 1
                    active_grid_points[x_start - current_position:x_end - current_position] = True

                current_position += element_width_grid_points

            # 5. Create k-Wave grid with correct syntax
            kgrid = kWaveGrid([Nx, Nz], [dx, dx])
            kgrid.setTime(self.kgrid.Nt, 1 / self.params['f_AQ'])

            # 6. Prepare delayed signals
            if factorT != 1:
                delayedSignal = self._apply_delay(dx=dx)
            else:
                delayedSignal = self.delayedSignal

            # Verify delayedSignal shape matches total_grid_points
            if delayedSignal.shape[0] != total_grid_points:
                raise ValueError(f"Expected delayedSignal to have {total_grid_points} rows, got {delayedSignal.shape[0]}")

            # Select signals for active grid points only
            source.p = delayedSignal[active_grid_points, :]

            # 7. Define sensor mask
            sensor = kSensor()
