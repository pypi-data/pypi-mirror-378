    ...
    def _generate_2Dacoustic_field_KWAVE(self, isGPU=True if config.get_process() == 'gpu' else False, show_log=True):
        ...
        try:
            ...
            if dx >= self.params['element_width']:
                dx = self.params['element_width'] / 2  # Ensure at least 2 points per element width
            ...
            active_list = np.zeros(self.params['num_elements'], dtype=bool)
            active_list[selected_indices] = True
            ...
            kgrid = kWaveGrid(Nx, dx, Nz, dx)
            kgrid.setTime(self.kgrid.Nt, 1 / self.params['f_AQ'])

            # Calculate element width in grid points
            ...
            # Set active elements in source mask
            ...
            # 5. Prepare delayed signals
            if factorT != 1:
                delayedSignal = self._apply_delay(dx=dx)
            else:
                delayedSignal = self.delayedSignal

            # Select signals for active elements only
            source.p = delayedSignal[active_list, :]

            # 6. Define sensor mask (covering the entire domain)
            sensor = kSensor()
            sensor.mask = np.ones((Nx, Nz))  # Note: Using Nx, Nz instead of self.params['Nx'], self.params['Nz']

            # 7. Calculate PML sizes (must be powers of 2 for FFT efficiency)
