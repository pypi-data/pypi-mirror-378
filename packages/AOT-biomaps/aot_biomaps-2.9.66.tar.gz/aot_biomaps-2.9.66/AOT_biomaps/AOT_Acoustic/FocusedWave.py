...
class FocusedWave(AcousticField):
    ...
    def _apply_delay(self, dx=None):
        ...
        try:
            ...
            # Calculer max_distance uniquement pour les éléments actifs
            active_element_positions = element_positions[active_elements]
            distance =  np.sqrt((active_element_positions - self.focal_line)**2 + self.params['Foc']**2)
            
            max_delay_samples = int(np.max(distance)-np.min(distance)) / self.params['c0'] / self.kgrid.dt
            ...
            for elem_idx in active_elements:
                ...
                for grid_idx in range(start_grid, end_grid):
                    ...
                    delay_samples = int(np.round((distance) / self.params['c0'] / self.kgrid.dt))
