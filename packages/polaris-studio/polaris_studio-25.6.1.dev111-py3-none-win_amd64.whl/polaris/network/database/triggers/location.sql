...
begin
    ...
    set
        ...
        walk_setback = round(coalesce(st_distance(new.geo, (select geo from Transit_Walk where walk_link= new.link)), 0), 8),
        ...
        bike_setback = round(coalesce(st_distance(new.geo, (select geo from Transit_Bike where bike_link= new.link)), 0), 8)
    where
        Location.rowid = new.rowid;
...
begin
    ...
    set
        ...
        walk_setback = coalesce(round(coalesce(st_distance(new.geo, (select geo from Transit_Walk where walk_link= new.link)), 0), 8), old.walk_setback),
        ...
        bike_setback = coalesce(round(coalesce(st_distance(new.geo, (select geo from Transit_Bike where bike_link= new.link)), 0), 8), old.bike_setback)
    where
        Location.rowid = new.rowid;
    INSERT OR IGNORE INTO Geo_Consistency_Controller VALUES('Location', 'geo');
end;
...
begin
    update Location
        set
            walk_setback = coalesce(round(coalesce(st_distance(new.geo, (select geo from Transit_Walk where walk_link= new.link)), 0), 8), new.walk_setback)
        where Location.rowid = new.rowid;
end;
...
begin
    update Location
        set bike_setback = coalesce(round(coalesce(st_distance(new.geo, (select geo from Transit_Bike where bike_link= new.link)), 0), 8), old.bike_setback)
    where Location.rowid = new.rowid;
end;
