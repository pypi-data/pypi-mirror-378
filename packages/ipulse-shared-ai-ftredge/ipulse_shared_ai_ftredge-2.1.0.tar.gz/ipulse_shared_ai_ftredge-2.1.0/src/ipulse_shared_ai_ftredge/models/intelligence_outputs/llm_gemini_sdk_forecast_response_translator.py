...
from .time_series_llm_prediction import TimeSeriesLLMPredictionResponse, PredictionValuePoint, KeyRisks
from ipulse_shared_base_ftredge.enums import Currency, AIFramework
import uuid

class LLMGeminiSDKRForecastesponseTranslator:
    """
    ...
    def gemini_sdk_to_llm_response(
        ...
        # Convert forecast points from Gemini schema to database model
        forecast_points = [
            PredictionValuePoint(
                forecast_date=point.forecast_date,
                forecast_value=point.forecast_value,
                forecast_value_upper_bound=point.forecast_value_upper_bound,
                forecast_value_lower_bound=point.forecast_value_lower_bound,
                confidence_score=point.confidence_score,
                milestones_and_events=point.milestones_and_events
            )
            for point in parsed_schema.price_forecast
        ]
        ...
        # Convert key risks from Gemini schema to database model
        key_risks = KeyRisks(
