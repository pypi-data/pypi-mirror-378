...
class SimulationRunner:
    ...
    def poll_once(self, run: Run) -> Run:
        ...
        for sim in run.simulations:
            ...
            should_trigger_eval = is_terminal and not getattr(sim, "eval_id", None)

            if should_trigger_eval:
                try:
                    eval_id = self.api.start_evaluation(sim.simulation_id)
                except HTTPStatusError as exc:
                    # Otherwise, wrap in a clear, user-copyable error with scenario_id.
                    raise UpstreamServiceError.from_httpx_error(
                        exc,
                        scenario_id=sim.scenario_id,
                        run_id=run.run_id,
                        simulation_id=sim.simulation_id,
                        endpoint="POST /evals/evaluate",
                        user_message=(
                            "Failed to start evaluation for your simulation. "
                            "This is likely an upstream service issue."
                        ),
                    ) from exc
                sim.eval_id = eval_id
