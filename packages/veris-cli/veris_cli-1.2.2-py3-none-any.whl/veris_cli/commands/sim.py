...
def launch(
    ...
    if getattr(cfg, "agent", None) is None:
        ...
        # Validate MCP is reachable. Any response (even 404) counts as reachable; network errors do not.
        try:
            httpx.get(cfg.agent.mcp_url, timeout=3.0, follow_redirects=True)
        except httpx.RequestError as exc:
            raise ConfigurationError(
                message="Unable to reach configured agent MCP URL.",
                hint=(
                    f"Tried: {cfg.agent.mcp_url}. Ensure your FastAPI app is running and ngrok tunnel is active. "
