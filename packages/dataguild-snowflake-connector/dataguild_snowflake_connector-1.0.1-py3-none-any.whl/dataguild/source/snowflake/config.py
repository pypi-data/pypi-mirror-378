...
Clean and comprehensive configuration classes for Snowflake integration in DataGuild.
Maintains all original function and class names while providing enhanced functionality,
validation, and production-ready features.
...
from dataguild.configuration.common import AllowDenyPattern, ConfigModel
from dataguild.configuration.pattern_utils import UUID_REGEX
from dataguild.configuration.source_common import (
    EnvConfigMixin,
    LowerCaseDatasetUrnConfigMixin,
    PlatformInstanceConfigMixin,
)
from dataguild.configuration.time_window_config import BaseTimeWindowConfig
from dataguild.configuration.validate_field_removal import pydantic_removed_field
from dataguild.configuration.validate_field_rename import pydantic_renamed_field
from dataguild.api.incremental_properties_helper import (
    IncrementalPropertiesConfigMixin,
)
from dataguild.glossary.classification_mixin import (
...
from dataguild.source.sql.sql_config import SQLCommonConfig, SQLFilterConfig
...
from dataguild.utilities.global_warning_util import add_global_warning
from dataguild.utilities.str_enum import StrEnum
...
class QueryDedupStrategyType(Enum):
    """Query deduplication strategies for performance optimization."""
    STANDARD = "STANDARD"
    NONE = "NONE"


class TagOption(StrEnum):
    """Tag extraction options with lineage considerations."""
...
class DatabaseId:
    """Database identifier for Snowflake shares."""
    database: str = Field(
...
class SnowflakeShareConfig(ConfigModel):
    """Configuration for Snowflake data sharing."""
    ...
    def source_database(self) -> DatabaseId:
        """Get the source database identifier."""
        return DatabaseId(self.database, self.platform_instance)


class SnowflakeFilterConfig(SQLFilterConfig):
    """Enhanced filtering configuration for Snowflake objects."""

    database_pattern: AllowDenyPattern = Field(
        AllowDenyPattern(
            allow=[r"^JAFFLE_SHOP_DB$"],
            deny=[r"^UTIL_DB$", r"^SNOWFLAKE$", r"^SNOWFLAKE_SAMPLE_DATA$", r"^SNOWFLAKE_LEARNING_DB$"],
        ),
    ...
    def validate_legacy_schema_pattern(cls, values: Dict) -> Dict:
        """Validate schema pattern configuration."""
...
class SnowflakeIdentifierConfig(
    ...
    """Configuration for Snowflake identifiers and URN generation."""
...
class SnowflakeUsageConfig(BaseUsageConfig):
    """Configuration for Snowflake usage statistics extraction."""
...
class SnowflakeConfig(
    ...
    """Base Snowflake configuration class."""
...
class SnowflakeV2Config(
    ...
    """Comprehensive Snowflake V2 configuration with all advanced features."""

    # ✅ Add missing domain attribute
    domain: Optional[str] = Field(
        default=None,
        description="Domain configuration for Snowflake connection",
    )
    ...
    # Add missing view inclusion flag
    include_views: bool = Field(
        default=True,
        description="Whether to include database views in ingestion"
    )

    # Add missing table inclusion flag (boolean version)
    include_tables_bool: bool = Field(
        default=True,
        description="Whether to include database tables in ingestion"
    )

    # Add missing streams inclusion flag
    include_streams: bool = Field(
        default=True,
        description="Whether to include database streams in ingestion"
    )

    # Add missing procedures inclusion flag
    include_procedures: bool = Field(
        default=True,
        description="Whether to include database procedures in ingestion"
    )
    ...
    def validate_convert_urns_to_lowercase(cls, v):
        """Validate URN lowercase conversion setting."""
        if not v:
            add_global_warning(
    ...
    def validate_include_column_lineage(cls, v, values):
        """Validate column lineage dependency on table lineage."""
    ...
    def validate_unsupported_configs(cls, values: Dict) -> Dict:
        """Validate configuration combinations and constraints."""
        ...
        if not include_technical_schema and any([include_profiles, delete_detection_enabled]):
            raise ValueError(
    ...
    def validate_shares(
        ...
        """Validate Snowflake shares configuration."""
