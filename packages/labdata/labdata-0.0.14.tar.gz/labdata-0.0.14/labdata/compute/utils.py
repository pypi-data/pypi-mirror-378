            ...
            with ssh_connect(target['address'],target['user'],target['permission_key']) as conn:
                for j in jobids:
                    ...
                    slurmjob = slurm_schedule_remote(cmd,  
                                                     ...
                                                     log_dir = '$LABDATA_PATH/remote_jobs',
                                                     container_path = container_file,
                                                     project = project,
                                                     database_user = prefs['database']['database.user'],
                                                     database_password = decrypt_string(prefs['database']['database.password']),
                ...
                if len(exists):
                    ...
                    if len(idx):
                        ...
                        if force_submit:
                            ...
                            (self.schema.ComputeTask() & f'job_id = {job_id}').delete(safemode = False) # deleting a previous job because of force_submit
                        else:
                            continue
            else:
