...
class FullAnalyser:
    ...
    def _log_progress(self, current: int, total: int, file_path: str) -> None:
        ...
        if self.progress:
            ...
            if self.colour:
                print(
                    f"{Fore.GREEN}Analysing ({current}/{total}){Style.RESET_ALL} [{percentage:.1f}%]: {file_path}"
                )
            else:
                print(f"Analysing ({current}/{total}) [{percentage:.1f}%]: {file_path}")
    ...
    def analyse_file(self, file_path: str) -> FileAnalysisResult:
        ...
        self._log_info(f"Starting analysis of {file_path}")
        try:
            content: t.Optional[str] = self._read_file_content(file_path, result)

            if content is None or not content.strip():
                self._log_warning(f"Skipping empty file: {file_path}")
                return result
        ...
        self._log_info(
            f"Completed analysis of {file_path} in {result.analysis_time:.2f}s"
        )
        return result
        ...
        for analyser_name, analyser in analysers.items():
            try:
                if analyser.tree:
                    ...
                    self._log_info(
                        f"Completed {analyser_name} analysis for {file_path}"
                    )
            except Exception as error:
                self._log_error(
    ...
    def analyse_project(
        ...
        if total_found > max_files:
            self._log_warning(
                f"Limiting analysis to {max_files} files (found {total_found})"
            )
            python_files = python_files[:max_files]

        if self.progress:
            print(f"Found {len(python_files)} Python files to analyse")
        ...
        self._log_info("Performing project-level duplication analysis...")
        ...
        if self.progress:
            if self.colour:
                print(
                    f"\n{Fore.GREEN}Generating report for {len(file_results)} files...{Style.RESET_ALL}"
                )
            else:
                print(f"\nGenerating report for {len(file_results)} files...")
