...
class MacOsOptions:
	def __init__(self,
		...
		target_architecture: _enums.TargetArchitecture | None = None,
...
class PyExporter:
	def __init__(self,
	             ...
	             clean: bool = False,
	             log_level: _enums.LogLevel | None = None,
	             contents_directory: str | Path | None = None,
	             windowed: bool = False,
	             console: bool = True,
	             icon: str | Path | None = None,
	             hide_console: _enums.HideConsole | None = None,
	             debug: _enums.Debug | None = None,
		...
		self._collects: list[tuple[_enums.CollectType, str]] = []
	...
	def add_collect(self, collect_type: _enums.CollectType | str, *module_names: str):
		"""
		...
		"""
		if not (_utils.get_enum_str(collect_type) in _utils.get_enum_data(_enums.CollectType).values()):
	...
	def get_pyinstaller_options(self, pretty: bool = True):
		...
		for opt, value in options.items():
			key, mod = _enums.OptionsMap.get(re.sub(r'^_+', '', str(opt)))
...
print(PyExporter('_main.py', name='jw-mwb', one_file=True, clean=True, no_confirm=True, log_level=_enums.LogLevel.WARN).get_pyinstaller_options())