...
from ILCDIRAC.Core.Utilities.CombinedSoftwareInstallation import redirectSoftware
...
class DiracILC(Dirac):
  ...
  def _checkapp(self, platform, appName, appVersion):
    ...
    if app_version or app_version_cvmfs:
      return S_OK()
    
    if appName == redirectSoftware(appName):
    ...
    return self._checkapp(platform, redirectSoftware(appName), appVersion)
