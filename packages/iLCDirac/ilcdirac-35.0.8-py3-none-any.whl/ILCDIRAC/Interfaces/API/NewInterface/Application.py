...
class Application(object):  # pylint: disable=too-many-instance-attributes
  ...
  Any new application should inherit from this class.
  """
  ...
  def setSteeringFile(self, steeringfile):
    ...
    """
    self._checkArgs({'steeringfile': (str,)})
  ...
  def setInputFile(self, inputfile):
    ...
    :param inputfile: Input file (data, not steering) to pass to the application. Can be local file or LFN:
    :type inputfile: string or list
    """
    kwargs = {"inputfile": inputfile}
    if not isinstance(inputfile, (six.string_types, list)):
  ...
  def setExtraCLIArguments(self, arguments):
    ...
    :param str arguments:
    """
    self._checkArgs({"arguments": (str,)})
    self.extraCLIArguments = arguments
    return S_OK()

  def listAttributes(self):
    """Method to list attributes for users.

    Doesn't list any private or semi-private attributes
    """
...
########################################################################################
  ...
  def _setApplicationModuleAndParameters(self, stepdefinition):
    """Create Application Module, add it to a Step and set values to Module.

    Called in every applications
    """
  ...
  def _createModuleDefinition(self):
    ...
    As it's generic code, all apps will use this.
    """
  ...
  def _applicationModule(self):
    """Create the module for the application, and add the parameters to it.

    Overloaded by every application class.
    """
    raise NotImplementedError('Must be implemented in Application')

  def _applicationModuleValues(self, moduleinstance):
    """Set the values for the modules parameters.

    """
    pass

  def _userjobmodules(self, stepdefinition):
    """Method used to return the needed module for UserJobs.

    It's different from the ProductionJobs
  ...
  def _checkFinalConsistency(self):
    ...
    Used to check that everything is fine, in particular that
  ...
  def _checkWorkflowConsistency(self):
    ...
    Used to check the workflow consistency: linking between
    applications
  ...
  def _addBaseParameters(self, stepdefinition):
    """Add to step the default parameters.

    appname, version, steeringfile, (nbevts, Energy), LogFile, InputFile, OutputFile, OutputPath.
    """
  ...
  def _setBaseStepParametersValues(self, stepinstance):
    ...
    stepinstance.setValue('ExtraCLIArguments', six.moves.urllib.parse.quote(self.extraCLIArguments))
  ...
  def _addParametersToStep(self, stepdefinition):
    """Method to be overloaded by every application.

    Add the parameters to the given step.
  ...
  def _setStepParametersValues(self, stepinstance):
    ...
    For all parameters that are not to be linked,
  ...
  def _resolveLinkedStepParameters(self, stepinstance):
    """Method to be overloaded by every application that resolve what are the linked parameters (e.g. OuputFile and InputFile).

    See :class:`~ILCDIRAC.Interfaces.API.NewInterface.Applications.StdhepCut.StdhepCut` for example.
    Called from Job.
    """
    return S_OK()
  ...
  def _checkArgs(self, argNamesAndTypes):
    ...
    for argName, argType in argNamesAndTypes.items():
  ...
  def _reportError(self, message, short_name='', **kwargs):
    ...
    Gets caller method name and arguments, formats the
