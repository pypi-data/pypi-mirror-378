...
class Cubes(
    ...
    def _delete_delegate_keys(self, keys: AbstractSet[CubeName], /) -> None:
        ...
        for key in keys:
            graphql_input = DeleteCubeInput(cube_name=key)
            graphql_client.delete_cube(input=graphql_input).set_output_validator(
                create_delete_status_validator(
                    key, lambda output: output.delete_cube.status
                )
            )
