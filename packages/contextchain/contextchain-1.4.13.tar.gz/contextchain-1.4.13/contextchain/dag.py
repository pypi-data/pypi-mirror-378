...
# Removed module-level import: from .context_engineer import ContextEngineer
...
class GenerateResponseTask(Task):
    def __init__(self, metadata: TaskMetadata, llm_optimizer: BaseLLMClient, context_engineer):  # Type annotation removed
        super().__init__(metadata)
        self.llm_optimizer = llm_optimizer
        self.context_engineer = context_engineer
...
class DAGEngine:
    def __init__(self, max_parallel_tasks: int = 10, vector_store=None, llm_optimizer=None, context_engineer=None):
        if context_engineer is None:
            from .context_engineer import ContextEngineer  # Lazy import to break circular dependency
