...
class ContextChainConfig(BaseModel):
    ...
    llm_model: str = os.getenv("LLM_MODEL", "llama3")  # Default to llama3
...
class ContextChain:
    ...
    def __init__(self, config: ContextChainConfig, llm_client: Optional[BaseLLMClient] = None):
        ...
        logger.info(f"ContextChain v2.0 initialized with LLM provider: {config.llm_provider}, model: {config.llm_model}")
    ...
    async def generate_insights(self, query: str, context: Optional[str] = None,
        ...
        try:
            ...
            response = ContextChainResponse(
                ...
                budget_allocation=vars(budget),  # Changed from budget.__dict__
    ...
    async def _log_interaction(self, query: str, complexity: QueryComplexity, budget: BudgetAllocation,
        ...
        try:
            ...
            await self.storage.log_interaction(
                ...
                query=query,
                complexity=vars(complexity),  # Changed from complexity.dict()
                budget_allocation=vars(budget),  # Changed from budget.dict()
