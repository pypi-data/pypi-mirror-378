...
class SSHConnector:
    ...
    def exec(self, cmd: str, cwd: Optional[str] = None, env: Optional[dict] = None, timeout: int = 300):
        ...
        print(f"[DEBUG] Executing remote command: {cmd} (cwd={cwd}, env={env})")
        client = self._client_connected()
        if cwd:
            cmd = f"cd {cwd} && " + cmd
        if env:
    ...
    def put_bytes(self, data: bytes, remote_path: str, mode: int = 0o644) -> None:
        print(f"[DEBUG] Uploading {len(data)} bytes to {remote_path} with mode {oct(mode)}")
        client = self._client_connected()
    ...
    def put_file(self, local_path: str, remote_path: str, mode: int = 0o644) -> None:
        ...
        """
        print(f"[DEBUG] Uploading file {local_path} to {remote_path} with mode {oct(mode)}")
        client = self._client_connected()
        sftp = client.open_sftp()
        # ensure parent dirs
        parent = posixpath.dirname(remote_path)
    ...
    def get_file(self, remote_path: str, local_path: str) -> None:
        ...
        Raises IOError/OSError on failure.
        """
        print(f"[DEBUG] Downloading remote file {remote_path} to {local_path}")
        client = self._client_connected()
        sftp = client.open_sftp()
    ...
    def list_files(self, remote_path: str, recursive: bool = False) -> list[str]:
        ...
        print(f"[DEBUG] Listing files in remote directory {remote_path} (recursive={recursive})")
    ...
    def get(self, remote_path: str, local_path: str) -> None:
        ...
        def _download_dir(rdir: str, ldir: str) -> None:
            ...
            for entry in sftp.listdir_attr(rdir):
                ...
                if stat.S_ISDIR(entry.st_mode):
                    ...
                    print(f"[DEBUG] Downloading file {r_child} to {l_child}")
        ...
        try:
            if _is_dir(remote_path):
                ...
                print(f"[DEBUG] Downloading file {remote_path} to {dest}")
