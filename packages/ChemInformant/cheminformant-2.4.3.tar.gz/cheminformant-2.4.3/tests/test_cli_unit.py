...
class TestMainFetch:
    def test_main_fetch_with_table_format(self):
        ...
        with mock.patch(
            "sys.argv", ["chemfetch", "caffeine", "--props", "cas,molecular_weight"]
        ):
    ...
    def test_main_fetch_with_sql_format(self):
        ...
        with mock.patch(
            "sys.argv", ["chemfetch", "caffeine", "--format", "sql", "-o", tmp_path]
        ):
            with mock.patch("ChemInformant.cli.get_properties", return_value=test_data):
    ...
    def test_main_fetch_value_error_handling(self):
        with mock.patch("sys.argv", ["chemfetch", "caffeine"]):
            with mock.patch(
                "ChemInformant.cli.get_properties",
    ...
    def test_main_fetch_custom_properties(self):
        ...
        with mock.patch(
            "sys.argv", ["chemfetch", "caffeine", "--props", "molecular_formula,xlogp"]
        ):
            with mock.patch("ChemInformant.cli.get_properties", return_value=test_data):
                with mock.patch("builtins.print") as mock_print:
                    cli.main_fetch()
...
class TestMainDraw:
    ...
    def test_main_draw_not_found_error(self):
        with mock.patch("sys.argv", ["chemdraw", "invalid_compound"]):
            with mock.patch(
                "ChemInformant.cli.draw_compound",
                side_effect=NotFoundError("invalid_compound"),
            ):
                with mock.patch("builtins.print") as mock_print:
                    with pytest.raises(SystemExit) as exc_info:
                        cli.main_draw()
    ...
    def test_main_draw_general_exception(self):
        with mock.patch("sys.argv", ["chemdraw", "caffeine"]):
            with mock.patch(
                "ChemInformant.cli.draw_compound",
                side_effect=Exception("Drawing failed"),
            ):
                with mock.patch("builtins.print") as mock_print:
                    with pytest.raises(SystemExit) as exc_info:
                        cli.main_draw()
