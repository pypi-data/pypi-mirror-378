...
def task_rawdata_collect(args):
    logger, file, pms, replicates, discarding, exportraw, output_folder = args
    ...
    if exportraw:
        res_df.to_excel(f'{output_folder}/raw_data/raw_data_{strain}.xlsx')
...
def task_logtransform(args):
    ...
    for index, row in df.iterrows():
        # math.log --> base e
        # math.log10 --> base 10
        # math.log1p --> base e; queal to log(1 + x), works for x >= 0
...
def collect_raw_data(logger, cores, input_folder, pms, replicates, discarding, exportraw, output_folder):
    logger.info(f"Collecting raw data...")
    
    
    if exportraw:
        os.makedirs(f'{output_folder}/raw_data/', exist_ok=True)
    ...
    with confu.ProcessPoolExecutor(max_workers=cores) as executor:  
        futures = []
        for file in files:
            future = executor.submit(task_rawdata_collect, (logger, file, pms, replicates, discarding, exportraw, output_folder))
