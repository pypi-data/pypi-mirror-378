...
# 安装基础OpenGL库（所有环境都需要）
echo "=== 安装基础OpenGL库 ==="
apt-get install libgles2-mesa-dev mesa-utils libgl1-mesa-dev mesa-common-dev libglu1-mesa-dev -y
...
if [ "$has_nvidia" = true ]; then
    ...
    # 安装NVIDIA EGL支持库
    apt-get install libnvidia-egl-wayland1 libnvidia-egl-gbm1 -y
    
    # # 安装NVIDIA CUDA库（用于硬件编解码）
    # apt-get install nvidia-cuda-toolkit -y
    
    # # 安装NVIDIA OpenGL库（包含libEGL_nvidia.so.0）
    # apt-get install libnvidia-gl-535 -y
