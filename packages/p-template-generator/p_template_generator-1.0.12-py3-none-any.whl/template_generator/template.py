...
def maybeMesa(useHardwareEncode=True, useHardwareDecode=True):
    return template_env.maybeMesa(useHardwareEncode, useHardwareDecode)
...
def executeTemplate(data, searchPath="", useAdaptiveSize=False, useAdaptiveDuration=False, printLog=False, oneVideoTimeout=240, useHardwareEncode=True, useHardwareDecode=True):
    ...
    #--gpu 根据环境检测结果生成命令行参数
    # 注意：命令行参数生成逻辑在template.py中，便于参数相关开发人员查看和修改
    if sys.platform == "linux":
        # 检测是否使用Mesa软件渲染
        # maybeMesa()函数检测headless环境或用户禁用硬件编码时返回True
        if maybeMesa(useHardwareEncode, useHardwareDecode):
            extArgs += ["--call_mesa"]
        
        # 检测是否使用软件编码/解码
        # maybeSoftWare()函数检测无NVIDIA硬件支持或用户禁用硬件编码时返回True
        if maybeSoftWare(useHardwareEncode, useHardwareDecode):
            extArgs += ["--call_software_encoder"]
            extArgs += ["--call_software_decoder"]
    else:
        # 非Linux平台的处理：直接根据用户参数决定
    ...
    if result.returncode == 0:
        ...
        should_retry_with_software = (
            ...
            "[Could not open video codec]" in err_msg and  # 错误日志包含codec错误
        ...
        if should_retry_with_software and sys.platform == "linux": 
            ...
            retry_command.append("--call_mesa")
            ...
            # 执行软编重试
            retry_result = subprocess.run(retry_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd=binary_dir, timeout=oneVideoTimeout*output_cnt, env=env)
...
def generateTemplate(config, output, searchPath, printLog=True, useHardwareEncode=True, useHardwareDecode=True): 
    ...
    # 确保EGL相关环境变量存在
    if 'MESA_GL_VERSION_OVERRIDE' not in env:
        env['MESA_GL_VERSION_OVERRIDE'] = '3.3'
    if 'MESA_GLSL_VERSION_OVERRIDE' not in env:
        env['MESA_GLSL_VERSION_OVERRIDE'] = '330'
    if 'LIBGL_ALWAYS_SOFTWARE' not in env:
        env['LIBGL_ALWAYS_SOFTWARE'] = '1'
    if 'GALLIUM_DRIVER' not in env:
        env['GALLIUM_DRIVER'] = 'llvmpipe'
    if 'EGL_PLATFORM' not in env:
        env['EGL_PLATFORM'] = 'x11'
    if 'MESA_GLSL_CACHE_DISABLE' not in env:
        env['MESA_GLSL_CACHE_DISABLE'] = '1'
