...
class PosteriorAgCoords:
    ...
    def plot_dist_between_pair(
        ...
        ax = ax or plt.gca()
        return ax.hist(
            self.dist_between_pair(a, b, center_coords=center_coords)
            .to_numpy()
            .ravel(),
            **kwds,
        )
    ...
    def summarise_pair_dist_hdi(
        ...
        **kwds,
        ...
        data = [
            xr.concat(
                [self.dist_between_pair(*pair, center_coords=False) for pair in groups],
                dim="dist",
            )
        ]
        names = ["Data"]
        ...
        if show_null or show_corrected:
            null = xr.concat(
                [self.dist_between_pair(*pair, center_coords=True) for pair in groups],
                dim="dist",
            )
