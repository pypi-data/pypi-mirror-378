# sparks/simulators/deterministic/compile_ode.py
...
# --------------------------------------------------------------------------------------
# Class factories with caches (built once per State class, then reused)
# --------------------------------------------------------------------------------------


class _ViewBase:
    """
    Base class for state view representations.
    """
...
class OdeSolutionState(Generic[StateType]):
    ...
    Usage: driver = OdeSolutionState(ModelState, sol.sol, attach_derived=True)
           s = driver(t)
    """
