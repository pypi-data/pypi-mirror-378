...
class CodeAudit:
    ...
    def filter_by_file(self, file):
        if file.strip() and '.py' in file.strip() and not os.path.basename(file).startswith("__") and not os.path.basename(file).startswith("000"):
            return True
        else:
            return False
    ...
    def get_app_from_file(self, file_name: str, project_apps: list[str], is_join=False):
        """
        ...
        for app in project_apps:
            ...
            if app in parts:
                ...
                if is_join:
                    return os.path.join(app, str(relative_path))
