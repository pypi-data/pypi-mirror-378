...
class SikuliPlusLibrary:
    ...
    Import example:
        Library    SikuliPlusLibrary    language=en_US    similarity=0.8    timeout=5

    Main options (as arguments or env vars):
        ...
        highlight_time: Highlight duration (default 2.0)
        action_speed:  Delay between actions (default 0.1)
        language:      Keyword language (default en_US)
        screen_id:     Monitor index (default 0)
    """
    def __init__(self, language: str = "en_US", **kwargs) -> None:
        """
        Initialize the library for Robot Framework.

        Args:
            language: Keyword language (e.g. en_US, pt_BR)
            **kwargs: See class docstring for supported options
        """
