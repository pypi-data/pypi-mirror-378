    ...
    *,
    ...
    for i, fl in enumerate(parser.css('div[jsname="IWWDBc"], div[jsname="YdtKid"]')):
        ...
        for item in fl.css("ul.Rk10dc li")[
            ...
            # Get prices
            price_element = item.css_first(".YMlIz.FpEdX, .U3gSDe .FpEdX")
            price = safe(price_element).text() or "Price unavailable"

            # Create a unique identifier for the flight to avoid duplicates
            flight_identifier = (
                name,
                flight_code,
                " ".join(departure_time.split()),
                " ".join(arrival_time.split()),
                duration,
                price.replace(",", ""),
            )
            if flight_identifier in seen_flights:
                continue
