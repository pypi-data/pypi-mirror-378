...
from justrunalready.verify import verify_linux, verify_macos, verify_windows
from justrunalready.wrappers.loader import load_wrappers


def _cmd_bundle(args: argparse.Namespace) -> int:
    cfg = load_config(Path(args.config))
    platform = _normalize_platform(getattr(args, "platform", "auto"))
    if platform.startswith("linux"):
        from justrunalready.platforms.linux import bundle as bundle_linux
        ...
        if rc == 0 and args.wrap:
            _run_wrapper(args.wrap, cfg, platform)
        return rc
    if platform in ("macos",):
        from justrunalready.platforms.macos import bundle as bundle_macos
    ...
    if platform.startswith("win") or platform == "windows":
        from justrunalready.platforms.windows import bundle as bundle_windows
...
def _cmd_verify(args: argparse.Namespace) -> int:
    ...
    platform = _normalize_platform(getattr(args, "platform", "auto"))
    if platform.startswith("linux"):
        return verify_linux(cfg)
    if platform in ("macos",):
        return verify_macos(cfg)
    if platform.startswith("win") or platform == "windows":
        return verify_windows(cfg)
    print(f"Unsupported platform: {platform}", file=sys.stderr)
    return 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_info.add_argument("--platform", default="auto", help="Platform: auto|linux|macos|windows")
    p_info.add_argument("--wrap", default="", help="Optional wrapper: appimage|dmg|zip")
    p_info.add_argument("--format", default="env", choices=["env", "json"], help="Output format")
