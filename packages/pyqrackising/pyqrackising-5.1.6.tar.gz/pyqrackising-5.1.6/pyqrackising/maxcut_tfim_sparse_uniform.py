...
@njit
def local_repulsion_choice(G_cols, G_rows, n, m):
    """
...
@njit(parallel=True)
def sample_for_solution(G_data, G_rows, G_cols, shots, thresholds):
    n = G_rows.shape[0] - 1
    ...
    for s in prange(shots):
        ...
        sample = local_repulsion_choice(G_cols, G_rows, max_weight, n, m)
        solutions[s] = sample
...
@njit(parallel=True)
def init_z(G_data, G_rows, G_cols):
    n_qubits = G_rows.shape[0] - 1
    ...
    for r in prange(n_qubits):
        ...
        degree = end - start
        val = np.count_nonzero(G_data[start:end])
...
def maxcut_tfim_sparse(
    ...
    degrees = init_J_and_z(G_m.data, G_m.indptr, G_m.indices)
    ...
    if IS_OPENCL_AVAILABLE and grid_size >= 128:
        ...
        h_mult = 2.0 / tot_t
        theta = init_theta(delta_t, tot_t, h_mult, n_qubits, degrees)
        ...
        opencl_context.maxcut_hamming_cdf_kernel(
            ...
            np.int32(n_qubits), deg_buf, args_buf, theta_buf, ham_buf
        )
        ...
        maxcut_hamming_cdf(n_qubits, degrees, quality, hamming_prob)

    best_solution, best_value = sample_for_solution(G_m.data, G_m.indptr, G_m.indices, shots, hamming_prob)
