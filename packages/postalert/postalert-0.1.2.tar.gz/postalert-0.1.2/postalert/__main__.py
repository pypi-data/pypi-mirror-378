...
_last_msg_time = 0
_last_request_time = 0    # <== Declare global here
...
def safe_request(session, method, url, **kwargs):
    global _last_request_time  # <== fix NameError by declaring global here
...
def search_requests_ids(keyword: str):
    ...
    session = requests.Session()
    response = safe_request(session, "POST", POSTMAN_HOST + GLOBAL_SEARCH_ENDPOINT,
                            json=format_search_request_body(keyword, 0, MAX_SEARCH_RESULTS),
                            headers=HEADERS)
    if response.status_code != 200:
        fail("Error fetching request IDs from Postman.com.", True)
    count = response.json()["meta"]["total"]["request"]
    ids = parse_search_response(response)
    print(f"[DEBUG] Total requests matching keyword '{keyword}': {count}")
    if count > MAX_SEARCH_RESULTS:
        max_pages = math.ceil(count / MAX_SEARCH_RESULTS)
        for i in range(1, max_pages):
            offset = i * MAX_SEARCH_RESULTS
            if offset > MAX_OFFSET:
                break
            r = safe_request(session, "POST", POSTMAN_HOST + GLOBAL_SEARCH_ENDPOINT,
                             json=format_search_request_body(keyword, offset, MAX_SEARCH_RESULTS),
                             headers=HEADERS)
            if r.status_code != 200:
                print("[WARN] Error fetching paged results from Postman.com, skipping.")
                continue
            ids.extend(parse_search_response(r))
    return ids

def parse_search_response(search_response):
    json_data = search_response.json()
    if "data" not in json_data:
        fail("No data field in search response.", True)
    data = json_data["data"]
    ids = []
    for d in data:
        request_item = {}
        request_id = d["document"]["id"]
        workspaces_raw = d["document"].get("workspaces", [])
        workspace_ids = [w["id"] for w in workspaces_raw]
...
def main():
    ...
    for keyword in keywords:
        ...
        request_infos, all_fetched_request_ids = search_request_info_collection_all_ids(keyword,
                                                                                       args.include,
                                                                                       args.exclude,
                                                                                       args.extend_workspaces,
                                                                                       args.raw,
                                                                                       args.strict,
                                                                                       output_folder)
