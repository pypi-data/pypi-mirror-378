...
class NTHUComplaintsMCP:
    ...
    def _setup_tools(self) -> None:
        ...
        async def test_submit_complaint(
            ...
            Returns:
                API 回應結果
            """
            async def submit_api_call(**params):
                submission = ComplaintSubmission(**params)
            ...
            return await ErrorHandler.safe_api_call(
                operation="申訴提交 API",
                api_func=submit_api_call,
                success_message="✅ 申訴提交測試完成",
                fail_message="❌ 申訴提交測試失敗",
                email=email,
                name=name,
        ...
        async def test_track_complaint(
            ...
            Returns:
                API 回應結果
            """
            async def track_api_call(**params):
                tracker = ComplaintTracker(**params)
            ...
            return await ErrorHandler.safe_api_call(
                operation="申訴追蹤 API",
                api_func=track_api_call,
                success_message="✅ 申訴追蹤測試完成",
                fail_message="❌ 申訴追蹤測試失敗",
        ...
        async def test_get_complaint_details(
            ...
            Returns:
                API 回應結果
            """
            async def details_api_call(**params):
                details = ComplaintDetails(**params)
            ...
            return await ErrorHandler.safe_api_call(
                operation="申訴詳情獲取 API",
                api_func=details_api_call,
                success_message="✅ 申訴詳情獲取測試完成",
                fail_message="❌ 申訴詳情獲取測試失敗",
                case_number=case_number,
                verification_code=verification_code,
            )
        ...
        async def check_api_connection() -> Dict[str, Any]:
            ...
            async def connection_check():
                async with httpx.AsyncClient() as client:
                    ...
                    response.raise_for_status()
                    return {"base_url": self.base_url, "status_code": response.status_code}

            return await ErrorHandler.safe_api_call(
                operation="API 連接檢查",
                api_func=connection_check,
                success_message="✅ API 連接正常",
                fail_message="❌ 無法連接到 API",
            )
        ...
        async def run_full_api_test(
            ...
            try:
                # 1. 檢查連接
                ErrorHandler.log_info("檢查 API 連接")
                ...
                # 4. 獲取詳情
                ErrorHandler.log_info("測試申訴詳情獲取")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Tools count not easily accessible in this FastMCP version
            print("🛠️  Tools setup completed", file=sys.stderr)
            print("✅ Server initialization complete", file=sys.stderr)

            print("🟢 [stderr] MCP server 正在啟動...", file=sys.stderr)  # 新增啟動訊息
            self.mcp.run()
            print("🟠 [stderr] MCP server 已正常關閉", file=sys.stderr)  # 新增關閉訊息
        except Exception as e:
