...
def login_user(email: str, password: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not user_doc:
            ...
            # Save to Firestore
            firebase_client.db.collection('users').document(user['uid']).set(default_user_data)
            user_data = default_user_data
        else:
            user_data = user_doc
            # Update last login time
            firebase_client.db.collection('users').document(user['uid']).update({
                'lastLoginAt': datetime.now().isoformat()
            })
...
def submit_complaint(
    ...
    try:
        ...
        complaint_data = {
            ...
            "submittedBy": user_session["uid"] if user_session and not anonymous else None,
            "submitterEmail": user_session["email"] if user_session and not anonymous else None,
...
def get_complaint_status(case_number: str) -> Dict[str, Any]:
    """
    查詢申訴狀態 (無需登入，適用於匿名申訴)
    ...
    try:
        ...
        # Return basic status information (safe for anonymous access)
        complaint_status = {
            "case_number": complaint_data.get("caseNumber"),
            "title": complaint_data.get("title"),
            "category": complaint_data.get("category"),
        ...
        # Only include responses if it's not sensitive
        public_responses = []
        for response in complaint_data.get("responses", []):
            if response.get("type") == "public":
                public_responses.append({
                    "content": response.get("content"),
                    "timestamp": response.get("timestamp")
                })

        complaint_status["public_responses"] = public_responses
