...
def login_user(email: str, password: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # In a real implementation, you would use Firebase Auth client SDK
        # or implement your own authentication mechanism

        # For now, we'll get user by email and assume authentication is handled elsewhere
...
def get_user_complaints_summary(session_token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if user_session["role"] == "admin":
            ...
            query = complaints_ref.order_by('submittedAt', direction=firestore.Query.DESCENDING)
        else:
            # Regular users can only see their own complaints
            query = complaints_ref.where('submittedBy', '==', user_session["uid"]).order_by('submittedAt', direction=firestore.Query.DESCENDING)
        ...
        for doc in complaints:
            ...
            complaint_list.append({
                ...
                "submitted_at": data.get("submittedAt").strftime("%Y-%m-%d %H:%M:%S") if data.get("submittedAt") else None,
                "last_updated": data.get("lastUpdated").strftime("%Y-%m-%d %H:%M:%S") if data.get("lastUpdated") else None,
    ...
    try:
        ...
        for response in complaint_data.get("responses", []):
            responses.append({
                ...
                "timestamp": response.get("timestamp").strftime("%Y-%m-%d %H:%M:%S") if response.get("timestamp") else None
            })
        ...
        for status in complaint_data.get("statusHistory", []):
            status_history.append({
                ...
                "timestamp": status.get("timestamp").strftime("%Y-%m-%d %H:%M:%S") if status.get("timestamp") else None
            })
        ...
        complaint_details = {
            ...
            "submitted_at": complaint_data.get("submittedAt").strftime("%Y-%m-%d %H:%M:%S") if complaint_data.get("submittedAt") else None,
            "last_updated": complaint_data.get("lastUpdated").strftime("%Y-%m-%d %H:%M:%S") if complaint_data.get("lastUpdated") else None,
