...
class VehiclesSSRService:
    ...
    async def get_vehicles_for_catalog(
        ...
        try:
            ...
            # Use new pagination fields if available, fallback to old ones
            has_next = getattr(response, 'has_next', getattr(response, 'next', None) is not None)
            has_previous = getattr(response, 'has_previous', getattr(response, 'previous', None) is not None)
