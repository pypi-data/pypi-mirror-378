...
class ActivityLogHandler(MemoryHandler):
    def __init__(self, endpoint, token, client, timeout=10, verify=True, max_retries=3, *args, **kwargs):
        ...
        self.token = token
        self.client = client  # Use the passed httpx client
    ...
    def close(self):
        """Close the handler by calling parent's close method."""
        # Call parent's close to properly clean up MemoryHandler state
        super().close()
