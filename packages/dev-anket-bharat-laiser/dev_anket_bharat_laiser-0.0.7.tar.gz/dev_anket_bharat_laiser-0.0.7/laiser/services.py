    ...
    def align_skills_to_taxonomy(self,raw_skills: List[str],document_id: str = '0',description: str = '',similarity_threshold: float = 0.20,top_k: int = 25,) -> pd.DataFrame:
        ...
        try:
            ...
            for skill in raw_skills:
                ...
                best_label = str(hits[0]['Skill']).strip()  # ESCO preferredLabel from index
                # map label -> tag if available in combined skills
                try:
                    taxonmy_skill_name = combined_df.loc[combined_df['SkillLabel'] == best_label, 'SkillLabel'  ].iloc[0]
                except Exception:
                ...
                rows.append({
                    ...
                    'Skill Tag': best_label,                        # canonical tag/id
                    'Knowledge Required': '[]',
                    'Task Abilities': '[]',
            ...
            return pd.DataFrame(rows, columns=[
                'Research ID','Description','Raw Skill','Skill Tag','Correlation Coefficient'
            ])
