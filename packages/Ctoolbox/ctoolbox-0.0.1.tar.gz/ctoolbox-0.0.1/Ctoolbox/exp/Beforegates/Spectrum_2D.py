...
def circuit(qubits, if_iso,flux, ctx=None):
    circ = init_biasdelay(qubits, if_iso, ctx)
    circ += [(("setBias", 'flux', flux), q) for q in qubits]
    circ += [(('Delay',1e-6),q) for q in qubits]
    circ += [(('R',0),q) for q in qubits]
    circ += [(('Delay',1e-6),q) for q in qubits]
    circ += readout(qubits, if_iso, ctx)
    return circ

def characterize(qubits, freq_st, freq_ed, freq_points,flux_st, flux_ed, flux_points, width, amp, signal, if_iso, preview, plot):
