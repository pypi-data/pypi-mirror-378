...
def characterize(qubits, freq_st, freq_ed, freq_points, signal, if_iso):
...
def analyze(thistask, rid=None, fignum1row=7, eachfigsize=(3,2.5), include='all'):
...
def plot_overview(thistask, rid=None, qnum1row=7,fignum1row=7, eachfigsize=(3,2.5)):
    if thistask is not None:
        result = thistask.result()
        rid = thistask.rid
    else:
