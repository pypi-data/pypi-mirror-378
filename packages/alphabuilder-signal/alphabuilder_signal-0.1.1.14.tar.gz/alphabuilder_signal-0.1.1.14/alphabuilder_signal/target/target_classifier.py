...
#----------xxx---------------
tc = TargetClassifier(tickers=['AAPL', 'MSFT', 'META'], start_date='2020-01-01', verbose=True, combined=False)
results = (tc.peak_detection(windows=[1, 2, 3])
           .trend_detection()
           .trough_detection(windows=[1, 2, 3, 4])
           .get_data()
           )

results['META'].head(50)