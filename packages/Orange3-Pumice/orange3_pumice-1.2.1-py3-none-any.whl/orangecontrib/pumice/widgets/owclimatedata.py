...
Months = [_tr.m[2, "January"], _tr.m[3, "February"], _tr.m[4, "March"], _tr.m[5, "April"], _tr.m[6, "May"], _tr.m[7, "June"],
          _tr.m[8, "July"], _tr.m[9, "August"], _tr.m[10, "September"], _tr.m[11, "October"], _tr.m[12, "November"], _tr.m[13, "December"]]
...
class OWClimateData(OWWidget):
    name = _tr.m[14, "Climate Data"]
    description = _tr.m[15, "Climate data"]
    icon = "icons/climatedata.svg"
    priority = 10

    class Error(OWWidget.Error):
        select_single = Msg(_tr.m[16, "Select a single station"])
        invalid_in_selection = Msg(_tr.m[17, "Input data does not have a column 'Station'"])

    class Warning(OWWidget.Warning):
        missing_stations = Msg(_tr.m[18, "Some selected stations are missing in the data set"])

    class Inputs:
        stations = Input(_tr.m[19, "Weather Stations"], Table)

    class Outputs:
        data = Output(_tr.m[20, "Climate Data"], Table)
    ...
    def __init__(self):
        ...
        tf = gui.radioButtonsInBox(
            self.controlArea, self, "time_selection", box=_tr.m[21, "Time Frame"],
            callback=self.time_selection_changed)
        tf.layout().setSpacing(1)
        gui.appendRadioButton(
            tf, _tr.m[22, "All-time monthly means"], insertInto=gui.hBox(tf))
        tf.layout().addSpacing(6)
        gui.appendRadioButton(
            tf, _tr.m[23, "Monthly means by decades"], insertInto=gui.hBox(tf))
        b = gui.hBox(tf)
        gui.appendRadioButton(
            tf, _tr.m[24, "Means for chosen month by decades"], insertInto=b)
        gui.comboBox(
            b, self, "month_index",
            items=Months,
        ...
        if INCLUDE_DAILY_VALUES:
            gui.appendRadioButton(
                tf, _tr.m[25, "Daily values"], insertInto=gui.hBox(tf))

        self.station_selector = ss = gui.radioButtonsInBox(
            self.controlArea, self, "geo_selection", box=_tr.m[26, "Weather Stations"],
        ...
        gui.appendRadioButton(
            ss, _tr.m[27, "All stations (averages by country)"], insertInto=next(boxi))
        b = next(boxi)
        gui.appendRadioButton(
            ss, _tr.m[28, "Countries on continent:"], insertInto=b)
        gui.comboBox(
            b, self, "continent",
            items=Continents,
        ...
        b = next(boxi)
        gui.appendRadioButton(ss, _tr.m[29, "Country: "], insertInto=b)
        gui.comboBox(
            b, self, "country",
            items=Countries,
            sendSelectedValue=True,
        ...
        b = next(boxi)
        gui.appendRadioButton(
            ss, _tr.m[30, "Single station: "], insertInto=b)
        gui.comboBox(
            b, self, "station",
            items=Stations,
            sendSelectedValue=True,
        ...
        vb = gui.vBox(self.controlArea, _tr.m[31, "Values"])
        b = gui.hBox(vb)
        gui.checkBox(
            b, self, "get_temperature", _tr.m[32, "Temperature: "],
            callback=self.value_selection_changed
        )
        gui.comboBox(
            b, self, "temperature_value",
            items=[_tr.m[33, "Average"], _tr.m[34, "Minimum"], _tr.m[35, "Maximum"], _tr.m[36, "Span"]],
            sendSelectedValue=False,
            callback=self.value_selection_changed,
        )
        gui.rubber(b)
        ...
        gui.checkBox(
            gui.hBox(vb), self, "get_precipitation", _tr.m[37, "Precipitation"],
            callback=self.value_selection_changed
        )
    ...
    def _decades_monthly(self):
        ...
        meta_attrs = [DiscreteVariable(_tr.m[38, "Decade"], values=decades)]
