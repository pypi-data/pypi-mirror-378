...
def _do_eigen_post_request(requesturl,payload,auth):
    ...
    if not str(resp.status_code).startswith("2"):
        logging.warning("Request to " + requesturl + " failed with status code " + str(resp.status_code))
        success = False
        errors = resp.text
    else:
        success = True
...
def validate_date_field(date_value, field_name):
    ...
    if isinstance(date_value, str):
        ...
        raise ValueError(f"Field '{field_name}' must be a datetime object, ISO format string, or relative time ('now', 'today', 'yesterday'). Got: {date_value}")
    ...
    raise ValueError(f"Field '{field_name}' must be a datetime object, ISO format string, timestamp, or relative time. Got type: {type(date_value)}")
