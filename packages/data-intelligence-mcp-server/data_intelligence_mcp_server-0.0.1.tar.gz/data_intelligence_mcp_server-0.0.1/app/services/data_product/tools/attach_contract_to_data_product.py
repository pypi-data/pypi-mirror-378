...
@service_registry.tool(
    name="data_product:attach_url_contract_to_data_product",
    description="""
...
async def attach_url_contract_to_data_product(
    ...
    # LOGGER.info(
    #     f"Attaching URL contract {contract_url} with name {contract_name} to the data product draft {data_product_draft_id}."
    # )
    ...
    try:
        await client.post(
            url=f"{settings.service_url}/data_product_exchange/v1/data_products/-/drafts/{request.data_product_draft_id}/contract_terms/{request.contract_terms_id}/documents",
            headers=headers,
            data=json,
        )
        ...
        # LOGGER.error(f"Failed to run attach_url_contract_to_data_product tool. Error while attaching URL contract to data product: {str(e)}")
        raise ServiceError(
            f"Failed to run attach_url_contract_to_data_product tool. Error while attaching URL contract to data product: {str(e)}"
        )

    # LOGGER.info(
    #     f"Attached URL contract {contract_url} with name {contract_name} to the data product draft {data_product_draft_id}."
    # )
