...
@service_registry.tool(
    name="data_product:attach_business_domain_to_data_product",
    description="""
...
async def attach_business_domain_to_data_product(
    ...
    # LOGGER.info(
    #     f"Attaching business domain {domain} to the data product draft {data_product_draft_id}."
    # )
    ...
    try:
        response = await client.post(
            url=f"{settings.service_url}/v2/asset_types/ibm_data_product_domain/search?catalog_id={DPH_CATALOG_ID}&hide_deprecated_response_fields=false",
            headers=headers,
            data=json,
        )
        ...
        raise ServiceError(
            f"Failed to run attach_business_domain_to_data_product tool. Error while getting business domain information: {str(e)}"
        )
    ...
    if not domain_id:
        # LOGGER.error(
        #     f'Domain name "{domain}" is not found, so it is not attached to {data_product_draft_id}. Here are the available domains: {available_domains}'
        # )
        raise ServiceError(
            f'Domain name "{request.domain}" is not found, so it is not attached to {request.data_product_draft_id}. Here are the available domains: {available_domains}'
        )
    ...
    try:
        response = await client.patch(
            url=f"{settings.service_url}/data_product_exchange/v1/data_products/-/drafts/{request.data_product_draft_id}",
            headers=headers,
            data=json,
        )
    except ExternalAPIError:
        ...
        # LOGGER.error(f"Failed to run attach_business_domain_to_data_product tool. Error while attaching business domain to data product: {str(e)}")
        raise ServiceError(
            f"Failed to run attach_business_domain_to_data_product tool. Error while attaching business domain to data product: {str(e)}"
        )

    # LOGGER.info(
    #     f"Business domain {domain} attached to the data product draft {data_product_draft_id}."
    # )
