...
def get_embeddings(source_graph_number, qc_solver_to_use, data_dir):
    ...
    if os.path.isfile(file_path):
        ...
        # source graph
        graph = GraphProperties(graph_id=source_graph_number)
        source_graph = nx.Graph()
        source_graph.add_nodes_from([k for k in range(graph.graph_data['num_nodes'])])
        source_graph.add_edges_from(graph.graph_data['edge_list'])
