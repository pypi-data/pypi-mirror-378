...
# 使用示例：
entities = predict_entities("100%负荷联合循环试验工况下，汽轮机高压缸效率为87.37%，与2023年大修前试验结果（85.94%）相比，提高约1.43个百分点。汽轮机机输出毛功率为128.570MW，汽轮机试验热耗率为9209.1kJ/(kW.h)，试验热效率为39.09%", model, tokenizer)
print("发现的实体:")
for entity in entities:
    print(f"  实体文本: {entity['text']}")
    print(f"  字符位置: {entity['start']}-{entity['end']}")
    print(f"  对应Tokens: {entity['tokens']}")
    print()
