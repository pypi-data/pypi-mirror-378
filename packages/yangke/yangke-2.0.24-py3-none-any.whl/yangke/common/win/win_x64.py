...
def find_pic(template: str, screen: str = "temp.png", threshold: float = 0.8):
    """
    从图片中查找另一张图片的位置，找到的坐标必然为正值，该方法对光照敏感，不同光照可能导致匹配错误；

    methods = {
    cv2.TM_SQDIFF: "平方差匹配 - 最小值为最佳匹配",
    cv2.TM_SQDIFF_NORMED: "归一化平方差匹配 - 对光照敏感",
    cv2.TM_CCORR: "相关匹配 - 对亮度敏感",
    cv2.TM_CCORR_NORMED: "归一化相关匹配 - 比TM_CCORR稍好",
    cv2.TM_CCOEFF: "相关系数匹配 - 考虑整体统计特性",
    cv2.TM_CCOEFF_NORMED: "归一化相关系数匹配 - 最佳选择，对光照变化鲁棒"
}
...
def find_pic1(template: str, screen: str = "temp.png", threshold: float = 0.8):
    """
    ...
    :param template: 小图片，即在大图片中查找该图片的相对位置，如果是数组，颜色通道顺序需要为RGB
    :param threshold: 阈值，越大则要求越相似，建议0.7以上，否则容易错误匹配
    :return: exists, (x,y), similarity
    """
    try:
        scr = pic2ndarray(screen)
        tp = pic2ndarray(template)
        # 改用 TM_CCOEFF_NORMED 方法
    ...
    # 对于 TM_CCOEFF_NORMED，使用最大值作为相似度
    similarity = max_val

    # 总是返回最匹配的位置，但通过 exists 标记是否达到阈值
    exist = similarity >= threshold

    logger.debug(f"找图结果: 相似度={similarity:.2f}, 阈值={threshold}, 匹配={'是' if exist else '否'}")
    ...
    # 对于 TM_CCOEFF_NORMED，使用最大值位置
    coordinate = max_loc
    return exist, coordinate, similarity


def find_pic_sift(template, screen, threshold=0.7):
    """
    使用SIFT特征点匹配
    """
    scr = pic2ndarray(screen)
    ...
    # 转换为灰度图
    if len(scr.shape) == 3:
        gray_scr = cv2.cvtColor(scr, cv2.COLOR_RGB2GRAY)
    else:
        gray_scr = scr

    if len(tp.shape) == 3:
        gray_tp = cv2.cvtColor(tp, cv2.COLOR_RGB2GRAY)
    else:
        gray_tp = tp

    # 创建SIFT检测器，兼容不同版本的OpenCV
    try:
        # OpenCV 4.x及以上版本
    ...
    # 检测关键点和描述符
    kp1, des1 = sift.detectAndCompute(gray_tp, None)
    kp2, des2 = sift.detectAndCompute(gray_scr, None)
    ...
    if len(good_matches) > 4:
        ...
        if M is not None:
            ...
            # 返回中心点坐标
            center_x = int(np.mean([point[0][0] for point in dst]))
            center_y = int(np.mean([point[0][1] for point in dst]))

            # 计算匹配质量（基于匹配点数量）
            similarity = len(good_matches) / max(len(kp1), len(kp2))

            return True, (center_x, center_y), similarity
