...
def send_teams_message(notification: TeamsNotification) -> bool:
    ...
    try:
        ...
        print(f"[DEBUG] Teams response: {response.status_code} - {response.text}")
        ...
        print(f"[DEBUG] Exception sending Teams message: {str(e)}")

    notification.save(update_fields=["status", "message"])
    return False  # ✅ Only return False if failed
