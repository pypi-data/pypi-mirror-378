...
try:
    ...
    STANDARD_AVAILABLE = False  # 强制使用独立实现
except ImportError:
...
def create_sample_config(output_path: str = "sif_config_template.json") -> None:
    """
    创建示例配置文件
...
def process_with_config_file(
    config_file: str, quiet: bool = False, use_standalone: bool = False
) -> Dict[str, Any]:
    """
    使用配置文件处理航线规划
    ...
    try:
        ...
        # 简化版本：只使用独立实现
        from .api.stdin_processor import process_with_standalone, setup_logging, create_timestamp_dir, generate_output_files
...
def main() -> int:
    ...
    # 其他选项
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="详细模式，显示完整信息（默认为静默模式）"
    )
