...
class TestAgentPressTools:
    ...
    def test_get_description(self):
        ...
        assert (
            AgentPressTools.SB_FILES_TOOL.get_description()
            == "Read, write, and edit files"
        )
        assert (
            AgentPressTools.SB_SHELL_TOOL.get_description() == "Execute shell commands"
        )
        assert (
            AgentPressTools.BROWSER_TOOL.get_description()
            == "Browse websites and interact with web pages"
        )
        assert (
            AgentPressTools.WEB_SEARCH_TOOL.get_description()
    ...
    def test_get_description_invalid_tool(self):
        ...
        # Mock the get_description method to test the error case
        with pytest.raises(ValueError, match="No description found for invalid_tool"):
            # This will fail because INVALID_TOOL is not in the descriptions dict
            AgentPressTools.get_description(MockTool.INVALID_TOOL)
...
class TestMCPTools:
    ...
    @patch("ba.tools.FastMCPClient")
    @pytest.mark.asyncio
    async def test_mcp_tools_initialize_without_client(self, mock_fastmcp):
