...
class AgentCreateRequest:
    ...
    agentpress_tools: dict[AgentPressTools, AgentPress_ToolConfig] | None = None
...
class AgentsClient:
    """SDK client for belaraby Agents API with httpx client supporting custom headers"""
    ...
    def __init__(
        ...
        Args:
            base_url: Base URL of the API (e.g., "https://belaraby.ai/api")
    ...
    def _handle_response(self, response: httpx.Response) -> dict[str, Any]:
        ...
        # Handle redirect responses (authentication issues)
        if response.status_code in [301, 302, 303, 307, 308]:
            raise httpx.HTTPStatusError(
                f"Authentication required. Please check your API key. Redirected to: {response.content.decode('utf-8', errors='replace')}",
                request=response.request,
                response=response,
            )
        ...
        if 'application/json' not in content_type:
            content_str = response.content.decode('utf-8', errors='replace')[:200]
            raise ValueError(f"Expected JSON response but got content-type: {content_type}. Content: '{content_str}'")
    ...
    async def get_pipedream_tools(
        self, agent_id: str, profile_id: str, version: str | None = None
...
client = create_agents_client(
    base_url="https://belaraby.ai/api",
