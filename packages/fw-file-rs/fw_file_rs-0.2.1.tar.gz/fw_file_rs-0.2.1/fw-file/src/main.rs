...
fn main() -> io::Result<()> {
    ...
    match &cli.command {
        Commands::GetDcmMeta { file_path, tags } => {
            ...
            let bytes = read_until_pixel_data(&mut file).expect("Failed to read DICOM metadata");
            let tag_slices: Vec<&str> = tags.iter().map(|s| s.as_str()).collect();

            match get_dcm_meta(&bytes, &tag_slices) {
                Ok(meta) => {
                    if meta.is_empty() {
        ...
        Commands::DeidDcm {
...

            {
                ...
                let dcm_header_bytes = read_until_pixel_data(&mut source_file).unwrap();
