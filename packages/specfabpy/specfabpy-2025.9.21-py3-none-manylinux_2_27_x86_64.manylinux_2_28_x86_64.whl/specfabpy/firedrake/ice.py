...
#        E_EIE.vector()[:] = self.sf.E_EIE_arr(...)
    ...
    def get_lamxi(self):
        ...
        Ix = np.argmax(mi[:, :, 0], axis=1)  # (n_nodes,) -> index of vector with max x
        Iy = np.argmax(mi[:, :, 1], axis=1)  # (n_nodes,) -> index of vector with max y
        Iz = np.argmax(mi[:, :, 2], axis=1)  # (n_nodes,) -> index of vector with max z

        rows = np.arange(len(lami))
        lami_sorted = np.stack([
            lami[rows, argmax_x],
            lami[rows, argmax_y],
            lami[rows, argmax_z]
        ], axis=1)
