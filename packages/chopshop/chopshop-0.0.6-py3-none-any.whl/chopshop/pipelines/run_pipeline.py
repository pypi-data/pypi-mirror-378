#!/usr/bin/env python3
"""
ChopShop Pipeline Runner (robust templating + flexible call resolution)
...
- Calls:
    * "cs.*"      → call via a ChopShop() instance (e.g., cs.text.analyze_with_dictionaries)
    * dotted path → import and call any function (e.g., chopshop.helpers.feature_gather.aggregate_features)
...
def resolve_call(call_name: str, cs: ChopShop):
    """
    - 'cs.audio.convert_to_wav' → getattr chain on ChopShop instance
...
def run_item_step_for_one_input(
    *, step: dict, input_path: Path, cs: ChopShop, item_artifacts: Dict[str, Any],
...
def run_global_step(
    *, step: dict, cs: ChopShop, globals_ctx: Dict[str, Any], vars_ctx: Dict[str, Any], manifest_path: Path
) -> Tuple[str, Dict[str, Any], Dict[str, Any]]:
    call = step["call"]
    params = step.get("with", {})
...
def main():
    ap = argparse.ArgumentParser(description="ChopShop Pipeline Runner (robust templating + flexible calls)")
    ...
    group.add_argument("--preset", help="Preset name (chopshop/pipelines/presets/<name>.yaml)")
    ...
    for idx, step in enumerate(steps, 1):
        ...
        if scope == "item":
            ...
            status, new_globals, err = run_global_step(
                step=step, cs=cs, globals_ctx=globals_ctx, vars_ctx=vars_ctx, manifest_path=out_manifest_path
            )
            if status != "ok":
