...
class BILLIHandler(FormatHandler):
    ...
    def _create_header(self, width, height, quality, data_size):
        ...
        header = struct.pack(
            '<5sBIIBI',  # little-endian: 5s=signature, B=version, I=width, I=height, B=quality, I=data_size
            self.signature,
            self.version,
            width,
            height,
    ...
    def _parse_header(self, header_data):
        ...
        signature, version, width, height, quality, data_size = struct.unpack('<5sBIIBI', header_data)
