...
class AutoSchema(Base):
    ...
    def add_hybrid_property_field(self, attribute: str, original_attribute: str, field_type: Any | None):
        ...
        if field_type is None:
            ...
            hybrid_prop = getattr(type(self.model), original_attribute, None)
            fget = getattr(hybrid_prop, "fget", None)
            if fget is not None:
                field_type = getattr(fget, "__annotations__", {}).get("return")

        schema_field_cls = field_type if isinstance(field_type, type) else fields.Str
