...
cdef class AdaptiveFormula:
    ...
    cdef dict weight_history  # Enterprise: track weight evolution
    cdef dict magnitude_cache  # Enterprise: cache field magnitudes
    cdef double adjustment_factor  # Enterprise: factor de ajuste (0.3 default)
    ...
    cdef tuple _optimize_params(self, str field_name, double similarity, value):
        """ML optimization - Enterprise/Pro with weight calibration"""
        cdef double optimized_weight, optimized_criticality
        cdef double expert_weight, proposed_weight, final_weight
        cdef double magnitude, normalized_magnitude
        
        # Get expert weights from config
        expert_weight = <double>self.config[field_name].get('weight', 1.0)
        expert_criticality = <double>self.config[field_name].get('criticality', 1.5)
        ...
        if self.tier == 'enterprise' and len(self.history) > 20:
            ...
            if isinstance(value, (int, float)):
                self.magnitude_cache[field_name].append(abs(<double>value))
    ...
    cpdef void set_adjustment_factor(self, double factor):
