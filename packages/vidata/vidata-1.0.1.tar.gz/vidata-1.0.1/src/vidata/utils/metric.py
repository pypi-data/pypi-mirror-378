...
class ConfusionMatrixTorch:
    def __init__(self, num_classes, ignore_index: int | None = None,
                 dtype: torch.dtype = torch.long, device: torch.device | None = None):
        self.num_classes = int(num_classes)
        self.ignore_index = ignore_index
        self.mat = torch.zeros((self.num_classes, self.num_classes),
                               dtype=dtype, device=(device or torch.device("cpu")))
    ...
    def update(self, pred: torch.Tensor, gt: torch.Tensor) -> None:
        ...
        # 1D histogram -> reshape to (n, n)
        conf = torch.bincount(inds, minlength=self.num_classes * self.num_classes).reshape(self.num_classes, self.num_classes)
