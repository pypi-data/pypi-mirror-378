...
from . import dataset, secret, provider, routing_cli
from .client import (
    create_workspace,
    delete_workspace,
    list_workspaces,
...
def login():
    ...
    try:
        ...
        def make_auth_request(method: str, path: str, **kwargs) -> httpx.Response:
            ...
            base_url = os.getenv("MIXTRAIN_PLATFORM_URL", "https://platform.mixtrain.ai/api/v1")
            url = f"{base_url}{path}"
...
workspace_app = typer.Typer(
    help="Manage workspaces", invoke_without_command=True
)
...
def create_workspace_cmd(
    ...
    try:
        ...
        rprint(f"[green]✓[/green] Successfully created workspace '{workspace_data.get('name')}'!")
