...
class RoutingCondition(BaseModel):
    ...
    field: str = Field(description="Field path to evaluate (e.g., 'user.tier', 'request.type')")
...
class RoutingTarget(BaseModel):
    ...
    weight: float = Field(default=1.0, ge=0.0, le=1.0, description="Routing weight (0.0-1.0)")
    label: Optional[str] = Field(None, description="Optional label for this target")
    timeout_ms: Optional[int] = Field(None, ge=100, description="Timeout in milliseconds")
    retry_count: Optional[int] = Field(None, ge=0, le=5, description="Number of retries")

    # Provider-specific fields
    function_name: Optional[str] = Field(None, description="Function name for Modal provider")
    request_class: Optional[str] = Field(None, description="Request class for Modal provider")
    headers: Optional[Dict[str, str]] = Field(None, description="Custom HTTP headers")

    # Runtime fields
    is_shadow: bool = Field(default=False, description="Whether this is a shadow target")
...
class RoutingRule(BaseModel):
    ...
    description: Optional[str] = Field("", description="Description of what this rule does")
    priority: int = Field(default=0, description="Priority (higher values evaluated first)")
    ...
    def validate_targets(cls, v, values):
        ...
        if strategy == RoutingStrategy.SPLIT:
            ...
            if len(v) != 2:
                raise ValueError('Shadow strategy requires exactly 2 targets (primary and shadow)')
...
class RoutingConfig(BaseModel):
    ...
    description: Optional[str] = Field("", description="Description of this configuration")
...
class RoutingResult(BaseModel):
    ...
    explanation: str = Field(description="Human-readable explanation of the routing decision")
    execution_time_ms: Optional[float] = Field(None, description="Time taken for routing decision")
    metadata: Optional[Dict[str, Any]] = Field(
...
class TestRequest(BaseModel):
    ...
    request_data: Dict[str, Any] = Field(description="Sample request data to test routing against")
    expected_rule: Optional[str] = Field(None, description="Expected rule name for validation")
