...
class DataLoader:
    ...
    def sync_regions(
        ...
        DataLoader.check_check_passed(check_passed=checks_passed)
        ...
        general_df = general_db.read_tables(Regions)
        company_df = company_db.read_tables(Regions)

        print("general df:", general_df)
        ...
        if not inconsistent.is_empty():
            preview = inconsistent.head(5).to_dicts()
            raise ValueError(
    ...
    def write_company(
        ...
        *,
        ...
        raise ValueError("Regions not synchronized") if not regions_synced else None
...
def _copy_direct_quarter_worker(
    *,
    schema: str,
    table: str,
    quarter_key: str,
...
def write_time_features(
    ...
    *,
    ...
    if table_is_empty:
        ...
        # Build quarterly key: "YYYY-Qn"
        qdf = df.with_columns(
            [
                pl.col(time_column).dt.year().alias("_year"),
                pl.col(time_column).dt.quarter().alias("_q"),
            ]
        ).with_columns(
            pl.concat_str([pl.col("_year").cast(pl.Utf8), pl.lit("-Q"), pl.col("_q").cast(pl.Utf8)]).alias(
                "_quarter_key"
            )
        )

        # Sort within each quarter for locality (ts, space)
        ...
        # Prepare quarterly payloads
        logger.info("Preparing quarterly batches for direct COPY...")
        quarterly_batches: list[tuple[str, bytes | str, bool, int]] = []  # (qkey, payload, is_binary, rows)
        for _, subdf in qdf.group_by("_quarter_key", maintain_order=True):
            subdf = subdf.drop(["_year", "_q"]).sort(sort_cols)

            if can_binary:
                # Try to use psycopg v3's binary COPY writer format via pandas-like pipe:
                # We don't have a turnkey binary encoder for Polars; CSV is our portable fallback.
                # To still leverage binary when possible, we use psycopg v3 text encoding only if a
                # simple adapter isn't available. If you have a custom binary packer, plug it here.
                # For now we fall back to CSV even with psycopg v3 (copy_expert FORMAT binary requires
                # packed tuples). Keep the switch for future upgrade.
                buf = BytesIO()
                subdf.drop("_quarter_key").write_csv(buf)  # fallback CSV
                payload = buf.getvalue().decode("utf-8")
                quarterly_batches.append(
                    (subdf.select(pl.col("_quarter_key")).unique().item(), payload, False, subdf.height)
                )
            else:
                # CSV payload
                buf = BytesIO()
                subdf.drop("_quarter_key").write_csv(buf)
                payload = buf.getvalue().decode("utf-8")
                quarterly_batches.append(
                    (subdf.select(pl.col("_quarter_key")).unique().item(), payload, False, subdf.height)
                )

        logger.info("Prepared %d quarterly batches for direct COPY.", len(quarterly_batches))
            ...
            if max_workers > 1:
                with ProcessPoolExecutor(max_workers=max_workers) as ex:
                    futs = [
                        ...
                        for (qkey, payload, is_bin, rows) in quarterly_batches
                    ]
                    for f in as_completed(futs):
                        pbar.update(f.result())
            else:
                for qkey, payload, is_bin, rows in quarterly_batches:
                    processed = _copy_direct_quarter_worker(
                        schema=schema,
                        table=table_name,
                        quarter_key=qkey,
                        payload=payload,
