...
hybrid_array_cache = []
class BoolHybridArray(MutableSequence,Exception):
    ...
    def __init__(self, split_index: int, size=None, is_sparse=False ,Type:Callable = None) -> None:
        ...
        for ref, existing_hash in hybrid_array_cache:
            ...
            try:
                if self.size != existing_array.size:
                    continue
                elif all(self[i] == existing_array[i] for i in range(len(self))):
                    self._cached_hash = existing_hash
                    return
            except Exception:
                continue
        new_hash = len(hybrid_array_cache)
    ...
    def __next__(self):
        ...
        except Exception:self.generator = iter(self)
    def __contains__(self, value) -> bool:
        if not isinstance(value, (bool,BHA_bool,np.bool_)):return False
    ...
    def __and__(self, other) -> BoolHybridArray:
        ...
        return BoolHybridArr(map(operator.and_, self, other))
    def __int__(self):
        if self.size == 0:
            return 0
    ...
    def __or__(self, other) -> BoolHybridArray:
        ...
        return BoolHybridArr(map(operator.or_, self, other))
    ...
    def __xor__(self, other) -> BoolHybridArray:
        ...
        return BoolHybridArr(map(operator.xor, self, other))
    ...
    def __copy__(self) -> BoolHybridArray:
        return self.copy()
    def find(self,value):
        return np.array([i for i in range(len(self)) if self[i]==value],dtype = np.int64)
    ...
    def count(self, value) -> int:
        value = bool(value)
        return sum(v is value for v in self)
    def optimize(self) -> None:
        self.__dict__ = BoolHybridArr(self).__dict__
        return self
    def memory_usage(self, detail=False) -> dict | int:
        small_mem = len(self.small)//8
        large_mem = len(self.large)*4
        equivalent_list_mem = 40 + 8 * len(self)
        equivalent_numpy_mem = 96+len(self)
        ...
        return {
            ...
            "密集区占用": small_mem,
            "稀疏区占用": large_mem,
            "对比原生list节省": f"{(1 - total/equivalent_list_mem)*100:.1f}%",
            "对比numpy节省": f"{(1 - total/equivalent_numpy_mem)*100:.1f}%" if equivalent_numpy_mem > 0 else "N/A",
            "是否需要优化": "是" if need_optimize else "否",
            "优化理由/说明": optimize_reason if need_optimize else "当前存储模式已适配数据特征，无需优化"
        }
    def get_shape(self):
        return (self.size,)
def BoolHybridArr(lst: Iterable, is_sparse=None, Type = None) -> BoolHybridArray:
    ...
    arr = BoolHybridArray(split_index, size, is_sparse, Type)
    ...
    arr.large = array.array('I', arr.large)
    return arr
def TruesArray(size, Type = None):
    split_index = min(size//10, math.isqrt(size))
    split_index = max(split_index, 1)
    return BoolHybridArray(split_index,size,Type = Type)
def FalsesArray(size, Type = None):
    split_index = min(size//10, math.isqrt(size))
    split_index = max(split_index, 1)
    return BoolHybridArray(split_index,size,True,Type = Type)
Bool_Array = np.arange(2,dtype = np.uint8)
class BHA_bool:
    def __init__(self,value):
        self.data = Bool_Array[1] if value else Bool_Array[0]
...
class BHA_List(list):
    ...
    def __repr__(self):
        return str(self)
    def __or__(self,other):
        return BHA_List([a|b for a,b in zip(self,other)])
    def __and__(self,other):
        return BHA_List([a&b for a,b in zip(self,other)])
...
builtins.BHA_Bool = lambda v:T if v else F
