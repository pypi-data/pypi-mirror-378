...
class kartverketsAPI:
    ...
    def transform_single_geonorge(self,response : dict) -> list[dict] | None:
        ...
        if json_data:
            ...
            return None
    def decode_address(self,address : str = None, lat = None, lon = None) -> list:
        raw_output_address = None
        clean_address = None

        try:
            self.logger.debug(f'Searching for address {address} with geonorge')
            raw_output_address = self.search_address(address=address,lat = lat, lon = lon)
        except Exception as e:
            self.logger.error(f'Error getting address {address}: {e}')
    ...
    async def get_transactions(self, transfer_type : Literal["active","historical","all"],ownership_type : Literal["eier","andel"] ,lat = None, lon = None,address : str = None ,section_num : int = None,coop_unit_num : int =None, coop_org_num : int = None):
        ...
        if lat and lon:
            result = self.decode_address(lat = lat, lon = lon)
