...
class PdfStream(object):
    ...
    def to_document(self, *, land_scape: bool = False, a4: bool = False) -> DocumentPdf:
        ...
        if not self.collection_pages.is_null:
            if _doc is None:
                ...
                _doc.add_pages(self.collection_pages.pages)
        return _doc

    def thresold(self, *, land_scape: bool = False, a4: bool = False, dpi=300) -> DocumentPdf:
        images: list[ImageObject] = self.collection_images.images
        if not self.collection_pages.is_null:
            tmp_doc = DocumentPdf.create_from_pages(self.collection_pages.pages)
    ...
    def to_files_images(
        ...
        if not self.collection_images.is_empty:
            image_stream.add_images(self.collection_images.images)
        if not self.collection_pages.is_null:
    ...
    def to_files_pdf(
        ...
        pages_document = []
        if not self.collection_pages.is_null:
            pages_document.extend(self.collection_pages.pages)
        if not self.collection_images.is_empty:
