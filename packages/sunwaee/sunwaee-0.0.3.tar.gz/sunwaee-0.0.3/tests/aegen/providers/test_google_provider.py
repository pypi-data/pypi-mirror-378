...
from sunwaee.aegen.providers.google import (
    ...
    google_sse_adapter,
    google_response_adapter,
)


@pytest.fixture
def google_api_key():
    return "AIzaSy-test-google-api-key-12345"
...
class TestGoogleProvider:
    ...
    def test_google_headers_adapter_success(self, monkeypatch, google_api_key):
    ...
    def test_google_headers_adapter_missing_key(self, monkeypatch):
    ...
    def test_google_messages_adapter_missing_messages(self):
        with pytest.raises(
            ValueError, match="Messages are required for google messages adapter"
        ):
            google_messages_adapter()
    ...
    def test_google_tools_adapter_missing_tools(self):
        with pytest.raises(
            ValueError, match="Tools are required for google tools adapter"
        ):
            google_tools_adapter()
    ...
    def test_google_payload_adapter_missing_messages(self):
        with pytest.raises(
            ValueError, match="Messages are required for google payload adapter"
        ):
            google_payload_adapter()

    def test_google_sse_adapter(self):
        mapping = google_sse_adapter()
    ...
    def test_google_response_adapter(self):
        mapping = google_response_adapter()
