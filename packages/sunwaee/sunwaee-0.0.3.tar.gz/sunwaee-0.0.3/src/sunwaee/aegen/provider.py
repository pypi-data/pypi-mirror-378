...
class Provider(BaseModel):
    ...
    name: str
    url: str
    headers_adapter: typing.Callable[[dict], dict] = Field(
        default_factory=lambda: default_headers_adapter
    )
    payload_adapter: typing.Callable[[dict], dict] = Field(
        default_factory=lambda: default_payload_adapter
    )
    messages_adapter: typing.Callable[[list[dict]], list[dict]] = Field(
        default_factory=lambda: default_messages_adapter
    )
    tools_adapter: typing.Callable[[list[dict]], list[dict]] = Field(
        default_factory=lambda: default_tools_adapter
    )
    sse_adapter: typing.Callable[[], dict] = Field(
        default_factory=lambda: default_sse_adapter
    )
    response_adapter: typing.Callable[[], dict] = Field(
