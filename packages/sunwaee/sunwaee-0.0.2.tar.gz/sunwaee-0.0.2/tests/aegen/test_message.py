...
class TestMessage:
    ...
    def test_message_from_dict_tool_call(self, sample_message_tool_call):
        v_message = Message.from_dict(sample_message_tool_call)
        assert v_message.role == "assistant"
        assert v_message.content == "Here's a tool call."
        assert len(v_message.tool_calls) == 1
        assert v_message.tool_calls[0].id == "tc_123"
        assert v_message.tool_calls[0].type == "function"
        assert v_message.tool_calls[0].name == "search_codebase"
        assert v_message.tool_calls[0].arguments == {"query": "auth implementation"}
    ...
    def test_message_from_dict_missing_content(self):
        message = {"role": "user"}
        with pytest.raises(ValueError, match="Message requires 'role' and 'content'"):
            Message.from_dict(message)

    def test_message_from_dict_invalid_tool_calls(self):
        message = {
            "role": "assistant",
            "content": "some tc",
