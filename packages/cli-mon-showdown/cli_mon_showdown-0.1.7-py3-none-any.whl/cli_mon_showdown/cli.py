...
def pack_team(path: str, formatname: str = "gen7ou", ps_path: str = None) -> str:
    ...
    if ps_path is None:
        ps_path = showdown_wrapper.find_pokemon_showdown()
    ...
    if not ps_executable.exists():
        raise RuntimeError(f"Pokemon Showdown not found at {ps_executable}")
    
    try:
        # First, pack the team using Pokemon Showdown
        pack_result = subprocess.run(
            ["node", str(ps_executable), "pack-team"],
            input=team_content,
            capture_output=True,
            text=True,
            check=True,
    ...
    try:
        ...
        validate_result = subprocess.run(
            ["node", str(ps_executable), "validate-team", formatname],
            input=team_content,
            capture_output=True,
            text=True,
            timeout=30
