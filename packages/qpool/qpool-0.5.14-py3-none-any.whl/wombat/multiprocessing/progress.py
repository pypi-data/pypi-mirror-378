...
def _update_progress_bars(
    ...
    dictionary but does not write to it. It does not hold a long-running lock
    to ensure the UI remains responsive, at the cost of potential minor
    inconsistencies between the 'Total' and worker counts in a single refresh.
    """
    try:
        # Read counts without a long-running lock for responsiveness. Each `.get()`
        # call will manage its own short-lived lock.
