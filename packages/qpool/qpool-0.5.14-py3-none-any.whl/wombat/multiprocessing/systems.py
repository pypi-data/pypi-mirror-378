...
class AccountingSystem(BaseSystem):
    ...
    async def _increment_count(
        ...
        await worker.increment_local_account_count(key, value)
    ...
    async def on_batch_received(batch: list[Task], worker: "Worker"):
        """
        Processes 'received' counting rules for an entire batch of tasks.

        This hook is responsible for the 'initial', 'generated', and 'logs' counts.
        It aggregates counts for the entire batch and performs bulk updates
        to the shared accounting store to minimize lock contention. It also removes
        the `Uncounted` or `Generated` marker traits to prevent re-counting on retries.
        """
        batch_counts = {"initial": 0, "generated": 0, "logs": 0}

        for task in batch:
            processed_a_received_event = False
            for trait in task.traits[:]:  # Iterate over copy
                if trait.counting_rules:
                    for rule in trait.counting_rules:
                        if rule.on_event == "received":
                            # We know 'generated' and 'logs' are the only ones.
                            if rule.counter_name in batch_counts:
                                batch_counts[rule.counter_name] += 1
                                processed_a_received_event = True

            if not processed_a_received_event:
                batch_counts["initial"] += 1
        ...
        # Perform bulk updates for each counter if the count is greater than zero.
        update_tasks = [
            _increment_worker_raw_count(worker, key, value)
    ...
    async def on_task_received(task: "Task", worker: "Worker") -> None:
        ...
        Initial task counting is handled by the `on_batch_received` hook to
        prevent extreme lock contention under high task loads.
        """
        pass
