...
class KS3ClientError(Exception):
    ...
    def __init__(self, reason, request_id=None, *args):
        super(KS3ClientError, self).__init__(reason, *args)
        self.reason = reason
        self.request_id = request_id

    def __repr__(self):
        return 'KS3ClientError: %s, request_id: %s' % (self.reason, self.request_id)
...
class KS3ServerError(Exception):
    def __init__(self, status, reason, body=None, request_id=None, *args):
        super(KS3ServerError, self).__init__(status, reason, body, request_id, *args)
        self.status = status
        self.reason = reason
        self.body = body or ''
        self.request_id = request_id
        ...
        if self.body:
            ...
            self.message = 'No body in response.'
    ...
    def __repr__(self):
        return '%s: %s %s\n%s' % (self.__class__.__name__,
                                  self.status, self.reason, self.body if self.body else self.request_id)
...
class StorageResponseError(KS3ServerError):
    ...
    def __init__(self, status, reason, body=None, request_id=None):
        self.resource = None
        super(StorageResponseError, self).__init__(status, reason, body, request_id=request_id)
...
class S3ResponseError(StorageResponseError):
    ...
    def __init__(self, status, reason, body=None, request_id=None):
        super(S3ResponseError, self).__init__(status, reason, body, request_id=request_id)


class KS3ResponseError(S3ResponseError):
    """
    Error in response from KS3.
    """

    def __init__(self, status, reason, body=None):
        super(KS3ResponseError, self).__init__(status, reason, body)
