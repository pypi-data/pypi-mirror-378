...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Plotting.py 8986 2025-07-23 03:39:41Z jhill $
...
def plot(*pargs,**kwargs):
	...
	if allvec:
		for i in range(len(pargs)):
			if isinstance(pargs[i], numpy.matrix):  pargs[i] = numpy.asarray(pargs[i])
...
def imagesc(img, x=None, y=None, axes=None, hold=False, drawnow=True, aspect='image', balance=None, colorbar=False, grid=False, ydir='reverse', **kwargs):
	...
	tol = 1e-10
	regular = ( len(x) == 1 or (numpy.ptp(numpy.diff(x)) < tol) ) and ( len(y) == 1 or (numpy.ptp(numpy.diff(y)) < tol) )
