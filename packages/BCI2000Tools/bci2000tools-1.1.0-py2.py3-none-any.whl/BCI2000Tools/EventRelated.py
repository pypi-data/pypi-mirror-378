...
# ///////////////////////////////////////////////////////////////////////////
# $Id: EventRelated.py 9000 2025-08-05 18:07:32Z jhill $
...
if __name__ == '__main__':
	...
	def TwoNumbers( s, allowNone=True ):
		seq = ast.literal_eval( s )
		x = a, b = [ None if allowNone and element is None else float(element) for element in seq ] # will throw an exception if there aren't exactly two elements, both interpretable as floats
		return x
	...
	parser.add_argument(       "--refractoryMsec",   metavar='MSEC',              default=800, type=float, help="minimum number of milliseconds from one trigger to the next" )
...
# (matplotlib and scipy are now imported lazily when needed, so that the BCI2000Tools package as a whole does not depend on them)
...
def siqr( x, q=0.25, axis=-1 ):
	...
	if numpy.asarray( q ).size == 1:
		if q < 0:
			import scipy.stats # python -m conda install scipy   or   python -m pip install scipy
...
def ResolveState( states, stateName ):
	if not stateName: return None, None
	stateName, bit = ( stateName + ':' ).split( ':' )[ :2 ]
	if '*' in stateName:
	...
	content = numpy.asarray( states[ stateName ] ).flatten()
	if bit:
		content = ( content & ( 2 ** abs( int( bit ) ) ) ) != 0 
		if int( bit ) < 0: content = ~content
	content = content.astype( float )
	return stateName, content
...
class EpochSet( Bunch ):
	...
	def __init__( self,
		...
		plotEach=False,
		yScale=None,
		# adding anything here? take care to add it also to the recursive call to EpochSet() below
	 ):
		...
		try: b.params.Stimuli
		except: self.stimuli = [ 'untitled_trigger_event' ]
		else:
		...
		if removeChannels:
			...
			# DO NOT USE { ChannelNames, SourceChOffset, SourceChGain } AND FRIENDS BELOW THIS POINT
		
		if rereference:
			if rereference.upper().replace( '+', '' ).replace( '-', '' )  == 'CAR':
				if '-' in rereference:
					stds = numpy.asarray( rawSignal ).std( axis=1 )
		...
		triggerChannelIndex = self.channels.find_labels( self.trigger )[ 0 ]
		if triggerChannelIndex is None: self.triggerState, triggerSignal = ResolveState( states, self.trigger )
		else:                           self.triggerState, triggerSignal = None, numpy.asarray( rawSignal )[ triggerChannelIndex ]
			
		self.response = responseState
		self.responseState, responseSignal = ResolveState( states, self.response )
		self.responseEvents = None if responseSignal is None else numpy.array( events( responseSignal ) )
			
		if keep: self.triggerSignal, self.responseSignal = triggerSignal, responseSignal
		...
		stimulusCode = states.get( 'StimulusCode', None )
		if stimulusCode is not None: stimulusCode = numpy.asarray( stimulusCode ).ravel()
		self.softwareEvents = numpy.array( [] if stimulusCode is None else events( stimulusCode ), dtype=int )
		epochIndices = self.hardwareEvents
		self.usingHardwareEvents = True
		if not epochIndices.size:
			if stimulusCode is None: raise NoDataError( 'no hardware events found in %s, and no StimulusCode state to fall back on' % self.trigger )
		...
		filterBoundsValid = [
			self.filterBand and self.filterBand[ 0 ] is not None and self.filterBand[ 0 ] > 0,
			self.filterBand and self.filterBand[ 1 ] is not None and self.filterBand[ 1 ] < self.samplesPerSecond / 2.0,
		]
		if self.filterOrder and any( filterBoundsValid ):
			import scipy.signal # python -m conda install scipy   or   python -m pip install scipy
			if   filterBoundsValid == [ False, True  ]:
				filterB, filterA = scipy.signal.butter( self.filterOrder, self.filterBand[ 1 ], btype='lowpass', fs=self.samplesPerSecond )
			elif filterBoundsValid == [ True,  False ]:
				filterB, filterA = scipy.signal.butter( self.filterOrder, self.filterBand[ 0 ], btype='highpass', fs=self.samplesPerSecond )
			elif filterBoundsValid == [ True,  True  ]:
		...
		if stimulusCode is None:
			matchedEpochIndices = self.hardwareEvents
			self.extraneousTriggerEvents = numpy.array( [], dtype=int )
		else:
			...
			self.extraneousTriggerEvents = numpy.array( list( set( epochIndices ) - set( matchedEpochIndices ) ), dtype=int )
		...
		self.epochIndices = numpy.asarray( self.epochIndices )
		...
		if stimulusCode is None:
			self.trialType = numpy.array( [ 1 ] * self.epochIndices.size )
			self.triggersMissing = 0
			self.triggersMisaligned = 0
			self.stimulusLatenciesMsec = []
		else:
			#self.trialType = stimulusCode.flatten()[ self.epochIndices ]
			...
			self.trialType = stimulusCode.flat[ matchedSoftwareEvents ]
	...
	def Combine( cls, objs ):
		...
		for field in 'params states seconds rawSignal triggerSignal responseSignal bandpassSignal'.split(): out.pop( field, None )
		for obj in objs[ 1: ]:
			for field in 'channelIsBad'.split(): out[ field ] |= obj[ field ]
			for field in 'hardwareEvents* softwareEvents* stimulusLatenciesMsec extraneousTriggerEvents* responseEvents* epochs spectra epochIndices* trialType epochIsBad'.split():
				isSampleIndex = field.endswith( '*' )
				field = field.strip( '*' )
				...
				incoming = obj.get( field, None )
				if incoming is not None and len( incoming ):
					incoming = numpy.asarray( incoming )
					if isSampleIndex: incoming = incoming + out.nSamplesTotal
					out[ field ] = numpy.concatenate( [ out[ field ], incoming ], axis=0 )
			for field in 'stimuli! channels! samplesPerSecond! usingHardwareEvents! epochTimeMsec! frequencyHz! filterBand filterOrder badChannelThreshold badEpochThreshold triggerState'.split():
				...
				if list( a.shape ) != list( b.shape ) or ( a != b ).sum():
					...
					print( message )
			for field in 'filenames warnings subjects datestamps triggersMissing triggersMisaligned nSamplesTotal'.split(): out[ field ] += obj[ field ]
		return out
	
	def ComputeResponses( self, select, minResponseMsec=100, maxResponseMsec=1000, returnTable=False ):
		if 'individualFiles' in self:
			individualResults = [ each.ComputeResponses( select=select, minResponseMsec=minResponseMsec, maxResponseMsec=maxResponseMsec, returnTable=returnTable ) for each in self.individualFiles ]
			if returnTable: return sum( individualResults, [] )
			else: pass # fall through and recompute on collated data			
		...
		stimulusEvents = self.epochIndices[ :select.size ][ select ]
		...
		responseLatenciesInMsec = ( numpy.array( matchingResponseEvents, dtype=float ) - stimulusEvents ) / self.samplesPerSecond * 1000.0
		if returnTable: return [ Bunch( filename=self.filenames[ 0 ], sampleIndexOfStimulus=si, sampleIndexOfResponse=ri, trialType=tt, responseLatencyInMsec=rt, missing=ri is None ) for si, ri, tt, rt in zip( stimulusEvents, matchingResponseEvents, self.trialType[ select ], responseLatenciesInMsec ) ]
		
		nLegalResponses = sum( 1 for event in matchingResponseEvents if event is not None )
		self.extraneousResponseEvents = numpy.array( [ event for event in self.responseEvents if event not in matchingResponseEvents ] )
		self.responseTargetEvents = stimulusEvents
		self.responseLatenciesInMsec = responseLatenciesInMsec
		self.responseSummary = '%.1f ms +/- %.1f SD (responded to %d out of %d events, %d extraneous response%s)' % ( numpy.nanmean( self.responseLatenciesInMsec ), numpy.nanstd( self.responseLatenciesInMsec ), nLegalResponses, stimulusEvents.size, len( self.extraneousResponseEvents ), '' if len( self.extraneousResponseEvents ) == 1 else 's' )
		return self
...
if __name__ == '__main__':
	...
	if ( OPTS.plot or OPTS.plotEach ):
		if epochs.filenames:
			...
			print( 'no files analyzed' ) # not sure why we would ever get here
	else:
