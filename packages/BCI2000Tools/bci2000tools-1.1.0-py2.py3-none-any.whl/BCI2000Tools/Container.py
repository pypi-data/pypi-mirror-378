...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Container.py 9063 2025-09-19 21:20:19Z jhill $
...
class Bunch( dict ):
	...
	def _write( self, filename, pickleProtocol=Unspecified, prefix='', suffix='' ):
		...
		if pickleProtocol is None:
			try: fh = openfcn( filename, mode + 't' ); fh.write( prefix + self._report( summarize=False ) + suffix )
			finally: fh.close()
		else:
			try: fh = openfcn( filename, mode + 'b' ); pickle.dump( self._convert( self, to_dict=True ), fh, protocol=pickleProtocol )
			finally: fh.close()
	...
	def _read( cls, filename, unpickle=Unspecified, go_into=( tuple, list ), func=None ):
		...
		if unpickle:
			try: fh = openfcn( filename, 'rb' ); instance = pickle.load( fh )              # instance is a dict at this point
			finally: fh.close()
		else:
			try: fh = openfcn( filename, 'rt' ); instance = ast.literal_eval( fh.read() )  # instance is a dict at this point
			finally: fh.close()
