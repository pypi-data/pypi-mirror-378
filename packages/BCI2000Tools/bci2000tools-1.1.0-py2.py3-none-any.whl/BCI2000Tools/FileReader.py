...
# ///////////////////////////////////////////////////////////////////////////
# $Id: FileReader.py 9063 2025-09-19 21:20:19Z jhill $
...
from . import Bootstrapping; from .Bootstrapping import USE_NUMPY_MATRIX # switch on/off in Bootstrapping.py
if USE_NUMPY_MATRIX: import warnings; warnings.filterwarnings( action='ignore', category=PendingDeprecationWarning, message='the matrix subclass is not the recommended way' ) # https://stackoverflow.com/a/53254739
...
class bcistream(object):
	...
	def __init__(self, filename, ind=-1):
		...
		for k,v in self.statedefs.items():
			...
			v['mult'] = (256.0 ** numpy.arange(nbytes, dtype=numpy.float64) / float(div))[None, :]
			self.statedefs[k] = v
	...
	def read(self, nsamp=1, apply_gains=True):
		...
		if USE_NUMPY_MATRIX: sig = numpy.asmatrix(sig)
	...
	def decode(self, nsamp='all', states='all', apply_gains=True):
		...
		for statename in statenames:
			...
			states[statename] = numpy.dot(sd['mult'], rawstates[sd['slice'],:] & sd['mask']).astype(numpy.int64)
			if USE_NUMPY_MATRIX: states[statename] = numpy.asmatrix(states[statename])
		return sig,states
	...
	def plotstates(self, states, whichstates=None, **kwargs):
		...
		if whichstates is not None:
			if isinstance(whichstates, basestring): whichstates = whichstates.replace( ',', ' ' ).split()
			labels = whichstates
		v = numpy.concatenate( [ numpy.asarray( states[ k ] ).ravel()[ None, : ] for k in labels ], axis=0 )
		ntraces,nsamp = v.shape
		#v = v - numpy.min(v,axis=1)
		sc = numpy.max(v,axis=1)[:, None]
		sc[numpy.where(sc==0.0)] = 1.0
		v = v / sc
		offsets = numpy.arange(1.0,ntraces+1.0)[None, :]
		v = v.T * -0.7 + offsets
		t = numpy.arange(nsamp, dtype=numpy.float32)[:, None] / self.samplingfreq_hz
	...
	def plotsig(self, sig, fac=3.0, ch=None):
		...
		if ch is not None:
			if isinstance(ch, basestring): ch = ch.replace( ',', ' ' ).split()
		...
		v = numpy.array(sig).T
		v -= numpy.median(v, axis=0)[None, :]
		offsets = numpy.arange(-1.0,ntraces+1.0)[None, :]
		offsets = offsets * max(v.std(axis=0)) * fac
		v += offsets[:,1:-1]
...
def ParseParam(param):
	...
	# special legacy case (suppress warning)
	if name == 'TransmitChList' and param[:2] == [ '1', '*' ]: param[:2] = [ '1', 'auto' ]
