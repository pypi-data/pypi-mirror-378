...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Parameters.py 9000 2025-08-05 18:07:32Z jhill $
...
def convert(val, type, name='', check_nesting=False, mode='Value', opts=()):
	...
	if type.endswith('list'):
		...
		if name == 'TransmitChList' and valstr == '*': valstr = 'auto'
		suppressWarning = (valstr == 'auto')
		...
		if type == 'float':
			...
			elif mode=='NumericValue' and 'q' not in opts and not suppressWarning: print( 'WARNING: invalid float value "%s" in parameter%s' % (str(val),namestr) )
		if type == 'int':
			if intval is not None: numval = intval
			elif mode=='NumericValue' and 'q' not in opts and not suppressWarning: print('WARNING: invalid int value "%s" in parameter%s' % (str(val),namestr) )
		if type == 'string':
			val = str(val)
...
class Param(object):
	...
	def __getitem__(self, sub):
		...
		if not hasattr(sub, '__len__') or len(sub) == 1:
			...
			a = numpy.array(self.Value, ndmin=2, copy=False)
			if a.ndim != 2: raise ValueError( 'matrix must be 2-dimensional' )
			result = a.__getitem__(sub)
...
def read_parameter_lines(f):
	close = lambda: None
	if isinstance(f, basestring): f = open(f, 'rb'); close = f.close
	start = f.tell()
...
class ParamList(list):
	def __init__(self, x=(), opts=()):
		...
		"""
		close = lambda: None
		if isinstance(x, basestring) and len(x) < 1024 and not '\n' in x and os.path.isfile(x): x = open(x, 'rb'); close = x.close
