...
from pydantic import BaseModel, VERSION as P_VERSION
...
if TYPE_CHECKING:
    from .main import RelationshipInfo, Model
...
def get_fields_set(
        object: InstanceOrType["Model"],
) -> Union[Set[str], Callable[[BaseModel], Set[str]]]:
    return object.model_fields_set


def init_pydantic_private_attrs(new_object: InstanceOrType["Model"]) -> None:
    object.__setattr__(
        new_object,
...
def sqlmodel_validate(
    ...
    if not is_table_model_class(cls):
        new_obj: _TModel = cls.__new__(cls)
    else:
...
def sqlmodel_init(*, self: "Model", data: Dict[str, Any]) -> None:
