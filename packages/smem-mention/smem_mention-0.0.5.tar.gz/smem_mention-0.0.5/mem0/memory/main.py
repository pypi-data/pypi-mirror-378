...
from mem0.memory.storage import SQLiteManager, PostgreSQLManager
...
def _create_database_manager(history_db_path: str) -> Union[SQLiteManager, PostgreSQLManager]:
    """
    Create appropriate database manager based on the path/URI provided.
    
    Args:
        history_db_path (str): Either a SQLite file path or PostgreSQL connection URI
        
    Returns:
        Union[SQLiteManager, PostgreSQLManager]: Appropriate database manager instance
    """
    # Check if it's a PostgreSQL connection URI
    if (history_db_path.startswith('postgresql://') or 
        history_db_path.startswith('postgres://') or
        history_db_path.startswith('postgresql+psycopg2://') or
        history_db_path.startswith('postgresql+psycopg://')):
        return PostgreSQLManager(history_db_path)
    else:
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        self.db = _create_database_manager(self.config.history_db_path)
    ...
    def reset(self):
        ...
        # Reset the database using the manager's reset method
        if hasattr(self.db, "reset"):
            self.db.reset()
        elif hasattr(self.db, "connection") and self.db.connection:
            if hasattr(self.db.connection, "execute"):
                # SQLite connection fallback
                self.db.connection.execute("DROP TABLE IF EXISTS history")
            self.db.connection.close()

        logger.info("Reset history database successfully")

        self.db = _create_database_manager(self.config.history_db_path)

        logger.info("Create new history database successfully")
        ...
        logger.info("Reset vector store successfully")
