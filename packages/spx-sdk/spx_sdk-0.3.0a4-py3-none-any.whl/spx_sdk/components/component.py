...
class SpxComponent(AbstractContextManager):
    ...
    def update(self, inst_name: str, cfg: Any) -> "SpxComponent":
        """
        Update an existing instance's configuration at runtime, mirroring _populate logic for a single entry.
        Returns the updated component (or None if not found).
        """
        instance = self.children.get(inst_name)
        if instance is None:
            self.logger.warning(f"Cannot update non-existent instance '{inst_name}' in component '{self.name}'")
            return None
        instance._populate(cfg)
        return instance
