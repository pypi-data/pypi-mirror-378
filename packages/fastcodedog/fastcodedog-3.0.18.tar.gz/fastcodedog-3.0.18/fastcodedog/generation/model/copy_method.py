    ...
    def _get_model_copy_texts(self, src_instance_name, dest_instance_name, model,
        ...
        for _, relationship in model.relationships.items():
            if relationship.name in relations_should_copy:
                if relationship.name not in relations_should_new_and_copy:    # 直接用等号赋值
                    # 这里配置容易出错，给个报错
                    if ((relationship.name in specified_attribute_values and specified_attribute_values[relationship.name])
                        ...
                        raise ValueError(f'关联关系{relationship.name}配置错误，只有新创建的对象（relations_should_new_and_copy中）才支持配置specified_attribute_values、relations_should_copy。在没有配置relations_should_new_and_copy的情况下，将采用=进行赋值')
                ...
                if relationship.is_list:
                    ...
                    texts.extend(self._get_model_copy_texts(sub_src_instance_name, sub_dest_instance_name,
                                                            ...
                                                            relations_should_copy[relationship.name],
                                                            relations_should_new_and_copy[relationship.name] if relationship.name in relations_should_new_and_copy else None,
                                                            indent=sub_indent))
