...
from metenox.prices import (
    get_eve_type_price,
    get_fuel_block_price,
    get_magmatic_gases_price,
)
...
class Moon(models.Model):
    ...
    def update_price(self):
        ...
        # TODO try to do a request to do this in one go
        hourly_harvest_value = sum(
            get_eve_type_price(moon_goo) * moon_goo_amount
    ...
    def fuel_price(cls) -> float:
        ...
        hourly_price = (
            METENOX_MAGMATIC_GASES_PER_HOUR * get_magmatic_gases_price()
            + METENOX_FUEL_BLOCKS_PER_HOUR * get_fuel_block_price()
        )
...
class Metenox(models.Model):
    ...
    stored_value = models.FloatField(default=0)
    ...
    def get_stored_moon_materials_value(self) -> float:
        ...
        # TODO optimize price fetching
        total = 0
        for stored_material in self.stored_moon_materials.all():
            total += (
                get_eve_type_price(stored_material.product) * stored_material.amount
            )
