...
from metenox.prices import MAGMATIC_TYPE_ID, get_fuel_blocs_type_ids
...
def update_metenox(
    ...
    fuel_blocs_type_ids = get_fuel_blocs_type_ids()
    for asset in metenox_assets:
        if asset["location_flag"] == "StructureFuel":
            if asset["type_id"] == MAGMATIC_TYPE_ID:
                metenox.set_magmatic_gases(asset["quantity"])
            elif asset["type_id"] in fuel_blocs_type_ids:
    ...
    metenox.stored_value = metenox.get_stored_moon_materials_value()
...
def update_prices():
    ...
    logger.info("Updating %s moons", moons.count())
