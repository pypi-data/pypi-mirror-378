...
class MedicalSummarizationPipeline:
    ...
    def generate_medical_summary(self, text, max_length=1024, min_length=300,
    ...
    # def chunk_medical_text(self, text, chunk_size=1000, overlap=100):
    ...
    #     return sections
    def chunk_medical_text(self, text, chunk_size=1500, overlap=100):
        words = text.split()
        ...
        # Increase threshold for splitting on headers (from 50 -> 300)
        ...
        for word in words:
            ...
            # Check if we hit a section boundary
            if any(header in word.lower() for header in medical_headers) and len(current_section) > header_threshold:
                sections.append(" ".join(current_section))
                current_section = []
...
# Load the spaCy NER model
    ...
    def get_named_entities(self,text):
        """Extracts a set of unique named entities from a given text."""
        doc = nlp(text)
        entities = set()
        for ent in doc.ents:
            # Normalize the entity text to handle case and remove duplicates
            entities.add(ent.text.strip().lower())
        return entities

    def calculate_ner_score(self,original_text, summarized_text):
        """Calculates the NER F1-Score for consistency."""

       # Step 1: Get entities from both texts
        original_entities = self.get_named_entities(original_text)
        summary_entities = self.get_named_entities(summarized_text)

    # Convert sets to a list for readability in output
        print(f"Original Entities: {list(original_entities)}")
        print(f"Summary Entities: {list(summary_entities)}")

        # Handle cases where one or both sets are empty to avoid division by zero
        if not original_entities and not summary_entities:
            return 1.0  # Perfect score if there are no entities in either text
        if not summary_entities:
            return 0.0  # No entities in summary, so recall is 0

        # Step 2: Calculate Intersection and Cardinalities
        intersection = original_entities.intersection(summary_entities)
        num_intersection = len(intersection)
        num_original = len(original_entities)
        num_summary = len(summary_entities)

        # Step 3: Calculate Precision, Recall, and F1-Score
        precision = num_intersection / num_summary
        recall = num_intersection / num_original
    ...
    def evaluate_medical_summary(self, generated_summary, reference_summary, original_text=None):
        ...
        if original_text:
            ...
            ner_score = self.calculate_ner_score(original_text, generated_summary)
            results['ner_score'] = ner_score
    ...
    def medical_summarize_and_evaluate(self, text, reference_summary=None,
        ...
        length_configs = {
            ...
            "comprehensive": {"max_length": 1200, "min_length": 200}
        }
    ...
    def compare_medical_summaries(self, summaries_dict, metrics_list=None, output_json="medical_summaries.json"):
        ...
        for idx, (name, summary) in enumerate(summaries_dict.items()):
            ...
            if metrics_list and idx < len(metrics_list) and metrics_list[idx] is not None:
                ...
                flesch_score = evaluation.get('flesch_reading_ease', 0)
                compression_ratio = evaluation.get('compression_ratio', 1)


                # Enhanced scoring for medical content
                final_score = (
                    0.5 * bert_f1 +           # Semantic similarity
            ...
            results.append((name, summary, final_score, evaluation, entities))
        ...
        # Find best summary
        best = max(results, key=lambda x: x[2]) # Use index 2 for final_score
