...
def get_medical(file_path):
    # Create medical pipeline instead of travel pipeline
    pipeline = create_medical_pipeline()

    sample_text = setx_medical(file_path)
    ...
    pipeline.compare_medical_summaries(summaries_to_compare, metrices, file_path)