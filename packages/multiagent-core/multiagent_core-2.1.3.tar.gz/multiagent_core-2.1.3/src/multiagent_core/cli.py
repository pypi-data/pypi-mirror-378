...
def _generate_project_structure(target_dir):
    """Generate project structure programmatically instead of copying files"""
    try:
        target_dir = Path(target_dir)
        
        # Create .multiagent directory first
        ...
        components_data = {
            ...
            "components": {
                "core": {"version": "2.1.0", "installed": True, "docker_enabled": False}
            },
...
"""
        ...
        # Generate basic VS Code settings under .multiagent/
        vscode_dir = multiagent_dir / '.vscode'
        vscode_dir.mkdir(exist_ok=True)
        console.print("Generating VS Code settings in .multiagent/.vscode/...")
        ...
        # Set up GitHub directory with templates from package, but simple workflow
...
def _install_git_hooks(project_path):
    ...
    try:
        ...
        if not git_hooks_dir.exists():
            console.print("[yellow].git/hooks directory not found - not a git repository?[/yellow]")
            return False
