...
def init(dry_run):
    ...
    if not dry_run:
        success = _copy_framework_files(cwd)
        if not success:
...
def _copy_framework_files(target_dir):
    """Copy framework files from installed package to target directory"""
    try:
        # Get package directory - handle WSL/Windows paths
        import multiagent_core
        package_location = multiagent_core.__file__
        
        # If it's a WSL path that looks like \\wsl.localhost\Ubuntu\..., convert it
        ...
        # Ensure target is a clean path
        if '\\\\wsl.localhost\\Ubuntu\\' in str(target_dir):
            target_str = str(target_dir).replace('\\\\wsl.localhost\\Ubuntu\\', '/').replace('\\', '/')
            target_dir = Path(target_str)
        else:
            target_dir = Path(target_dir)
        
        # Copy .claude directory (only if it doesn't exist)
        claude_source = package_path / '.claude'
        claude_target = target_dir / '.claude'
        if claude_source.exists() and not claude_target.exists():
            console.print("Setting up .claude directory...")
            import shutil
            shutil.copytree(claude_source, claude_target)
        
        # Merge .github directory contents (excluding version-management workflow)
        ...
        if github_source.exists():
            ...
            for item in github_source.rglob('*'):
                if item.is_file():
                    ...
                    # Skip the version-management workflow (specific to multiagent-core)
        ...
        # Copy other framework directories
        other_dirs = ['core', 'config', 'environments', 'templates']
        for dir_name in other_dirs:
            source_dir = package_path / dir_name
            target_subdir = target_dir / dir_name
            if source_dir.exists() and not target_subdir.exists():
                console.print(f"Setting up {dir_name}...")
                import shutil
                shutil.copytree(source_dir, target_subdir)
        
        # Create .multiagent directory and components.json
        ...
        components_data = {
            ...
            "components": {
                "core": {"version": "2.0.4", "installed": True, "docker_enabled": False}
            },
        ...
        # Create initial VERSION file for project (not for PyPI publishing)
...
def _install_git_hooks(project_path):
    ...
    try:
        ...
        hooks_source = package_path / '.claude' / 'hooks'
        git_hooks_dir = project_path / '.git' / 'hooks'
        
        if hooks_source.exists() and git_hooks_dir.exists():
            import shutil
            for hook_file in hooks_source.glob('*'):
                if hook_file.is_file() and not hook_file.name.endswith('.md'):
                    ...
                    console.print(f"Installed hook: {hook_file.name}")
            
            console.print("[green]Git hooks installed successfully![/green]")
            return True
        else:
            console.print("[yellow]No git hooks found in package or .git directory missing[/yellow]")
            return False
