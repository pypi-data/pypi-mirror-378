...
def _recommend_additional_components(project_path):
    ...
    console.print(f"    Install: [dim]pip install multiagent-devops[/dim]")
...
def _generate_project_structure(target_dir):
    """Generate project structure programmatically - actively reorganizes existing structure"""
    try:
        target_dir = Path(target_dir)
        
        # Clean up any misplaced files from previous broken installs
        console.print("Setting up .multiagent directory structure...")
        
        # Move any misplaced config/core/environments files to .multiagent
        for misplaced_dir in ['config', 'core', 'environments']:
            misplaced_path = target_dir / misplaced_dir
            if misplaced_path.exists():
                console.print(f"Moving misplaced {misplaced_dir}/ to .multiagent/{misplaced_dir}/")
                import shutil
                target_path = target_dir / ".multiagent" / misplaced_dir
                if target_path.exists():
                    shutil.rmtree(target_path)
                shutil.move(str(misplaced_path), str(target_path.parent))
        
        # Create .multiagent directory  
        multiagent_dir = target_dir / ".multiagent"
        multiagent_dir.mkdir(exist_ok=True)
        console.print("Setting up .multiagent directory...")
        ...
        components_data = {
            ...
            "components": {
                "core": {"version": current_version, "installed": True, "docker_enabled": False}
            },
        ...
        # Create documentation under .multiagent/
        docs_dir = multiagent_dir / 'docs'
        docs_dir.mkdir(exist_ok=True)
        console.print("Generating documentation in .multiagent/docs/...")
...
## Structure
- `.multiagent/` - Framework files (all framework content lives here)
- `.claude/` - Claude Code agent configurations  
- `.github/` - GitHub workflows and issue templates

## Commands
```bash
python -m multiagent_core.cli status    # Check installation
python -m multiagent_core.cli upgrade   # Update components
```

For more information, see: https://github.com/vanman2024/multiagent-core
"""
        ...
        console.print("Setting up .claude directory...")
        ...
        # Set up framework files in .multiagent directory - ALWAYS reorganize
        console.print("Setting up framework files in .multiagent/...")
        import shutil
        
        # Always refresh config directory to .multiagent/config/
        config_source = package_path / 'config'
        config_target = multiagent_dir / 'config'
        if config_target.exists():
            shutil.rmtree(config_target)
        if config_source.exists():
            shutil.copytree(config_source, config_target)
        
        # Always refresh core directory to .multiagent/core/
        core_source = package_path / 'core'
        core_target = multiagent_dir / 'core'
        if core_target.exists():
            shutil.rmtree(core_target)
        if core_source.exists():
            shutil.copytree(core_source, core_target)
            
        # Always refresh templates directory to .multiagent/templates/
        templates_source = package_path / 'templates'
        templates_target = multiagent_dir / 'templates'
        if templates_target.exists():
            shutil.rmtree(templates_target)
        if templates_source.exists():
            shutil.copytree(templates_source, templates_target)
        
        # Always refresh .claude directory from package
        claude_source = package_path / 'claude'
        console.print("Setting up .claude directory...")
        # Clear and rebuild .claude directory
        if claude_dir.exists():
            shutil.rmtree(claude_dir)
        claude_dir.mkdir(exist_ok=True)
        if claude_source.exists():
            for item in claude_source.rglob('*'):
                if item.is_file():
                    relative_path = item.relative_to(claude_source)
                    target_file = claude_dir / relative_path
                    target_file.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(item, target_file)
        
        # Always refresh VS Code settings from package
        vscode_source = package_path / '.vscode'
        vscode_target = target_dir / '.vscode'
        console.print("Setting up VS Code settings...")
        if vscode_target.exists():
            shutil.rmtree(vscode_target)
        if vscode_source.exists():
            shutil.copytree(vscode_source, vscode_target)
        
        # Always refresh GitHub directory with templates from package
        github_source = package_path / '.github'
        github_target = target_dir / '.github'
        console.print("Setting up GitHub workflows...")
        # Clear and rebuild GitHub workflows
        if github_target.exists():
            shutil.rmtree(github_target)
        github_target.mkdir(exist_ok=True)
        if github_source.exists():
            for item in github_source.rglob('*'):
                if item.is_file():
                    relative_path = item.relative_to(github_source)
                    # Skip the complex version-management workflow (specific to multiagent-core)
                    if 'version-management.yml' in str(relative_path):
                        continue
                    target_file = github_target / relative_path
                    target_file.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(item, target_file)
        
        # Generate simple version management workflow for user projects
        workflows_dir = github_target / 'workflows'
        workflows_dir.mkdir(exist_ok=True)
        simple_workflow = workflows_dir / 'version-management.yml'
        workflow_content = '''name: Project Version Management
...
'''
        ...
        with open(simple_workflow, 'w') as f:
            f.write(workflow_content)
            
        # Create initial project VERSION file (not for PyPI publishing)
        version_file = target_dir / "VERSION"
        if not version_file.exists():
            console.print("Creating project VERSION file...")
            import datetime
            version_data = {
                "version": "0.1.0",
                "commit": "initial",
        ...
        console.print(f"[red]Error setting up framework: {e}[/red]")
        return False
