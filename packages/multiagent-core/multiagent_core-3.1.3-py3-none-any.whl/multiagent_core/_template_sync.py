...
def sync_templates(template_names: Iterable[str] | None = None, *, quiet: bool = False) -> List[SyncResult]:
    ...
    for name in names:
        ...
        if not src.exists():
            ...
            if not quiet:
                print(f"⚠️  {name} not found in repository; removed packaged copy if present")
            continue
