...
def _detect_available_clis():
    ...
    # Check each CLI with simpler detection
    
    # Gemini CLI - check if command exists
    try:
        result = subprocess.run(['which', 'gemini'], capture_output=True, text=True, timeout=3)
        if result.returncode == 0:
            status['Gemini CLI'] = {'available': True, 'version': 'Available'}
        else:
            status['Gemini CLI'] = {'available': False, 'version': None}
    except:
        status['Gemini CLI'] = {'available': False, 'version': None}
    
    # Qwen CLI - check if command exists  
    try:
        result = subprocess.run(['which', 'qwen'], capture_output=True, text=True, timeout=3)
        if result.returncode == 0:
            status['Qwen CLI'] = {'available': True, 'version': 'Available'}
        else:
            status['Qwen CLI'] = {'available': False, 'version': None}
    except:
        status['Qwen CLI'] = {'available': False, 'version': None}
...
def _copy_framework_files(target_dir):
    ...
    try:
        ...
        package_path = Path(multiagent_core.__file__).parent
        ...
        # Merge .github directory contents
