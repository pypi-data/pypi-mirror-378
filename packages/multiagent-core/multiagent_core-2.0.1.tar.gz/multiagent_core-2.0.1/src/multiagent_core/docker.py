...
def run_github_operation_in_docker(operation_type, repo_name=None, work_dir=None, github_token=None):
    """Run GitHub operations inside Docker to avoid WSL path issues
    
    Args:
        operation_type (str): 'create_repo', 'git_init', 'install_hooks'
        repo_name (str): Repository name (for create_repo)
    ...
    try:
        ...
        docker_commands = ['docker']
        mount_path = str(work_dir)
        
        if '\\\\wsl.localhost\\' in str(work_dir):
            docker_commands = ['wsl', 'docker']
            mount_path = str(work_dir).replace('\\\\wsl.localhost\\Ubuntu', '').replace('\\', '/')
        elif hasattr(os, 'uname') and 'Microsoft' in os.uname().release:
            mount_path = str(work_dir)
        
        # Use Alpine Linux with git for lightweight operations
        docker_run_base = docker_commands + [
            'run', '--rm', '-i',
        ...
        if github_token:
            docker_run_base.extend(['--env', f'GITHUB_TOKEN={github_token}'])
        
        docker_run_base.append('alpine/git')
        
        if operation_type == 'create_repo':
            # Install gh CLI and create repo in Alpine
            commands = [
                'apk add --no-cache curl tar',
                'curl -L https://github.com/cli/cli/releases/latest/download/gh_*_linux_amd64.tar.gz | tar -xz --strip=1 -C /usr/local/bin gh_*_linux_amd64/bin/gh',
                'git config --global --add safe.directory /workspace',
                'git config --global user.email "action@github.com"',
                'git config --global user.name "MultiAgent Core"',
                'git add . || true',
                'git commit -m "Initial commit from multiagent-core" || true',
                f'gh repo create {repo_name} --public'
            ]
            
        elif operation_type == 'git_init':
            commands = [
                'git config --global --add safe.directory /workspace',
                'git config --global user.email "action@github.com"',
                'git config --global user.name "MultiAgent Core"',
        ...
        elif operation_type == 'install_hooks':
            commands = [
                'git config --global --add safe.directory /workspace',
                'mkdir -p .git/hooks',
                '''cat > .git/hooks/pre-commit << 'HOOK_EOF'
#!/bin/sh
echo "🤖 MultiAgent pre-commit hook"
echo "📦 Checking framework consistency..."
HOOK_EOF''',
                'chmod +x .git/hooks/pre-commit'
            ]
        else:
            console.print(f"[red]Unknown operation type: {operation_type}[/red]")
            return False
        ...
        result = subprocess.run(
            docker_run_base + ['sh', '-c', command_str],
            capture_output=True,
            text=True,
        ...
        if result.returncode == 0:
            console.print(f"[green]Docker operation '{operation_type}' completed successfully[/green]")
            if result.stdout:
                console.print(f"[dim]{result.stdout.strip()}[/dim]")
            return True
        else:
            console.print(f"[red]Docker operation '{operation_type}' failed[/red]")
            if result.stderr:
...
def create_docker_setup(use_docker=False):
    ...
    if use_docker:
        ...
        cat > .multiagent/core/docker-utils.py << 'EOF'
# Docker utilities for multi-agent development
import subprocess
import os
from pathlib import Path
...
def run_command(cmd, work_dir=None):
    ...
    return subprocess.run(cmd, shell=True, cwd=work_dir, capture_output=True, text=True)

def convert_wsl_path(path):
    \"\"\"Convert WSL paths for Windows tools\"\"\"
    if '\\\\\\\\wsl.localhost\\\\' in str(path):
        return str(path).replace('\\\\\\\\wsl.localhost\\\\Ubuntu', '').replace('\\\\\\\\', '/')
    return str(path)
EOF

        cat > .multiagent/config/project-config.yml << 'EOF'
...
EOF
        ...
        cat > .multiagent/environments/development.yml << 'EOF'
...
EOF
        ...
        cat > .github/workflows/version-management.yml << 'EOF'
...
jobs:
  version-management:
        ...
        cat > .releaserc.json << 'RELEASE_EOF'
        {
          ...
          "plugins": [
            ...
            [
              "@semantic-release/exec",
              {
                "prepareCmd": "echo '{\"version\": \"v\${nextRelease.version}\", \"commit\": \"'\$GITHUB_SHA'\", \"build_date\": \"'\$(date -u +%Y-%m-%dT%H:%M:%SZ)'\", \"build_type\": \"production\"}' > VERSION && if [ -f pyproject.toml ]; then sed -i 's/version = \".*\"/version = \"\${nextRelease.version}\"/' pyproject.toml; fi"
              }
            ],
            [
              "@semantic-release/git",
    ...
    - name: Project version management complete
      if: always()
      run: |
        echo "Project version management workflow completed"
        echo "📦 Current version: \$(cat VERSION | jq -r .version 2>/dev/null || echo 'unknown')"
        echo ""
        echo "🚀 Project uses semantic versioning with conventional commits"
EOF
        ...
        cat > package.json << 'EOF'
{
  "name": "project-name",
  "version": "0.1.0",
  "description": "Project created with multiagent-core",
  "private": true,
  "scripts": {
