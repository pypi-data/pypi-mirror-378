...
class Diresa(nn.Module):
    ...
    def from_hyper_param(
        ...
        DIRESA can be built using feed forward layers, CNN layers or a combination of both.
        The argument `dense_units` is used to add feedforward layers, where the length of the tuple represents depth of the network and each individual value represents the number of nodes per layer.
        ...
        :param activation: activation function shared by all the sub-modules.
    ...
    def get_ordering(self) -> torch.Tensor | None:
        """
    ...
    def reset_ordering(self) -> torch.Tensor | None:
        """
