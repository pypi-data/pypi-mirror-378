...
def compress_base64_image_if_needed(base64_image, max_size_bytes=1_000_000, quality=70):
    # Decode base64 to bytes
    image_bytes = base64.b64decode(base64_image)
    if len(image_bytes) <= max_size_bytes:
        return base64_image  # No compression needed
...
class IdentityVerification:
    ...
    def extract_front_id_info(self, front_id, country, nationality):
        ...
        if country == 'QAT':
            ...
            try:
                st = time.time()
                front_id = compress_base64_image_if_needed(front_id,quality=50)
                processed_front_id = self.image_conversion(front_id)
                logging.info(f'----------------Time taken for image conversion front: {time.time() - st} seconds\n')

                st = time.time()
                compressed_image = BytesIO()
                processed_front_id.save(compressed_image, format="JPEG", quality=100, optimize=True)
                compressed_image_data = compressed_image.getvalue()
