...
async def weaviate_get_schema_info() -> dict[str, Any]:
    ...
    try:
        ...
        for collection_name, collection_info in schema_result.items():
            ...
            count = 0
            if not count_result.get("error") and count_result.get("results"):
...
async def weaviate_validate_collection_exists(collection_name: str) -> dict[str, Any]:
    ...
    Example:
        ...
        if result["exists"]:
            print(f"Collection exists with {result['property_count']} properties")
        ```
    """
    try:
        service = WeaviateService()
    ...
    Example:
        ...
        if not props.get("error"):
            for prop_name, prop_info in props["properties"].items():
                print(f"{prop_name}: {prop_info['data_type']} - {prop_info['description']}")
        ```
    """
    try:
        service = WeaviateService()
