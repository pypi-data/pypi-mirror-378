...
logger = Logger("log_files/prompts")
...
def prompt_config(PERSONALIZED_PROMPTS_API, prompt_app):
    ...
    try:
        ...
        if prompts_setting["systemPrompt"]["systemPrompt"]:
            ...
            #logger.info(f"#### System Prompt {support_template} ##########")
            #logger.info(f"#### REQUIRE_CITATION_STATEMENT {REQUIRE_CITATION_STATEMENT} ##########")
        else:
        ...
        if (
            ...
            # logger.info(f"#### Search Summary Prompt {INITIAL_SEARCH_SUMMARY_TEMPLATE} ##########")
        ...
        logger.error(f"Error while fetching prompt configuration from db {e}")
