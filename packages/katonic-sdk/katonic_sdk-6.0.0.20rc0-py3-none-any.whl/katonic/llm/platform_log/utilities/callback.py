...
from ..utilities.logutils import Logger, handle_exception
...
logger = Logger("log_files/callback")
...
class CustomCallbackHandler(BaseCallbackHandler):
    def __init__(
        ...
        provider, _ = get_llm_provider(self.model_name, logger)
        ...
        try:
            ...
            #logger.info(f"Pricing of model: {self.model_pricing_dict}")
        except Exception:
            ...
            logger.error(
                f"Cost information is not available in metadata.{error_message}"
            )

    def get_answer_validity(self, input, output):
        logger.info("performing answer validity")
        messages = answer_validation(logger, query=input, response=output)
        self.llm_prompt = dict_based_prompt_to_langchain_prompt(messages)
        llm = initialize_llm_models(self.model_name, logger)
        # logger.info(f"ANSWER VALIDITY INPUT PROMPT: {str(self.llm_prompt)}")
        ...
        logger.info("Completed answer validity")
        ...
        try:
            if len(self.output_text) > 0:
                try:
                    ...
                    logger.error(f"Answer validation failed due to {str(e)}")
        except Exception as e:
            logger.error(f"Populating cost data failed for Answer Validity, {str(e)}")
        try:
            ...
            logger.info(f" Message Id in callback: {self.message_id}")    
            payload = {
            ...
            logger.info("Updating rate limits increment data to database.")
            increment_rate_limits_data(
            ...
            if response.json()["status"] == 200:
                logger.info("Cost has been added successfully....")
            else:
                logger.info(f"Cost details hasn't updated due to, {str(response.text)}")

        except Exception as e:
            logger.error(f"failed while posting cost data: {str(e)}")
    ...
    def save_citations(self):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Citations details hasn't updated due to {str(response.text)}"
                )
        except Exception as e:
