...
def main():
    ...
    parser = argparse.ArgumentParser(description='AI Boutique Assistant MCP Server')
    parser.add_argument('--port', type=int, default=int(os.getenv('PORT', '8080')),
                        help='Port to run the server on (default: 8080)')
    parser.add_argument('--stdio', action='store_true',
                        help='Force stdio mode (for ADK integration)')
    parser.add_argument('--http', action='store_true', 
                        help='Force HTTP mode (for web/API access)')
    args = parser.parse_args()
    
    # Determine server mode
    if args.stdio:
        # Explicit stdio mode requested
        logger.info("🔧 Starting in stdio mode (forced)")
        asyncio.run(run_stdio_server())
    elif args.http:
        # Explicit HTTP mode requested  
        logger.info(f"🌐 Starting in HTTP mode on port {args.port} (forced)")
        asyncio.run(run_http_server(port=args.port))
    elif sys.stdin.isatty() == False:
        # Auto-detect: Running as child process - use stdio mode
        logger.info("🔧 Starting in stdio mode (auto-detected)")
        asyncio.run(run_stdio_server())
    else:
        # Auto-detect: Running standalone - use HTTP mode
        logger.info(f"🌐 Starting in HTTP mode on port {args.port} (auto-detected)")
        asyncio.run(run_http_server(port=args.port))
