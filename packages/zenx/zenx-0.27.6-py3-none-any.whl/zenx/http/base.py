...
import asyncio
from typing import Any, ClassVar, Type, Dict
...
class SessionManager(ABC):
    ...
    def __init__(self, client: "HttpClient", logger: BoundLogger, settings: Settings) -> None:
        ...
        self.session_init_args = {"proxy": settings.PROXY}
    ...
    def set_session_init_args(self, kwargs: Dict) -> None:
        self.session_init_args.update(kwargs)
...
class HttpClient(ABC):
    ...
    async def _session_request(self, session: Session, **kwargs) -> Any:
        try:
            ...
            self.logger.error("request", url=kwargs.get("url"), client=self.name, session_id=session.id)
            raise
        else:
            return response
