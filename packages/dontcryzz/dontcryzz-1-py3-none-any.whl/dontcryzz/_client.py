...
class ZaloAPI(object):
	...
	def login(self, phone, password, imei, user_agent=None):
		...
		self.onLoggedIn(self.fetchAccountInfo().profile.get("displayName"))
	...
	def leaveGroup(self, grid, silent=1, language="vi"):
	        payload = {
	            "grids": [grid],
	            "imei": self._imei,
	            "silent": silent,
	...
	def sendReport(self, user_id, reason=0, content=None, thread_type=ThreadType.GROUP):
		"""Send report to Zalo.
	...
	def onLoggingIn(self, phone=None):
		...
		"""
		logger.debug("> > Ank La DontCry < <".format(phone))
