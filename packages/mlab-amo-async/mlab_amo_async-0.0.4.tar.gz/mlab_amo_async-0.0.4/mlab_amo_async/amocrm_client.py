    ...
    async def get_call_links(self, contact_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, note in enumerate(call_notes):
                ...
                if call_link:
                    # Добавляем параметры аутентификации, если их нет, и если call_link - это строка
                    if isinstance(call_link, str) and account_id and user_id and "userId" not in call_link:
                        if "?" in call_link:
                    ...
                    # Добавляем параметр загрузки, если call_link - это строка
                    if isinstance(call_link, str) and "download=" not in call_link:
