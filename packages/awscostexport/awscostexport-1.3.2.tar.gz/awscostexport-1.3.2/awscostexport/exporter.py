...
def build_account_filter(account_filter):
    """Build AWS Cost Explorer filter for account inclusion/exclusion"""
    ...
    if account_filter.get('include'):
        filters.append({
            'Dimensions': {
                'Key': 'LINKED_ACCOUNT',
                'Values': account_filter['include']
            }
        })
    
    # Exclude accounts filter
    if account_filter.get('exclude'):
        filters.append({
            'Not': {
                'Dimensions': {
                    'Key': 'LINKED_ACCOUNT', 
                    'Values': account_filter['exclude']
                }
            }
        })
...
def export_marketplace_spend(ce_client, date_range, output_dir, account_filter=None):
    """Export AWS Marketplace spend separately"""
    print("Exporting marketplace spend...")
    
    # Build base filter
    base_filter = build_account_filter(account_filter)
    marketplace_filter = {
        'Or': [
            {'Dimensions': {'Key': 'SERVICE', 'Values': ['AWS Marketplace']}},
            {'Dimensions': {'Key': 'SERVICE', 'Values': ['AWS Marketplace Entitlement']}}
        ]
    }
    
    # Combine filters
    if base_filter:
        filter_param = {'And': [base_filter, marketplace_filter]}
    else:
        filter_param = marketplace_filter
    ...
    for result in response['ResultsByTime']:
        for group in result['Groups']:
            ...
            # Create a readable product name from service and usage type
            product_name = f"{service} - {usage_type}" if usage_type else service
            marketplace_costs[product_name] += cost
...
def export_service_breakdown(ce_client, date_range, output_dir, account_filter=None):
    """Export service cost breakdown"""
    print("Exporting service breakdown...")
    
    # Build filters
    base_filter = build_account_filter(account_filter)
    
    # Exclude marketplace from main service breakdown
    not_marketplace_filter = {
        'Not': {
            'Or': [
                {'Dimensions': {'Key': 'SERVICE', 'Values': ['AWS Marketplace']}},
                {'Dimensions': {'Key': 'SERVICE', 'Values': ['AWS Marketplace Entitlement']}}
    ...
    # Combine filters
    if base_filter:
        filter_param = {'And': [base_filter, not_marketplace_filter]}
    else:
        filter_param = not_marketplace_filter
...
def export_ec2_details(ce_client, date_range, output_dir, account_filter=None):
    """Export EC2 cost details"""
    print("Exporting EC2 details...")
    
    # Build filters
    base_filter = build_account_filter(account_filter)
    ec2_filter = {'Dimensions': {'Key': 'SERVICE', 'Values': ['EC2 - Other', 'Amazon Elastic Compute Cloud - Compute']}}
    
    # Combine filters
    if base_filter:
        filter_param = {'And': [base_filter, ec2_filter]}
    else:
        filter_param = ec2_filter
    
    # Get line item types to properly account for Savings Plans
    response = ce_client.get_cost_and_usage(
        TimePeriod=date_range,
        Granularity='MONTHLY',
        Metrics=['UnblendedCost'],
    ...
    it_response = ce_client.get_cost_and_usage(
        TimePeriod=date_range,
        Granularity='MONTHLY',
        Metrics=['UnblendedCost'],
        Filter=filter_param,
    ...
    for result in it_response['ResultsByTime']:
        for group in result['Groups']:
    ...
    with open(csv_file, 'w', newline='') as f:
        ...
        for i, (itype, cost) in enumerate(sorted_types[:20]):
            avg_monthly = cost / len(it_response['ResultsByTime'])
            percentage = (cost / total * 100) if total > 0 else 0
            writer.writerow([itype, f"${cost:,.2f}", f"${avg_monthly:,.2f}", f"{percentage:.1f}%"])
    
    print(f"✓ EC2 instance types saved to {csv_file}")

def export_rds_details(ce_client, date_range, output_dir, account_filter=None):
    ...
    # Combine filters
    if base_filter:
        filter_param = {'And': [base_filter, rds_filter]}
    else:
        filter_param = rds_filter
...
def export_container_services(ce_client, date_range, output_dir, top_services, account_filter=None):
    """Export container services (ECS, EKS, Fargate) if they're in the top services"""
    container_services = ['Amazon EC2 Container Service', 'Amazon Elastic Container Service for Kubernetes', 'AWS Fargate']
    
    # Check if any container services are in top services
    found_services = []
    for service in container_services:
        if any(service in top[0] for top in top_services):
            found_services.append(service)
    
    if found_services:
        print(f"Exporting container services: {', '.join(found_services)}...")
        try:
            # Build filters
            base_filter = build_account_filter(account_filter)
            container_filter = {'Dimensions': {'Key': 'SERVICE', 'Values': found_services}}
            
            # Combine filters
            if base_filter:
                filter_param = {'And': [base_filter, container_filter]}
            else:
                filter_param = container_filter
...
def export_data_transfer(ce_client, date_range, output_dir, account_filter=None):
    ...
    try:
        # Build filters
        base_filter = build_account_filter(account_filter)
        data_filter = {'Dimensions': {'Key': 'USAGE_TYPE_GROUP', 'Values': ['EC2: Data Transfer', 'EC2: NAT Gateway']}}
        
        # Combine filters
        if base_filter:
            filter_param = {'And': [base_filter, data_filter]}
        else:
            filter_param = data_filter
...
def export_ebs_volumes(ce_client, date_range, output_dir, account_filter=None):
    ...
    try:
        ...
        # Combine filters
        if base_filter:
            filter_param = {'And': [base_filter, ebs_filter]}
        else:
            filter_param = ebs_filter
...
def export_comprehensive_reservation_analysis(ce_client, output_dir, account_filter=None):
    ...
    for service in SERVICES_WITH_RESERVATIONS:
        try:
            ...
            # Combine filters
            if base_filter:
                filter_param = {'And': [base_filter, service_filter]}
            else:
                filter_param = service_filter
