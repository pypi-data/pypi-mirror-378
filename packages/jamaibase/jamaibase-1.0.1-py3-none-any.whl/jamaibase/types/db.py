...
class ModelConfigUpdate(ModelInfo):
    ...
    embedding_dimensions: int | None = Field(
        None,
        description=(
            "Reduced embedding size. Defaults to None (no reduction). "
            "For example: `openai/text-embedding-3-large` has `embedding_size` of 3072 "
            "but can be shortened to `embedding_dimensions` of 256; "
            "`cohere/embed-v4.0` has `embedding_size` of 1536 "
            "but can be shortened to `embedding_dimensions` of 256."
...
class OrganizationUpdate(_BaseModel):
    ...
    @field_validator("external_keys", mode="after")
    @classmethod
    def filter_empty_external_keys(cls, v: dict[str, str]) -> dict[str, str]:
        # Return a new dictionary containing only items with non-empty, non-whitespace string values.
        return {k: v.strip() for k, v in v.items() if v.strip()}
...
class VerificationCodeCreate(VerificationCodeUpdate):
    ...
    expiry: DatetimeUTC = Field(
        description="Code expiry datetime (UTC). Defaults to expiry in 7 days.",
    )
