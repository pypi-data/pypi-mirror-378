...
class ChatRequestBase(BaseModel):
    ...
    temperature: float = Field(
        ...
        description=(
            ...
            "Note that this parameter will be ignored for models that do not support it, "
    ...
    thinking_budget: PositiveNonZeroInt | None = Field(
        1,
        description=(
            "The number of tokens to reserve for the model's internal 'reasoning' process. "
            "These tokens are used by the model to reason through complex problems before generating a final response. "
            "This parameter is only applicable to models from some providers that support extended thinking, "
            "such as models from Gemini, Anthropic and OpenAI. "
            "If set, the model will have up to this many tokens available for its internal reasoning, "
            "in addition to the tokens used for the visible response. "
            "However for OpenAI models, this parameter only control the turning on and off of reasoning, "
            "when thinking budget more than 0 turning on, while None or 0 turning off."
    ...
    reasoning_effort: Literal["minimal", "low", "medium", "high"] = Field(
        "low",
        description=(
            "The level of reasoning effort the model should apply, checked after `thinking_budget`. "
            "`minimal` only applicable for GPT-5 model series. "
            "Will be ignored if the model does not support it."
        ),
    )
    ...
    def hyperparams(self) -> dict[str, Any]:
        ...
        return self.model_dump(
            exclude_none=True, exclude={"object", "messages", "rag_params", "agent_mode"}
        )
...
class ChatRequest(ChatRequestBase):
    ...
    def validate_params(self):
        ...
        # if self.thinking_budget and self.reasoning_effort:
        #     raise ValueError(
        #         "To avoid conflicts, please specify either 'reasoning_effort' or 'thinking_budget', but not both."
        #     )
        # if self.thinking_budget and self.thinking_budget > self.max_tokens:
        #     raise ValueError("'max_tokens' cannot be set lower than 'thinking_budget'.")
        return self
