...
class BaseConfig(msgspec.Struct):
    ...
    @classmethod
    def load(cls, parse_cli_args: bool = True) -> Self:
        """Load configuration from available sources.

        Args:
            parse_cli_args: bool parameter, enable, parsing cli args, default=False. 
        ...
        config_path: str | None = cli_values.get("config", None) or env_values.get(
            "config", None
        ...
        merged_data = update_recursive(
            yaml_values, update_recursive(env_values, cli_values)
