...
def run_ExpertBasedGraml_panda_tutorial():
    recognizer = ExpertBasedGraml(domain_name=PANDA, env_name="PandaMyReachDense")
    ...
    problem_name = env_property.goal_to_problem_str(np.array([[-0.5, -0.5, 0.1]]))
    actor = DeepRLAgent(
        domain_name=PANDA,
    ...
    closest_goal = recognizer.inference_phase(
        partial_sequence, np.array([[-0.5, -0.5, 0.1]]), 0.5
    )
    print(
        f"closest_goal returned by GRAML: {closest_goal}\nactual goal actor aimed towards: [-0.5, -0.5, 0.1]"
    )
